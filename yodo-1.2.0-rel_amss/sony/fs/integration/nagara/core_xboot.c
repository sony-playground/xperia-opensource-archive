...
int core_get_root_key_hash(unsigned char *buf, unsigned int len)
{
	...
	unsigned char buf_tmp[PLATFORM_ROOT_KEY_HASH_LEN];
	...
	memcpy(dummy_buf, (const void *)PLATFORM_ROOT_KEY_HASH_ADDR,
                                        DUMMY_PLATFORM_ROOT_KEY_HASH_LEN);

	while(buf_i < PLATFORM_ROOT_KEY_HASH_LEN)
	{
		memcpy((buf_tmp + buf_i), (dummy_buf + dummy_index), 7);
		buf_i= buf_i + 7;
