...
int execute_module_xboot_reset_arb(
		...
		const char *match)
{
	...
	unsigned char num_func_call_expected = 0;
	...
	case MT_XB_EXECUTE:
		...
		/* Set Authenticated Channel Command */
		res = integration_set_property(INTG_PROP_AUTHCH_CMD,
				const_dut_auth_channel,
				const_dut_auth_channel_len);
		if (res != FCSERR_OK) {
			log("Set AuthChCmd failed with %d\n", res);
			return res;
		}

		/* Emulate updated nonce in secdata (RPMB) */
		res = integration_set_property(INTG_PROP_SD_NONCE,
			const_dut_authch_sake_nonce,
			const_dut_authch_sake_nonce_len);
		if (res != FCSERR_OK) {
			log("Set of INTG_PROP_SD_NONCE failed with %d\n", res);
			return res;
		}
		...
		res = xboot_check_auth_cmd(ctx);
		if (res != FCSERR_OK) {
			log("xboot_check_auth_cmd failed %d\n", res);
			goto test_failure;
		}
		...
		/* SecData (RPMB) should have been updated */
		num_func_call_expected = 1;
		res = integration_is_equal_property(INTG_PROP_CNT_WRSECDATA,
			(const void *)&num_func_call_expected,
			sizeof(num_func_call_expected));
		if (res != FCSERR_OK) {
			log("CNT_WRSECDATA check failed with %d\n", res);
			goto test_failure;
		}
		/* Check cleared nonce in SecData (RPMB) */
		res = integration_is_equal_property(INTG_PROP_SD_NONCE,
			const_dut_sec_data_sake_nonce,
			sizeof(const_dut_sec_data_sake_nonce));
		if (res != FCSERR_OK) {
			log("set of INTG_PROP_SEC_DATA failed with %d\n",
				res);
			goto test_failure;
		}

		/* Misc TA unit is erased */
		res = integration_is_equal_property(INTG_PROP_AUTHCH_CMD,
			NULL,
			0);
		if (res != FCSERR_OK) {
			log("PROP_AUTHCH_CMD check failed with %d\n", res);
			goto test_failure;
		}

		/* VBMeta ARB is reset */
		num_func_call_expected = 1;
		res = integration_is_equal_property(INTG_PROP_CNT_RSTVBMARB,
			(const void *)&num_func_call_expected,
			sizeof(num_func_call_expected));
		if (res != FCSERR_OK) {
			log("CNT_RSTVBMARB check failed with %d\n", res);
			goto test_failure;
		}
