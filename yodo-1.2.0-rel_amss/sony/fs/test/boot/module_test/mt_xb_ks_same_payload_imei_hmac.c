...
/*
 * Description:
 * If xflkeystore and xflkeystorebak have same payload, and xflkeystore is
 * bound to the IMEI HMAC and xflkeystorebak is bound to plain IMEI, and both
 * have the same keystore counter, the valid HMAC key from MiscTA unit 2164
 * should be copied to MiscTA unit 66673, and xflkeystore copied to
 * xflkeystorebak.
 *
 * Setup:
 * 1. xflkeystorebak is valid and bound to plain IMEI.
 * 2. xflkeystore is valid, bound to IMEI HMAC and has same counter as
 *    xflkeystorebak.
 * 3. There is a valid HMAC key in MiscTA unit 2164.
 *
 * Execution:
 * 1. xflkeystore will be validated successfully.
 * 2. The HMAC key will be copied to MiscTA unit 66673.
		...
		const char *match)
{
	...
	case MT_XB_EXECUTE:
		...
		/* Put a valid HMAC key to low MiscTA */
