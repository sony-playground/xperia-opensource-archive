		...
		const char *match)
{
	...
	case MT_XB_EXECUTE:
		...
		/* Clear primary keystore */
		res = integration_clear_property(INTG_PROP_KS);
		if (res != FCSERR_OK) {
			log("Clear keystore failed with %d\n", res);
			return res;
		}
		...
		/* If keystore is empty, then we shall boot XFL */
		...
		if (cert_count != const_dut_emb_valid_n_xfl_cert) {
			log("Unexpected cert_count %d\n", cert_count);
			goto test_failure;
		}

		/* Check certs */
		for (i = 0; i < const_dut_emb_valid_n_xfl_cert; i++) {
