...
/* Startup of normal Phone SW with power button.
   This test case adds a delay longer than long press
   timeout. This is the ordininary behaviour due to
   rom image verification and other boot stages. */
/* Power button press time (2000 ms) must be larger than
   delay (1500 ms) so vibrate will be triggered before
   release of button */
...
int execute_module_xboot_delay_normal_setup(
		...
		const char *match)
{
	...
	case MT_XB_EXECUTE:
		...
		/* Delay time longer than long press time to represent
		   platform verify and load/verify of various boot layers */
		xbpal_delay_us(1500*1000);
