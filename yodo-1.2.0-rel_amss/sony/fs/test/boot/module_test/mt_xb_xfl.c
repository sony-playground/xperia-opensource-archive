...
static int module_setup_hwstate(fcsmt_log_t log)
{
	...
	struct integration_hwstate state = {
		...
		.vldwn_button_pressed_time = 1500,
		...
		const char *match)
{
	...
	case MT_XB_EXECUTE:
		...
		/* Set sake random value */
		res = integration_set_property(INTG_PROP_URAND,
			const_dut_authch_sake_nonce,
			const_dut_authch_sake_nonce_len);
		if (res != FCSERR_OK) {
			log("Set of INTG_PROP_URAND failed with %d\n", res);
			return res;
		}
		...
		/* XFL should be started */
		res = xboot_gen_auth_ch_nonce(ctx);
		if (res != FCSERR_OK) {
			log("xboot_gen_auth_ch_nonce failed %d\n", res);
			goto test_failure;
		}
		...
		res = integration_is_equal_property(INTG_PROP_SD_NONCE,
			...
			const_dut_authch_sake_nonce_len);
		if (res != FCSERR_OK) {
			log("SecData nonce check failed with %d\n", res);
			goto test_failure;
		}

		/* Force call of pal function for read ARB */
		cnt_buf = (unsigned long *)malloc(
				sizeof(const_dut_sec_data_rollback_counter));
		if (cnt_buf == NULL) {
			log("Failed to allocate count buffer\n");
			return -FCSERR_ENOMEM;
		}
		res = xboot_get_rollback_counters(cnt_buf,
			sizeof(const_dut_sec_data_rollback_counter)/
			sizeof(const_dut_sec_data_rollback_counter[0]));
		if (res != FCSERR_OK) {
			log("Get VBMeta ARB failed with %d\n", res);
			goto test_failure;
		}
		if (memcmp(cnt_buf,
				const_dut_sec_data_rollback_counter,
				sizeof(const_dut_sec_data_rollback_counter))) {
			log("VBMeta ARB check failed with %d\n", res);
			goto test_failure;
		}
