		...
		size_t *embed_rootcert_list_count)
{
	...
	/* Test Config Root */
	embedded_roots[0].usage_flags = INTEMUL_CERT_CONFIG_USAGE;
	embedded_roots[0].rootcerts = x_conf_root_cert_test;
	embedded_roots[0].rootcerts_length = sizeof(x_conf_root_cert_test);

	/* Test XFL root */
	embedded_roots[1].usage_flags = INTEMUL_CERT_XFL_USAGE;
	embedded_roots[1].rootcerts = x_xfl_root_cert_test;
	embedded_roots[1].rootcerts_length =
