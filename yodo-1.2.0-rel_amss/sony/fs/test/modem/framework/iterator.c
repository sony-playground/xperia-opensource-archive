		...
		struct iterator_info *it, const char *match)
{
	...
	while (it->cur_module < it->n_modules) {
		...
		if (strncmp(p_module->name, match, cmp_len) == 0)
			break;
		p_module = NULL;
		(it->cur_module)++;
	}
...
int add_module(struct iterator_info *it, const char *name, fcsmt_func_t func)
{
	...
	if ((it->module == NULL) && (it->n_allocated <= it->n_modules)) {
		next_allocation = malloc(sizeof(struct iterator_info) *
					(it->n_allocated+10));
		if (next_allocation == NULL)
			return -FCSERR_ENOMEM;
		if (it->module != NULL) {
			memcpy(next_allocation, it->module,
				sizeof(struct iterator_info) * it->n_modules);
			free(it->module);
		}
		it->n_allocated += 10;
				...
				struct iterator_tcstate *tcstate))
{
	...
	if (it == NULL || export_tcstate == NULL)
		return -FCSERR_EINVAL;
		...
		const char *match)
{
	...
	if (log == NULL || it == NULL || match == NULL)
		return -FCSERR_EINVAL;

	if (it->module == NULL || it->n_modules == 0)
		return -FCSERR_EINVAL;
		...
		struct iterator_info *it)
{
	...
	for (i = 0; i < it->n_modules; i++) {
		...
		if (p_module == NULL || p_module->name == NULL) {
