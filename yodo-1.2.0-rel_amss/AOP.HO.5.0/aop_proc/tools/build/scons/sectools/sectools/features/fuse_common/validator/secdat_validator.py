...
class SecDatValidator(BaseValidator):
    ...
    def __init__(self, oem_config_path, qti_config_path,
                 user_config_path, ui_config_path,sec_dat_path, config, feature, output_dir, debug, TOOL_NAME, dummy=False):
        '''
        Constructor
        '''
        ...
        self.secdat_parser = SecdatParseGen(feature, dummy=self.dummy)
    ...
    def validate(self):
        ...
        try:
            if self.config_parser.data_model.compare(self.secdat_parser.data_model, 'config files', 'sec.dat', dummy=self.dummy):
