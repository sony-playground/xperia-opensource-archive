...
def dump(debug_data): #dump_path, parse_path, memory, debug_info, target, branch):
    ...
    rpm_ulogContext_type = debug_info.variables[b'rpm_ulogContext'].vartype
    ...
    if os.path.exists(rpm_external_log):
        ...
        parser = os.path.join(parse_path, 'aop_log_hfam.py')
        parser2 = os.path.join(parse_tbl, 'tracer_event_tbl.h')
        parser_params = ['py', '-3', parser, '-f', rpm_external_log, '-t', target, '-tbl', parser2 ]#, '-b', branch]
        if os.name == 'posix':
            parser_params = ['python3', parser, '-f', rpm_external_log, '-t', target, '-tbl', parser2 ]#, '-b', branch]

        try:
            pretty_output = subprocess.check_output(parser_params)
            pretty_output = pretty_output.decode("utf-8")
    ...
    if os.path.exists(ddr_external_log):
        print_p('\t\tPost-processing the AOP "DDR" log...')
        parser = os.path.join(parse_path, 'aop_log_hfam.py')
        parser2 = os.path.join(parse_tbl, 'tracer_event_tbl.h')
        parser_params = ['py', '-3', parser, '-f', ddr_external_log, '-t', target, '-tbl', parser2 ]#, '-b', branch]
        if os.name == 'posix':
            parser_params = ['python3', parser, '-f', ddr_external_log, '-t', target, '-tbl', parser2 ]#, '-b', branch]
