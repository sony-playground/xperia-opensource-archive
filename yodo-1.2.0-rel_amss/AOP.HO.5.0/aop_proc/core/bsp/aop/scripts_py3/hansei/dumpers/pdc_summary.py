...
from dumpers.pdc_registers import masks, unit_masks, pdc_fields, version, status_val, enable_pdc_val, disable_clk_gate_val, enable_bank_val, irq_status_val, irq_cfg_val
...
def tick_format(ticks):
    return '0x%016X |%14.6f' % (ticks, ticks/19200000.0)
...
def dump(debug_data): 
	...
	if target_ver in targ_spec_config:
		target = target_ver
	debug_data['printer'].p("Dumping pdc Summary registers...")
	...
	if not os.path.exists(os.path.join(dump_path, "pdc")):
		os.makedirs(os.path.join(dump_path, "pdc"))
	...
	############################################################
			...
			if (subs_list[xx]!="debug"): # no profilng for debug
				...
				for d in range(eval(subs_list[xx]+"_no_drv")):
					...
					print("\t\t\t      PDC Sleep sequence Start : " + tick_format(dnStart) + " [Valid: "+str(bool(dnStartValid))+", Overflow: "+str(bool(dnStartOverflow))+"]" ,file=pdc_summary_file)
					print("\t\t\t      PDC Sleep sequence End   : " + tick_format(dnEnd) + " [Valid: "+str(bool(dnEndValid))+", Overflow: "+str(bool(dnEndOverflow))+"]"  ,file=pdc_summary_file)
					print("\t\t\t      PDC Wake sequence Start  : " + tick_format(upStart) + " [Valid: "+str(bool(upStartValid))+", Overflow: "+str(bool(upStartOverflow))+"]" ,file=pdc_summary_file)
					print("\t\t\t      PDC Wake sequence End    : " + tick_format(upEnd) + " [Valid: "+str(bool(upEndValid))+", Overflow: "+str(bool(upEndOverflow))+"]" ,file=pdc_summary_file)
