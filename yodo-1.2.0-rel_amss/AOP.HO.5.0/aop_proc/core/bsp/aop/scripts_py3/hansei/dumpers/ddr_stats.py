...
def dump(debug_data): #dump_path, memory, debug_info):
    ...
    if (check_member('ddr_stats', aop_core_dump)):
        ...
        try:
            freq_switch_address = debug_info.variables[b'ddr_stats'].die
        except:
        ...
        try:
            ...
            ddr_address = debug_info.variables[b'ddr_stats'].address # FIXME: get right address by magic
...
################################################################################################################################			
    ...
    if (check_member('ddr_status', aop_core_dump)):
        ...
        try:
            freq_switch_address = debug_info.variables[b'ddr_status'].die###################################
        except:
            ddr_file_name = os.path.join(dump_path, 'ddr_stats.txt')
            with open(ddr_file_name, 'w') as ddr_file:
        ...
        try:
			#not in core dump, but elf is close enough to provide a correct pointer
            ddr_status_address = debug_info.variables[b'ddr_status'].address # FIXME: get right address by magic
...
################################################################################################################################			
    if (check_member('ddr_cfg', aop_core_dump)):
        #ddr_cfg  in in core dump
        ddr_cfg = aop_core_dump.ddr_cfg 
        ddr_cfg = cast(ddr_cfg, 'LOCAL_DDRCfgType', memory, debug_info)
    else:
        try:
            ddr_cfg_address = debug_info.variables[b'ddr_cfg'].die
        except:
            ddr_file_name = os.path.join(dump_path, 'ddr_stats.txt')
            with open(ddr_file_name, 'w') as ddr_file:
        ...
        try:
            #not in core dump, but elf is close enough to provide a correct pointer
            ddr_gbl_address = debug_info.variables[b'ddr_cfg'].address # FIXME: get right address by magic
            ddr_cfg = decode_object('ddr_cfg', ddr_gbl_address, None, memory, debug_info, die=ddr_cfg_address)
        except:
            #not correct in elf, find absolute address (very hacky)
            with open(ddr_file_name, 'w') as ddr_file:
                print("\n !!! DDR_cfg DOES NOT EXIST !!! \n", file=ddr_file)
...
def dump_ddr(dump_path, ddr_cfg, ddr, ddr_status, memory, debug_info):
    ...
    with open(ddr_file_name, 'w') as ddr_file:
        ...
        try:
            ...
            for x in range(0, ddr_cfg.nNumMCStates):
                print("|{0:>9d}|{1:>10d}|{2:>12d}|{3:>13d}|{4:>6s}|{5:>10s}|{6:>10s}|{7:>10s}|{8:>14s}|{9:>12d}|{10:>14d}|{11:>20d}|"  .format(\
                                                                ...
                                                                ddr_cfg.pMCCfg[x].freq_mhz,\
                                                                ddr_cfg.pMCCfg[x].clk_period,\
                                                                ddr_cfg.pMCCfg[x].freq_enable,\
                                                                ddr_cfg.pMCCfg[x].mode.decode('utf-8'),\
                                                                rail_level[ddr_cfg.pMCCfg[x].vddcx],\
                                                                rail_level[ddr_cfg.pMCCfg[x].vddmx],\
                                                                rail_level[ddr_cfg.pMCCfg[x].vdda],\
        ...
        try:
            ...
            for y in range(0, ddr_cfg.nNumSHUBStates):
                print("|{0:>9d}|{1:>10d}|{2:>12d}|{3:>13d}|{4:>6s}|{5:>10s}|{6:>10s}|{7:>10s}|{8:>14s}|{9:>12d}|{10:>14d}|{11:>20d}|"  .format(\
                                                                ...
                                                                ddr_cfg.pSHUBCfg[y].freq_mhz,\
                                                                ddr_cfg.pSHUBCfg[y].clk_period,\
                                                                ddr_cfg.pSHUBCfg[y].freq_enable,\
                                                                ddr_cfg.pSHUBCfg[y].mode.decode('utf-8'),\
                                                                rail_level[ddr_cfg.pSHUBCfg[y].vddcx],\
                                                                rail_level[ddr_cfg.pSHUBCfg[y].vddmx],\
                                                                rail_level[ddr_cfg.pSHUBCfg[y].vdda],\
        ...
        try:
            ...
            print("\t{0:32s}{1:10s}"  .format(	"bcm_alc_pending:",str(ddr_status.bcm_alc_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"bcm_acv_pending:",str(ddr_status.bcm_acv_pending)	), file=ddr_file)
            ...
            print("\t{0:32s}{1:10s}"  .format(	"dest_mc_cp:",hex(ddr_status.dest_mc_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"dest_shub_cp:",hex(ddr_status.dest_shub_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"next_mc_cp:",hex(ddr_status.next_mc_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"next_shub_cp:",hex(ddr_status.next_shub_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"mc_lvl_double:",hex(ddr_status.mc_lvl_double)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"cur_mc_cp:",hex(ddr_status.cur_mc_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"cur_shub_cp:",hex(ddr_status.cur_shub_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"ddr_rm_level:",hex(ddr_status.ddr_rm_level)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"bcm_mc_cp:",hex(ddr_status.bcm_mc_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"bcm_shub_cp:",hex(ddr_status.bcm_shub_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"bcm_acv_cp:",hex(ddr_status.bcm_acv_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"bcm_alc_cp:",hex(ddr_status.bcm_alc_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"reg_mc_cp:",hex(ddr_status.reg_mc_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"reg_shub_cp:",hex(ddr_status.reg_shub_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"flush_complete:",hex(ddr_status.flush_complete)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"pcb_mask:",hex(ddr_status.pcb_mask)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"lb_activations:",hex(ddr_status.lb_activations)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"xo_vote:",hex(ddr_status.xo_vote)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"xo_on:",hex(ddr_status.xo_on)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"xo_on_req_id:",hex(ddr_status.xo_on_req_id)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"xo_on_start_time:",hex(ddr_status.xo_on_start_time)	), file=ddr_file)
            
            print("\t{0:32s}{1:10s}"  .format(	"dcc_vote:",hex(ddr_status.dcc_vote)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"dcc_on:",hex(ddr_status.dcc_on)	), file=ddr_file)
            ...
            print("\t{0:32s}{1:10s}"  .format(	"Cx_vote:",rail_level[ddr_status.arcs[0].vote]	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"Cx_last_vote:",rail_level[ddr_status.arcs[0].last_vote]	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"Cx_vote_pending:",hex(ddr_status.arcs[0].vote_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"Cx_vote_req_id:",hex(ddr_status.arcs[0].req_id)	), file=ddr_file)
            
            print("\t{0:32s}{1:10s}"  .format(	"Mx_vote:",rail_level[ddr_status.arcs[1].vote]	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"Mx_last_vote:",rail_level[ddr_status.arcs[1].last_vote]	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"Mx_vote_pending:",hex(ddr_status.arcs[1].vote_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"Mx_vote_req_id:",hex(ddr_status.arcs[1].req_id)	), file=ddr_file)
            
            print("\t{0:32s}{1:10s}"  .format(	"VDDA_vote:",rail_level[ddr_status.arcs[2].vote]	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"VDDA_last_vote:",rail_level[ddr_status.arcs[2].last_vote]	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"VDDA_vote_pending:",hex(ddr_status.arcs[2].vote_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"VDDA_vote_req_id:",hex(ddr_status.arcs[2].req_id)	), file=ddr_file)
            ...
            print("\t{0:32s}{1:10s}"  .format(	"pmic_vote:",hex(ddr_status.pmic_vote)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"pmic_vote_pending:",hex(ddr_status.pmic_vote_pending)	), file=ddr_file)
            
            print("\t{0:32s}{1:10s}"  .format(	"voting_in_progress:",hex(ddr_status.voting_in_progress)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"votes_pending:",hex(ddr_status.votes_pending)	), file=ddr_file)
            
            print("\t{0:32s}{1:10s}"  .format(	"shrm_cp:",hex(ddr_status.shrm_cp)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"tcs0_pending:",hex(ddr_status.tcs0_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"tcs1_pending:",hex(ddr_status.tcs1_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"tcs2_pending:",hex(ddr_status.tcs2_pending)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"mc_floor:",hex(ddr_status.mc_floor)	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"mc_steps[0]:",hex(ddr_status.mc_steps[0])	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"mc_steps[1]:",hex(ddr_status.mc_steps[1])	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"mc_steps[2]:",hex(ddr_status.mc_steps[2])	), file=ddr_file)
            print("\t{0:32s}{1:10s}"  .format(	"mc_steps[3]:",hex(ddr_status.mc_steps[3])	), file=ddr_file)
