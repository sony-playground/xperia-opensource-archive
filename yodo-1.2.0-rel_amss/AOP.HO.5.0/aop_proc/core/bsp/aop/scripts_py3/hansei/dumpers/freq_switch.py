...
def dump(debug_data): #dump_path, memory, debug_info):
    ...
    if (check_member('freq_switch_table_ptr ', aop_core_dump)):
        ...
        try:
            freq_switch_type = debug_info.variables[b'freq_switch_table_ptr'].die
        except:
        ...
        try:
            ...
            freq_switch_address = debug_info.variables[b'freq_switch_table_ptr'].address # FIXME: get right address by magic
...
def dump_ddr(dump_path, rpm, freq_switch, memory, debug_info):
    ...
    with open(freq_switch_file_name, 'a') as freq_switch_file:
        ...
        print("\n\n ------------------------------------------------------------------------------------------------------------------------------------------", file=freq_switch_file)
        print("\n |   TimeStamp   |  Event  | powerstate_request | mc_steps | bcm_acv_alc | bcm_mc_request | bcm_shub_request | current_mc_cp | current_shub_cp |", file=freq_switch_file)
        print("\n ------------------------------------------------------------------------------------------------------------------------------------------", file=freq_switch_file)
        try:
            for x in range(smallest_indx-1,-1,-1):
                print("|{0:>15s} |{1:>9s}|{2:>20s}|{3:>9s}|{4:>9s}|{5:>16s}|{6:>18s}|{7:>15s}|{8:>17s}|"  .format(\
                                            ...
                                            hex(freq_switch[x].mc_steps),\
                                            hex(freq_switch[x].bcm_acv_alc),\
