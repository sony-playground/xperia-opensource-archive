...
def dump(debug_data): #dump_path, memory, debug_info):
    ...
    if (check_member(b'sleep_stats', aop_core_dump)):
        ...
        try:
            ...
            sleep_address = debug_info.variables[b'sleep_stats'].address # FIXME: get right address by magic
...
def dump_sleep(dump_path, rpm, sleep, memory, debug_info):
    ...
    with open(sleep_file_name, 'w') as sleep_file:
        ...
        print('\n\tTYPE:            %s' % convert_hex_to_ascii(sleep[2].stat_type), file=sleep_file)
        try:
            print('\t\tsleep_count:            %d' % sleep[2].count, file=sleep_file)
            print('\t\tlast_entered_at:        %s' % hex(sleep[2].last_entered), file=sleep_file)
            print('\t\tlast_exited_at:         %s' % hex(sleep[2].last_exited), file=sleep_file)
            print('\t\taccumulated_duration:   %s' % hex(sleep[2].total_duration), file=sleep_file)
        except:
            print('\t\tERROR READING sleep_stats[2]', file=sleep_file)
