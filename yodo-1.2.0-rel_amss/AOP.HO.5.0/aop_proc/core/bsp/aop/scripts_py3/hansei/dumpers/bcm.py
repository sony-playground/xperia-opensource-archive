...
from dumpers.cmd_db import bcm_reg_map, bcm_vcd_map
import traceback
...
####################################################################################################################        
    ...
    if (check_member('global_ddr_cfg ', aop_core_dump)):
        ...
        try:
            ddr_cfg_address = debug_info.variables[b'global_ddr_cfg'].die
        except:
        ...
        try:
            ...
            ddr_gbl_address = debug_info.variables[b'global_ddr_cfg'].address # FIXME: get right address by magic
    ...
    bcm_csv_file_name = os.path.join(dump_path, 'bcm_vt.csv')
    bcm_summary_csv_file_name = os.path.join(dump_path, 'bcm_vt_summary.csv')
    summary_file_name = os.path.join(dump_path, 'rpmh_summary.txt')

    with open(bcm_file_name, 'w') as bcm_file, open(summary_file_name, 'w') as summary_file:
        ...
        for x in range(0, no_vcds):
            ...
            ddr_mc_cp=hex(ddr_status.init_mc_cp) 
            ddr_shub_cp=hex(ddr_status.init_shub_cp) 
                    ...
                    if (x==0):
                        ...
                        if (ddr_mc_cp!=0x0):
                            ...
                            for y in data[key]["Clock Domains"]:
                                try :							
                                    print("\t|{0:3s}{1:>20s}|{2:>8s}|{3:>10s}|{4:>8s}|{5:>25s}|{6:30s}: {7:>17s}|" .format(v4,var_print,v1,v2,v3,v5,y["domain"],(str(y["freq"][int(ddr_mc_cp,16)]).split(":")[1][2:-2])), file=bcm_file)
                                    var_print=""  
                                    v1=""
                                    v2=""
                                    v3=""
                        ...
                        if (ddr_shub_cp!=0x0):
                            ...
                            for y in data[key]["Clock Domains"]:
                              try:
                                print("\t|{0:3s}{1:>20s}|{2:>8s}|{3:>10s}|{4:>8s}|{5:>25s}|{6:30s}: {7:>17s}|" .format(v4,var_print,v1,v2,v3,v5,y["domain"],(str(y["freq"][int(ddr_shub_cp,16)]).split(":")[1][2:-2])), file=bcm_file)
                                var_print=""  
                                v1=""
                                v2=""
                                v3=""
                        ...
                        if (agg_cp!=0):
                            ...
                            for y in data[key]["Clock Domains"]:
                              try:
                                print("\t|{0:3s}{1:>20s}|{2:>8s}|{3:>10s}|{4:>8s}|{5:>25s}|{6:30s}: {7:>17s}|" .format(v4,var_print,v1,v2,v3,v5,y["domain"],(str(y["freq"][agg_cp]).split(":")[1])[2:-2]), file=bcm_file)
                                var_print=""  
                                v1=""
                                v2=""
                                v3=""
        ...
        bcm_backend_seq_dict = OrderedDict()
        bcm_backend_seq_dict["VCD"] = []
        bcm_backend_seq_dict["SEQ_CURR_PC"] = []
        bcm_backend_seq_dict["IDLE"] = []
        ...
        for x in range(0, no_vcds):
            ...
            bcm_backend_seq_dict["VCD"].append(x)
            bcm_backend_seq_dict["SEQ_CURR_PC"].append(curr_pc)
            bcm_backend_seq_dict["IDLE"].append(seq_state)
        ...
        df = pd.DataFrame().from_dict(bcm_backend_seq_dict, orient="columns")
        df.to_csv(bcm_csv_file_name, index=False)

        print("\n\n ~~BCM Resource Summary~~", file=bcm_file)
        bcm_res_summary_data = []
        ...
        for regs in range(0,no_regs):
            ...
            for drv in range(0,no_drv):
                ...
                if bcm_vt != 0:
                    ...
                    bcm_res_summary_data.append({"Name":"BCM{}".format(regs), "id":regs, "DRV":targ_spec_config[target]['drv'][drv],
                                                 "Valid":vote_valid, "AB":vote_x, "IB":vote_y})
                BCM_votetable[regs][drv] = [vote_valid, vote_x, vote_y]
                #print(BCM_votetable[regs][drv])

        df = pd.DataFrame(bcm_res_summary_data)
        df.sort_values(by=["id"], inplace=True)
        df.drop(columns=["id"], inplace=True)
        df.to_csv(bcm_summary_csv_file_name, index=False)