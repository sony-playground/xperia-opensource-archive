...
#define CMD_DB_MAX_HW_ID_ENTRIES   110
...
/* Internal config structs to pass from parser to target*/
typedef struct{
  uint16_t arc_db_headers_cnt;
  uint16_t arc_aux_size;
  uint8_t *arc_aux;
  cmd_db_entry_header* arc_db_headers;
}cmddb_arc_struct_t;

typedef struct{
  uint16_t bcm_db_headers_cnt;
  uint16_t bcm_aux_size;
  uint8_t *bcm_aux;
  cmd_db_entry_header* bcm_db_headers;
}cmddb_bcm_struct_t;
