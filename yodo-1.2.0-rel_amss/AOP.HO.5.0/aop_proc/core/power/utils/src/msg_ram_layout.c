...
{
  aop_info_0_t *info = (aop_info_0_t*)msg_ram_sections[MSG_RAM_AOP_INFO_0].start;
  info->dictionary.aop_lpm_mon_offset = buffer_addr;
}
...
void msg_ram_init(void)
{
  ...
  info->dictionary.aop_ddr_reg_data_offset  = (uint32)&(info->aop_ddr_reg_data) - AOP_SS_MSG_RAM_START_ADDRESS_BASE;
  info->dictionary.aop_ddr_residency_offset = (uint32)&(info->ddr_residency)    - AOP_SS_MSG_RAM_START_ADDRESS_BASE;
  info->dictionary.starc_residency_offset   = (uint32)&(info->starc_residency)  - AOP_SS_MSG_RAM_START_ADDRESS_BASE;

  info->boot_cookie = 0x0;
}
...
/**
 * <!-- msg_ram_set_boot_time -->
 *
 * @brief Save AOP bootup time.
 */ 
void msg_ram_set_boot_time(uint64 start_time, uint64 end_time)
{
  aop_info_0_t *info = (aop_info_0_t*)msg_ram_sections[MSG_RAM_AOP_INFO_0].start;
  info->aop_boot_time[0] = start_time;
  info->aop_boot_time[1] = end_time;
}
...
{
  aop_info_0_t *info = (aop_info_0_t *) msg_ram_sections[MSG_RAM_AOP_INFO_0].start;
  DDR_RESIDENCY_INFO_T *ddr_res = &(info->ddr_residency);
  return ddr_res;
}
 ...
 *
 * @brief Return the pointer to a msg ram memory area where resource vote info are populated
 *
 */
void *msg_ram_get_vote_statis_area (void)
{
  aop_info_0_t *info = (aop_info_0_t *) msg_ram_sections[MSG_RAM_AOP_INFO_0].start;
  uint32_t *vote_info = (uint32_t *)&(info->ddr_residency.drv_arc_vote);
  return vote_info;
}
 ...
 *
 * @brief Return the pointer to a msg ram memory area where starc stats are populated
 *
 */ 
void *msg_ram_get_starc_stats_area (void)
{
  aop_info_0_t *info = (aop_info_0_t *) msg_ram_sections[MSG_RAM_AOP_INFO_0].start;
  STARC_RESIDENCY_INFO_T *starc_res = &(info->starc_residency);
  return starc_res;
}
...
{
  aop_ddr_reg_data_t *info = &((aop_info_0_t*)msg_ram_sections[MSG_RAM_AOP_INFO_0].start)->aop_ddr_reg_data;
  return info;
}
...
{
  return (msg_ram_sections[MSG_RAM_SMEM_TABLE].start+sizeof(aop_smem_table_t));
}
 ...
 *
 */ 
#if !defined(KUNO_STUBS) && !defined(PINNACLE_STUBS)
...
{
  return msg_ram_get_mem_area(MSG_RAM_TZ);
}
#endif
 ...
 *
 * @brief Return the pointer to a msg ram memory area where AOP profiling stats are stored
 *
 */ 
void *msg_ram_get_aop_prof_area (void)
 {
#if defined(AOP_PROF_ENABLED) || defined(DEBUG_LPM_VIOLATORS) || defined(RISCV_TARGET)
   return msg_ram_get_mem_area(MSG_RAM_AOP_PROF);
#else
  return NULL;
#endif 
 }

void msg_ram_memcpy(void* dest, const void *src, int size)
{
  uint8_t *pdest = (uint8_t*) dest;
  uint8_t *psrc = (uint8_t*) src;

  CORE_VERIFY(!(size % sizeof(uint32_t)));  //Always accepts 4byte aligned size to perform word copy
...
void msg_ram_memset(void* dest, int val, int size)
{
  uint32_t *pdest = dest;
  CORE_VERIFY(!(size % sizeof(uint32_t)));  //Always accepts 4byte aligned size to perform word copy
