...
Copyright 2012-2013,2020 by QUALCOMM Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/
/*===========================================================================
...
--------   ---        -------------------------------------------------------
03/23/20   rama       Changes to use config context to get crypto APIs
...
)
{
  ...
  CeMLHashAlgoType alg2;
  boot_crypto_if_type *crypto_if = NULL;
  ...
  do
  {
    ...
    status = boot_config_context_get_ptr(CONFIG_CONTEXT_CRT_HANDLE, CONFIG_CONTEXT_CRYPTO_INTERFACE, (void**)&crypto_if);
    if (status != BL_ERR_NONE || crypto_if == NULL)
    {
      break;
    }
    ...
    if ((ceml_init=crypto_if->CeMLInit())!= CEML_ERROR_SUCCESS)
    {
    ...
    if ((ceml_hash_init=crypto_if->CeMLHashInit(&hash_ctx->ctx, hash_ctx->alg))!=CEML_ERROR_SUCCESS)
    {
    ...
    ceml_hash_set_params = crypto_if->CeMLHashSetParam(hash_ctx->ctx,
...
)
{
  ...
  do
  {
    ...
    */
    ...
    if ((ceml_hash_Update = crypto_if->CeMLHashUpdate(hash_ctx->ctx, iovec)) != CEML_ERROR_SUCCESS)
...
)
{
  ...
  do
  {
    ...
    if ((ceml_hash_final = crypto_if->CeMLHashFinal(hash_ctx->ctx, &iovec)) != CEML_ERROR_SUCCESS )
    {
      status = (bl_error_boot_type)ceml_hash_final;
      break;
    }

    memcpy(digest_ptr, (uint8 *)(iovec.iov[0].pvBase), digest_buf_len);

    /* clear hash on stack */
    qmemset((void *) buf, 0, sizeof(buf));

    crypto_if->CeMLHashDeInit(&hash_ctx->ctx);

    if ((ceml_deinit = crypto_if->CeMLDeInit()) != CEML_ERROR_SUCCESS)
    {
