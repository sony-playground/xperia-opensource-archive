...
/*
 ...
 ============================================================================
 $Header: //components/rel/core.boot/6.1.1/QcomPkg/Include/ChipInfo.h#2 $
 ...
 * @param[out] pDefective Pointer to uint32 used to store the
 *                          defectiveness of the selected part.
 ...
 */
DALResult ChipInfo_GetDefectivePart(ChipInfoPartType ePart, uint32_t *pnMask);

/**
 * Retrieve a mask of CPUs in the current cluster which are marked as
 * defective in the PTE fuses. If the cluster has fewer than 32 CPUs,
 * the additional CPUs are marked as "not defective"
 *
 * @param[in] nCPUCluster CPU cluster to check.
 * @param[out] nMask      Pointer to uint32 to hold a mask of defective
 ...
 *          DAL_SUCCESS otherwise
 */
DALResult ChipInfo_GetDefectiveCPUs(uint32_t nCPUCluster, uint32_t *pnMask);
...
/**
 * @brief Gets product marketing data for the specified part.
 *
 * Restricted to the current chip family and FeatureCode+PCode combination.
 *
 * The marketing data varies for each part. Marketing data for
 * the specified part is stored in the appropriate member of
 * ChipInfoPartMarketingDataType (defined in ChipInfoDefs.h)
 *
 * @param[in] ePart Part whose data is being queried
 * @param[out] pData Caller-allocated buffer to hold marketing data
 * @return ChipInfoResult
 */
ChipInfoResult ChipInfo_GetMarketingData(
    ChipInfoPartType ePart,
    ChipInfoPartMarketingDataType *pData);
