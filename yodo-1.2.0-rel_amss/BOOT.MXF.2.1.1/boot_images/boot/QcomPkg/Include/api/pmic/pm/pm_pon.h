 ...
 *
 *  \n &copy; Copyright (c) 2012-2022 by Qualcomm Technologies Inc.  All Rights Reserved.
 */
...
/*! \enum pm_pon_pbs_fsm_status_type
   \brief PON PBS FSM State machine status type.
 */
typedef enum
{
  PM_PON_PBS_FSM_STATUS_DVDD_CFG,      /* dvdd_cfg state */
  PM_PON_PBS_FSM_STATUS_PON,           /* pon state */
  PM_PON_PBS_FSM_STATUS_POFF,          /* poff state */
  PM_PON_PBS_FSM_STATUS_ON,            /* on state */
  PM_PON_PBS_FSM_STATUS_RESET,         /* reset status */
  PM_PON_PBS_FSM_STATUS_OFF,           /* off state */
  PM_PON_PBS_FSM_STATUS_FAULT,         /* fault state  */
  PM_PON_PBS_FSM_STATUS_WARM_RESET,    /* warm reset state */
  PM_PON_PBS_FSM_STATUS_INVALID        /*INVALID*/
}pm_pon_pbs_fsm_status_type;


/*! \enum pm_pon_hw_fsm_status_type
   \brief PON HW FSM State machine status type.
 */
typedef enum
{
  PM_PON_HW_FSM_STATUS_NO_FAULT,      /* NO FAULT */
  PM_PON_HW_FSM_STATUS_HW_FAULT,      /* HW FAULT */
  PM_PON_HW_FSM_STATUS_INVALID        /* INVALID*/
}pm_pon_hw_fsm_status_type;

/*! \struct pm_pon_fsm_status_type
   \brief PON FSM types.
 */
typedef struct
{
  pm_pon_pbs_fsm_status_type pbs_fsm;  /*PON PBS FSM state types */
  pm_pon_hw_fsm_status_type  hw_fsm;   /*PON HW  FSM state types */
...
typedef enum
{                                                                                                      
  PM_PON_RESET_CFG_WARM_RESET,                                         /* Warm reset PMIC              */
  PM_PON_RESET_CFG_HARD_RESET,                                         /* Hard reset PMIC              */
  PM_PON_RESET_CFG_NORMAL_SHUTDOWN,                                    /* PMIC shutdown                */
  PM_PON_RESET_CFG_D_VDD_BATT_REMOVE_HARD_RESET,                       /* DVDD Hard reset              */
  PM_PON_RESET_CFG_D_VDD_BATT_REMOVE_SHUTDOWN,                         /* DVDD shutdown                */
  PM_PON_RESET_CFG_X_VDD_COIN_CELL_REMOVE_SHUTDOWN,                    /* XVDD shutdown                */
  PM_PON_RESET_CFG_X_VDD_COIN_CELL_REMOVE_HARD_RESET,                  /* XVDD Hard reset              */
  PM_PON_RESET_CFG_IMMEDIATE_X_VDD_COIN_CELL_REMOVE_SHUTDOWN,          /* XVDD+Shutdown                */
  PM_PON_RESET_CFG_WARM_RESET_THEN_D_VDD_BATT_REMOVE_SHUTDOWN,         /* WARMRESET+DVDD Shutdown      */
  PM_PON_RESET_CFG_WARM_RESET_THEN_X_VDD_COIN_CELL_REMOVE_SHUTDOWN,    /* WARM RESET + XVDD Shutdown   */
  PM_PON_RESET_CFG_WARM_RESET_THEN_SHUTDOWN,                           /* Warm Reset + Shutdown        */
  PM_PON_RESET_CFG_WARM_RESET_THEN_HARD_RESET,                         /* Warm reset + Hardreset       */
  PM_PON_RESET_CFG_WARM_RESET_THEN_D_VDD_BATT_REMOVE_HARD_RESET,       /* Warm reset + DVDD Hard Reset */
  PM_PON_RESET_CFG_WARM_RESET_THEN_X_VDD_COIN_CELL_REMOVE_HARD_RESET,  /* Warm reset + XVDD hard reset */
 ...
 */
typedef enum {
  /* PMIC7 PON IRQs,  use PMIC5 Enums for PMIC5 PON Projects (like Aurora)*/
  ...
  /* PMIC5 PON IRQs */
  PM_PON_IRQ_KPDPWR_ON_PMIC5      = PM_PON_IRQ_KPDPWR_N_S2,
  PM_PON_IRQ_RESIN_ON_PMIC5       = PM_PON_IRQ_RESIN_N_S2,
  PM_PON_IRQ_CBLPWR_ON_PMIC5      = PM_PON_IRQ_RESIN_AND_KPDPWR_S2,
  PM_PON_IRQ_KPDPWR_BARK_PMIC5    = PM_PON_IRQ_PMIC_WDOG_S2,
  PM_PON_IRQ_RESIN_BARK_PMIC5     = PM_PON_IRQ_PS_HOLD,
  PM_PON_IRQ_KPDPWR_RESIN_BARK_PMIC5 = PM_PON_IRQ_SW_RESET,
  PM_PON_IRQ_PMIC_WD_BARK         = PM_PON_IRQ_RESIN_N_DEB,
  PM_PON_IRQ_SOFT_RESET           = PM_PON_IRQ_KPD_PWR_N_DEB,
  PM_PON_IRQ_INVALID                   /* */
}pm_pon_irq_type;
...
typedef struct
{
  ...
  unsigned shutdown_seq : 1;   /*shutdown sequence */
  unsigned hard_reset_seq : 1; /*hard reset sequence */
...
typedef struct
{
  ...
  unsigned s3_reset : 4;                /* Stage 3 Reset */
...
{
  unsigned reserved1 : 4;            /* Reserved bits */  
  unsigned fault_n : 1;              /* Triggered by FAULT_N being low for more than expected clock cycles */
...
typedef struct
{
  ...
  uint32 known_rsn : 1;      /* known reason. If unknown read sid, pid and irq field */
  uint32 sid : 4;            /* slave id */
  uint32 pid: 8;             /* peripheral id */
  uint32 irq: 3;             /* interrupt */
...
typedef struct
{
  ...
  uint32 kpdpwr : 1;             /* KPDPWR_N trigger */
  uint32 known_rsn : 1;          /* known reason. If false then read sid, pid and irq field */
  uint32 sid : 4;                /* slave id */
  uint32 pid: 8;                 /* peripheral id */
  uint32 irq: 3;                  /* interrupt */
  uint32 reserved:8;              /* reserved  */
 ...
 *                time in usec, rounds up to the next largest
 *                number in usec if value passed is in between the
 *                permitted values: 60, 120,...,1000, 25000
 *
 *
 * @return  pm_err_flag_type
 *          PM_ERR_FLAG_INVALID_PARAMETER
 ...
 * @param[in] : threshold_mv (Range between 2875 and 3225 )
 * @param[in] : hyst_mv (Range between 175 and 425 )
 * @param[in] : enable (to enable the setting)
 *            
 * @return  pm_err_flag_type 
 *          PM_ERR_FLAG_INVALID_PARAMETER
 *          PM_ERR_FLAG_SUCCESS
...
/**
 * @brief This function configures OVLO Rising Voltage threshold and HYST Voltage threshold
 * 
 * @param[in] pmic_device_index. Primary PMIC: 0
 * @param[in] : threshold_mv (Range between 4200 and 6300 )
 * @param[in] : hyst_mv (Range between 250 and 1000 )
 * @param[in] : enable (to enable the setting)
 *            
 * @return  pm_err_flag_type 
 *          PM_ERR_FLAG_<ERROR_MESSAGE>
 *          PM_ERR_FLAG_SUCCESS
 *
 */
pm_err_flag_type
...
/**
* @brief This function performs target specific settings before 
*       issuing hard reset in ramdump path
*
* @return pm_err_flag_type
*          PM_ERR_FLAG_INVALID_PARAMETER
*          PM_ERR_FLAG_SUCCESS
