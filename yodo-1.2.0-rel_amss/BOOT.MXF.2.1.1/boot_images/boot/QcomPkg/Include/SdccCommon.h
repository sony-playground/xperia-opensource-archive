...
/** @file SdccCommon.h
  ...
  Copyright (c) 2014 - 2018, 2020 - 2022 Qualcomm Technologies, Inc. 
  All Rights Reserved. 
...
----------   ---    --------------------------------------------------------------
2022-02-17   jt     Add new mode for level shifter
2022-01-05   jt     Remove unused modes
2021-07-10   ss     Added bsp config structure in slot_type structure
2020-06-04   jt     Add current speed mode, voltage corner
2018-07-13   jt     Add SD card 100 MHz
2017-10-30   jt     Adjust delay functions
...
#define sdcc_udelay(us) sdcc_bsp_busy_wait(us)
#define sdcc_mdelay(ms) sdcc_bsp_busy_wait(1000*ms)
...
typedef enum
{
   ...
   SDCC_SD_FREQUENCY_100MHZ,      /* SD cards that support up to 100MHz */
...
typedef enum sdcc_card_mode_t
{
   ...
   SDCC_INIT_MODE,
   SDCC_DATA_TRANSFER_SDR50_LS_MODE, 
...
typedef struct sdcc_card_info {
   ...
   BOOLEAN                tuning_done;
   SDCC_CARD_MODE         card_mode; 
   sdcc_bsp_speed_mode    curr_speed_mode;      /* current operating speed mode */
   UINT8                  curr_volt_corner;     /* current operating voltage corner */
} sdcc_dev_type;
...
typedef struct sdcc_slot_data
{
   ...
   sdcc_bsp_config        sdcc_config; /* Configuration Parameters such as speed mode*/
} sdcc_slot_type;
