...
/**
 * Maximum length of a ChipInfoPartType's marketing name string,
 * including the terminating \0.
 *
 * TODO: Should this be part of the interface? What if something comes
 * along with a really long marketing name? We'll need an interface
 * update then, instead of it being an implementation detail.
 */
#define CHIPINFO_MAX_PART_MARKETING_NAME_LENGTH 32
...
/**
 * GPU marketing data.
 * Used by ChipInfo_GetMarketingData when ePart is CHIPINFO_PART_GPU.
 * Details are as specified by HLOS GPU SW.
 */
typedef struct ChipInfoGPUMarketingDataType
{
  uint32_t nChipID;
  uint32_t nVulkanID;
  char szName[CHIPINFO_MAX_PART_MARKETING_NAME_LENGTH];
} ChipInfoGPUMarketingDataType;

/**
 * Container for part-specific marketing data.
 * Each part has different marketing data needs, so each part has its
 * own ChipInfo<Part>MarketingDataType defined above.
 *
 * ePart identifies the MarketingDataType to access.
 */
typedef struct ChipInfoPartMarketingDataType
{
  ChipInfoPartType ePart;
  union
  {
    ChipInfoGPUMarketingDataType GPUData;
    // Structures for other parts.
  };
} ChipInfoPartMarketingDataType;
