/** @file
  ...
  Copyright (c) 2013-2022, 2022 Qualcomm Technologies Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 01/13/20   rh       Adding capture of hotkeys at early boot stage
...
// Key from boot
UINT8     SblBootKeyPress;

// SDAM for key storage
extern UINT16 g_sdam_pmic_idx;
extern UINT16 g_sdam_number;
extern UINT16 g_sdam_address;
...
void ApplySblButtonArray( UINT8 *pButtonArray )
{
   // Only porcess SBL stored keys if array is not empty
   if (SblBootKeyPress == 0)
      return;

   //for Power Key (Disabled since it is always sensed at SBL)
   //*(pButtonArray + 0) |= (SblBootKeyPress & POWER_BUTTON_BMAP) ? TRUE : FALSE;

   //for volume up
   *(pButtonArray + 1) |= (SblBootKeyPress & VOLUME_UP_BUTTON_BMAP) ? TRUE : FALSE;

   //for volume down
   *(pButtonArray + 2) |= (SblBootKeyPress & VOLUME_DN_BUTTON_BMAP) ? TRUE : FALSE;

   //for home
   *(pButtonArray + 3) |= (SblBootKeyPress & HOME_BUTTON_BMAP) ? TRUE : FALSE;
}
...
BOOLEAN PollForKey()
{
  ...
  if (numKeyRead == 1) 
  {
    DEBUG(( EFI_D_INFO, "ButtonsDxeTest: Disable HotkeyRestore %d \r\n", numKeyRead));
    HotKeyRestore = FALSE;
  }
  if (HotKeyRestore)
  {
    DEBUG(( EFI_D_INFO, "ButtonsDxeTest: Poll Key %d \r\n", HotKeyRestore));
    ApplySblButtonArray( pCurrButtonArray );
  }
  ...
  )
{
  ...
  // Reset Hotkey injection for callback event
  numKeyRead--;
}
...
void GetSdamStoredKeys()
{
  EFI_STATUS                    Status;
  UINT8                         data_tmp;
  EFI_QCOM_PMIC_SDAM_PROTOCOL  *sdam_protocol;
  SblBootKeyPress = 0;
  ...
  if (Status != EFI_SUCCESS)
  {
    DEBUG(( EFI_D_ERROR, "ButtonsDxe: ERROR, PMIC SDAM protocol not found!\r\n"));
  }
  Status = sdam_protocol->SdamMemRead(g_sdam_pmic_idx, g_sdam_number, g_sdam_address, 1, &data_tmp);

  if(Status == EFI_SUCCESS)
  {
    SblBootKeyPress = data_tmp;
    HotKeyRestore = TRUE;
    numKeyRead = 0;
    DEBUG(( EFI_D_ERROR, "ButtonsDxeTest: Keypress SDAM data payload %x \r\n", SblBootKeyPress));
  }
  else
  {
    DEBUG(( EFI_D_ERROR, "ButtonsDxe: Keypress PM SDAM read failed \r\n"));
  }
}
...
)
{
  ...
  // Get the key press stored during SBL boot time 
  GetSdamStoredKeys();
