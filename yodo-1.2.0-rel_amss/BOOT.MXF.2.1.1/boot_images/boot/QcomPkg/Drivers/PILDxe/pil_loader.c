...
PIL_STATUS PilLoadElfFile(PIL_SUBSYS_CTX_TYPE* Ctx)
{
      ...
      )
    {
            ...
            !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK)))
        {
          ...
          Status = GetFileSizeOverPxe(Cfg->ImagePath, &ElfSize);
          if (EFI_ERROR(Status)){
            PIL_ERROR_CFG(Cfg, "Failed to read from partition\n");
            return PIL_LOAD_ERR;
          }
          
          ElfBuffer = AllocatePages(ElfSize/EFI_PAGE_SIZE);
          if (ElfBuffer == NULL){
            PIL_ERROR_CFG(Cfg, "Out of resources to allocate %d Pages\n" , ElfSize/EFI_PAGE_SIZE);
            return PIL_LOAD_ERR;
          }

          Status = ReadFileOverPxe(Cfg->ImagePath, ElfBuffer, &ElfSize);
        }
        else
        {
    ...
    else if (Cfg->Type == ELF_SINGLE)
    {
        do {
          ...
          if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) && 
              !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK))) {
            Status = GetFileSizeOverPxe(ElfFilePath, (UINTN*)&ElfSize);
          }
          else {
          ...
          if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) &&
              !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK))) {
            Status = ReadFileOverPxe(ElfFilePath, (VOID*)ElfBuffer, &ElfSize);
          }
          else {
...
PIL_STATUS PilLoadMdtFile(PIL_SUBSYS_CTX_TYPE* Ctx)
{
    ...
    if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) &&
        !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK)))
    {
      MetaDataSize64 = MetaDataSize;
	  Status = GetFileSizeOverPxe(MetaDataFilePath, (UINTN*)&MetaDataSize64);
	  MetaDataSize = (UINT32)MetaDataSize64;
    }
    else
    {
    ...
    if((Cfg->Version >= PIL_SUBSYS_CFG_VERSION_V3) &&
        !StrnCmp(Cfg->MediaType, MEDIA_TYPE_NETWORK, StrLen(MEDIA_TYPE_NETWORK)))
    {
      Status = ReadFileOverPxe(MetaDataFilePath, (VOID*)MetaDataBuffer, (UINTN*)&MetaDataSizeRead);
    }
    else
    {
...
STATIC PIL_STATUS CopyImageName(PIL_SUBSYS_CTX_TYPE* Ctx, CHAR8* Buffer)
{
    ...
    UnicodeStrToAsciiStrS(Start, Buffer, FWNAME_SHORT_LEN);
    return PIL_SUCCESS;
}
