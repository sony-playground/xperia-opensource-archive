  ...
  )
{
  ...
  else if (OperationType == MemOperation)
  {
	...
	{
      Base = RESOURCE_BASE(RootBridgeInstance->ResAperture[PciRbResTypePMem]);
      Limit = RESOURCE_LIMIT(RootBridgeInstance->ResAperture[PciRbResTypePMem]);
	}
  }
  else
  {
  ...
  )
{
  ...
  else
  {
    ...
    Limit = RESOURCE_LIMIT(RootBridgeInstance->ResAperture[PciRbResTypePMem]);
    if ((Address >= Base) && (Address <= Limit))
      Address = PCI_ADDR_TO_SYS_ADDR (Address, RootBridgeInstance->ResAperture[PciRbResTypePMem]);
  }
