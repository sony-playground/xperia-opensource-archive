...
static int read_3rd_party_apps_cmac(void)
{
    ...
    for (uint32_t i = 0; i < MAX_3RD_PARTY_UEFI_APPS ; ++i) {
        ...
        // use a string rather than char as AsciiStrCat() uses string
        char num[2]={0}; 
...
EFI_STATUS spss_iar_main(PIL_SUBSYS_CFG_TYPE* spss_cfg)
{
    ...
    if (m_iar_state == IAR_STATE_ACTIVATED) {
        ...
        AsciiStrToUnicodeStrS(full_name_path, full_name_path_unicode, sizeof(full_name_path));
        ...
        AsciiStrToUnicodeStrS(image_path, spss_cfg->ImagePath, sizeof(image_path));
