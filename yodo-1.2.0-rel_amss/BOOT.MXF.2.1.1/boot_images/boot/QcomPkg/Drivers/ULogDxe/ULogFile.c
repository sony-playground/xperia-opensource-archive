/** @file ULogFile.c
 ...
 Copyright (c) 2014-2015, 2021, Qualcomm Technologies Inc. All rights reserved.
 ...
 */
EFI_STATUS ULogFile_SetLogDirectoryAndSize(VOID)
{
 ...
 */
EFI_STATUS ULogFile_IncrementBootCount(VOID)
{
  ...
  if (sBootCountNeedsUpdate == FALSE) {
    ...
    while (sBootCountStr) {
      ...
      AsciiStrCpyS(BootCountFilePath, AsciiStrLen(sLogDirectory) + AsciiStrLen(ULOGFILE_BOOTCOUNTFILE_NAME) + 1, sLogDirectory);
      AsciiStrCatS(BootCountFilePath, AsciiStrLen(sLogDirectory) + AsciiStrLen(ULOGFILE_BOOTCOUNTFILE_NAME) + 1, ULOGFILE_BOOTCOUNTFILE_NAME);
    ...
    AsciiStrCpyS(sOverlapSeparatorString, sOverlapSeparatorLen+1, ULOGFILE_OVERLAP_SEPARATOR_PREFIX);
    if (sBootCountStr != NULL) {
        AsciiStrCatS(sOverlapSeparatorString, sOverlapSeparatorLen+1, sBootCountStr);
    }
    AsciiStrCatS(sOverlapSeparatorString, sOverlapSeparatorLen+1, "\n");
...
ULogResult ULogFile_Open(ULogHandle * h, CONST CHAR8 *_FileName, UINT32 FileSize)
{
  ...
  *h = hdl;
  ...
  AsciiStrCpyS(FilePath, MAX_PATHNAME, sLogDirectory);
  AsciiStrCatS(FilePath, MAX_PATHNAME, Log->name);
  AsciiStrCatS(FilePath, MAX_PATHNAME, ULOGFILE_METADATAFILE_NAME);
...
ULogResult ULogFile_MakePathFromName(CONST CHAR8 *LogName, CHAR8 *FilePath)
{
  ...
  AsciiStrCpyS(PathStr, AsciiStrLen(sLogDirectory) + AsciiStrLen(LogName) + AsciiStrLen(ULOGFILE_EXTENSION) + 1, sLogDirectory);
  ...
  AsciiStrCatS(PathStr, AsciiStrLen(sLogDirectory) + AsciiStrLen(LogName) + AsciiStrLen(ULOGFILE_EXTENSION) + 1, LogName);
  if (AsciiStrLen(PathStr) < length) {
  ...
  AsciiStrCatS(PathStr, AsciiStrLen(sLogDirectory) + AsciiStrLen(LogName) + AsciiStrLen(ULOGFILE_EXTENSION) + 1, ULOGFILE_EXTENSION);
  if (AsciiStrLen(PathStr) < length) {
  ...
  StrnCpyS((CHAR16 *) FilePath, AsciiStrLen(sLogDirectory) + AsciiStrLen(LogName) + AsciiStrLen(ULOGFILE_EXTENSION) + 1,(CONST CHAR16 *) PathStr, length);
