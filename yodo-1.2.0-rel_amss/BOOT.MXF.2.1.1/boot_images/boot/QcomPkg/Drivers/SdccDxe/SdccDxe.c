    ...
    OUT EFI_STORAGE_DEV_INFO *DevInfo)
{
   ...
   DevInfo->SpecVersion = CardInfo.spec_version;

   /* Get Vendor Id- OEM ID for SD card (2 bytes) */
   DevInfo->VendorId = AllocateZeroPool (4); 
   if (NULL == DevInfo->VendorId) 
   {
      return EFI_OUT_OF_RESOURCES; 
   }
   DevInfo->VendorId[0] = (CardInfo.oem_id >> 8) & 0xff;
   DevInfo->VendorId[1] = CardInfo.oem_id & 0xff; 

   /* Get Product Id- 5 characters */
   DevInfo->ProductId = AllocateZeroPool (sizeof(CardInfo.prod_name)); 
   if (NULL == DevInfo->ProductId) 
   {
      return EFI_OUT_OF_RESOURCES; 
   }
   CopyMem(DevInfo->ProductId, CardInfo.prod_name, sizeof(CardInfo.prod_name)); 

   /* Serial number is a UINT32 and 2 characters represent 1 numerical byte + NULL */
   EfiStatus = HexToStringConverter(CardInfo.prod_serial_num, sizeof(CardInfo.prod_serial_num), &DevInfo->SerialNumber);
   ...
   EfiStatus = HexToStringConverter(CardInfo.prod_rev, sizeof(CardInfo.prod_rev), &DevInfo->FirmwareRevision);
   if (EFI_SUCCESS != EfiStatus)
   {
      return EfiStatus;
   }
