...
2018/04/24   kpa     Replace boot_from_emmc with boot_from_sdcc
2018/04/24   jt      Changes to RegisterListener 
2018/04/12   kpa     Added gEfiSDBootOptionGuid
2018/03/22   jt      Write 0s for EraseBlocks  
2018/03/08   jt      Initial version, specfically for SD card boot

=============================================================================*/

#include "SdccDxe_SDBoot.h"
...
/* SD dummy Erase Protocol */
static EFI_ERASE_BLOCK_PROTOCOL gSdEraseProtocol = {
   EFI_ERASE_BLOCK_PROTOCOL_REVISION,         /* Revision */
   512,                                       /* EraseLengthGranularity */
   SdEraseBlocks                            /* EraseBlocks */
};
   ...
   )
{
   ...
   EFI_TPL PrevTPL;
   SDCC_STATUS rc = SDCC_NO_ERROR;
   UINT8 *EraseBuffer = NULL; 
   UINTN max_transfer_size = 0, transfer_size = 0;

   /* Validate passed-in pointers are not NULL */
   if ((NULL == This) || (NULL == This->Media))
   {
      return EFI_INVALID_PARAMETER;
   }
   ...
   max_transfer_size = This->Media->BlockSize * SDCC_MAX_BLOCK_TRANSFER_SIZE; 
   ...
   while (Size > 0)
   {
      ...
      EraseBuffer = AllocateZeroPool(transfer_size);
      if (NULL == EraseBuffer)
      {
         DEBUG ((EFI_D_ERROR, "SdEraseBlocks NULL EraseBuffer"));
         EfiStatus = EFI_OUT_OF_RESOURCES;
         goto EraseEnd; 
      }

      rc = MMCHSWriteBlocks (This, MediaId, LBA, transfer_size, EraseBuffer); 
      if (rc != SDCC_NO_ERROR)
      {
         DEBUG ((EFI_D_ERROR, "SdEraseBlocks failed, status 0x%x", rc));
         EfiStatus = EFI_DEVICE_ERROR;  
         goto EraseEnd; 
      }
      else
      {
         ...
         LBA = LBA + (transfer_size / This->Media->BlockSize); 
  ...
  )
{
   ...
   status = gBS->InstallMultipleProtocolInterfaces (
                        ...
                        &gEfiEraseBlockProtocolGuid, &MmchsDeviceSd.EmmcErase,
                        NULL
                        );
   ...
   )
{
   ...
   /* Skip processing eMMC BlockIO handles for SD card 
      if not booting from Sdcc 
   */
   if (boot_from_sdcc () == FALSE)
   {       
       return EFI_SUCCESS;
   }
  ...
  if (EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "SdccDxeEntryPoint: Installing SD Boot Option Guid failed, Status = (0x%p)\r\n", Status));
  }
...
Install_SD_Handle:
   ...
   MmchsDeviceSd.EmmcErase      = gSdEraseProtocol; 

   SdCardDetectionHandler(DetectSdCardEvent, NULL);
   ...
   if (Status == EFI_SUCCESS)
   {
     RegisterListener(ScmInitialized, (VOID *)&MmchsDeviceSd.BlkIo); 
   }
   else
   {
