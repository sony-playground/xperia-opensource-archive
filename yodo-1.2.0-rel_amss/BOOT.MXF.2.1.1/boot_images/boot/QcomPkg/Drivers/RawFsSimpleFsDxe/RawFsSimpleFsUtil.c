...
//
EFI_FILE_SYSTEM_INFO customFsInfo = {
...
)
{
    ...
    EFI_FILE_SYSTEM_INFO             *fsInfoOut = NULL;
    EFI_FILE_SYSTEM_VOLUME_LABEL     *fsVolumeLabel = NULL;;
    UINTN                             size = 0;

    if (CompareGuid (InformationType, &gEfiFileSystemInfoGuid))
    {
        // Return filesystem info
        size = (sizeof (EFI_FILE_SYSTEM_INFO)) + (StrSize (pPartitionName)) - (sizeof (CHAR16));

        if (*BufferSize < size)
        {
            DEBUG((EFI_D_INFO, "SimpleFileSystemGetInfo:Buffer too small.\n"));
            *BufferSize = size;
    ...
    {
        // Return Volume Label
        size     = sizeof (EFI_FILE_SYSTEM_VOLUME_LABEL) + StrSize (pPartitionName) - sizeof (CHAR16);
        if (*BufferSize < size)
        {
            DEBUG((EFI_D_INFO, "SimpleFileSystemGetInfo:Buffer too small.\n"));
            *BufferSize = size;
    ...
    {
        DEBUG((EFI_D_WARN, "[RawFsSimpleFsDxe]SimpleFileSystemGetInfo:Information type not supported.\n"));
        return EFI_UNSUPPORTED;
    }
}
