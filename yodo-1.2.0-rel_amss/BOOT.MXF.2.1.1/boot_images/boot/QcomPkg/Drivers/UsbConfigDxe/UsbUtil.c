  ...
  )
{
  ...
  // No need of waiting for clock to be disabled and check whether its still enable or not.
  //These checks are already taken care in Clock Framework. But still keeping the code for future debug scenarios.
  #if 0
  ...
  )
{
  ...
  // reset clock.
  Status = ClockProtocol->ResetClock(ClockProtocol, ClockId, AssertFlag);

  ERR_CHK ("failed to reset %a clock", ClockName);
