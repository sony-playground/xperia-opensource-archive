...
)
{
    ...
    errFlag = pm_comm_write_byte(WriteParam.SlaveID, WriteParam.Address, WriteParam.Data);  // 1:0  
...
)
{
    ...
    switch(DriverTypeFunction.HapticsTypeFunction)
    {
       ...
       case EFI_PM_TEST_HAPTICS_AUTO_RES_CONFIG:
       {
           ...
           errFlag = pm_haptics_auto_res_cfg(autores.DeviceIndex, (pm_haptics_lra_res_calib_type)autores.CalibType , autores.CalibEop, (pm_haptics_lra_high_z_type)autores.ZType,
          autores.CalDuration, (pm_haptics_lra_auto_res_mode_type)autores.ModeType);
           break;
       }
...
)
{
    ...
    switch(DriverTypeFunction.PONTypeFunction)
    {
        ...
        {
              // Comment out below code since API pm_pon_overtemp_reset_cfg has been removed. Need to add it back before we uncomment below code
...
//            errFlag = pm_pon_overtemp_reset_cfg(OvertempResetCfg.DeviceIndex, (pm_pon_reset_cfg_type)OvertempResetCfg.ResetCfgType);
        ...
        {
              // Comment out below code since API pm_pon_get_soft_reset_reason has been removed. Need to add it back before we uncomment below code
...
)
{
    ...
    switch(DriverTypeFunction.SPMIDebugTypeFunction)
    {
        case EFI_PM_ENABLE_DEBUG_SPMI:
        {                                                                            
         errFlag = pm_comm_enable_debug_spmi_if ();
         break;
      }
      default:
         break;
    ...
    switch(DriverType)
    {
      ...
      case EFI_PM_COINCELL:
       ...
       case EFI_PM_BOB:
       {
        ...
        case EFI_PM_SPMI_DBG:
        {
            errFlag = TestSPMIDbgDriver(DriverTypeFunction,DriverParam,DriverOut);
           break;
        }
        default:
        {
...
void 
EFIAPI EFI_PmicPrmOsAuxDevInit()
{
  static BOOLEAN Initialized = FALSE;

  if(!Initialized)
  {
    pm_prm_os_aux_dev_init();
    Initialized = TRUE;
  }
}
