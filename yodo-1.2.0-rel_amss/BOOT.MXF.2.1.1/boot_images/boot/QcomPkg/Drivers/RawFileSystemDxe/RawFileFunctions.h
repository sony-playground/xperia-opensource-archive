...
[In] pPartitionGuid             --  Pointer to GUID of the partition (DPP, SMBIOS, etc.)
...
[In]  RawFSBlockIo                  --   Pointer to handle for Block IO protocol
[In]  pPartitionGuid                --   Pointer to GUID of the partition (DPP, SMBIOS, etc.)
                                         If NULL, always reads the header data from file system. 
                                         Othwerwise, uses cached header
...
[In]  pNewfileInfo          --  Structure that carries FILE_ENTRY and file data for the incoming file
[In]  RawFSBlockIo          --  Pointer to handle for Block IO protocol
[In]  pPartitionGuid         --  Pointer to Partition GUID
@return
EFI_SUCCESS
Other standard EFI_STATUS codes in the event of an error
*/
...
[In]  RawFSBlockIo          --  Pointer to handle for Block IO protocol
[In]  pHeaderBlocks         --  Pointer to FS_BLOCK structure that contains the header block start, number of blocks and block buffers
[In]  ppFSHeader            --  Pointer to a pointer to the start of the FS_HEADER information
[In]  pPartitionGuid        --  Pointer to GUID of the partition (DPP, SMBIOS, etc.)
...
/*
@brief
Function to find a file in the file system directory.
@param
[In]     pDirectoryName   --  Pointer to caller allocated memory that will hold the name of the directory
[In]     pPartitionGuid   --  Pointer to GUID of the partition (DPP, SMBIOS, etc.)
[In]     pFiletoRead      --  Pointer to caller allocated memory that will hold the filename 
                              whose size need to be returned.
[Out]    pFileSize        --  Pointer to UINT to store file size
[Out]    pNextFile        --  Pointer to next filename.
                              If pFiletoRead is NULL, pNextFile holds the first found file name in directory.
                              Otherwise, pNextFile holds the next file name found after pFiletoRead.
                              If no more files exist, nextfile returns NULL.
[In]     RawFSBlockIo     --  Pointer to handle for Block IO protocol
