...
/*=========================================================================
      Forward references
==========================================================================*/
extern ClockResult Clock_LogState ( ClockHandle hClient, ULogHandle  hULog, uint32_t    nFlags );
...
{
  DALResult status;
  status = Clock_LogState(pDrvCtxt->hClient, hULog, nFlags);
  return status;
}
...
(
  ...
  const char                  *szSource;
  CHAR16                       sUnicodeName[CLK_NAME_SZ];
  ...
  for (i = 0; i < pBSP->nNumClocks; i++)
  {
    ...
    if (pmClockDesc->nDebugMuxSel != HAL_CLK_NO_DEBUG_MUX_SEL)
    {
      if (pClock->pDebugMux != NULL)
      {
        if(strncmp(pClock->pDebugMux->szName,"apcs",4)!= 0)
        {
          nFreqHz = 0;
          bCalcSupported = TRUE;
          eResult = Clock_GetClockId(pDrvCtxt->hClient, pClock->szName, &nClockId);
          Clock_MeasureClockFrequency(pDrvCtxt->hClient, nClockId, &nFreqHz);
        }
      }
    }
    ...
    if (bCalcSupported)
    {
      DEBUG(( EFI_D_ERROR, " %s,\t%d,\t%d,\t%d,\t%u,\t%d,\t%d,",
         sUnicodeName, nIsOn, nIsEnabled, nReset, nFreqHzConfig,
         nFreqHz, pClock->pDomain->VRegRequest.eVRegLevel));
    ...
    {
      DEBUG(( EFI_D_ERROR, " %s,\t%d,\t%d,\t%d,\t%u,\t-1,\t%d,",
        sUnicodeName, nIsOn, nIsEnabled, nReset, nFreqHzConfig,
        pClock->pDomain->VRegRequest.eVRegLevel));
...
//  ULogHandle    ULhandle;
  ...
  case BDSClockLogStateToFile:
    ...
    status = Clock_LogState(pDrvCtxt->hClient, ULhandle, nLogMask);
    break;
#endif
