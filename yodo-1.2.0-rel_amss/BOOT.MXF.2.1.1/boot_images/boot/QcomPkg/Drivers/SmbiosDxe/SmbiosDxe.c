  ...
  )
{
  ...
  *NumberOfStrings = 0;
  ...
  while (*CharInStr != 0 || *(CharInStr+1) != 0) { 
    ...
    for (StrLen = 0 ; StrLen < gMaxLen; StrLen++) {
  ...
  for (Link = Head->ForwardLink; Link != Head; Link = Link->ForwardLink) {
    ...
    if ((UINTN)Record->Handle == *SmbiosHandle) {
      ...
      if (InputStrLen == TargetStrLen) {
        AsciiStrCpyS(StrStart, AsciiStrLen(String)+1, String);
        EfiReleaseLock (&Private->DataLock);
        return EFI_SUCCESS;
      }
  ...
  )
{
  ...
  if (mPrivateData.Smbios.MajorVersion < 2 || (mPrivateData.Smbios.MajorVersion == 2 && mPrivateData.Smbios.MinorVersion < 7)){
    gMaxLen = SMBIOS_STRING_MAX_LENGTH;
  } else if (mPrivateData.Smbios.MajorVersion < 3) {
    //
    ...
    //
    gMaxLen = SMBIOS_TABLE_MAX_LENGTH;
  } else {
    //
    ...
    //
    gMaxLen = SMBIOS_3_0_TABLE_MAX_LENGTH;
  }
  //
