/** @file UefiPlatCfg.c
  ...
  Copyright (c) 2012-2019, 2021, 2022 Qualcomm Technologies, Inc.  All 
...
void ProcessMemoryMapTokens (UINT8* Section, UINTN SectionOffset, UINT8* TokenPtrArray[], UINTN MaxTokens)
{
  ...
  AsciiStrCpyS (MemCfgName, MAX_MEM_LABEL_NAME, (CHAR8*)TokenPtrArray[2]);
  ...
  AsciiStrCpyS (mMemRegions[mNumMemRegions].Name, MAX_MEM_LABEL_NAME, MemCfgName);
...
ParseChipIDConfigParameters (UINT8* Section, UINTN SectionOffset, UINT8* TokenPtrArray[], UINTN MaxTokens)
{
  ...
  if (ChipIDConfigNum == ChipIDVal)
  {
    ...
    if (Index < ConfigTableEntryCount)
    {
        ...
        AsciiStrCpyS (ConfigTable[Index].Value, MAX_MEM_LABEL_NAME, (CHAR8*)TokenPtrArray[2]);           
    }
  }
}
