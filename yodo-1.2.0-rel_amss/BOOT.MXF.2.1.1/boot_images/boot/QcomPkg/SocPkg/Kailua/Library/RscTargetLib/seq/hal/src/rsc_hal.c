 ...
 */
void rsc_hal_config_br_addr(rsc_handle handle,uint8* rsc_base, uint32 idx, uint16 val)
{

if(rsc_is_cam(handle))
  HWIO_OUTXI(rsc_base, RSC_SEQ_CFG_BR_ADDR_i_CHN_TCS_DRV0, idx, val);
else  
 ...
 */
uint16 rsc_hal_copy_simple_cmd_seq(uint8 *rsc_base, uint16 start_adr, rsc_cmd_t* cmds, uint16 size,rsc_handle handle)
{
  uint16 max_cmds     = 0;
  ...
  for(i = 0; i < size; i++, addr++)
  {
	...
	if (rsc_is_cam(rsc)){
			 HWIO_OUTXMI(rsc_base, SEQ_MEM_CHN_TCS_m_DRV0,
             ((addr-1)/RSC_HAL_SIMPLE_CMDS_PER_REG), mask, val);
		 }
	else{
			 HWIO_OUTXMI(rsc_base, SEQ_MEM_m_DRV0, ((addr-1)/RSC_HAL_SIMPLE_CMDS_PER_REG), mask, val);
		 }
 ...
 * @return Number of commands copied to actual RSC HW (In successful call
          it must be equal to size).
 */
uint16 rsc_hal_copy_extended_cmd_seq(uint8 *rsc_base, uint16 start_adr, rsc_cmd_t* cmds, uint16 size,rsc_handle handle)
{
  uint16 max_cmds     = 0;
  uint16 cmds_copied  = 0;
  uint32 mask         = 0;
 ...
 * @return Number of commands copied to actual RSC HW (In successful call
          it must be equal to size).
 */
uint16 rsc_hal_copy_32_cmd_seq(uint8 *rsc_base, uint16 start_adr, rsc_cmd_t* cmds, uint16 size,rsc_handle handle)
{
  uint16 max_cmds     = 0;
  uint16 cmds_copied  = 0;
  ...
  max_cmds = rsc_hal_get_num_seq_cmd_words(rsc_base) * RSC_HAL_32_CMDS_PER_REG;
  ...
  for(i = 0; i < size; i++, addr++)
  {
        ...
        {
		if (rsc_is_cam(rsc)){
			HWIO_OUTXMI(rsc_base, SEQ_MEM_CHN_TCS_m_DRV0, ((addr-1)/RSC_HAL_32_CMDS_PER_REG), mask, val);
			}
		else{
			HWIO_OUTXMI(rsc_base, SEQ_MEM_m_DRV0, ((addr-1)/RSC_HAL_32_CMDS_PER_REG), mask, val);
			}
		}
	val =((uint32)(cmds[i]));
    cmds_copied++;
 ...
 * @param rsc_base: RSC instance's base address.
 */
void rsc_hal_conf_hidden_tcs_owner(uint8 *rsc_base, uint8 drv_id)
{
  HWIO_OUTXF(rsc_base, HIDDEN_TCS_CTRL_DRV0, HIDDEN_TCS_PDC_TIMER_DRV_OWNER, drv_id);
}
