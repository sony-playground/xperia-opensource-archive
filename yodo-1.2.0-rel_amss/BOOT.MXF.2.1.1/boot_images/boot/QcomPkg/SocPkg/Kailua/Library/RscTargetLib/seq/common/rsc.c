  ...
  if (NULL != mode->branch_info)
  {
      for (i = 0; i < mode->br_count; i++)
      {
        if (0 != mode->branch_info[i].br_addr)
        {
          ...
          rsc_hal_config_br_addr(handle, handle->addr, i, addr);
        }
      }
  }
                                      ...
                                      uint16 seq_length, uint32 *mode_id)
{
  ...
  uint16 (*copy_cmd_seq)(uint8*,  uint16, rsc_cmd_t*, uint16, rsc_handle);
  ...
  cmds_copied = copy_cmd_seq(handle->addr, handle->copied_cmds, seq, seq_length, handle);
...
rsc_result_t rsc_conf_hidden_tcs_ownership(rsc_handle handle, uint8 drv_id)
{
  ...
  rsc_hal_conf_hidden_tcs_owner(handle->addr, drv_id);
