...
2022-07-18   rn      Updated the sdcc_bsp_gpio_control for kailua
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
         ...
         if (NULL != fdt_handle)
         {
            ...
            {
               major_version = (chip_version >> 16) & 0xffff; 
               snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/sdc%d/HsrSettings/SDR104/V%d", slot_num, major_version);
            }
...
sdcc_bsp_err_type sdcc_bsp_volt_set (UINT32 driveno, sdcc_bsp_regulator_mode mode)
{
   ...
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      ...
      {
         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_mode(PMIC_G, PM_LDO_3, PM_SW_MODE_NPM))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_G, PM_LDO_3, PM_ON, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }

         if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_G, PM_LDO_3, 1200 * 1000, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }
      ...
      else if (SDCC_BSP_REGULATORS_OFF == mode) 
      {
         ...
         if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_G, PM_LDO_3, PM_OFF, TRUE))
         {
            return SDCC_BSP_ERR_PMIC; 
         }
      }
