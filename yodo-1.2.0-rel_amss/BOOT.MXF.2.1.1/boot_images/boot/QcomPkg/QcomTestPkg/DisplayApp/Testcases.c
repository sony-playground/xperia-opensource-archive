...
static EFI_STATUS DisplayPatternTest()
{
  ...
  {
    AsciiPrint("DisplayFillBackground: QueryMode() failed.\n");
  }
  else if (EFI_SUCCESS != (eStatus = DisplayFillBackground(LIGHT_GREY, 0, 0, pModeInfo->HorizontalResolution, pModeInfo->VerticalResolution)))
  {
    AsciiPrint("DisplayPatternTest: DisplayFillBackground() failed.\n");
  }
  else if (EFI_SUCCESS != (eStatus = DisplayFillBackground(DARK_GREY, pModeInfo->HorizontalResolution / 2, 0, pModeInfo->HorizontalResolution / 2, pModeInfo->VerticalResolution)))
  {
    AsciiPrint("DisplayPatternTest: DisplayFillBackground() failed.\n");
  }
...
{
  EFI_STATUS                        eStatus                  = EFI_SUCCESS;
  EFI_GUID                          sDisplayEdidActiveGUID   = EFI_EDID_ACTIVE_PROTOCOL_GUID;
  EFI_EDID_ACTIVE_PROTOCOL         *pDisplayEdidActiveProtocol;


  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol(&sDisplayEdidActiveGUID,
                                                    NULL,
                                                    (VOID **)&pDisplayEdidActiveProtocol)))
  {
    AsciiPrint("DisplayPanelEdidTest: LocateProtocol() failed.\n");
    eStatus = EFI_NOT_FOUND;
  }
  else
  {
