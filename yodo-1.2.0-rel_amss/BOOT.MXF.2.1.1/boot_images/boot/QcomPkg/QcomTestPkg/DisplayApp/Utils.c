...
static void GetPanelName(CHAR8 *pConfig, uint32 uConfigLength, CHAR8 *pPanelName, uint32 uPanelNameLength)
{
       ...
       (NULL != (pTagEnd   = AsciiStrStr(pConfig, "</PanelDescription>"))))
   {
     ...
     AsciiStrnCpyS(pPanelName, uPanelNameLength, pNameStart, uCopyAmount);        
   }
   ...
   {
     AsciiStrnCpyS(pPanelName, uPanelNameLength, "Panel (Unknown configuration)", AsciiStrLen("Panel (Unknown configuration)"));
   }
}
...
/* DisplayUtilProfilingLogs Helper
*/
static EFI_STATUS DisplayUtilProfilingLogs(CHAR8* pCommandLine, CHAR8* pArgs, UINTN uNumArgs)
{
  EFI_QCOM_DISPLAY_UTILS_PROTOCOL *pDisplayUtilsProtocol  = NULL;
  EFI_STATUS                       eStatus                = EFI_SUCCESS;
  ...
  if ((EFI_SUCCESS !=  eStatus) ||
      (NULL        ==  pDisplayUtilsProtocol))
  {
    AsciiPrint("Error: Locate display utils protocol failed with status: %r\n", eStatus);
  }
  else
  {
    // Dump profiling logs
    if (EFI_SUCCESS != (eStatus = pDisplayUtilsProtocol->DisplayUtilsGetProperty(EFI_DISPLAY_UTILS_PROFILING_LOG, NULL, NULL)))
    {
      AsciiPrint("Error: Display utils failed to dump profiling logs (status: %r)\n", eStatus);
    }
  }
...
UtilCmdEntry sUtilList[] = //zwy need modify this
{
    ...
    {"enableimagefv",
     ...
     {"profilerlogs",
      "Display performance profiling logs.",
      &DisplayUtilProfilingLogs},
