...
EFI_STATUS PmicNpatestApp_ProcessCommand (UINTN Argc, CHAR8** Argv, boolean *createClient, boolean *issueRequest)
{
  ...
  if((0 == AsciiStrCmp(arg_Array[0], "cc") )|| (0 == AsciiStrCmp(arg_Array[0], "CC")) || (1 == AsciiStrDecimalToUintn(arg_Array[0])))
   *createClient = TRUE;
  else
   *createClient = FALSE;
  if ((0 == AsciiStrCmp(arg_Array[1], "ir") )|| (0 == AsciiStrCmp(arg_Array[1], "IR")) || (1 == AsciiStrDecimalToUintn(arg_Array[1])))
    *issueRequest = TRUE;
  else
    *issueRequest = FALSE;
