  ...
  if ((*Walker)[0] == L'\"') {
    ...
    if (NextDelim - ((*Walker)+1) == 0) {
      ...
      *Walker = NextDelim + 1;
      ...
      StrnCpyS(*TempParameter, StrSize(*TempParameter)/sizeof (CHAR16), (*Walker)+1, NextDelim - ((*Walker) + 1));
      *Walker = NextDelim + 1;
    } else {
      //
      // last one... someone forgot the training quote!
      //
      StrCpyS(*TempParameter, StrSize(*TempParameter), *Walker);
      *Walker = NULL;
    }
...
*/
  ...
  *Argc = 0;
  ...
  while(Walker != NULL && *Walker != CHAR_NULL) {
    ...
    StrCpyS(NewParam, Size, TempParameter);
