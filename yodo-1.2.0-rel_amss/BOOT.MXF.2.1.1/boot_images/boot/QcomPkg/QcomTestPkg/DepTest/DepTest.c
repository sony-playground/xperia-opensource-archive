...
# Test command           Description                                                 Expected Results - Win                  Expected results - LA
===========================================================================================================================================================================
1 DepTest "-Alloc Code"  Trying to execute from code area, which is expected         No crash; Test should pass              Same as WP
2 DepTest "-Alloc Data"  Trying to execute from data area, should not be allowed     Should crash with "Instruction abort"   Same as WP
3 DepTest "-I FontDxe"   Trying to overwrite existing code area, which is marked RO  Should crash with "Data abort:"         Same as WP
4 DepTest "-A"           Same as Case-3 above                                        Should crash with "Data abort:"         Same as WP
5 DepTest                No option defaults to "-Alloc Data"                         Results should be same as "-Alloc Data" Same as WP
6 DepTest -X             Ignore; Dummy for now
7 DepTest -R             Ignore; Dummy for now, later can be enhanced to test RO data

  Copyright (c) 2016, 2018-2020,2021 Qualcomm Technologies Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/08/19   md      Fix -A and -I test failure. 
 01/18/18   ps      Kw error fix & test cases description
 02/26/16   vk      Fix KW
...
ProcessCommand (UINTN Argc, CHAR8** Argv)
{
  ...
  else if (AsciiStriCmp ((CHAR8*)CmdStr, "-I") == 0)
  {
    ...
    AsciiStrCpyS (TestImage, MAX_IMG_PATH_LEN, Argv[1]);
...
TestSingleImage (IN CHAR8* ImageName)
{
  ...
  for (i = 0; i < NumDbgImages; i++)
  {
    ...
    {  
      ...
      if (pName != NULL)
      {
        CurrentImage = AsciiStrStr(pName, "/DEBUG/");
        ...
        if (CurrentImage == NULL)
        {
          AsciiPrint("not this image \n");
          continue;
        }
        else
        {
          CurrentImage += AsciiStrLen ("\\DEBUG\\");
        }
      }
      if ((CurrentImage != NULL) && (AsciiStriCmp (ImageName, CurrentImage) == 0))
      {
        ...
        AsciiPrint ("[TEST]\n");
        return TestExecImage (Base, Size);
      }
      else
      {
          AsciiPrint("not this image \n");
...
TestAllImages (VOID)
{
  ...
  for (i = 0; i < NumDbgImages; i++)
  {
    ...
    {  
      ...
      if (pName != NULL && CurrentImage == NULL)
        CurrentImage = AsciiStrStr(pName, "\\DEBUG\\");
...
ClearExecNever (UINT64 Addr, UINT64 Size)
{
  ...
  //AsciiPrint ("%a():Clear XN, Start = 0x%016lp Size = 0x%016lx \n", __FUNCTION__, Addr, Size);
#if 0 
...
TestAlloc (VOID)
{
  ...
  if (TestOption == TestAllocCode)
  {
    ...
    AsciiPrint ("Expect device to Not crash; Test should pass \n");
  }
  else if(TestOption == TestAllocData)
    AsciiPrint ("Expect a crash with Instruction/Prefetch abort \n");
  ...
  )
{
  ...
  else if (TestOption == TestOneImg)
  {
    ...
    if (AsciiStrLen (TestImage) != 0) 
    {
      AsciiStrCatS (TestImage, MAX_IMG_PATH_LEN, ".dll");      
