  ...
  )
{
  ...
  CONST CHAR8        *ClockName = "gcc_sdcc1_apps_clk";
  CONST CHAR8        *FallBackClockName = "gcc_sdcc2_apps_clk";
  ...
  if (ClockProtocol && (EFI_SUCCESS == Status))
  {
    ...
    if(EFI_SUCCESS != Status)
    {
      ...
      Status = ClockProtocol->GetClockID (ClockProtocol, FallBackClockName, &ClockId);
      if(EFI_SUCCESS != Status)
      {
        AsciiPrint ("ERROR: Failed to Get clock ID for %a\r\n", FallBackClockName);
        return Status;
