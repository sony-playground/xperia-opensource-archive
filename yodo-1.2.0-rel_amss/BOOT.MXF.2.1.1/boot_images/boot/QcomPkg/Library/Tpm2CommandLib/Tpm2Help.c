...
--------    ---    ----------------------------------------------------------
03/31/21    shl     Added parameter check 
10/08/20    shl     Added pointer checking and CopyBufferToDigestList() 
  ...
  )
{
  ...
  if( DigestList == NULL || DigestList->count > HASH_COUNT )
  {
     DEBUG ((DEBUG_ERROR, "Parameter check failed.\n"));
     return FALSE;
  }
  ...
  )
{
  ...
  if( Buffer == NULL || DigestList == NULL || DigestList->count > HASH_COUNT )
  {
     DEBUG ((DEBUG_ERROR, "Parameter check failed.\n"));
     return Buffer;
  }
  ...
  )
{
  ...
  DigestList->count = SwapBytes32( *((UINT32 *)Buffer) );
  Buffer = Buffer +  sizeof(UINT32);

  if( DigestList->count > HASH_COUNT )
  {
     DEBUG ((DEBUG_ERROR, "Hash count is too big.\n"));
     return EFI_INVALID_PARAMETER;
  }

  for (Index = 0; Index < DigestList->count; Index++)
  {
     DigestList->digests[Index].hashAlg = SwapBytes16( *((UINT16 *)Buffer) );
     Buffer = Buffer + sizeof(UINT16);

     DigestSize = GetHashSizeFromAlgo (DigestList->digests[Index].hashAlg);
     if( DigestSize == 0 )
     {
        DEBUG ((DEBUG_ERROR, "Wrong hash Algo.\n"));
        return EFI_INVALID_PARAMETER;
     }

     CopyMem (&DigestList->digests[Index].digest, Buffer, DigestSize);
     Buffer = Buffer + DigestSize; 
  }
...
/**
  ...
  @return TPML_DIGEST_VALUES data size, 0 means failed.
**/
UINT32
EFIAPI
GetDigestListSize (
  ...
  )
{
  ...
  if( DigestList == NULL || Digest == NULL || DigestList->count > HASH_COUNT )
     return EFI_INVALID_PARAMETER;
