...
----------   ---   ---------------------------------------------------------
2022-01-19   wek   Remove lint/Klocwork warning.
2021-04-23   wek   Add flush operation to storage.
2020-06-12   cm    Fix issue of accessing raw data in case of sdcard
2018-01-23   wek   Fix issue for firmware upgrade returning fail on success
...
dp_res_t devprg_storage_sdcc_flush(void *device)
{
  (void) device;
...
{
  struct dp_storage_sdcc *dev = (struct dp_storage_sdcc *)device;
  SDCC_STATUS result = SDCC_ERR_UNKNOWN ;
  sdcc_mem_info_type info = {0};
                                      ...
                                      struct devprg_storage_info *dev_info)
{
  ...
  memset (&info, 0, sizeof(sdcc_mem_info_type));
  result = sdcc_handle_mem_get_device_info(dev->handle, &info);

  if (result != SDCC_NO_ERROR || info.block_len == 0)
  {
    error = translate_sdcc_error(result);
    return error;
  }
                                       ...
                                       uint32 size_in_bytes)
{
  ...
  memset (&sdcc_info, 0, sizeof(sdcc_mem_info_type));
  ...
  if(sdcc_info.block_len == 0 || size_in_bytes % sdcc_info.block_len != 0)
  {
...
dp_res_t devprg_storage_sdcc_info_raw_size(void *device, uint32 *size)
{
  ...
  if (info.card_type == SDCC_CARD_MMCHC)
    *size = 532; /* 532 is the same value as EMMC_RAW_DATA_SIZE in the
                  sdcc driver */
  else
    *size=0;
                                            ...
                                            uint32 *size)
{
  ...
  if (error == DEVPRG_SUCCESS)
  {
    ...
    for(i = 0; i < raw_size; i++)
    {
      ...
      if (result < 0 || result > 2) /* Compare with 2 to get around lint/SABOP warning. */
      {
        error = DEVPRG_ERROR_NOMEM;
        goto cleanup;
      }
