...
----------   ---   --------------------------------------------------------- 
2022-04-28   jt    Add sync cache 
2022-02-18   ssb   Initialized structures info, ufs_config and ufs_info
...
void *devprg_storage_ufs_open(uint32 slot, uint32 partition, dp_res_t *error)
{
  ...
  else if (partition == DEVPRG_BOOT_PARTITION_1)
    partition = 1;
  else if (partition == DEVPRG_BOOT_PARTITION_2)
    partition = 2;
...
dp_res_t devprg_storage_ufs_flush(void *device)
{
  struct dp_storage_ufs *dev = (struct dp_storage_ufs *) device; 
  int32 result; 
  dp_res_t error; 

  result = ufs_sync_cache (dev->handle, 0, 0);  
...

{
  ...
  config->bHPBControl = configure->bHPBControl;
  ...
  for(i = 0; i < number_of_luns; i++)
  {
    ...
    dest->wLUMaxActiveHPBRegions = src->wLUMaxActiveHPBRegions;
    dest->wHPBPinnedRegionStartIdx = src->wHPBPinnedRegionStartIdx;
    dest->wNumHPBPinnedRegions = src->wNumHPBPinnedRegions;
  }
}
                                      ...
                                      struct devprg_storage_cfg_data *configure)
{
  ...
  struct ufs_lun_config_descr ufs_config = {0}; 
