...
#define DEVPRG_VIP_TABLE_SIZE 16384

/* TME requires the VIP buffer to be 4k aligned so it can be locked.
 * The lock granularity is 4k, therefore the start and size should
 * be a multiple of 4k to avoid locking data that does not belong
 * to VIP. */
#if ((DEVPRG_VIP_TABLE_SIZE % DEVPRG_VIP_BUFFER_ALIGNMENT) != 0)
  #error "VIP buffer must be a multiple of the alignment"
#endif
...
static char devprg_vip_hash_table[DEVPRG_VIP_TABLE_SIZE] __attribute__((__aligned__(DEVPRG_VIP_BUFFER_ALIGNMENT)));
