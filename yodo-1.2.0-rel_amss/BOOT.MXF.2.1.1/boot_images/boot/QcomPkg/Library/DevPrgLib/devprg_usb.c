...
static void devprg_usb_al_bulk_shutdown(void)
{
...
dp_res_t devprg_usb_set_mode(enum devprg_transport_mode_key key,
                             unsigned int val)
{
  dp_res_t status = DEVPRG_SUCCESS;

  switch (key)
  {
  case DEVPRG_USB_ZLP_KEY:
    devprg_usb_al_bulk_set_zlp_mode(val);
    break;
  case DEVPRG_TRANSPORT_SHUTDOWN_KEY:
    devprg_usb_al_bulk_shutdown();
    break;
  default:
    status = DEVPRG_ERROR_INVAL_PARAM;
  }
