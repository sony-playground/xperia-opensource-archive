/** @file FwGptHelpers.c
  ...
  Copyright (c) 2011-2016, 2018, 2022 Qualcomm Technologies, Inc . All rights reserved.
   ...
   )
{
    ...
    ACTIVE_GPT_SUB_DATA *BootGPTSubDataPtr    = NULL;
    ACTIVE_GPT_SUB_DATA *GppGPTSubDataPtr     = NULL;
    ACTIVE_GPT_SUB_DATA **pBootGPTSubDataPtr = NULL;
    ACTIVE_GPT_SUB_DATA **pGppGPTSubDataPtr = NULL;
    ...
    if(BackupBootGpt == TRUE)
    {
      pBootGPTSubDataPtr = &BootGPTSubDataPtr;
    }
    
    if(BackupGppGpt == TRUE)
    {
      pGppGPTSubDataPtr = &GppGPTSubDataPtr;
    }
    
    Status = AllocateAndInitGPTBackupData(pBootGPTSubDataPtr, pGppGPTSubDataPtr);
    ...
    )
{
    ...
    EFI_GUID   BOOT_GUID;
    EFI_GUID   GPP_GUID;
    BOOLEAN    BootHasUpdate    = FALSE;
    BOOLEAN    GppHasUpdate     = FALSE;
    ...
    //make default value as flase and then let it modify by following checks
    //because there is chance of skipping to fill value in one of the parmeter
    *SwitchBootGpt = FALSE;
    *SwitchGppGpt = FALSE;

    Status = CheckIfStorageHasNewContent(List, List->Count, &BootHasUpdate, &GppHasUpdate);
    HANDLE_ERROR_LABEL(Status, CheckIfStorageHasNewContent, GetAllSwitchGptRequestsFromFwEntryListExit);
