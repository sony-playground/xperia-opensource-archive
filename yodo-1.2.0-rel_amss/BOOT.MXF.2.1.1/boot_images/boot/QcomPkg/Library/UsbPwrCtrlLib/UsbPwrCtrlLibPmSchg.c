...
/**
  Gets the Charger Active port.

  @param[OUT]   ChgActivePort   Pointer to active port number
...
*/
EFI_STATUS
UsbPwrCtrlLibPmSchg_GetChgActivePort(
  UINT8 *ChgActivePort)
{
  EFI_STATUS Status = EFI_SUCCESS;

  //Sanity Check
  if (NULL == ChgActivePort)
  {
    return EFI_INVALID_PARAMETER;
  }
  ...
  Status = PmicSchgProtocol->GetActivePort(ChgActivePort);
