                ...
                OUT CHAR8        ***ArgvArrayPtr)
{
  ...
  *ArgvArrayPtr = ArgStrPtr;
  ...
  if (WideCharArg)
    CmdStatus = UnicodeStrToAsciiStrS ((CHAR16 *)CmdStr, (AllocdMem + ArgsSize), StrLen ((CHAR16 *)CmdStr) + 2 );
  else
    CmdStatus = AsciiStrnCpyS ((AllocdMem + ArgsSize), StrLen ((CHAR16 *)CmdStr) + 2, CmdStr, AllocSize - ArgsSize);
