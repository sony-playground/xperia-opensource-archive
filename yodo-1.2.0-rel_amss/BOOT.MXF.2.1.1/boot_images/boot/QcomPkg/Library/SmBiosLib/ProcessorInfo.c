...
SMBIOS_PROCESSOR_INFO smbiosProcessorInfo =
{
  ...
  1536,                    // CurrentSpeed - Current speed in MHz
  ...
  5,                       // ***String5 - SerialNumberStringNum
...
EFI_STATUS UpdateProcessorInfoTable (EFI_SMBIOS_PROTOCOL *Smbios)
{
  ...
  Status = smbiosGetMaxSpeed(&(smbiosProcessorInfo.MaxSpeed));
  if (Status == EFI_SUCCESS)
  {
    maxSpeedGHZHigh = smbiosProcessorInfo.MaxSpeed / 1000;
    maxSpeedGHZLow = (smbiosProcessorInfo.MaxSpeed % 1000);
    maxSpeedGHZLowRem = (smbiosProcessorInfo.MaxSpeed % 10);
    ...
    {
      maxSpeedGHZHigh = (smbiosProcessorInfo.MaxSpeed / 10) + 1;
  ...
  Status = Smbios->Add(
    ...
    NULL,
    &ProcessorHandle,
    (EFI_SMBIOS_TABLE_HEADER*)&smbiosProcessorInfo
    );
  if(Status != EFI_SUCCESS)
  {
    return Status;
