...
static pwr_utils_mol_res_t *g_res_mol = NULL;
static size_t g_res_mol_count = 0;
...
void pwr_utils_lvl_init (void)
{
  ...
  for (i = 0; i < g_res_count; i++)
  {
    ...
    res->vlvls = (uint16 *)AllocatePool (data_len);
...
int pwr_utils_mol_resource_idx (const char *res_name)
{
  ...
  if ((res_name == NULL) ||(g_res_mol == NULL) || (g_res_mol_count == 0))
  {
    return -1;
  }
...
int pwr_utils_mol_hlvl (int resource_idx)
{
  ...
  int mol_vlvl = 0;
  int mapped_vlvl =0;
  if ((size_t)resource_idx >= g_res_mol_count)
  {
    return RAIL_VOLTAGE_LEVEL_INVALID;
  }
