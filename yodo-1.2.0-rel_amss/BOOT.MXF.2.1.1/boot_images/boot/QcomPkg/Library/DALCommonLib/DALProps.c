...
DALResult _DALSYS_GetDALPropertyInfo(DALDEVICEID DeviceId, DALProps * DALPROP_PropsInfo, DALSYSPropertyHandle hDALProps)
{
   uint32 dwPropsIdx = 0;
   ...
   if (pDALPropsDir)
   {
      for (dwPropsIdx=0;dwPropsIdx<pDALPropsDir->dwNumDevices;dwPropsIdx++)
      {
         if (DeviceId == pDALPropsDir->dwPropsDeviceOffset[dwPropsIdx][0])
         {
            ...
            hDALProps[0] = (DevCfgMemAddr) DALPROP_PropsInfo;
            hDALProps[1] = pDALPropsDir->dwPropsDeviceOffset[dwPropsIdx][1];
            return DAL_SUCCESS;
         }
      }
   }
   hDALProps[0] = (int)(intptr_t)NULL;
   hDALProps[1] = (int)(intptr_t)NULL;
      ...
      const StringDevice **ppStringDeviceObj, DALProps *pDALPtr)
{
   ...
   DevCfg_log("Failed- pszDevName:%s pDALPtr:0x%x", pszDevName, pDALPtr);
   return DAL_ERROR;
}
 
DALResult _DALSYS_GetDALPropertyStrInfo(const char *pszDevName, DALSYSPropertyHandle hDALProps, DALProps * DALPROP_PropsInfo)
{
   ...
   DevCfg_log("Failed- pszDevName:%s DALPROP_PropsInfo:0x%x", pszDevName, DALPROP_PropsInfo);
   return DAL_ERROR;
}
...
static uint32 GetNameOffsetOrIdVal(uint32 dwTypeNameOffset, byte *pbCurPropsPos,uint32 *pdwId) 
{   
...
static void _PopulateStringProperty(DALSYSPropertyVar *pPropVar, DALPropsDir *pPropsDir, byte *pbPropsBuf)
{
   uint32 dwOffset = 0;
   byte *pPropBase = (byte *)pPropsDir;
   memscpy(&dwOffset,sizeof(uint32),pbPropsBuf,sizeof(uint32));
   pPropVar->Val.pszVal = (char *)( pPropBase+pPropsDir->dwPropsStringSectionOffset+dwOffset);
   pPropVar->dwLen = strlen(pPropVar->Val.pszVal);
   pPropVar->dwType = DALSYS_PROP_TYPE_STR_PTR;
}

static void _PopulateUint32Property(DALSYSPropertyVar *pPropVar, byte *pbPropsBuf)
{
...
static void _PopulateByteSeqProperty(DALSYSPropertyVar *pPropVar, byte *pbPropsBuf)
{
...
static void _PopulateStructPtrProperty(DALSYSPropertyVar *pPropVar, DALProps *DALPROP_PropsInfo, byte *pbPropsBuf)
{
