...
void rpmh_os_init()
{
  ...
  InitRecursiveLock ("RpmhDrvr", (LockHandle **)&rpmh_os.lock);

  //Some configurations don't support LockLib, but are single threaded. 
  //Just add checks to see if the lock is null before locking/unlocking.
  //rpmh_core_verify_ptr(rpmh_os.lock);
}
...
void rpmh_clock_toggle (char *clk_name, boolean enable)
{
    ...
    if(!rpmh_os.clock_handle)
    {
        ...
        rpmh_core_verify( Clock_Attach (&(rpmh_os.clock_handle), "Rpmh") == CLOCK_SUCCESS);
    }

    rpmh_core_verify ( Clock_GetId( rpmh_os.clock_handle, clk_name, &clock_id) == CLOCK_SUCCESS);
    if(enable)
    {
      rpmh_core_verify ( Clock_Enable(rpmh_os.clock_handle, clock_id) == CLOCK_SUCCESS);
    }
    else
    {
      rpmh_core_verify ( Clock_Disable(rpmh_os.clock_handle, clock_id) == CLOCK_SUCCESS);
    }
}
...
void rpmh_lock(boolean isr)
{
  ...
  if(rpmh_os.lock != NULL)
  {
    AcquireLock (rpmh_os.lock);
  }
}	
