 ...
 */
/*
 * INFO: when using this macro, extend 'work' array to 3 in npa_client struct in npa_resource_common.h
 */
#if 0
...
typedef struct npa_struct
{
  ...
  CorePoolType batch_request_pool;
  CorePoolType batch_resource_pool;
...
typedef struct npa_batch_resource
{
  npa_resource_state         old_state;
  npa_resource_state         old_required;
  npa_resource_state         old_suppressible;
  npa_resource_state         old_semiactive;
  npa_resource_state         old_suppressible2;
  npa_resource_state_delta   old_headroom;
  npa_resource              *resource;
  struct npa_batch_resource *next;
} npa_batch_resource;

typedef struct npa_batch_request
{
  npa_client_handle         client;
  struct npa_batch_request *next;
} npa_batch_request;
...
/**
  ...
  *
  * @brief The core of NPA's request pipeline for batch requests.
