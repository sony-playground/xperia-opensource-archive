 ...
 *
 * Copyright (c) 2012-2016, 2018, 2020 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
/*=============================================================================
  ...
  ----------   ---     ---------------------------------------------------------
  2020-05-21   cmanoj  Support to Dump RPMB Frames
...
//Uncomment below macro to print RPMB Dump Frames
//#define ENABLE_RPMB_FRAME_DUMPS
#define rpmb_mdelay(ms) rpmb_bsp_busy_wait(1000*ms)
...
void rpmb_bsp_busy_wait(UINT32 usec)
{
    gBS->Stall(usec);
}

/* dump a given RPMB frame */
static void dump_rpmb_frame(uint8_t *ip_frame, const char *frame_type, uint32_t num_of_frames)
{
#ifdef ENABLE_RPMB_FRAME_DUMPS
   
    int i=0, j=0;
    //buf[] is of size 105 because strings at line number 124, 146 takes up 102 bytes
    //So 105 is maximum number of bytes for representing hex digits and 
    //strings at line 124, 146
    uint8_t *frame;
    char buf[105]={0};
    char *p=buf;
    UINTN BytesWritten = 0;
    
    BytesWritten = SerialPortFlush();    
    EnableSynchronousSerialPortIO();
    for (j=0;j<num_of_frames;j++)
    {
        DEBUG ((EFI_D_ERROR, "Printing %a frame (in hex)\n", frame_type));
        DEBUG ((EFI_D_ERROR, "________0__1__2__3__4__5__6__7__8__9__A__B__C__D__E__F__0__1__2__3__4__5__6__7__8__9__A__B__C__D__E__F\n"));

        frame = &ip_frame[j*RPMB_SECTOR_SIZE];

        for (i = 0; i < RPMB_SECTOR_SIZE; i++)
        {
            if (!(i % 32))
            {
            ...
            if (((i + 1) % 32) == 0)
            {
                ...
                DEBUG ((EFI_D_ERROR, "\n"));
                p = buf;
                rpmb_mdelay(TWO_HUNDRED_MILLISECONDS);
            }
            
        }
        DEBUG ((EFI_D_ERROR, "________F__E__D__C__B__A__9__8__7__6__5__4__3__2__1__0__F__E__D__C__B__A__9__8__7__6__5__4__3__2__1__0\n"));
        rpmb_mdelay(TWO_HUNDRED_MILLISECONDS);
    }
#endif
}
...
EFI_STATUS EFIAPI RPMBListenerAllocMem()
{
  ...
  UINTN uSize = sizeof(RPMB_LISTENER_PARAMS);
  ...
  RPMBListenerParams = ShmBridgeProtocol->ShmBridgeAllocate(ShmBridgeProtocol, &uSize, ShmBridgeBootSvcData);
...
EFI_STATUS EFIAPI RPMBListenerInit(UINT32 MediaId)
{
  ...
  do
  {
    ...
    {
      DEBUG ((EFI_D_ERROR, "RPMBInitProtocols: %r!\n", Status));
      break;
    }
  ...
  do
  {
    ...
    switch (*tCmdID)
    {
      case SD_MGR_MSG_RPMB_INIT_ID:
        DEBUG((RPMB_DBG_LVL, "RPMBListenerCallback: SD_MGR_MSG_RPMB_INIT_ID\n"));
        RPMBListenerPartiInit(BufferPtr, BufferLen);
        break;

      case SD_MGR_MSG_RPMB_READ_ID:
        DEBUG((RPMB_DBG_LVL, "RPMBListenerCallback: SD_MGR_MSG_RPMB_READ_ID\n"));
        RPMBListenerReadSectors(BufferPtr, BufferLen);
        break;

      case SD_MGR_MSG_RPMB_WRITE_ID:
        DEBUG ((RPMB_DBG_LVL, "RPMBListenerCallback: SD_MGR_MSG_RPMB_WRITE_ID\n"));
        RPMBListenerWriteSectors(BufferPtr, BufferLen);
        break;

      case SD_MGR_MSG_RPMB_PARTITION_ID: 
        DEBUG ((RPMB_DBG_LVL, "RPMBListenerCallback: **SD_MGR_MSG_RPMB_PARTITION_ID**\n"));
        RPMBListenerConfigPartitions(BufferPtr, BufferLen); 
        break; 

      default:
        DEBUG ((RPMB_DBG_LVL, "RPMBListenerCallback: Invalid SD_MGR_MSG_ID:%d\n", *tCmdID));
...
EFI_STATUS RPMBInitProtocols()
{
  ...
  if (NULL == RPMBListenerParams->RPMBProtocol)
  {
    ...
    {
       DEBUG ((EFI_D_ERROR, "Locate protocol failed: %r\n", Status));
       return Status;
    }
  }
...
void RPMBListenerPartiInit (void *BufferPtr, UINT32 BufferLen)
{
  ...
  do
  {
    ...
    if (SD_MGR_MSG_RPMB_INIT_ID == DevInitReq->CmdID)
    {
      if (!TZSDMgrRpmbVersionCheck(DevInitReq->Version))
      {
        ...
        DEBUG((RPMB_DBG_LVL, "RPMBListenerPartiInit: Version mismatch:%u\n", DevInitReq->Version));
        break;
      }

      if (NULL == RPMBListenerParams->RPMBProtocol)
      {
        DEBUG ((RPMB_DBG_LVL, "RPMBListenerPartiInit: RPMBProtocol NULL!\n"));
        break;
      }

      /* RPMB Size in Bytes */
      RPMBSize = RPMBListenerParams->RPMBProtocol->RPMBPartitionSizeInBytes;
      DEBUG ((RPMB_DBG_LVL, "RPMBListenerPartiInit: RPMBSize:%u\n", RPMBSize));
      ...
      DEBUG ((RPMB_DBG_LVL, "RPMBListenerPartiInit: NumSectors:%u RelWrCount:%u\n", 
            DevInitRsp->NumSectors, DevInitRsp->RelWrCount));
  ...
  DEBUG ((RPMB_DBG_LVL, "RPMBListenerPartiInit Response:%d, DevType:%d\n", 
        DevInitRsp->Status, DevInitRsp->DevType));
}
...
void RPMBListenerReadSectors (void *BufferPtr, UINT32 BufferLen)
{
  ...
  DEBUG((RPMB_DBG_LVL, "RPMBListenerReadSectors: NumHalfSectors:%d\n",RpmbRWReq->NumHalfSectors));
  ...
  do
  {
    ...
    if (SD_MGR_MSG_RPMB_READ_ID == RpmbRWReq->CmdID)
    {
      ...
      dump_rpmb_frame((uint8_t *)CmdPktBuffer, "request", RpmbRWReq->ReqBuffLen/RPMB_SECTOR_SIZE);
	  ...
	  dump_rpmb_frame((uint8_t *)RespBuffer, "response", RpmbRWRes->RspBuffLen/RPMB_SECTOR_SIZE);
    }
    else /* Error: Invalid Parameters */
    {
  ...
  DEBUG ((RPMB_DBG_LVL, "RPMBListenerReadSectors Response: %d\n", RpmbRWRes->Status));
}
...
void RPMBListenerWriteSectors (void *BufferPtr, UINT32 BufferLen)
{
  ...
  CopyMem(mTzSDRpmbRWReq, BufferPtr, ReqBuffLen);
  RpmbRWReq = mTzSDRpmbRWReq;
  DEBUG ((RPMB_DBG_LVL, "RPMBListenerWriteSectors: NumHalfSectors:%d, RelWrCount:%d\n",
        RpmbRWReq->NumHalfSectors, RpmbRWReq->RelWrCount));
  ...
  DEBUG ((RPMB_DBG_LVL, "RPMBListenerWriteSectors Response: %d\n", RpmbRWRes->Status));
}
...
void RPMBListenerConfigPartitions (void *BufferPtr, UINT32 BufferLen)
{
  ...
  DEBUG ((RPMB_DBG_LVL, "RPMBListenerConfigPartitions: DevID:%d, Version:%d\n", 
        PartiReq->DevID, PartiReq->Version));
  ...
  DEBUG ((RPMB_DBG_LVL, "RPMBListenerConfigPartitions Response: %d, NumPartitions: %d\n", 
        PartiRsp->Status, PartiRsp->NumPartitions));
}
