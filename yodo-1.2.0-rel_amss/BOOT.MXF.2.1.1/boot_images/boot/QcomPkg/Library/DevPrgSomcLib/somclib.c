 ...
 *                                                                            *
 ******************************************************************************
 * Copyright 2012, 2022 Sony Corporation                                      *
...
#include "devprg_target.h" // For devprg_get_soc_serial_number
...
{
  int res;
  struct devprg_sha256_data *handle = (struct devprg_sha256_data *)context;

  if (handle == NULL || data == NULL)
    return -FCSERR_EINVAL;

  res = devprg_sha256_update(handle, (uint8_t *)data, length);
  if (res != 0)
    return -FCSERR_EINVAL;
                       ...
                       size_t digest_size)
{
  ...
  if (handle == NULL || digest == NULL || digest_size < HASH_SIZE)
    return -FCSERR_EINVAL;

  res = devprg_sha256_final(handle, digest, digest_size);
  if (res != 0)
    return -FCSERR_EINVAL;
  ...
  uint16_t *sake_method, uint8_t *level)
{
  ...
  {
    case 0:
      ret = devprg_get_soc_serial_number(&die_id);
      if (ret != 0)
      {
        res = -FCSERR_ESTATE;
        goto error;
