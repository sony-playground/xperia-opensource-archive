...
----------   ---     -----------------------------------------------
2022-06-29   ss      Card Mode updation in HS400 mode
...
2020-06-04   jt      Remove unused parameters 
2020-05-22   vm      Add DLLConfig Settings in HS400ES 
2018-07-13   jt      Move tuning functions to sdcc_util.c 
2018-05-18   sb      Ignore cmd end bit error in HS200 tuning process
2018-02-21   sb      Correct the card size in sectors calculation
...
static SDCC_STATUS sdcc_configure_dll_hs400(sdcc_dev_type *sdcc_pdev)
{
   ...
   if ((NULL == sdcc_pdev) || (SDCC_DATA_TRANSFER_HS400_MODE != sdcc_pdev->card_mode))
   {
      return SDCC_ERR_INVALID_PARAM; 
   }
...
static SDCC_STATUS sdcc_enter_hs200_mode (sdcc_dev_type *sdcc_pdev)
{
   ...
   if ((NULL == sdcc_pdev) ||
       ((SDCC_DATA_TRANSFER_HS200_MODE != sdcc_pdev->card_mode) && (SDCC_DATA_TRANSFER_HS400_MODE != sdcc_pdev->card_mode)))
   {
      return SDCC_ERR_INVALID_PARAM; 
   }
   ...
   if (SDCC_DATA_TRANSFER_HS400_MODE == sdcc_pdev->card_mode)
   {
   ...
   else if (SDCC_DATA_TRANSFER_HS200_MODE == sdcc_pdev->card_mode)
   {
   ...
   HAL_sdhci_SetUHSMode(sdcc_pdev->driveno, (UINT16)SDCC_SDR104_MODE); 
   HAL_sdhci_HCSelectIn(sdcc_pdev->driveno, (UINT32)HAL_SDCC_CLOCK_UHS_MCLK); 

   sdcc_config_clk (sdcc_pdev, sdcc_pdev->card_mode); 
...
static SDCC_STATUS sdcc_enter_hs400_mode (sdcc_dev_type *sdcc_pdev)
{
   ...
   sdcc_hc_set_uhs_mode_select(sdcc_pdev, SDCC_SDR12_MODE); 
   ...
   sdcc_pdev->card_mode = SDCC_DATA_TRANSFER_HS400_MODE;
...
static SDCC_STATUS sdcc_enter_hs400_mode_with_es (sdcc_dev_type *sdcc_pdev)
{
   ...
   UINT32 timeout = 50;    
   UINT32 driveno = 0;
   UINT8 mclk_freq_calc = 0x28, rclk_delay = 104;
   ...
   HAL_sdhci_DLL2_ClockConfig(driveno, mclk_freq_calc, 0);
...
sdcc_config_mmc_modes_segment( sdcc_dev_type *sdcc_pdev )
{
   ...
   sdcc_ext_csd_type ext_csd;
   UINT8 boot_enabled_val = 0, platform_speed = 0;
   int i; 
   BOOLEAN DDRModeEnable = FALSE; 
   sdcc_bsp_speed_mode EmmcSpeedMode =  sdcc_pdev->pslot->sdcc_config.SpeedMode;
   ...
   platform_speed = sdcc_bsp_get_platform_speed_config(sdcc_pdev->driveno);
   if (EmmcSpeedMode != platform_speed) 
   {
       EmmcSpeedMode = platform_speed; 
   }

   /* Read the the speed mode flag */
   if (SDCC_DDR50_MODE == EmmcSpeedMode)
   {
   ...
   if ((SDCC_HS400_MODE == EmmcSpeedMode) && (ext_csd.card_type & SDCC_HS400_MMC_DDR_200MHZ_1_8V))
   {
   ...
   else if ((SDCC_HS200_MODE == EmmcSpeedMode) && (ext_csd.card_type & SDCC_HS200_SDR_200MHZ_1_8V))
   {
      sdcc_pdev->card_mode = SDCC_DATA_TRANSFER_HS200_MODE; 
   ...
   else 
   {
      if (DDRModeEnable && (ext_csd.card_type & SDCC_MMC_DDR_52MHZ))
      {
         ...
         sdcc_pdev->card_mode = SDCC_DATA_TRANSFER_DDR_MODE;
          ...
          * Set the clock to 50MHz */   
          rc = sdcc_hc_set_uhs_mode_select(sdcc_pdev, SDCC_DDR50_MODE); 
          if (rc != SDCC_NO_ERROR)
          {
