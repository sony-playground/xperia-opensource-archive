...
---------   ----   ----------------------------------------------------- 
2022-04-20  jt     HSR updates to configure LOW_FREQ_MODE
2022-02-17  jt     SDR50 tuning updates
2022-01-05  jt     Add SDR50 tuning capability 
2021-05-24  jt     Add device tree support
2020-06-04  jt     Add power management functions 
2020-05-22  vm     Add new tuning method
2018-07-13  jt     Add tuning functions 
2018-02-21  sb     Update for sdm640 
...
)
{
   ...
   sdcc_bsp_supported_frequencies frequencies; 

   sdcc_bsp_get_supported_freq (driveno, &frequencies); 
   ...
   case SDCC_DATA_TRANSFER_SDR50_LS_MODE:
      if ((card_type == SDCC_CARD_MMC) || (card_type == SDCC_CARD_MMCHC))
      {
         ...
         else
         {
            ...
            sdcc_mclk = SDCC_BSP_CLK_20_MHZ;
               ...
               (card_type == SDCC_CARD_UHS1))
      {
         /* check the SD clock freq */
         if (sdcc_pdev->mem.clk_freq == SDCC_SD_FREQUENCY_100MHZ)
         {
            sdcc_mclk = SDCC_BSP_CLK_100_MHZ;
         }
         else if (sdcc_pdev->mem.clk_freq == SDCC_SD_HIGH_FREQUENCY)
         {
            sdcc_mclk = frequencies.hs_mode;  
         }
         else
         {
            sdcc_mclk = SDCC_BSP_CLK_25_MHZ;
   ...
   case SDCC_DATA_TRANSFER_DDR_MODE:
      if ((SDCC_CARD_MMC == card_type) || (SDCC_CARD_MMCHC == card_type) || (SDCC_CARD_UHS1 == card_type))
      {
         if (sdcc_pdev->mem.clk_freq == SDCC_MMC_FREQUENCY_52MHZ)
         {
      ...
      {
         sdcc_mclk = SDCC_BSP_CLK_20_MHZ;
         DEBUG ((EFI_D_INFO, "sdcc_config_clk: DDR is only supported on MMC or UHS1 cards\n"));
      }
      break;
    ...
    case SDCC_DATA_TRANSFER_HS200_MODE:
       if ((SDCC_CARD_MMC == card_type) || (SDCC_CARD_MMCHC == card_type) || (SDCC_CARD_UHS1 == card_type))
       {
          if (SDCC_MMC_FREQUENCY_200MHZ == sdcc_pdev->mem.clk_freq)
          {
             ...
             sdcc_mclk = frequencies.hs200_mode; 
          }
       }
       else
       {
          sdcc_mclk = SDCC_BSP_CLK_20_MHZ;
          DEBUG((EFI_D_INFO, "sdcc_config_clk: HS200 is only supported on MMC or UHS1 cards\r\n"));
       }
       break;
    ...
    case SDCC_DATA_TRANSFER_HS400_MODE:
       if ((SDCC_CARD_MMC == card_type) || (SDCC_CARD_MMCHC == card_type))
       {
          if (SDCC_MMC_FREQUENCY_400MHZ == sdcc_pdev->mem.clk_freq)
          {
             ...
             sdcc_mclk = frequencies.hs400_mode;
          }
       }
       else
       {
...
*
******************************************************************************/
void sdcc_dll_mclk_freq_set (sdcc_dev_type *sdcc_pdev)
{
   UINT32 mclk_freq = 0;
...
} /* sdcc_dll_mclk_freq_set */

/******************************************************************************
* Name: sdcc_init_cm_dll_sdc4
*
* Description:
...
*
******************************************************************************/
SDCC_STATUS sdcc_init_cm_dll_sdc4 (sdcc_dev_type *sdcc_pdev)
{
   sdcc_bsp_hsr_settings_type hsr_settings; 
   sdcc_bsp_speed_mode speed_mode = SDCC_SDR104_MODE; 
   SDCC_STATUS rc = SDCC_NO_ERROR;
   UINT32 driveno = sdcc_pdev->driveno;
   UINT8 low_freq_mode = 0; 
   UINT8 mclk_freq_calc = SDCC_BSP_DLL_CONFIG_2_DEFAULT_MCLK_FREQ_CALC_VAL;
   UINT8 rclk_delay = SDCC_BSP_DDR_CONFIG_DEFAULT_RCLK_DELAY_VAL;
   UINT8 sdc4_config_msb = SDCC_BSP_DLL_CONFIG_3_DEFAULT_SDC4_CONFIG_MSB_VAL;
   UINT32 dll_user_ctl = SDCC_BSP_DLL_USER_CTRL_DEFAULT_VAL; 
   UINT32 dll_config_init = SDCC_BSP_DLL_CONFIG_DEFAULT_VAL; 

   if (sdcc_pdev->card_mode == SDCC_DATA_TRANSFER_SDR50_LS_MODE) 
   {
      speed_mode = SDCC_SDR50_MODE; 
   }

   if (sdcc_bsp_get_hsr_settings (driveno, speed_mode, &hsr_settings) == SDCC_BSP_NO_ERROR) 
   {
       dll_config_init = hsr_settings.dll_config & HWIO_HC_REG_DLL_CONFIG_INIT_BMSK; 
       rclk_delay = hsr_settings.ddr_config & HWIO_HC_REG_DDR_CONFIG_PRG_RCLK_DLY_BMSK; 
       sdc4_config_msb = hsr_settings.dll_config_3 & HWIO_HC_REG_DLL_CONFIG_3_SDC4_CONFIG_MSB_BMSK; 
       dll_user_ctl = hsr_settings.dll_user_ctrl; 
       mclk_freq_calc = (hsr_settings.dll_config_2 & HWIO_HC_REG_DLL_CONFIG_2_MCLK_FREQ_CALC_BMSK) >> HWIO_HC_REG_DLL_CONFIG_2_MCLK_FREQ_CALC_SHFT;
       low_freq_mode = (hsr_settings.dll_config_2 & HWIO_HC_REG_DLL_CONFIG_2_LOW_FREQ_MODE_BMSK) >> HWIO_HC_REG_DLL_CONFIG_2_LOW_FREQ_MODE_SHFT; 
   }
 
   HAL_sdhci_PowerSave(driveno, FALSE);
   HAL_sdhci_DllRstPdn(driveno, TRUE);
   HAL_sdhci_DllInit(driveno, dll_config_init); 
   sdcc_dll_mclk_freq_set(sdcc_pdev);
   HAL_sdhci_DllUserCtl(driveno, dll_user_ctl);
   HAL_sdhci_DLL2_ClockConfig(driveno, mclk_freq_calc, low_freq_mode);

   if (driveno == SDCC_BSP_SLOT_1)
   {
       HAL_sdhci_DllConfig3(driveno, sdc4_config_msb);
   }
   else
   {
      if (SDCC_DATA_TRANSFER_HS200_MODE == sdcc_pdev->card_mode)
      {
         HAL_sdhci_DllConfig3(driveno, 0x10); 
      }
      else if (SDCC_DATA_TRANSFER_HS400_MODE == sdcc_pdev->card_mode)
      {
         HAL_sdhci_DllConfig3(driveno, 0x01); 
      }
   }

   HAL_sdhci_ConfigDDR(driveno, rclk_delay); 
   HAL_sdhci_DllRstPdn(driveno, FALSE);
   HAL_sdhci_DllEnable(driveno, TRUE);
   HAL_sdhci_DllCkOutEnable(driveno, TRUE);
   HAL_sdhci_DLLClockEnable(driveno, TRUE);
   rc = HAL_sdhci_DllPollStatusLock(driveno);
   HAL_sdhci_PowerSave(driveno, TRUE);
   
   return rc;
} /* sdcc_init_cm_dll_sdc4 */
...
*    for enabling HS200/SDR104 mode.
*
* Arguments:
*    sdcc_pdev        [IN/OUT] : pointer to device type
*
...
*
******************************************************************************/
SDCC_STATUS sdcc_hs200_execute_tuning (sdcc_dev_type *sdcc_pdev)
{
   SDCC_STATUS rc = SDCC_NO_ERROR;
   UINT32 driveno = sdcc_pdev->driveno;
   ...
   /* Check if the device is HS200/400 or SDR50 with level shifter (requires tuning) */
   if ((SDCC_DATA_TRANSFER_HS200_MODE != sdcc_pdev->card_mode) &&
       (SDCC_DATA_TRANSFER_HS400_MODE != sdcc_pdev->card_mode) && 
       (SDCC_DATA_TRANSFER_SDR50_LS_MODE != sdcc_pdev->card_mode))
   {
   ...
   rc = sdcc_init_cm_dll_sdc4(sdcc_pdev);
   if (rc != SDCC_NO_ERROR)
   {
...
RetryTuning:
   ...
   do
   {
      /* Note: CDR Enable needs to be set as part of CMD 19/21. It will
      ...
      if (sdcc_pdev->card_type == SDCC_CARD_UHS1)
      {
         sdcc_cmd.cmd    = SD_CMD19; 
      }
...
)
{
   ...
   if ((sdcc_pdev->card_type == SDCC_CARD_SD) || 
       ...
       (sdcc_pdev->card_type == SDCC_CARD_UHS1))
   {
      mult = 100; 
   }
...
/******************************************************************************
* Name: sdcc_change_speed
*
* Description:
*  This function changes the speed mode.
*
* Arguments:
*    sdcc_pdev[IN]      : pointer to device structure
...
*
******************************************************************************/ 
SDCC_STATUS sdcc_change_speed (sdcc_dev_type *sdcc_pdev, 
                               sdcc_bsp_speed_mode speed_mode)
{
   if (SDCC_BSP_SLOT_1 == sdcc_pdev->driveno)
   {
      return sdcc_sd_switch_to_hs (sdcc_pdev, &speed_mode);
   }
   else
   {
...
/******************************************************************************
* Name: sdcc_clock_scale_down
*
* Description:
*  This function performs clock scaling down.
*
* Arguments:
*    sdcc_pdev[IN]      : pointer to device structure
*    volt_corner[IN]    : voltage corner to scale to
*    speed_mode[IN]     : speed mode to change to 
* 
* Returns:
*    SDCC_NO_ERROR if successful
*    Error code otherwise
*
******************************************************************************/ 
SDCC_STATUS sdcc_clock_scale_down (sdcc_dev_type *sdcc_pdev, 
                                   UINT8 volt_corner, 
                                   UINT8 speed_mode)
{
   BOOLEAN sdr50_tuning = FALSE; 
   SDCC_STATUS rc = SDCC_NO_ERROR;
   UINT32 driveno = sdcc_pdev->driveno;

   /* If we will be in the same speed mode, just change the internal voltage corner */
   if (speed_mode == sdcc_pdev->curr_speed_mode)
   {
      sdcc_pdev->curr_volt_corner = volt_corner; 
      return SDCC_NO_ERROR;
   }

   /* For SD card (slot 1), we are only supporting scaling down from SDR104 to SDR50 if SDR50 tuning is not required.
      Otherwise, SDR104 to SDR25 and SDR50 to SDR25 are possible. */
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      sdr50_tuning = (sdcc_pdev->pslot->capabilities.HAL_sdhci_Reg_44_46 & HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_BMSK) >> HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_SHFT;

      if (sdr50_tuning) 
      {
         if (!(((SDCC_SDR104_MODE == sdcc_pdev->curr_speed_mode) || (SDCC_SDR50_MODE == sdcc_pdev->curr_speed_mode)) && (SDCC_SDR25_MODE == speed_mode)))
         {
            return SDCC_BSP_ERR_FEATURE_UNSUPPORTED;
         }          
      }
      ...
      {
         if (!((SDCC_SDR104_MODE == sdcc_pdev->curr_speed_mode) && (SDCC_SDR50_MODE == speed_mode)))
         {
            return SDCC_BSP_ERR_FEATURE_UNSUPPORTED;
         }
      }
   ...
   /* Change to the speed_mode */
   rc = sdcc_hc_set_uhs_mode_select (sdcc_pdev, speed_mode);
   if (SDCC_NO_ERROR != rc)
   {
      DEBUG ((EFI_D_INFO, "Clock scaling down failed %x\n", rc));
      return rc;
   }

   sdcc_pdev->curr_speed_mode = speed_mode;
   sdcc_pdev->curr_volt_corner = volt_corner;
...
/******************************************************************************
* Name: sdcc_clock_scale_up
*
* Description:
*  This function performs clock scaling up. 
*
* Arguments:
*    sdcc_pdev[IN]      : pointer to device structure
*    volt_corner[IN]    : voltage corner to scale to
...
*
******************************************************************************/
SDCC_STATUS sdcc_clock_scale_up (sdcc_dev_type *sdcc_pdev, 
                                 UINT8 volt_corner, 
                                 UINT8 speed_mode)
{
   BOOLEAN sdr50_tuning = FALSE; 
   ...
   /* For SD card (slot 1), we are only supporting scaling up from SDR50 to SDR104 if SDR50 tuning is not required.
      Otherwise, SDR25 to SDR50 and SDR25 to SDR104 are possible */
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      sdr50_tuning = (sdcc_pdev->pslot->capabilities.HAL_sdhci_Reg_44_46 & HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_BMSK) >> HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_SHFT;
      ...
      if (sdr50_tuning) 
      {
         if (!((SDCC_SDR25_MODE == sdcc_pdev->curr_speed_mode) && ((SDCC_SDR50_MODE == speed_mode) || (SDCC_SDR104_MODE == speed_mode))))
         {
            return SDCC_BSP_ERR_FEATURE_UNSUPPORTED;
         }
      }
      ...
      {
         if (!((SDCC_SDR50_MODE == sdcc_pdev->curr_speed_mode) && (SDCC_SDR104_MODE == speed_mode)))
         {
            return SDCC_BSP_ERR_FEATURE_UNSUPPORTED;
         }
      }
   ...
   if (SDCC_NO_ERROR != rc)
   {
      DEBUG ((EFI_D_INFO, "Clock scaling up failed %x\n", rc));
      return rc;
   }

   /* Re-initialize DLL and restore the initial DLL phase */
   if (SDCC_NO_ERROR != sdcc_init_cm_dll_sdc4 (sdcc_pdev))
   {
      DEBUG ((EFI_D_INFO, "Clock scaling up failed to initialize DLL\n"));
      return SDCC_ERR_UNKNOWN;
   }

   if (HAL_sdhci_ConfigCmDllPhase (driveno, sdcc_pdev->selected_phase))
   {
      DEBUG ((EFI_D_INFO, "Clock scaling up failed to configure DLL phase\n"));
      return SDCC_ERR_UNKNOWN;
   }
