 ...
 *
 * Copyright (c) 2014-2016, 2018, 2020, 2022 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
...
----------  ---     ----------------------------------------------------- 
2022-08-08  jt      Print message if card is not SDR104 capable
   ...
   for ( i = 0; i < SDCC_SD_DETECT_MAX_RETRIES; i++ )
   {
      ...
      { 
         if ( sdcc_cmd.resp[0] & SDCC_HC_CCS )
         {
            ...
            /* SD memory card. If S18A bit is set, the card is a UHS-1 card. */
            if ( sdcc_cmd.resp[0] & SDCC_S18A )
            {
               card_type = SDCC_CARD_UHS1; 
               DEBUG ((EFI_D_INFO, "Found UHS-1 card after %dth retry\n", i));
            }
            else
            {
...
)
{
   ...
   scr->sd_spec3 = (UINT8)(value >> 7);
   /* sd_spec4: Spec. Version 4.00 or higher */
   scr->sd_spec4 = (UINT8)((value & 0x04) >> 2);
...
*    supports.  It also sends CMD6 to set (mode 1) the card to the
*    highest speed the card supports. 
*
* Arguments:
*    sdcc_pdev    [IN/OUT] : pointer to device type
*
...
******************************************************************************/
SDCC_STATUS 
sdcc_sd_switch_to_hs( sdcc_dev_type *sdcc_pdev, sdcc_bsp_speed_mode *intended_speed )
{
   static BOOLEAN warning_printed = FALSE; 
   BOOLEAN sdr50_tuning = FALSE, ddr50_supported = FALSE, tuning_failed = FALSE; 
   SDCC_STATUS     rc         = SDCC_NO_ERROR;
   sdcc_cmd_type   sdcc_cmd;
   sdcc_switch_data_type switch_status;
   sdcc_bsp_speed_mode speed_mode = SDCC_SDR12_MODE;
   UINT16 cmd6_speed_mode_val = SDCC_CMD6_DS; 
   UINT16 access_mode = SDCC_ACCESS_MODE_HS;

   /* query switchable for high speed to discover the supported access modes */
   SetMem(gTempBuffer, 64, 0);
   ...
   if ( SDCC_NO_ERROR != rc )
   {
      DEBUG ((EFI_D_INFO, "Failed for querying access mode func\n"));
      return rc;
   }
   ...
   {
      DEBUG ((EFI_D_INFO, "Invalid in querying access mode func\n"));
      return SDCC_ERR_UNKNOWN;
   }

   ddr50_supported = (sdcc_pdev->pslot->capabilities.HAL_sdhci_Reg_44_46 & HAL_SDHCI_CAPS_DDR_50_SUPPORT_BMSK) >> HAL_SDHCI_CAPS_DDR_50_SUPPORT_SHFT;
   ...
   /* Print message if card is not SDR104 capable */
   if (!(SDCC_ACCESS_MODE_SDR104 & switch_status.funcgrp_support_func[0]))
   {
      if (FALSE == warning_printed)
      {
         DEBUG((EFI_D_WARN, "SD card is not SDR104 capable\n"));
         warning_printed = TRUE;
      }
   }
 
SelectMode:
   /* Determine the maximum supported speed mode */ 
   if ( ( ( NULL == intended_speed ) && ( SDCC_ACCESS_MODE_SDR104 & switch_status.funcgrp_support_func[0] ) && ( FALSE == tuning_failed ) ) ||
        ( ( NULL != intended_speed ) && ( SDCC_SDR104_MODE == *intended_speed ) ))
   {
      speed_mode = SDCC_SDR104_MODE;
      cmd6_speed_mode_val = SDCC_CMD6_SDR104; 
      access_mode = SDCC_ACCESS_MODE_SDR104;
   }
   else if ( ( ( NULL == intended_speed ) && ( TRUE == ddr50_supported ) && ( SDCC_ACCESS_MODE_DDR50 & switch_status.funcgrp_support_func[0] ) ) ||
             ( ( NULL != intended_speed ) && ( TRUE == ddr50_supported ) && ( SDCC_DDR50_MODE == *intended_speed ) ) )
   {
      speed_mode = SDCC_DDR50_MODE;
      cmd6_speed_mode_val = SDCC_CMD6_DDR50; 
      access_mode = SDCC_ACCESS_MODE_DDR50; 
   }
   else if ( ( ( NULL == intended_speed ) && ( SDCC_ACCESS_MODE_SDR50 & switch_status.funcgrp_support_func[0] ) ) || 
             ( ( NULL != intended_speed ) && ( SDCC_SDR50_MODE == *intended_speed ) ) )
   {
      speed_mode = SDCC_SDR50_MODE;
      cmd6_speed_mode_val = SDCC_CMD6_SDR50; 
      access_mode = SDCC_ACCESS_MODE_SDR50;
   }
   else if ( ( ( NULL == intended_speed ) && ( SDCC_ACCESS_MODE_HS & switch_status.funcgrp_support_func[0] ) ) ||
             ( ( NULL != intended_speed ) && ( SDCC_SDR25_MODE == *intended_speed ) ) )
   {
      speed_mode = SDCC_SDR25_MODE;
      cmd6_speed_mode_val = SDCC_CMD6_HS; 
      access_mode = SDCC_ACCESS_MODE_HS; 
   }
   else if ( ( NULL == intended_speed ) || 
             ( ( NULL != intended_speed ) && ( SDCC_SDR12_MODE == *intended_speed ) ) )
   {
      speed_mode = SDCC_SDR12_MODE;
      cmd6_speed_mode_val = SDCC_CMD6_DS;
      access_mode = SDCC_ACCESS_MODE_HS;
   }
   else
   {            
   ...
   /* Check if access mode that we want to switch is busy */
   if ( ( SDCC_BUSY_STATUS_DEF == switch_status.data_struct_vers ) &&
        ( access_mode & switch_status.funcgrp_busy_status[0] ) )
   {
      DEBUG ((EFI_D_INFO, "Query: Access mode %d is busy\n", access_mode));
      return SDCC_ERR_UNKNOWN;
   }

   /* CMD6 mode 1 to switch to the chosen access mode */
   SetMem(gTempBuffer, 64, 0);
   SetMem(&switch_status, sizeof(sdcc_switch_data_type), 0);
   sdcc_cmd.cmd_arg = SDCC_SET_MODE | SDCC_QUERY_SUPPORTED_FUNCS | cmd6_speed_mode_val;
   sdcc_cmd.flags = SDCC_TRANSFER_FLAG_IS_READ_TRANSFER;
   sdcc_pdev->mem.block_mode = SDCC_MEM_BYTE_MODE;
   ...
   if ( SDCC_NO_ERROR != rc )
   {
      DEBUG ((EFI_D_INFO, "Failed in command to switch to access mode %d\n", access_mode));
      return rc;
   }
   ...
   if ( ( 0 == switch_status.max_curr_consumption ) ||
        ( cmd6_speed_mode_val != switch_status.funcgrp_status[0] ) )
   {
      DEBUG ((EFI_D_ERROR, "Failed in switching to access mode %d\n", access_mode));
      return SDCC_ERR_UNKNOWN;
   }
   ...
   if ( ( SDCC_BUSY_STATUS_DEF == switch_status.data_struct_vers ) &&
        ( access_mode & switch_status.funcgrp_busy_status[0] ) )
   {
      DEBUG ((EFI_D_INFO, "Switch: Access mode %d is busy\n", access_mode));
      return SDCC_ERR_UNKNOWN;
   }
   ...
   /* Update the new UHS mode and execute tuning for SDR104 */
   rc = sdcc_hc_set_uhs_mode_select( sdcc_pdev, speed_mode ); 
   if ( SDCC_NO_ERROR == rc )
   {
      /* Execute tuning if required. If tuning fails, disable DLL and
         change timing mode to SDR2S. Then, go back and select the
         next best speed mode. */
      if ( TRUE == sdcc_pdev->tuning_needed )
      {
         ...
         if ( SDCC_NO_ERROR != rc )
         {
            ...
            sdcc_pdev->card_mode = SDCC_DATA_TRANSFER_MODE; 
            HAL_sdhci_PowerSave( sdcc_pdev->driveno, FALSE );
            HAL_sdhci_DllRstPdn( sdcc_pdev->driveno, TRUE );
            HAL_sdhci_CdrEnable( sdcc_pdev->driveno, FALSE ); 
            HAL_sdhci_DLLClockEnable( sdcc_pdev->driveno, FALSE );
            HAL_sdhci_DllCkOutEnable( sdcc_pdev->driveno, FALSE );                    
            HAL_sdhci_DllEnable( sdcc_pdev->driveno, FALSE );
            HAL_sdhci_PowerSave( sdcc_pdev->driveno, TRUE );

            if ( ( ( NULL != intended_speed ) && ( SDCC_SDR104_MODE == *intended_speed ) ) || 
                 ( ( NULL != intended_speed ) && ( SDCC_SDR50_MODE == *intended_speed ) && ( TRUE == sdr50_tuning ) ) )
            {
               return rc;
            }

            rc = sdcc_hc_set_uhs_mode_select( sdcc_pdev, SDCC_SDR25_MODE );
            if ( SDCC_NO_ERROR != rc )
            {
               return rc; 
            }
