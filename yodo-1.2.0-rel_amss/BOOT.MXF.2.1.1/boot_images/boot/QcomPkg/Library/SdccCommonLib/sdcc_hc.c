...
----------  ---     --------------------------------------------------- 
2022-06-28  ss      Reorganised Target Specific Function calls
...
2021-04-27  jt      Check for retuning in transfer function
...
sdcc_init_sdhci_mode (UINT32 driveno)
{
   ...
   if(SDCC_BSP_NO_ERROR != sdcc_bsp_get_config(driveno, &(sdcc_pdata->sdcc_config)))
   {
	  DEBUG ((EFI_D_ERROR, "sdcc_init_sdhci_mode: can't get config parameters\n"));
      return FALSE;
   };
   ...
   /* Set the default voltage corner */
   sdcc_bsp_default_volt_corner (&sdcc_pdev->curr_volt_corner);
   ...
   /* Is ADMA supported? */
   if (( TRUE == sdcc_pdata->sdcc_config.DmaEnabled) && 
...
sdcc_partial_init_sdhci_mode (sdcc_dev_type *sdcc_pdev)
{
   ...
   if (TRUE == sdcc_pdev->pslot->sdcc_config.DmaEnabled && 
...
*
******************************************************************************/
SDCC_STATUS sdcc_hc_set_uhs_mode_select (sdcc_dev_type *p_device, 
                                         sdcc_bsp_speed_mode mode)
{
   UINT32 driveno = 0; 
   ...
   switch (mode)
   {
   case SDCC_SDR104_MODE:
      p_device->mem.clk_freq = SDCC_MMC_FREQUENCY_200MHZ; 
      p_device->card_mode = SDCC_DATA_TRANSFER_HS200_MODE;
      select_in = HAL_SDCC_CLOCK_UHS_MCLK;     
      p_device->tuning_needed = TRUE;  
      break;
      
   case SDCC_DDR50_MODE:
      p_device->mem.clk_freq = SDCC_MMC_FREQUENCY_52MHZ;
      p_device->card_mode = SDCC_DATA_TRANSFER_DDR_MODE; 
      select_in = HAL_SDCC_CLOCK_EDGE_DDR; 
      break;

   case SDCC_SDR50_MODE:
      p_device->mem.clk_freq = SDCC_SD_FREQUENCY_100MHZ;
      p_device->card_mode = SDCC_DATA_TRANSFER_MODE; 
      select_in = HAL_SDCC_CLOCK_EDGE_FEEDBACK; 
      if (p_device->pslot->capabilities.HAL_sdhci_Reg_44_46 & HAL_SDHCI_CAPS_USE_TUNING_FOR_SDR50_BMSK)
      {
         p_device->card_mode = SDCC_DATA_TRANSFER_SDR50_LS_MODE; 
         select_in = HAL_SDCC_CLOCK_UHS_MCLK; 
         p_device->tuning_needed = TRUE; 
      }
      break;
      
   case SDCC_SDR25_MODE:
      p_device->mem.clk_freq = SDCC_MMC_FREQUENCY_52MHZ;
      if (SDCC_BSP_SLOT_1 == driveno)
      {
         p_device->mem.clk_freq = SDCC_SD_HIGH_FREQUENCY;
      }
      p_device->card_mode = SDCC_DATA_TRANSFER_MODE;
      select_in = HAL_SDCC_CLOCK_EDGE_FEEDBACK; 
      break;   
      
   case SDCC_SDR12_MODE:
   ...
   sdcc_config_clk(p_device, p_device->card_mode);
                                         ...
                                         UINT16 transfer_size)
{
   ...
   if (p_device->tuning_needed && (!p_device->tuning_in_progress) && (!p_device->tuning_done))
   {
      if ((SDCC_DATA_TRANSFER_HS200_MODE == p_device->card_mode) || (SDCC_DATA_TRANSFER_SDR50_LS_MODE == p_device->card_mode))
      {
         rc = sdcc_hs200_execute_tuning(p_device);
         if (rc != SDCC_NO_ERROR)
         {
...
SDCC_STATUS sdcc_command_sdhci_mode (sdcc_dev_type *sdcc_pdev, sdcc_cmd_type *cmd)
{
   ...
   {
      if ((SDCC_DATA_TRANSFER_HS200_MODE == sdcc_pdev->card_mode) || (SDCC_DATA_TRANSFER_SDR50_LS_MODE == sdcc_pdev->card_mode))
      {
   ...
   if ((SDCC_DATA_TRANSFER_HS400_MODE == sdcc_pdev->card_mode) || 
       (SDCC_DATA_TRANSFER_HS200_MODE == sdcc_pdev->card_mode) || 
