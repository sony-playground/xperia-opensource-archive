...
EFI_STATUS ChargerLib_Exit( VOID )
{
  ...
  if(FALSE == ChgExitDone)
  {
    ...
    //PMIC access is revoked upon entering BDS menu. Configuring wdog at EBL exit will cause violation. Leave Wdog enabled.	
...
EFI_STATUS ChargerLib_GetShipMode(BOOLEAN* Enabled)
{
  ...
  AsciiStrToUnicodeStrS((CHAR8 *)gChargerLibCfgData.shipmode_cfg.shipMode_str, UniStr, CHARGERLIB_SHIPMODE_STR_LEN);
