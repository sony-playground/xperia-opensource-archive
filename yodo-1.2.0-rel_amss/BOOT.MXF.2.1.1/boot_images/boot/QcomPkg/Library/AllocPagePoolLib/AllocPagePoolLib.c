...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/22/19   kpa     Added Locks to support multithreaded invocation
...
STATIC BOOLEAN MemLockInitialized = FALSE;
LockHandle *MemLockPtr =NULL;

EFI_STATUS InitMemoryMutex (VOID)
{
  EFI_STATUS Status;
  if (MemLockInitialized == FALSE)
  {
    Status = InitRecursiveLock ("MEMMUTEX", &MemLockPtr);
    if (EFI_ERROR (Status))
    {
      DEBUG ((EFI_D_ERROR, "InitNamedLock Failed - %r\n", Status));
      return Status;
    }    
    MemLockInitialized = TRUE;
  }  
  
  return EFI_SUCCESS;
}

EFI_STATUS AcquireAllocatorLock (VOID)
{
  if (MemLockInitialized == TRUE)
    AcquireLock (MemLockPtr);
  
  return EFI_SUCCESS;
}

EFI_STATUS ReleaseAllocatorLock (VOID)
{
  if (MemLockInitialized == TRUE)
    ReleaseLock (MemLockPtr);
...
VOID *
EFIAPI
AllocatePagesInternal (IN UINTN Pages)
{
#ifdef ENABLE_ASLR
...
{
  VOID *BufferPtr = NULL;
  AcquireAllocatorLock();  
  BufferPtr = AllocatePagesInternal (Pages);
...
{
  AcquireAllocatorLock ();   
  FreePagesInternal (Buffer, Pages);
  ReleaseAllocatorLock ();  
}