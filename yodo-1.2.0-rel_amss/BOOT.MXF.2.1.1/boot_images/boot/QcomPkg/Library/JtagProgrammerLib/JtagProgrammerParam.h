...
----------   ---     ----------------------------------------------------------
2020-11-04   sa      Changed jtag_prog_param elements from UINT64 to uintnt.
...
/* 
This structure is used to communcate between TRACE32 Debugger and our FLASH 
plug-in. T32 flash programming routine expects this struct to be in specific
format as defined in flash programming guide. Each element size in the struct
should correspond to architecture integer size (32/64 bit).
*/
struct struct_jtag_prog_param {
   uintnt mod_addr;           // in: Flash module address
   uintnt reserved;           // in: Not used
   uintnt width;              // in: Data width - Not used
   uintnt offset;             // in: Not used
   uintnt addr;               // in: address of the datacard to write/erase
   uintnt data_length;        // in: size of data to write/erase
   uintnt timing;             // in: Clock frequency - Not used
   uintnt cmd_status;         // in: 1=program, 2=erase, 5=erase, out: 0=o.k., 0x100=fail
...
#define JTAG_CMD_BULK_ERASE            0x00000050
#define JTAG_CMD_UNIT_TEST_1           0x00000051
