...
----------   ---     ----------------------------------------------------------
2022-04-01   ss      EEPROM support
2020-08-31   wek     Fix klocwork warning on va_list initalization.
...
UINT32 (*jsdcc_mem_bulk_erase)(void);
UINT32 (*jsdcc_mem_unit_test1)(void);
...
void main_c(void)
{
  ...
  case JTAG_CMD_BULK_ERASE:
    outputMessage("Executing Bulk Erase\n");
    ret_code = jsdcc_mem_bulk_erase();
    break;

  case JTAG_CMD_UNIT_TEST_1:
	  	if (device_type == MEM_DEVICE_EEPROM)
        {
          outputMessage("Executing EEPROM Unit Test\n");
        }
		else
        {
      outputMessage("Executing SPINOR Unit Test\n");
        }
    ret_code =jsdcc_mem_unit_test1();
    break;
  default:
...
UINT32 do_probe(void)
{
  ...
  else if (device_type == MEM_DEVICE_EEPROM)
  {
    ret_code = do_probe_eeprom();
  }
  ...
  else if (device_type == MEM_DEVICE_SPINOR)
  {
    ...
    jsdcc_mem_bulk_erase = jsdcc_mem_erase_bulk_spi;
    jsdcc_mem_unit_test1 = jsdcc_mem_unit_test_spi;
  ...
  {
	jsdcc_mem_read = jsdcc_mem_read_eeprom;
	jsdcc_mem_write = jsdcc_mem_write_eeprom;
	jsdcc_mem_erase = jsdcc_mem_erase_eeprom;
    jsdcc_mem_bulk_erase = jsdcc_mem_erase_bulk_eeprom;
	jsdcc_mem_unit_test1 = jsdcc_mem_unit_test_eeprom; 
	jsdcc_mem_erase_partition = jsdcc_mem_erase_partition_eeprom;
	jsdcc_mem_compute_crc = jsdcc_mem_compute_crc_eeprom;
	jsdcc_mem_print_info = jsdcc_mem_print_info_eeprom;
	jsdcc_mem_program = jsdcc_mem_program_eeprom;
	jsdcc_mem_chip_erase = jsdcc_mem_chip_erase_eeprom;
	jsdcc_mem_set_active_boot = jsdcc_mem_set_active_boot_eeprom;
  }
...
int reflect(int data, const UINT32 len)
{
  int ref = 0;
  UINT32 i;
