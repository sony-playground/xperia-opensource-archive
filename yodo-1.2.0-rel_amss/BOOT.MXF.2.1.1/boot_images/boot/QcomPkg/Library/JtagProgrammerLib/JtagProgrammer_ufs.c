...
----------   ---     ----------------------------------------------------------
2019-07-12   jt      Fix correct index for set hw partition
2019-06-04   jt      Open LUNs only when they are needed
2018-12-12   jt      Move alloc units calculation to UFS driver
2017-12-01   yps     Replace banned functions
...
struct ufs_lun_config_descr storage_extras;
...
struct ufs_handle *partition_config = NULL;           /* User partition handle */
struct ufs_handle *partition_handles[MAX_PARTITIONS] = {0};
...
UINT32 jsdcc_mem_read_ufs(void)
{
    ...
    {
       partition_handles[current_handle_index] = ufs_open(0, current_handle_index);
    }

    pdata = jtag_prog_param.data;
    rc = ufs_read(partition_handles[current_handle_index], pdata,
...
UINT32 jsdcc_mem_write_ufs(void)
{
    ...
    pdata = jtag_prog_param.data;
    rc = ufs_write(partition_handles[current_handle_index], pdata,
                   jtag_prog_param.addr, jtag_prog_param.data_length);
...
UINT32 jsdcc_mem_program_ufs(void)
{
   ...
   if (jtag_prog_param.data_length < BLOCK_SIZE) {
      ...
      memscpy(sector_buffer + sector_buffer_offset, sizeof(sector_buffer) - sector_buffer_offset, jtag_prog_param.data, jtag_prog_param.data_length);
   ...
   if (NULL != pdata) {
      rc = ufs_write(partition_handles[current_handle_index], pdata, redirect_offset,
                     blocks_to_write);
...
UINT32 jsdcc_mem_erase_ufs(void)
{
   ...
   while(jtag_prog_param.data_length > 0)
   {
       ...
       if (partition_handles[current_handle_index] != NULL) {
         pdata = jtag_prog_param.data;
...
UINT32 jsdcc_mem_compute_crc_ufs(void)
{
   ...
   pdata = jtag_prog_param.data;
   rc = ufs_read(partition_handles[current_handle_index], pdata, jtag_prog_param.addr,
                 sectors_to_read);
...
UINT32 jsdcc_mem_chip_erase_ufs(void)
{
    ...
    for (i = 0; i < (max_partitions - 1); i++)
    {
        ...
        if (NULL != partition_handles[i])
        {
            outputMessage("Calling ufs_format(LUN %d) \n", i);
            rc = ufs_format(partition_handles[i]);
            if (rc != 0)
            {
...
UINT32 jsdcc_mem_set_hw_partition_ufs(void)
{
   ...
   {
     if (NULL == partition_handles[jtag_prog_param.addr])
     {
        partition_handles[jtag_prog_param.addr] = ufs_open(0, jtag_prog_param.addr);
...
UINT32 jsdcc_mem_firmware_update_ufs(void)
{
   ...
   if (NULL != pdata) {
      ...
      rc = ufs_fw_update_write (partition_handles[current_handle_index], blocks_to_write * BLOCK_SIZE,
