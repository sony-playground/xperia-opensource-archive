...
pm_err_flag_type pm_schg_misc_get_sysok_reason(uint32 pmic_index, pm_schg_misc_sysok_reason_type *sysok_reason)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_register_data_type data = 0;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(pmic_index);
...
pm_err_flag_type pm_schg_misc_bark_bite_wdog_pet(uint32 pmic_index)
{
  ...
  else
  {
    ...
    err_flag = pm_comm_write_byte_mask(SLAVE_ID, wdog_pet, mask, 0x01);
  }
...
pm_err_flag_type pm_schg_misc_aicl_cmd(uint32 device_index, pm_schg_misc_aicl_cmd_type cmd)
{
  ...
  else
  {
    ...
    err_flag = pm_comm_write_byte_mask(device_index, aicl_cmd_reg, data, mask);
  }
