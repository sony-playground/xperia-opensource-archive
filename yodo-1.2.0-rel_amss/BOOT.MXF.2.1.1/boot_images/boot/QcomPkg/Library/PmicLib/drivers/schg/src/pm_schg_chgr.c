...
pm_err_flag_type pm_schg_chgr_enable_src(uint32 pmic_index, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(pmic_index);
...
pm_err_flag_type pm_schg_chgr_get_chgr_status(uint32 pmic_index, pm_schg_chgr_status_type *charger_status)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
...
pm_err_flag_type pm_schg_chgr_set_jeita_fvcomp_cfg(uint32 pmic_index, uint32 fv_comp_hot, uint32 fv_comp_cold)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_register_data_type data = 0;
  ...
  else
  {
    ...
    err_flag |= pm_comm_write_byte_mask(SLAVE_ID, fv_cmp_cold_cfg, mask, data);

    /* JEITA Float Voltage Compensation in hot soft limit  */
    data = fv_comp_hot / PM_SCHG_JEITA_FLOAT_VOLTAGE_STEP_CONSTANT;
    err_flag |= pm_comm_write_byte_mask(SLAVE_ID, fv_cmp_hot_cfg, mask, data);
  }
...
pm_err_flag_type pm_schg_chgr_set_jeita_ccomp(uint32 pmic_index, uint32 ccomp_hot, uint32 ccomp_cold)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_register_data_type data = 0;
  ...
  else
  {
    ...
    err_flag |= pm_comm_write_byte_mask(SLAVE_ID, fcc_cmp_cold_cfg, mask, data);

    /* JEITA Charge Current Compensation in hot soft limit */
    data = ccomp_hot / PM_SCHG_JEITA_CHARGE_CURRENT_STEP_CONSTANT;
    err_flag |= pm_comm_write_byte_mask(SLAVE_ID, fcc_cmp_hot_cfg, mask, data);
  }
...
pm_err_flag_type pm_schg_chgr_get_schg_pmic_info(uint32 *pmic_index, uint32 *slave_id, uint32 *charger_count, uint32 max_charger)
{
  uint32 i = 0;
