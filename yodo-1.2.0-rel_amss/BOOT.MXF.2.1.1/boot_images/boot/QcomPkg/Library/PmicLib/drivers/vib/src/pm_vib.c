...
/* =======================================================================
$Header: //components/rel/core.boot/6.1.1/QcomPkg/Library/PmicLib/drivers/vib/src/pm_vib.c#1 $
$Author: pwbldsvc $
...
/* //Vib perepheral do not exist PMI8994.  Need to use Haptics instead
   ...
   switch (mode)
   {
      case PM_VIB_MODE__MANUAL:
         err_flag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib_play, 0x80, enable << 7);
         if (err_flag != PM_ERR_FLAG_SUCCESS)
         {
            goto pm_vib_enable_exit;
         }
         ...
         {
            err_flag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib_en_ctl, 0x80, (pm_register_data_type)(enable << 7));
            if (err_flag != PM_ERR_FLAG_SUCCESS)
            {
               goto pm_vib_enable_exit;
            }
...
/* //Vib perepheral do not exist PMI8994.  Need to use Haptics instead
   ...
   {
      err_flag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib_vmax_cfg, 0x3E, (pm_register_data_type)(voltage / 116));
   }
*/
...
/* //Vib perepheral do not exist PMI8994.  Need to use Haptics instead
   ...
   errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, VIB_CFG1_REG_MASK, VIB_CFG1_ACTUATOR_TYPE_VALUE);
   ...
   if (VIB_CFG1_ACTUATOR_TYPE_VALUE == 1) // ERM actuator
   {
      ...
      errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, 0x7F, data);
      if (errFlag != PM_ERR_FLAG_SUCCESS)
      {
         goto pm_vib_init_exit;
      }
   ...
   else
   {
      ...
      errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, 0x01, data);
      if (errFlag != PM_ERR_FLAG_SUCCESS)
      {
         goto pm_vib_init_exit;
      }
      ...
      errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, 0xFF, data);
      if (errFlag != PM_ERR_FLAG_SUCCESS)
      {
         goto pm_vib_init_exit;
      }
   ...
   errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, VIB_ILIM_SEL_MASK, VIB_ILIM_SEL_VAL);
   if (errFlag != PM_ERR_FLAG_SUCCESS)
   {
      goto pm_vib_init_exit;
   }
   ...
   errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, VIB_INTERNAL_PWM_FREQ_SEL_MASK, VIB_INTERNAL_PWM_FREQ_SEL_VAL);
   if (errFlag != PM_ERR_FLAG_SUCCESS)
   {
      goto pm_vib_init_exit;
   }
   ...
   errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, VIB_EXTERNAL_PWM_FREQ_SEL_MASK, VIB_EXTERNAL_PWM_FREQ_SEL_VAL);
   if (errFlag != PM_ERR_FLAG_SUCCESS)
   {
      goto pm_vib_init_exit;
   }
   ...
   errFlag = pm_comm_write_byte_mask(vib_ptr->comm_ptr->slave_id, reg_vib, VIB_PWM_CAP_SEL_MASK, VIB_PWM_CAP_SEL_VAL);
   if (errFlag != PM_ERR_FLAG_SUCCESS)
   {
      goto pm_vib_init_exit;
   }
