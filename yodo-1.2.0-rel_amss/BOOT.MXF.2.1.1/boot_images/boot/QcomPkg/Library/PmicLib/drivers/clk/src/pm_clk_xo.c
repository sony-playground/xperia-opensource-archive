...
$Header: //components/rel/core.boot/6.1.1/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk_xo.c#1 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_clk_xo_mode(uint8 pmic_chip, pm_clk_type periph, pm_clk_xo_mode_type mode)
{
  ...
  err_flag = pm_comm_write_byte_mask(clk_ptr->comm_ptr->slave_id, reg, 0x03, (pm_register_data_type)mode);

  return err_flag;
}

pm_err_flag_type 
pm_clk_xo_trim (uint8 pmic_chip, 
                pm_clk_type periph, 
                uint32 trim_value)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  ...
  err_flag = pm_comm_write_byte_mask(clk_ptr->comm_ptr->slave_id, reg, 0x3F, (pm_register_data_type)trim_value);
