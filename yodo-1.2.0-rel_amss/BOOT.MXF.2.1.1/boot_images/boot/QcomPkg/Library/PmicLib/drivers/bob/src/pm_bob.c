...
$Header: //components/rel/core.boot/6.1.1/QcomPkg/Library/PmicLib/drivers/bob/src/pm_bob.c#1 $
...
pm_bob_ocp_latched_status_clear(uint8 pmic_chip, uint8 peripheral_index,  boolean *ocp_occured)
{
   ...
   else
   {
       ...
       err_flag |= pm_pwr_ocp_latched_status_clear_alg(&(bob_ptr->pm_pwr_data), bob_ptr->comm_ptr, peripheral_index, is_broadcast_cfg_present, ocp_occured);
   }
