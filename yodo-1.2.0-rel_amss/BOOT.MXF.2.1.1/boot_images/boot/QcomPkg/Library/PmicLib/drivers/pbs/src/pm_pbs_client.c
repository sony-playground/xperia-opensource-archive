...
$Header: //components/rel/core.boot/6.1.1/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_client.c#1 $
$Author: pwbldsvc $
...
#define PBS_CLIENT_BASE_ADDR(pbs_core_index, pbs_client_index)  pbs_client_ptr->base_address[pbs_core_index] + (pbs_client_ptr->pbs_client_reg_table->peripheral_offset*pbs_client_index)
/*===========================================================================

                        FUNCTION DEFINITIONS

===========================================================================*/
pm_err_flag_type pm_pbs_client_enable(uint8 pmic_device_index, pm_pbs_core_type pbs_core_index, pm_pbs_client_type pbs_client_index, boolean enable)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type reg = 0;
    pm_pbs_data_type *pbs_client_ptr = pm_pbs_get_data(pmic_device_index);
    uint8 data = 0;
    uint8 mask = 0x80;
    ...
    err_flag = pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, reg, mask, data);

    return err_flag; 
}

pm_err_flag_type pm_pbs_client_enable_status(uint8 pmic_device_index, pm_pbs_core_type pbs_core_index, pm_pbs_client_type pbs_client_index, boolean *status)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_register_address_type reg = 0;
    pm_pbs_data_type *pbs_client_ptr = pm_pbs_get_data(pmic_device_index);
    ...
    err_flag = pm_comm_read_byte_mask(pbs_client_ptr->comm_ptr->slave_id, reg, mask, &data);
...
pm_err_flag_type pm_pbs_client_sw_trigger(uint8 pmic_chip, pm_pbs_core_type pbs_core_index, pm_pbs_client_type pbs_client_index, pm_pbs_client_trigger trigger, 
                                          uint8 *trigger_argument, uint8 *trigger_return_value)
{
    pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
    pm_pbs_data_type *pbs_client_ptr = pm_pbs_get_data(pmic_chip);
    uint8 data = 0x80;
    ...
    if (trigger_argument != NULL)
    {
        ...
        err_flag |= pm_comm_write_byte(pbs_client_ptr->comm_ptr->slave_id, scratch1, *trigger_argument);
    }

    trig_ctl = base_addr + pbs_client_ptr->pbs_client_reg_table->trig_ctl;

    // Trigger the PBS sequence
    err_flag |= pm_comm_write_byte(pbs_client_ptr->comm_ptr->slave_id, trig_ctl, data);
    ...
    if (trigger_return_value != NULL)
    {
        ...
        err_flag |= pm_comm_read_byte(pbs_client_ptr->comm_ptr->slave_id, scratch2, trigger_return_value);
    }
...
pm_err_flag_type
pm_pbs_client_hw_trigger_enable(uint8 pmic_chip, pm_pbs_core_type pbs_core_index, pm_pbs_client_type pbs_client_index, pm_pbs_client_trigger hw_trigger, pm_pbs_client_trigger_edge_type trigger_edge, boolean enable)
{
    pm_err_flag_type err_flag                      = PM_ERR_FLAG_SUCCESS;
    ...
    else //Disable triggering
{
    ...
    switch (trigger_edge)
  {
    ...
    err_flag = pm_comm_write_byte(pbs_client_ptr->comm_ptr->slave_id, en_ctl_addr, 0x00);
    err_flag |= pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, hw_trig_re_en_addr, hw_trig_re_fe_en_mask, hw_trigger_re_data);
    err_flag |= pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, hw_trig_fe_en_addr, hw_trig_re_fe_en_mask, hw_trigger_fe_data);

    err_flag |= pm_comm_write_byte(pbs_client_ptr->comm_ptr->slave_id, trig_latch_clr_addr, 0xFF);
    err_flag |= pm_comm_write_byte(pbs_client_ptr->comm_ptr->slave_id, en_ctl_addr, 0x80);

    //Enables/Disable RIF memory interface and the RIF Memory Access Mode. When enabled the PBS can run concurrently.
    err_flag |= pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, base_addr + MEM_INTF_CFG_ADDR, base_addr + MEM_INTF_CFG_RMSK, 0x80); //0x7040 = 0x80 : Enables 
    err_flag |= pm_comm_write_byte_mask(pbs_client_ptr->comm_ptr->slave_id, base_addr + MEM_INTF_CFG_ADDR, base_addr + MEM_INTF_CFG_RMSK, 0x00); //0x7040 = 0x00 : Disable 
