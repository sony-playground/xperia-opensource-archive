/*! \file
*
*  \brief  pm_scpq_typec.c driver implementation.
...
#define ADDRESS(x) (scpq_ptr->base_address + \
                    scpq_ptr->typec_register->peripheral_offset + \
                    scpq_ptr->typec_register->x)
...
pm_err_flag_type pm_scpq_usb_typec_vconn_enable(uint32 pmic_index, pm_scpq_usb_typec_vconn_enable_type enable_type)
{
  ...
  else if(enable_type >= PM_SCPQ_USB_TYPEC_VCONN_INVALID)
  {
    err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
  }
  else
