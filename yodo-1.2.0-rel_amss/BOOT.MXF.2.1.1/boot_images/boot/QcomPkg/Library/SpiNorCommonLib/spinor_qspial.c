...
static SPINOR_STATUS config_cmd_spi(QSPI_CMD *qspi_config_cmd, SPI_COMMAND_TYPE cmd_mode, uint8_t opcode);

SPINOR_STATUS spial_init(bool init_status)
{
...
SPINOR_STATUS config_cmd_spi(QSPI_CMD *qspi_config_cmd, SPI_COMMAND_TYPE cmd_mode, uint8_t opcode)
{
  QSPI_CMD *qspi_cmd;
  ...
  if (cmd_mode == qspi_read_mode)
  {
    ...
    case READ_DDR_1_1_1_MODE:
      qspi_cmd->cmd_mode     = DDR_1BIT;
      qspi_cmd->addr_mode    = DDR_1BIT;
      qspi_cmd->data_mode    = DDR_1BIT;
    ...
    case READ_DDR_1_2_2_MODE:
      qspi_cmd->cmd_mode     = DDR_1BIT;
      qspi_cmd->addr_mode    = DDR_2BIT;
      qspi_cmd->data_mode    = DDR_2BIT;
      qspi_cmd->opcode       = spinor_dev.config_data->read_ddr_opcode;
      qspi_cmd->dummy_clocks = spinor_dev.config_data->read_ddr_wait_state;
      break;
