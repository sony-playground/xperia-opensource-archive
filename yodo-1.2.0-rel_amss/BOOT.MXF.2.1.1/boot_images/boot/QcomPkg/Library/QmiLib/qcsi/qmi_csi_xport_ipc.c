...
static int find_in_rx_q(struct qmi_csi_xport_handle *xp, ipc_router_address_type *addr)
{
  ...
  if ((buf == NULL) && (xp->rx_handling != NULL) &&
      (memcmp(&xp->rx_handling->src_addr, addr, sizeof(*addr)) == 0))
  {
    buf = xp->rx_handling;
  }
  return buf ? 1 : 0;
  ...
  switch(event)
  {
    ...
    case IPC_ROUTER_EVENT_CLOSED:
    {
      ...
      while(buf)
     {
        ...
        buf=buf->link.next;
        if(buf_to_free->msg)
           FREE(buf_to_free->msg);
        ipc_router_packet_free(&buf_to_free->pkt);
        FREE(buf_to_free);
      }
      UNLOCK(xp->rx_q_lock);
 ...
 )
{
  ...
  while((buf = dequeue_rx(xp)) != NULL)
  {
    ...
    LOCK(xp->rx_q_lock);
    /* Current RX buf handling is done */
    xp->rx_handling = NULL;
    UNLOCK(xp->rx_q_lock);
    FREE(buf);
  }
