...
extern int ipc_router_core_open
(
...
extern int ipc_router_core_send_conf
(
 ipc_router_client_type *client,
 ipc_router_address_type *addr
);

extern int ipc_router_core_send
(
 ipc_router_client_type *client,
...
{
  struct qmi_cci_xport_handle *xp = CALLOC_QCCI(1, sizeof(struct qmi_cci_xport_handle));
  if(xp)
  {
    xp->clnt = clnt;
 ...
 )
{
  ...
  if(num_entries)
  {
    servers = MALLOC_QCCI(sizeof(ipc_router_server_type) * (*num_entries));
    if(!servers)
      return 0;
  }
