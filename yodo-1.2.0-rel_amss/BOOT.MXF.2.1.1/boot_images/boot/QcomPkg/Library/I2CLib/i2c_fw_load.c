...
i2c_status fw_init(plat_device_config *dcfg)
{
   ...
   uint32      qupv3_se_protocol = HWIO_INXF(se_base, QUPV3_SEn_GENI_FW_REVISION, PROTOCOL);
   uint32      qupv3_hw_ver;
   uint32 mask,i,result;
   uint32      is_SE_Minicore = HWIO_INXF(se_base, QUPV3_SEn_SE_HW_PARAM_2, GEN_USE_MINICORES);
   ...
   if(!dcfg->i2c_hub)
   {
      mask = HWIO_INXF(se_base, QUPV3_SEn_SE_HW_PARAM_1, RX_FIFO_DEPTH);
   }
   else
   {
      mask = 16;
