...
- Bit 6: Debug check in Image certificate field
? IsProductionDevice check:
- Check bits 0 AND 1 AND 2 AND 6 in status.value
? JTAG disable check:
...
? Image Cert Debug disabled check:
- Check bit 6 only in status.value
*/
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 05/22/19   dp      Added Image certificate debug check for security state
...
/**
  Check if debug check in Image Certificate passed
...
IsImageCertDebugDisabled(void)
{
    ...
    // Bit#6 Debug Enabled Check
    if(((secure_state >> 6) & 0x1)) // Need to check if it is 1 unlike previous bits
        return TRUE; // Debug disabled, production device

    return FALSE; // Debug enabled
}
