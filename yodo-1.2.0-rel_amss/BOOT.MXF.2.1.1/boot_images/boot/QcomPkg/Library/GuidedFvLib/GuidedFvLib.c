...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/21/19   md      Updated call to ProcessFvImageFile API
                                 ...
                                 CHAR8    *Args OPTIONAL)
{
  ...
  AsciiStrToUnicodeStrS(AppName, AppNameStr, AsciiStrSize(AppName));
  ...
  if (Args)
  {
    ...
    AsciiStrToUnicodeStrS (Args, ArgsStr, AsciiStrSize(Args));
    ...
    else
    {
      ...
      LoadOptionMem = AllocatePool(ImageInfo->LoadOptionsSize);
      ImageInfo->LoadOptions = LoadOptionMem;
      if (ImageInfo->LoadOptions != NULL)
        StrCpyS(ImageInfo->LoadOptions, ImageInfo->LoadOptionsSize/sizeof(CHAR16), ArgsStr);
      else
        DEBUG((EFI_D_WARN, "Could not pass arguments\n"));
    }
    FreePool (ArgsStr);
                      ...
                      OUT UINTN  *FileSize)
{
  ...
  AsciiStrToUnicodeStrS(FileName, FileNameStr, AsciiStrSize(FileName));
