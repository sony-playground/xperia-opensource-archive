...
* 
*/
static boolean calculate_segment_offsets
(
  void * elf_base, 
  Elf64_Ehdr *ehdr, 
  uint8 **hash_seg_base, 
  ...
  uint8 **prog_seg_base, 
  uint32 *prog_seg_size
)
{
  int i = 0;
  uint32 flags, loadable_seg_count =0;
  Elf64_Phdr * phdr = (Elf64_Phdr *)( ( (uint8 *)elf_base ) + ehdr->e_phoff );
  Elf64_Phdr * current_phdr;

  if( elf_base == NULL || ehdr == NULL ||
      hash_seg_base == NULL || hash_seg_size == NULL ||
      prog_seg_base == NULL || prog_seg_size == NULL
    )
  {
    return FALSE;
  }
  for (i = 0; i < ehdr->e_phnum; i++)
  {
    ...
    {
      if(loadable_seg_count > 1)
      {
        DEBUG(( EFI_D_ERROR, "ERROR: Loadable Seg not unique. Processing %d", loadable_seg_count ));
      }      
...
static boolean calculate_segment_offsets_32
(
  void * elf_base, 
  Elf32_Ehdr *ehdr_32, 
  uint8 **hash_seg_base,
...
)
{
  ...
  for (i = 0; i < ehdr_32->e_phnum; i++)
  {
    ...
    else if (MI_PBT_ELF_PT_LOAD ==  current_phdr->p_type)
    {
      if(loadable_seg_count > 1)
      {
        DEBUG(( EFI_D_ERROR, "WARN: Loadable Seg not unique. Processing %d", loadable_seg_count ));
      }
      *prog_seg_base = ( (uint8 *)elf_base ) + current_phdr->p_offset;
      *prog_seg_size += current_phdr->p_memsz;
      loadable_seg_count++;
  ...
  )
{
  ...
  if( elf_base == NULL || meta_base == NULL ||
      prog_base == NULL || prog_size == NULL
    )
  {
    return EFI_ABORTED;
  }  
  ...
  if ( elf_format == ELF_CLASS_32 )
  {
    ...
    status = calculate_segment_offsets_32(elf_base, ehdr_32, &hash_seg_base, 
                                          &hash_seg_size,&prog_seg_base, 
                                          &prog_seg_size);
    if(status == FALSE)
    {
      return EFI_ABORTED;
    ...
    {
      DEBUG(( EFI_D_ERROR, "Error: Hash seg size is zero"));
    }
  }
  else if ( elf_format == ELF_CLASS_64 )
  {
    ...
    /* Calculate hash and program segment offsets */
    status = calculate_segment_offsets(elf_base, ehdr, &hash_seg_base, 
                                      &hash_seg_size, &prog_seg_base, 
                                      &prog_seg_size);
    if(status == FALSE)
    {
