...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 03/25/22   bn      Addressed unused variable
 06/22/21   PCR     Added changes to manage GPIOs/Clocks for multiple clients
...
)
{
	...
	if((polarity || phase) == 0 ) 
	...
	else if(polarity == 0)
	 {
	  ...
	  config.cs_toggle = !param.eCSMode;
      ...
      status = spi_full_duplex(spi_handle,&config,&desc,num_descriptors,NULL,NULL,TRUE,TRUE);
