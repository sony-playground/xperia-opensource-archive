/*============================================================================
 ...
 $Header: //components/rel/core.boot/6.1.1/QcomPkg/Library/ChipInfoLib/ChipInfo.c#2 $
...
DALResult ChipInfo_GetDefectiveCPUs(uint32 nCPUCluster, uint32 *pnMask)
{
  ...
  if (nCPUCluster == 0 && !ChipInfoCtxt.nNumClusters)
  {
     ...
     * successfully with pnMask = "not defective/unknown", instead of
...
/* ===========================================================================
 **  FUNCTION      ChipInfo_GetMarketingData
 **
 **  DESCRIPTION   See ChipInfo.h
 ** ======================================================================== */
...
{
  if (!pData || ePart == CHIPINFO_PART_UNKNOWN)
  {
    return CHIPINFO_ERROR_INVALID_PARAMETER;
  }
  ...
  if (!ChipInfoCtxt.pPartMarketingData ||
      !ChipInfoCtxt.nNumPartsWithMarketingData)
  {
    return CHIPINFO_ERROR_NOT_SUPPORTED;
  }

  for (int i = 0; i < ChipInfoCtxt.nNumPartsWithMarketingData; i++)
  {
    if (ChipInfoCtxt.pPartMarketingData[i]->ePart == ePart)
    {
      ChipInfo_memcpy(
        pData,
        sizeof(ChipInfoPartMarketingDataType),
        ChipInfoCtxt.pPartMarketingData[i],
        sizeof(ChipInfoPartMarketingDataType));
      return CHIPINFO_SUCCESS;
    }
  }
