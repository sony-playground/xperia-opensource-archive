...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 09/14/120  cm       Fix in MountDesiredFVFromBuffer() GUID checks
...
MountDesiredFVFromBuffer(IN VOID *Buffer, 
                         IN UINTN BufferSize,
                         IN  EFI_GUID *DesiredFvGuid, OPTIONAL
                         IN EFI_GUID *DesiredFVFileGUID OPTIONAL)
{
  EFI_STATUS    Status = EFI_NOT_FOUND;
  ...
  /* If DesiredFvGuid is mentioned and we have mounted the desired FV then exit */
  if(DesiredFvGuid != NULL && GuidedFvIsMounted(DesiredFvGuid))
    return EFI_SUCCESS;
