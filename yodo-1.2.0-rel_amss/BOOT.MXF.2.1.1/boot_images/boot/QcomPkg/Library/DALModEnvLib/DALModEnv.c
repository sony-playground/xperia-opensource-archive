...
void DALSYS_InitMod(DALSYSConfig * pCfg)
{
   if (pCfg)
   {
      ...
      if (configDefined)
      {
         DEBUG ((EFI_D_ERROR, "Module cannot re-initialize DAL module environment\n"));
         ASSERT_EFI_ERROR (0);
      }
      configDefined = 1;

      //populate the DALProps structure which contains the string device info list
      gpDALModStringDeviceInfoList->dwDeviceSize = pCfg->stringDeviceInfoList.dwLen;
      gpDALModStringDeviceInfoList->pDevices = pCfg->stringDeviceInfoList.pDevices;
      gpDALModDriverInfoList = &pCfg->numericDriverInfoList;

      getDALSYSProtocol();
      gpDALSYSInitFncPtr->init(gpDALModDriverInfoList, gpDALModStringDeviceInfoList, &gpDALSYSFncPtr);
   }
   else
   {
      if (!gpDALSYSFncPtr)
...
UINTN 
DALSYS_memscpy(VOID * pDest, UINTN iDestSz, CONST VOID * pSrc, UINTN iSrcSize)
{
