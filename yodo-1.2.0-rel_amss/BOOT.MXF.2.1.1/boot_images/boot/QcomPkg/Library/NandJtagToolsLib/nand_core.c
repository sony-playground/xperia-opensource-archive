 ...
 *   order.
 *
 *   $Header: //components/dev/core.boot/5.1.1/glosch.core.boot.5.1.1.nand_cfg_olympic/QcomPkg/Library/NandJtagToolsLib/nand_core.c#1 $
 *   $DateTime: 2022/04/04 03:16:28 $
 *   $Author: glosch $
 *
 * when         who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 01/07/21     sa      Pass page count as parameter.
 * 08/01/20     sa      Workaround for Rumi.
 * 07/08/20     sa      Close all references to mibib handle.
...
int flash_prog_erase_bad_block_table()
{
  ...
  bbt_magic_page = (mibib.flash_info.nand_info.pages_per_block * bbt_block);
  ...
  result = flash_prog_read_pages(mibib.flash_handle, &mibib.flash_info,
    bbt_magic_page, 1, read_type, page_buffer);
  ...
  {
    if (FLASH_DEVICE_DONE != nand_block_get_state(mibib.flash_handle,
      bbt_block, &blk_state))
    {
    ...
    if (NAND_DEVICE_DONE != nand_erase(mibib.flash_handle, bbt_block, 1))
    {
  ...
  int count)
{
  ...
  memset((void *)magic_buffer, 0xFF, MAX_PAGE_SIZE);
  ...
  unsigned char *src_main, unsigned char *src_spare )
{
  ...
  if (enable_read_verify)
  {
    ...
    result = flash_prog_read_pages_and_udata(pctl->flash_handle, &pctl->flash_info,
    pctl->current_page, 1, read_type, page_buffer, readback_spare_buf,
    pctl->spare_bytes_len);
    ...
    for (i=0; i < main_bytes_to_compare; i++)
    {
      if (*vsrc1++ != *vsrc2++)
      {
        ...
        FLASHLOG(1,("src at 0x%08X  readback at 0x%08X\n", 
          (uintnt)src_main, (uintnt)page_buffer));
        FLASHLOG(1,("Error: verify failure\n"));
        FLASHLOG(1,("fmmv: Page = %ld Block = %ld \n", 
          (uintnt)pctl->current_page,
          (uintnt)pctl->current_block));
  ...
  unsigned char *spare_bytes)
{
  ...
  result = flash_prog_write_page_and_udata (pctl->flash_handle, &pctl->flash_info,
    pctl->current_page, NAND_ACCESS_MAIN_SPARE, src, temp_spare_buff,
    pctl->spare_bytes_len);
  ...
  result = flash_prog_read_pages_and_udata(pctl->flash_handle, &pctl->flash_info,
    pctl->current_page,1, NAND_ACCESS_MAIN_SPARE, page_buffer, spare_buffer,
    pctl->spare_bytes_len);
...
int flash_prog_erase_partition(parti_ctl_t pctl)
{
  ...
  int result = FLASH_DEVICE_DONE;
  int pageno;
  FLASHLOG (3, ("Read the 1st page to partition to check if it is erased.\n"));

  /* Need to read only the 1st page, so read page 0 */
  pageno = 0;
  
  result = flash_prog_read_pages(pctl->flash_handle, &pctl->flash_info,
    pageno, 1, read_type, page_buffer);

  if (FLASH_DEVICE_DONE != result)
  {
    if (FLASH_DEVICE_FAIL_PAGE_ERASED == result)
    {
      FLASHLOG (3, ("Partition already erased. SKIP erasing.\n"));
      return FLASH_PROG_SUCCESS;
    }
  }

  FLASHLOG (3, ("Partition not  erased. Erasing the whole partition.\n"));
#endif
  /* Erase the whole partition */
  for (blockno=0; blockno<(pctl->flash_info.nand_info.block_count); blockno++)
  {
    KICK_WATCHDOG();

    if (DAL_SUCCESS != nand_block_get_state(pctl->flash_handle,
      blockno, &blk_state))
    {
      return FLASH_PROG_FAIL;
    }
    ...
    /* erase the block */
    if (NAND_DEVICE_DONE != nand_erase(pctl->flash_handle, blockno, 1))
    {
      FLASHLOG (1,("Error: device failed during erase\n"));
      SET_ERR (ERR_ERASE_FAIL);
      return FLASH_PROG_FAIL;
...
uint32 flash_prog_erase_some_of_chip (uint32 start_block)
{
  ...
  KICK_WATCHDOG();
  //outputMessage ("\n\n------ Erase Chip ------\n");
  //outputMessage ("Probing flash device: \n");
  FLASHLOG (1, ("\n\n------ Erase Chip ------\n"));
  ...
  for (blockno=start_block; blockno<mibib.flash_info.nand_info.block_count;
    blockno++)
  {
    KICK_WATCHDOG();

    if (FLASH_DEVICE_DONE != nand_block_get_state(mibib.flash_handle, blockno,
      &blk_state))
    {
      return FLASH_PROG_FAIL;
    }
...
int flash_prog_find_starting_erase_block (flash_partable_t ram_parti_ptr)
{
  ...
  parti_page = (newest_mibib_block *
    mibib.flash_info.nand_info.pages_per_block) +
    MIBIB_PAGE_PARTITION_TABLE;

  result = flash_prog_read_pages(mibib.flash_handle, &mibib.flash_info,
    parti_page, 1, read_type, page_buffer);
...
uint32 flash_prog_init_parti(uint32 size)
{
  ...
  memset(oldest_usr_parti_buffer, 0xFF,
  ...
  if (size > mibib.flash_info.nand_info.page_size_bytes)
  {
  ...
  newest_mibib_block = mibib.flash_info.nand_info.mibib_info.new_mibib_block;
  oldest_mibib_block = mibib.flash_info.nand_info.mibib_info.old_mibib_block;
  ...
  memset(oldest_parti_buffer, 0xFF,
    mibib.flash_info.nand_info.page_size_bytes);
  ...
  /* Close all the handles to this partition so that the next call can read the new mibib */ 
      
  if (NAND_DEVICE_DONE !=  nand_close(mibib.flash_handle, 1))
  {
     FLASHLOG(3, ("Error closing all device handle \n"));
     SET_ERR (ERR_DEV_MISSING);
     return FLASH_PROG_FAIL;
  }
...
uint32 flash_prog_deinit_tools(void)
{
  ...
  {
    if (NAND_DEVICE_DONE != nand_close(mibib.flash_handle, 0))
    {
...
static int flash_prog_parse_factory_img_string_table(parti_ctl_t pctl)
{
  ...
  while ((size > 0) && (hdr_parsed_total == FALSE))
  {
    ...
    if (strcmp(factory_img_str, "page_bytes_user") == 0)
    {
      ...
      if (pctl->flash_info.nand_info.page_size_bytes != page_size_from_device)
      {
        FLASHLOG ( 1,("\n Page size MISMATCH"));
    ...
    if (strcmp(factory_img_str, "block_pages") == 0)
    {
      ...
      if (pctl->flash_info.nand_info.pages_per_block != pages_in_block)
      {
    ...
    if (strcmp(factory_img_str, "device_blocks") == 0)
    {
      ...
      if (pctl->flash_info.nand_info.block_count < device_blocks)
      {
      ...
      (pctl->avail_pages_in_block == 0)))
    {
      while (new_partition_found == FALSE)
      {
        ...
        next_parti_start_addr =
          ...
          pctl->page_size * pctl->flash_info.nand_info.pages_per_block +
          pctl->factory_img.bin_img_offset;
    ...
    if (pctl->record_header_parsed == 0)
    {
            ...
            (pctl->curr_record_addr != 0))
      {
        ...
        pctl->current_page = (start_spare_block + erase_blk_count + bb_count) \
                * pctl->flash_info.nand_info.pages_per_block;
...
int flash_prog_get_flash_info(void)
{
  ...
  nand_core_strcpy((uint8 *)mibib.dev_params.flash_name,
    mibib.flash_info.nand_info.device_name, FLASH_PROG_NAME_SIZE);

  mibib.dev_params.num_blocks = mibib.flash_info.nand_info.block_count;
  mibib.dev_params.page_size = mibib.flash_info.nand_info.page_size_bytes;
  mibib.dev_params.pages_in_block = mibib.flash_info.nand_info.pages_per_block;
  prog_param.reserved = (uintnt)&mibib.dev_params;
    ...
    unsigned char *src_spare)
{
  ...
  enum nand_block_state bad_block = NAND_BLOCK_BAD;
...
next_good_block:
  ...
  page_idx=failed_block * pctl->flash_info.nand_info.pages_per_block;
  for (; page_idx <= failed_page; page_idx++)
  {
    ...
    if (page_idx < failed_page)
    {
      ...
      result = flash_prog_read_pages_and_udata(pctl->flash_handle, &pctl->flash_info,
        page_idx, 1, read_type, page_buffer, readback_spare_buf,
        pctl->spare_bytes_len);
  ...
  if (FLASH_DEVICE_DONE != result)
  {
    if (FLASH_DEVICE_DONE != nand_block_set_state(pctl->flash_handle,
