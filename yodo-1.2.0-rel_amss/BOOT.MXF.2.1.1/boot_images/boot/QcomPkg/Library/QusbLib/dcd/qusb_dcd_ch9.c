...
int qusb_dcd_control_transfer(struct qusb_dcd_dsc_device* dev, int is_tx, void* buff, uint32 len)
{
  ...
  if (is_tx==1)
  {
    qusb_bus_log(DCD_CONTROL_TRANSFER_TX_LOG, 0, len);
  }
  else
  {
    qusb_bus_log(DCD_CONTROL_TRANSFER_RX_LOG, 0, len);
  }
...
int handle_get_descriptor(struct qusb_dcd_dsc_device* dev, struct usb_setup_packet* req)
{
  ...
  switch ( dsc->desc_type ) 
  {
    ...
    {
      if(((dev->speed != QUSB_SUPER_SPEED) && (dev->speed != QUSB_SUPER_SPEED_PLUS))
  ...
  switch ( req->bRequest )
  {
    ...
    case USB_SETUP_REQ_GET_INTERFACE:
      ...
      { 
        ...
        memset((void*)dev->setup_data, 0, sizeof(dev->setup_data));
                          ...
                          struct qusb_dcd_dsc_endpoint* ep, int halt)
{
  ...
  qusb_bus_log(HANDLE_EP_HALT_LOG, 0, (ep_num * 10) + is_tx);

  if ( halt != 0 ) 
  {
    qusb_uart_log("set_ep_halt", (ep_num << 4) + is_tx);
    qusb_dci_stall_endpoint(ep_num, is_tx);
  } 
  else 
  {
    qusb_uart_log("clear_ep_halt", (ep_num << 4) + is_tx);
    qusb_dci_cancel_unstall_requeue(ep_num, is_tx);
  }
  return 0;
}
...
uint32 qusb_dcd_init(struct qusb_dcd_dsc_device* dev)
{
  ...
  dev->shutdown_loop_iteration=500;
  
  status = qusb_dci_init(dev->core_id, dev, &dci_callbacks);
  return status;
}
...
void qusb_dcd_shutdown(struct qusb_dcd_dsc_device* dev)
{
  ...
  for (i = 0; (QUSB_URB_STATUS_ACTIVE == dev->setup_tx_urb.transfer_status) && (i < dev->shutdown_loop_iteration); i++)
  {
    qusb_dcd_poll(dev);
  }
