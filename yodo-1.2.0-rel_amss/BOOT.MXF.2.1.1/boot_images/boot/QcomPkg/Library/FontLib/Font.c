...
Copyright 2016-2018,2020,2022 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
===========================================================================*/
...
--------   ---        -------------------------------------------------------
07/18/22   yps        Add null pointer check
...
)     
{
  ...
  memset((void *)&clear_pixel,0x0, sizeof(pixel_data_type));
  ...
  { 
    ...
    for(row_index =0; row_index < GLYPH_HEIGHT_IN_PIXEL; row_index++ ) 
    {
      ...
      for(column_index =0; column_index < GLYPH_WIDTH_IN_PIXEL; column_index++)
      {
        ...
        {
          memcpy((void *)buff_ptr, (void *)current_glyph->pixel_data, sizeof(pixel_data_type));
        }
        else
        {
          memset((void *)buff_ptr,0x0, sizeof(pixel_data_type));
        }
  ...
  do
  {
    ...
    for(src_row_index =0; src_row_index < GLYPH_HEIGHT_IN_PIXEL; src_row_index++)
    {
      ...
      for(src_col_index =0; src_col_index < GLYPH_WIDTH_IN_PIXEL; src_col_index++)
      {
        ...
        {
          memcpy((void *)dst_pixel_ptr, (void *)src_pixel_ptr, sizeof(pixel_data_type));
          dst_pixel_ptr++;
        }
        src_pixel_ptr++;
      }
      ...
      for(scale_index = SCALE_1X; scale_index < scale_factor; scale_index++)
      {
        memcpy((void *)dst_pixel_ptr, (void *) dst_pixel_row_ptr,
 ...
 )
{
  ...
  memset((void *)font_ptr,0x0, sizeof(font_info_type));
              
  /* Initialize Display Driver*/
  DisplayInitialize(&(font_ptr->gframe_buffer_info), uflags);

  if(font_ptr->gframe_buffer_info.pBufferAddr == NULL)
    return FALSE;
  ...
  WriteBackInvalidateDataCacheRange((void *)font_ptr->gframe_buffer_info.pBufferAddr,
                 font_ptr->gframe_buffer_info.uStride * font_ptr->gframe_buffer_info.uHeight);
...
)
{
  ...
  message_length = strlen((char*)message_ptr);
...
{
  boolean status = FALSE;
  BL_VERIFY(font_ptr != NULL, BL_ERR_NULL_PTR_PASSED|BL_ERROR_GROUP_BOOT);  
  if(scale <= SCALE_MAX)
  {
