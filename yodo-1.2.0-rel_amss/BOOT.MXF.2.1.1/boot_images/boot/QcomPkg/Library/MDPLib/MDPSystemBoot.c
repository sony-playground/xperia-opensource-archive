...
static const sMDSS_BaseAddressMappings asMDSS_BaseAddressMappings[MDSS_BASEADDRESSMAPINGS_MAX] = {
    {EFICHIPINFO_FAMILY_WAIPIO,  (uintPtr)0x0AE00000},
    {EFICHIPINFO_FAMILY_KAILUA,  (uintPtr)0x0AE00000},
	{EFICHIPINFO_FAMILY_AURORA,  (uintPtr)0x0AE00000},
};
...
static MDP_Status MDP_ReadFrameBufferInfo(uint64 *puFrameBufferAddr, uint32 *puFramebufferSize)
{
  ...
  else
  {
    ...
    if (0 != (uFdtRet = fdt_get_node_handle(&hNode, pBlob, "/soc/disp_rdump_region")))
    {
      eStatus = MDP_STATUS_NO_RESOURCES;
    }
    else if (0 != (uFdtRet = fdt_get_uint32_prop_list(&hNode, "reg", (VOID*)&uDTBConfig, sizeof(uDTBConfig))))
    {
