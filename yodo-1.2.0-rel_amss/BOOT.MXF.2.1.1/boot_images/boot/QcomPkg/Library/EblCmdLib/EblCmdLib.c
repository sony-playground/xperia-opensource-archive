/** @file EblCmdLib.c
  ...
  Copyright (c) 2010-2016,2018,2019, 2021 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/12/21   bpk     call ConnectAllDrivers() in EblMountFvCommand function
 08/26/19   kh      Add display SetMode
 06/20/19   am      Replaced AsciiStrStr() in ImageHandleToPdbFileName() to GetImageNameFromPath()
 03/21/19   md      Added mountfv -f option
 01/23/19   md      Added cls command to clear display.
 10/09/18   yw      Added GetChipInfo command to support Axiom automation
 08/20/18   yw      Added MountFv GUID support, added ToggleWdog cmd
 07/23/18   kpa     Added MountFv support
  ...
  )
{
  ...
  Status = GetImageNameFromPath (Pdb, &ImageNameStart);
  if (ImageNameStart == NULL)
  {
    return "";
  }
...
EFI_STATUS
EblToggleWdogCommand (
  IN UINTN  Argc,
  IN CHAR8  **Argv
  )
{
    EFI_STATUS Status;
    BOOLEAN OldEnableHWWdogVar, NewEnableHWWdogVar;
    ...
    Status = gRT->GetVariable (L"ForceEnableHWWdog", &gQcomTokenSpaceGuid,
                               NULL, &VarSize, (UINT8*)&OldEnableHWWdogVar);
    if (EFI_ERROR(Status))
    {
        // first time to toggle, wdog will be enabled 
        NewEnableHWWdogVar = TRUE;
    }
    else {
        NewEnableHWWdogVar = !OldEnableHWWdogVar;
    }
    
    Status = gRT->SetVariable (L"ForceEnableHWWdog", &gQcomTokenSpaceGuid,
                 EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,
                 (UINTN)sizeof(NewEnableHWWdogVar), &NewEnableHWWdogVar);
    if (EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR, "Failed to set variable\n"));
        return Status;
    }
    ...
    if (EFI_ERROR(Status) || VariableServicesProtocol == NULL) {
        DEBUG((EFI_D_ERROR, "Failed to locate variable service\n"));
        return Status;
    }
    
    Status = VariableServicesProtocol->FlushVariableNV(VariableServicesProtocol);
    if (EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR, "Failed to flush NV variable\n"));
        return Status;
    }

    if (NewEnableHWWdogVar)
        DEBUG((EFI_D_WARN, "New wdog status : ON\n"));
    else
        DEBUG((EFI_D_WARN, "New wdog status : OFF\n"));
  ...
  )
{
    ...
    if (EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR, "Failed to get chipinfo protocol %r\n", Status));
        return Status;
    }
   
    Status = ChipInfoProtocol->GetChipVersion( ChipInfoProtocol, &ChipVersion);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get chip version %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "ChipVersion: 0x%x\n", ChipVersion));
       
    Status = ChipInfoProtocol->GetRawChipVersion(ChipInfoProtocol, 
                                                 &RawDate);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get raw chip version %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "RawChipVersion: 0x%x\n", RawDate));

    Status = ChipInfoProtocol->GetChipId(ChipInfoProtocol, &ChipId);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get chip ID %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "ChipId: 0x%x\n", ChipId));
        
    Status = ChipInfoProtocol->GetRawChipId(ChipInfoProtocol, &RawDate);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get raw chip ID %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "RawChipId: 0x%x\n", RawDate));
        
    Status = ChipInfoProtocol->GetChipIdString(ChipInfoProtocol, 
                                               (CHAR8*)&ChipIdString, 
                                               EFICHIPINFO_MAX_ID_LENGTH);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get chip ID string %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "ChipIdString: %a\n", ChipIdString));
        
    Status = ChipInfoProtocol->GetChipFamily(ChipInfoProtocol, 
                                               &ChipFamily);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get chip family %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "ChipFamily: 0x%x\n", ChipFamily));
        
    Status = ChipInfoProtocol->GetModemSupport(ChipInfoProtocol, 
                                               &ModemSupport);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get modem support %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "ModemSupport: 0x%x\n", ModemSupport));
        
    Status = ChipInfoProtocol->GetMarketingNameString(ChipInfoProtocol, 
                                               (CHAR8*)&MarketingNameString,
                                               EFICHIPINFO_MAX_NAME_LENGTH);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get Marketing Name String %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "MarketingNameString: %a\n", MarketingNameString));
        
    Status = ChipInfoProtocol->GetSerialNumber(ChipInfoProtocol, &chipserialnumber);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get chip serial number %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "ChipSerialNumber: 0x%x\n", chipserialnumber));
        
    Status = ChipInfoProtocol->GetFoundryId(ChipInfoProtocol, &FoundryId);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get Foundry ID %r\n", Status));
    else
                DEBUG((EFI_D_ERROR, "FoundryId: 0x%x\n", FoundryId));

    Status = ChipInfoProtocol->GetRawDeviceFamily(ChipInfoProtocol, &RawDate);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get Raw Device Family %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "RawDeviceFamily: 0x%x\n", RawDate));
        
    Status = ChipInfoProtocol->GetRawDeviceNumber(ChipInfoProtocol, &RawDate);
    if (EFI_ERROR(Status))
        DEBUG((EFI_D_ERROR, "Failed to get Raw Device Number %r\n", Status));
    else
        DEBUG((EFI_D_ERROR, "RawDeviceNumber: 0x%x\n", RawDate));
  ...
  )
{
  ...
  do {
    ...
    AsciiStrToUnicodeStrS(Argv[1], VariableName, VariableNameSize);
    ...
    AsciiStrToUnicodeStrS(Argv[2], VendorGuidUniStr, VendorGuidUniStrSize);
  ...
  )
{
  ...
  Buffer[BufferSize] = 0;
  AsciiStrToUnicodeStrS(Buffer, OutputBuffer, 2*BufferSize);
...
DisplayGetMode (void)
{
  ...
  if (EFI_SUCCESS != (eStatus = gBS->LocateProtocol (&sGfxOutputGUID,
                                                     ...
                                                     (VOID **)&pGfxProtocol)))
  {
     AsciiPrint ("DisplayGetMode: LocateProtocol() failed.\n");
     return eStatus;
  }
  else
  {
     ...
     do {
       if (EFI_SUCCESS == (eStatus = pGfxProtocol->QueryMode(pGfxProtocol, ModeNum, &nModeInfoSize, &pModeInfo)))
       {
          AsciiPrint("\t%d] %dx%d (Format:%d)\n", ModeNum, pModeInfo->HorizontalResolution, pModeInfo->VerticalResolution, (UINT32)pModeInfo->PixelFormat);
          ModeNum++;
       }
  ...
  )
{
   ...
   {
      ModeNum = AsciiStrDecimalToUintn (Argv[1]);
                                                     ...
                                                     (VOID **)&pGfxProtocol)))
    {
     ...
     {
        if (EFI_SUCCESS == (eStatus = pGfxProtocol->SetMode(pGfxProtocol, ModeNum)))
        {
...
GLOBAL_REMOVE_IF_UNREFERENCED const EBL_COMMAND_TABLE mLibCmdTemplate[] =
{
  ...
  {
    "mountfv partitionlabel;",
    " Mounts a FV from partition",
    NULL,
    EblMountFvCommand
  },
  {
  ...
  {
    "SetMode;",
    " Set Display specified Resolution",
    NULL,
    EblDisplaySetMode
  },
};
  ...
  )
{
  ...
  SetCmdHistory ("start fv3:\\mptest");           // Launch Mptest
