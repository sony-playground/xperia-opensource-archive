...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 05/28/19   pameta  Marking Pending Capsule NV variable
 02/01/19   rajb    Merged latest EDK2 code changes for progress bar rendering, from MdeModulePkg
 02/05/19	akeshan	Addition of capsule storage type
 02/12/18   prema   Change to skip nested capsule header while deriving FMP header
...
extern EDKII_FIRMWARE_MANAGEMENT_PROGRESS_PROTOCOL  *gFmpProgress;
  ...
  )
{
  ...
  Seconds = 5 * 60;
  Color   = NULL;
  if (gFmpProgress != NULL) {
    Seconds = gFmpProgress->WatchdogSeconds;
    Color   = &gFmpProgress->ProgressBarForegroundColor;
  }
  ...
  )
{
  ...
  for (Index = 0; Index < mCapsuleTotalNumber; Index++) {
    ...
    if (CompareGuid (&CapsuleHeader->CapsuleGuid, &gWindowsUxCapsuleGuid)) {
      ...
      Status = ProcessThisCapsuleImage (CapsuleHeader, CapsuleFileName, NULL);
  ...
  for (Index = 0; Index < mCapsuleTotalNumber; Index++) {
    ...
    if (!CompareGuid (&CapsuleHeader->CapsuleGuid, &gWindowsUxCapsuleGuid)) {
      ...
      // as we are allowing code flow to reach this point so that we can reach the delete
      // operation lying later to this if condition
      if (((!FirstRound) || (EmbeddedDriverCount == 0)) &&
          (mCapsuleStatusArray [Index] == EFI_NOT_READY)) {

        DEBUG((DEBUG_INFO, "ProcessThisCapsuleImage - 0x%x\n", CapsuleHeader));
        ResetRequired = FALSE;
        Status = ProcessThisCapsuleImage (CapsuleHeader, CapsuleFileName, &ResetRequired);
        mCapsuleStatusArray [Index] = Status;
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "DxeCapsuleProcessLib.c ProcessCapsules capsule storage type is 0x%X\n", (UINTN) CapsuleStorage));

  if ((!mDxeCapsuleLibEndOfDxe) ||
      (PcdGetBool(PcdSingleStageCapsuleUpdate))) {
    UpdateStatus = ProcessTheseCapsules(TRUE, CapsuleStorage);

    if (AreAllImagesProcessed()) {
      FlushPersistedCapsules(CapsuleStorage);

      IsCapsulePendingInPersistedMedia = BdsLibGetVariableAndSize (L"IsCapsulePendingInPersistedMedia", 
        &gEfiGlobalVariableGuid, 
        &Size);
      if (IsCapsulePendingInPersistedMedia != NULL) {
        if (*IsCapsulePendingInPersistedMedia) {
         *IsCapsulePendingInPersistedMedia = FALSE;
         DEBUG ((EFI_D_INFO, "DxeCapsuleProcessLib.c Clearing IsCapsulePendingInPersistedMedia variable"));
         Status = gRT->SetVariable(L"IsCapsulePendingInPersistedMedia",
                                   &gEfiGlobalVariableGuid,
                                   EFI_VARIABLE_NON_VOLATILE|EFI_VARIABLE_BOOTSERVICE_ACCESS|EFI_VARIABLE_RUNTIME_ACCESS,
                                   sizeof(UINT64),
                                   IsCapsulePendingInPersistedMedia);
        }
        FreePool (IsCapsulePendingInPersistedMedia);
        IsCapsulePendingInPersistedMedia = NULL;
      }
    }
