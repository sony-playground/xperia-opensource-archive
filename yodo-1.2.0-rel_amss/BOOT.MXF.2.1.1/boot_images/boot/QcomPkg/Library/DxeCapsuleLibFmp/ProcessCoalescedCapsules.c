...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 02/01/19   rajb    Updated capsule image process API
 09/27/18   prema   Changes done for CalculateCrc32 handling
 02/22/17   gparam  Changes corresponding to the changes done in XBL to
                    support FMP capsule processing
...
STATIC UINT8 OverflowBuffer[CAPSULE_PART_GUID_TABLE_BLOCK_SIZE];

// TODO: Giri: Migrate this to not assume persisting storage as partition
  ...
  )
{
  ...
  do {
    ...
    if (EFI_ERROR (Status))
    {
      ...
      CapsuleGuidTable->Crc32 = CalculateCrc32(CapsuleGuidTable,
                                               sizeof(CAPSULE_PARTITION_GUID_TABLE));

    }
  
    // Confirm block sizes match
    if (CapsuleGuidTable->BlockSize != CAPSULE_PART_GUID_TABLE_BLOCK_SIZE)
    {
      Status = EFI_UNSUPPORTED;
      break;
    }
    ...
    for(Index = 0; Index < CapsuleTotalNumber; Index++)
    {
      ...
      if ((CapsuleGuidTable->GuidCount < CAPSULE_GUID_ENTRIES_MAX) && 
          (CapsuleGuidTable->FreeBlocks*CAPSULE_PART_GUID_TABLE_BLOCK_SIZE >= CapsuleSize))
      {    
        ...
        CapsuleGuidTable->FreeBlocks -= 
          (GuidEntry->ImageSize >> CAPSULE_PART_GUID_TABLE_BLOCK_SHIFT) + 
          (((GuidEntry->ImageSize%CAPSULE_PART_GUID_TABLE_BLOCK_SIZE) > 0)?1:0);
