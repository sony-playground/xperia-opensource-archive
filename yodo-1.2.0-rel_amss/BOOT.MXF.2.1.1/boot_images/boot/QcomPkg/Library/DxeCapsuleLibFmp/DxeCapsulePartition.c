/** @file
  ...
  Copyright (c) 2017-2019, 2021 Qualcomm Technologies, Inc. All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 09/27/18   prema   Added FlushBlock handling changes wherever missed to handle
                    and changes done for CalculateCrc32 handling
  ...
  )
{
  ...
  Status = CapsuleBlkIo->FlushBlocks(CapsuleBlkIo);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Failed to do FlushBlocks Status: %r\n", Status));
    Status = EFI_SUCCESS;
  }
...
{
  EFI_STATUS                   Status      = EFI_SUCCESS;
  EFI_STATUS                   FlushStatus = EFI_SUCCESS;
  UINT32                       *MemLoc = NULL;
  UINT32                       HeaderSize = 0;
  ...
  {
    FlushStatus = CapsuleBlkIo->FlushBlocks(CapsuleBlkIo);
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_WARN, "Invalid capsule persisting media content found. Re-Initializing the contents\n"));
