/** @file ListVars.c
  ...
  Copyright (c) 2010-2012,2014,2019,2021 Qualcomm Technologies Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 05/06/21   ka      support dumping HwErr record variables, remove BS_NV_S which is not a valid tabletype
 05/24/19   bd      Improve attributes printing
...
#define RT_NV_NS_HR_ATTRIBUTES  (EFI_VARIABLE_BOOTSERVICE_ACCESS | \
                              EFI_VARIABLE_RUNTIME_ACCESS | \
                              EFI_VARIABLE_NON_VOLATILE  | \
                              EFI_VARIABLE_HARDWARE_ERROR_RECORD)


static CHAR8 *gVarAttributes [] = 
{
  //Macro values defined in file - MdePkg/Include/Uefi/UefiMultiPhase.h 
                                           // Value    Shift Count to get 1 at Bit-0
  "NV",                                    // 0x0001         0
  "BS",                                    // 0x0002         1
  "RT",                                    // 0x0004         2 
  "HW_ERR",                                // 0x0008         3
...
DumpStore ( VOID )
{
  ...
  do
  {
    ...
    Status = gRT->QueryVariableInfo(RT_NV_NS_HR_ATTRIBUTES, 
                                    &MaxStoreSize, 
                                    &RemStoreSize, 
                                    &MaxVarSize);
    AsciiPrint("QueryVariableInfo:\n"
               ...
               "  RemainingVariableStorageSize: %ld\n"
               "  MaximumVariableSize: %ld\n",
               RT_NV_NS_HR_ATTRIBUTES, "RT_NV_NS_HR",
               MaxStoreSize, 
               RemStoreSize, 
               MaxVarSize);

    if (Status != EFI_SUCCESS)
      AsciiPrint("Failed to query Attributes = 0x%X, Status = 0x%X.\n", RT_NV_NS_HR_ATTRIBUTES, Status);    
  ...
  for (VarCount = 0;;VarCount++)
  {
    ...
    AsciiPrint("  Attributes: 0x%x  [", Attributes);
    SizeOfArray = sizeof(gVarAttributes)/sizeof(CHAR8 *);
    Index = 0x0;
    while(Attributes && Index < SizeOfArray)
    {      
      if(Attributes & 0x01)
      {        
        AsciiPrint("%a", gVarAttributes[Index] );
        Attributes = Attributes >> 1;
        if(Attributes) AsciiPrint(" | "); //do not print for last attribute
      }
      else
      {
        Attributes = Attributes >> 1;
