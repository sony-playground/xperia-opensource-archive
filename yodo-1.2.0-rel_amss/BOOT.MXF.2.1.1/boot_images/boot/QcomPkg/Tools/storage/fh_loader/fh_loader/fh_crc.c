/* @file fh_crc.c
  ...
  This file implements CRC16 calculation
...
*
* NOTE: The x^16 tap is left off; it is implicit.
*/
#define CRC_16_POLYNOMIAL       0x8408
...
word crc_16_calc(byte *buf_ptr, word len);
...
*    len      [IN]: Number of data bits to calculate the CRC over
...
word crc_16_calc(byte *buf_ptr, word len)
{
  ...
  */
  for(crc_16 = CRC_16_SEED; len >= 8; len = (word)(len - 8), buf_ptr++)
  {
    crc_16 = (word)
             (crc16_table[(crc_16 >> (16 - 8)) ^ *buf_ptr] ^ (crc_16 << 8));
  }
  ...
  */
  if(len != 0)
  {
    ...
    data = (word)(((word)(*buf_ptr)) << (word)(16 - 8));

    while(len-- != 0)
    {
      /* Is MSB of XOR a 1 */
      if(((crc_16 ^ data) & ((word)1 << 15)) != 0)
      {

        crc_16 = (word)(crc_16 << 1);   /* Left shift CRC          */
      ...
      data = (word)(data << 1);           /* Left shift data         */
    }
  }

  return(word)(~crc_16);      /* return the 1's complement of the CRC */
...
*
* Description:
* This is a wrapper over the function crc_16_calc and converts the input length
* from number of bytes to number of bits.
*
* Arguments:
*    *buf_ptr [IN]: Pointer to bytes containing the data to CRC. The bit stream
*                   starts in the MS bit of the first byte.
...
{
  return crc_16_calc(buf_ptr, (word)(len * 8));
}