...
# --------   ---     -----------------------------------------------------------
# 12/22/21   bxr     Corrected compile error for python3
# 07/15/21   ck      Added -x --exclude-paths-file
# 04/20/20   ck      Corrected single file error count
...
def main():
  ...
  parser.add_option("-l", "--log",
                    ...
                    help="Log error to file.")

  parser.add_option("-x", "--exclude-paths-file",
                    action="store", type="string", dest="exclude_paths_file",
		    help="Path to file that contains newline separated paths that are to be excluded from scanning.")
  ...
  # If an exclude paths file was given make sure it exists and read the file paths in
  if options.exclude_paths_file:
    if not os.path.exists(options.exclude_paths_file):
      parser.error("Exclude paths file provided does not exist.")

    exclude_paths_list = parse_exclude_paths_file(options.exclude_paths_file)
  else:
    exclude_paths_list = []
  ...
  if options.file_name:
    ...
    if not evaluate_copyright(options.file_name,
      ...
      # Log file name if applicable
      if options.log_file:
        f.write(options.file_name + "\n")
    ...
    if not options.recursive:
      for file_name in os.listdir(options.base_directory):
        ...
        if any(file_name.endswith(ext) for ext in file_extensions):
          ...
          if time.gmtime(os.path.getmtime(os.path.join(options.base_directory, file_name))) <= \
            ...
            if not evaluate_copyright(os.path.join(options.base_directory,file_name),
              ...
              # Log file name if applicable
              if options.log_file:
                f.write(os.path.join(os.path.join(root,file_name)+"\n"))
      ...
      for root, directories, files in os.walk(options.base_directory):
        # If root exists in exclude paths list then skip file evaluation
        if path_in_exclude_paths_list(exclude_paths_list,
                                      root):
          # Log file name if applicable
          if options.log_file:
            f.write("Path " + root + " skipped via exclude paths list entry\n")
        ...
        for file_name in files:
          ...
          if any(file_name.endswith(ext) for ext in file_extensions):
            ...
            # Ensure file modification date is newer or equal to default or one supplied
            if time.gmtime(os.path.getmtime(os.path.join(root, file_name))) <= \
               file_modification_minimum:
...
###############################################################################
# path_in_exclude_paths_list
###############################################################################
def path_in_exclude_paths_list(exclude_paths_list,
                               path_under_test):

  # Loop through each exclude paths list entry and see if path_under_test begins with it
  for current_exclude_path in exclude_paths_list:
    # Correct directory separators
    current_exclude_path = current_exclude_path.replace("\\", os.sep)
    current_exclude_path = current_exclude_path.replace("/", os.sep)

    if current_exclude_path in path_under_test:
      return True
...
###############################################################################
# parse_exclude_paths_file
###############################################################################
def parse_exclude_paths_file(file_path):

  exclude_paths_list = []

  # Open file and read each line into exclude paths list
  try:
    with open(file_path) as file:
      for line in file:
        # Strip off new line character and add to exclude paths list
        exclude_paths_list.append(line.strip('\n'))
  except:
    print("\nERROR: An error occured while parsing the exclude paths file: " + \
      file_path)
    sys.exit()

  return exclude_paths_list
