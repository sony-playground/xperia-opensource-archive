...
*/
/*=============================================================================
  Copyright (c) 2011-2012, 2014, 2019-2021 Qualcomm Technologies, Incorporated.
  All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/08/21 � pohanw �Move Find partition & Mount/unmount to USBMSD added v10003
...
/** @defgroup EFI_USB_MSD_PERIPHERAL_PROTOCOL  EFI_USB_MSD_PERIPHERAL_PROTOCOL 
 *  @ingroup INTERFACES 
 */
 /** @defgroup  EFI_USB_MSD_PERIPHERAL_PROTOCOL_prot PROTOCOL
 *  @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL 
 */ 
 /** @defgroup  EFI_USB_MSD_PERIPHERAL_PROTOCOL_apis APIs
 *  @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL 
 */ 

/** @defgroup  EFI_USB_MSD_PERIPHERAL_PROTOCOL_data DATA_STRUCTURES
 *  @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL 
 */

/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_data */
/** Protocol version. */
#define EFI_USB_MSD_PERIPHERAL_PROTOCOL_REVISION  0x00010002
#define EFI_USB_MSD_PERIPHERAL_PROTOCOL_REVISION_V3  0x00010003
...
struct _PartitionCtx
{
  EFI_HANDLE               *Handle;
  CHAR8                    Name[PARTITION_NAME_LENGTH];
  EFI_BLOCK_IO_PROTOCOL    *BlkIo;
  UINT16                   Parent;
  BOOLEAN                  Found;
  EFI_ERASE_BLOCK_PROTOCOL *Erase;
};
typedef struct _PartitionCtx PartitionCtx;
...
/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_apis
  @par Summary
...
/* EFI_USB_MSD_GET_DEVICE_SPEED */
/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_apis
@par Summary
Returns valid speed of connection when enumeration is complete else invalid speed.

@param[in]   This             Pointer to the EFI_USB_MSD_PERIPHERAL_PROTOCOL instance.
@param[out]   BusSpeed        USB bus speed of device.
...
typedef
EFI_STATUS
(EFIAPI *EFI_USB_MSD_GET_DEVICE_SPEED)(
  IN  EFI_USB_MSD_PERIPHERAL_PROTOCOL *This,
  OUT EFI_USB_BUS_SPEED               *BusSpeed
);

/* EFI_USB_MSD_UNMOUNT_HANDLE */
/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_apis
@par Summary
Unmount the partition from DUT, so we can enumerate that partition for mass storage.

@param[in]   *Handle          handle to the partition to unmount
...
typedef
EFI_STATUS
(EFIAPI *EFI_USB_MSD_UNMOUNT_HANDLE)(
  IN    EFI_HANDLE               *Handle
);


/* EFI_USB_MSD_MOUNT_HANDLE */
/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_apis
@par Summary
mount the partition to DUT

@param[in]   CHAR16    *PartitionLabel   partition label to mount
...
typedef
EFI_STATUS
(EFIAPI *EFI_USB_MSD_MOUNT_HANDLE)(
  IN    CHAR16    *PartitionLabel
);

/* EFI_USB_MSD_FIND_PARTITIONS */
/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_apis
@par Summary
return partition table for mass stoage app to use

@param[out]   PartitionCtx **   pointer to partition table
@param[out]   UINT16*           Number of partition exist in the table
...
typedef
EFI_STATUS
(EFIAPI *EFI_USB_MSD_FIND_PARTITIONS)(
...
/*===========================================================================
  PROTOCOL INTERFACE
===========================================================================*/
/** @ingroup EFI_USB_MSD_PERIPHERAL_PROTOCOL_prot
  @par Summary
...
struct _EFI_USB_MSD_PERIPHERAL_PROTOCOL {
  ...
  /// Get Bus Speed
  ///
  EFI_USB_MSD_GET_DEVICE_SPEED              GetDeviceSpeed;
  ///
  /// Unmount partition from device
  ///
  EFI_USB_MSD_UNMOUNT_HANDLE                UnmountFatPartition;   /* Added in version 1.3 */
  ///
  /// mount partition from device
  ///
  EFI_USB_MSD_MOUNT_HANDLE                  MountFatPartition;     /* Added in version 1.3 */
  ///  
  /// Return full partition table
  ///
  EFI_USB_MSD_FIND_PARTITIONS               FindPartitions;        /* Added in version 1.3 */
};
