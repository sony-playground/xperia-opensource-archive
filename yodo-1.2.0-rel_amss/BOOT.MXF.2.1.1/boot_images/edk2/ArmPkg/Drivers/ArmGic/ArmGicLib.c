  ...
  )
{
  ...
  #define GIC_IRQ_MAX 1020   /* @see GIC spec, GICD_TYPER ::ITLinesNumber */
/* UEFI ARM GIC driver incorrectly calculating the maximum number of IRQs (FIX).
   ARM GIC Interrupt 1020-1023 are reserved and SW should always limit max number
   of interrupts to 1020. (refer attached ARM GIC spec) TZ GIC driver already
   limiting max interrupt to 1020 */
  return MIN(32 * ((MmioRead32 (GicDistributorBase + ARM_GIC_ICDICTR) & 0x1F) + 1),GIC_IRQ_MAX);
#else
