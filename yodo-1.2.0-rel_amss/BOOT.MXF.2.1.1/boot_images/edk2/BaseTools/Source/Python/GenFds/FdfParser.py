      ...
      # nested include support
        ...
        while self._GetNextToken():
            ...
            if self._Token == TAB_DEFINE:
                ...
                # SONY_BEGIN (pseudo conditional 'include')
                if IncludedFile[0:8] == 'SomcPkg/':
                    if (os.getenv('USES_FEATURE_XBOOT', None) != 'yes'):
                         print("*** IncludedFile "+IncludedFile+" OVERRODE to /dev/null ***")
                         IncludedFile = '/dev/null'
                # SONY_END (pseudo conditional 'include')
                #
