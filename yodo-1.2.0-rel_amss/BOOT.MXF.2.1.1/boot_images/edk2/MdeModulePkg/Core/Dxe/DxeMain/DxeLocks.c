...
Copyright (c) 2011-2014, 2016, 2019 - 2021 Qualcomm Technologies, Inc. All rights reserved.
...
extern EFI_EVENT       gYieldEvent;
#define YIELD_EVENT_CB_INTERVAL 1000 
...
{
  EFI_STATUS            Status;
  static UINT64 AccumulatedTime = 0;

  DxeLock ();

  /* See if the following logic belongs here or need to find a proper home */
  if (Microseconds < YIELD_EVENT_CB_INTERVAL)
    AccumulatedTime += Microseconds;
  /*
    Give back control to Scheduler after every 1ms.
    This is required for timer callbacks to be processed.
    Without this change any code calling Stall API will block the callbacks for that duration.
  */
  if ((AccumulatedTime >= YIELD_EVENT_CB_INTERVAL) || (Microseconds >= YIELD_EVENT_CB_INTERVAL))
  {
    AccumulatedTime = 0;
