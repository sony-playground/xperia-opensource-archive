...
static ssize_t dsi_display_camera_flash_br_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	struct dsi_panel *panel = display->panel;
	...
	if (sscanf(buf, "%d", &brightness) < 0) {
		pr_err("[%s]:sscanf failed to set flash_br, keep current value =%d\n",
						__func__, panel->flash_br);
		goto flash_br_error;
	}

	panel->flash_br = brightness;
	pr_info("[%s]:camera flash mode %s, flash_br = %d.\n", __func__,
				panel->flash_br > 0 ? "ON" : "OFF",
				panel->flash_br);
...
static ssize_t dsi_display_camera_flash_br_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "%u", display->panel->flash_br);
}

static ssize_t dsi_display_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_display *display = dev_get_drvdata(dev);
...
static ssize_t dsi_display_area_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	memset(panel->area_count, 0, sizeof(u32) * panel->area_count_table_size);
	mutex_unlock(&display->display_lock);

	return snprintf(buf, DSI_BUF_SIZE, "%s\n", area_count_str);
}

static ssize_t dsi_display_flm2_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	struct dsi_panel *panel = display->panel;
...
static struct device_attribute panel_attributes[] = {
	...
	__ATTR(camera_flash_brightness, S_IRUSR|S_IRGRP|S_IWUSR|S_IWGRP,
		dsi_display_camera_flash_br_show, dsi_display_camera_flash_br_store),
	__ATTR(panel_id, S_IRUSR, dsi_display_id_show, NULL),
	__ATTR(area_count, (S_IRUSR | S_IRGRP), dsi_display_area_count_show, NULL),
	__ATTR(flm2_mode, (S_IWUSR|S_IWGRP), NULL, dsi_display_flm2_mode_store),
};
