...
static void dsi_panel_init_area_count(struct dsi_panel *panel);
static void dsi_panel_deinit_area_count(struct dsi_panel *panel);
static void dsi_panel_update_area(struct dsi_panel *panel, u32 level);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (panel->flash_br) {
		u32 flash_bl_lvl = panel->flash_br * panel->bl_config.bl_max_level
			/ (panel->hdr_props.peak_brightness / 10000);
		pr_info("[%s]Override brightness for display flash. %d nit, bl_lvl %d to %d\n",
			__func__, panel->flash_br, bl_lvl, flash_bl_lvl);
		bl_lvl = flash_bl_lvl;
	}

	//Adjust register value considering min/max reg val
	bl_lvl = dsi_panel_driver_adjust_brightness(panel, bl_lvl);

	dsi_panel_update_area(panel, bl_lvl);
...
static int dsi_panel_parse_misc_features(struct dsi_panel *panel)
{
	...
	DSI_ERR("%s: ulps during suspend feature %s\n", __func__,
		(panel->ulps_suspend_enabled ? "enabled" : "disabled"));
				...
				bool trusted_vm_env)
{
	...
	rc = of_property_read_u32(of_node, "somc,brightness-adjust-type", &tmp);
	panel->spec_pdata->br_adjust_type = !rc ? tmp : 0;

	rc = of_property_read_u32(of_node, "somc,brightness-adjust-transition-point", &tmp);
	panel->spec_pdata->br_adjust_tp = !rc ? tmp : panel->bl_config.brightness_max_level;

	rc = of_property_read_u32(of_node, "somc,brightness-adjust-min-level", &tmp);
	panel->spec_pdata->br_adjust_min_lvl = !rc ? tmp : panel->bl_config.bl_min_level;

	rc = of_property_read_u32(of_node, "somc,brightness-adjust-max-level", &tmp);
	panel->spec_pdata->br_adjust_max_lvl = !rc ? tmp : panel->bl_config.bl_max_level;

	rc = of_property_read_u32(of_node, "somc,brightness-adjust-peak-level", &tmp);
	panel->spec_pdata->br_adjust_peak_lvl = !rc ? tmp : panel->bl_config.bl_max_level;

	dsi_panel_init_area_count(panel);
...
void dsi_panel_put(struct dsi_panel *panel)
{
	...
	dsi_panel_deinit_area_count(panel);
...
int dsi_panel_set_flm2_mode(struct dsi_panel *panel, int mode)
{
	...
	mutex_lock(&panel->panel_lock);
	panel->spec_pdata->flm_mode = mode;
    /* update the backlight register value for the FLM mode */
	rc = dsi_panel_set_backlight(panel, panel->spec_pdata->bl_level);
	if (rc)
...
int dsi_panel_set_hbm(struct dsi_panel *panel, u32 mode)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, hbm_mode, false);
	if (rc)
...
static void dsi_panel_init_area_count(struct dsi_panel *panel)
{
	...
	panel->now_area = 0;
	panel->start_jiffies = jiffies;

	rc = of_property_read_u32(panel->panel_of_node, "somc,area_count_table_size", &tmp);
	if (!rc) {
		panel->area_count_table_size = tmp;
	} else if (rc != -EINVAL) {
		pr_err("%s: Unable to read area_count_table_size\n", __func__);
		return;
	}

	if (panel->area_count_table_size > 0) {
		panel->area_count_table = kzalloc(sizeof(int) * panel->area_count_table_size, GFP_KERNEL);
		if (!panel->area_count_table) {
			pr_err("%s: area_count_table kzalloc error ", __func__);
			panel->area_count_table_size = 0;
			return;
		}

		panel->area_count = kzalloc(sizeof(u32) * panel->area_count_table_size, GFP_KERNEL);
		if (!panel->area_count) {
			kfree(panel->area_count_table);
			pr_err("%s: area_count kzalloc error ", __func__);
			panel->area_count_table_size = 0;
			return;
		}

		rc = of_property_read_u32_array(panel->panel_of_node, "somc,area_count_table",
			panel->area_count_table, panel->area_count_table_size);
		if (rc < 0) {
			kfree(panel->area_count_table);
			kfree(panel->area_count);
...
static u32 dsi_panel_get_area(struct dsi_panel *panel, u32 level)
{
	int i;

	for (i = 0; i < panel->area_count_table_size; i++) {
		if (i == 0) {
			if (level == panel->area_count_table[i])
				break;
		} else {
			if (level <= panel->area_count_table[i])
				break;
...
void dsi_panel_update_areacount(struct dsi_panel *panel, int new_area)
{
	u32 now;
	u32 duration;

	now = jiffies;
	duration = (now - panel->start_jiffies) >= 0 ?
		(now - panel->start_jiffies) : (now + panel->start_jiffies);
	panel->area_count[panel->now_area]
			= panel->area_count[panel->now_area]
				+ jiffies_to_msecs(duration);

	panel->now_area = new_area;
	panel->start_jiffies = now;
}

static void dsi_panel_update_area(struct dsi_panel *panel, u32 level)
{
	u32 new_area = dsi_panel_get_area(panel, level);

	if (panel->now_area != new_area)
		dsi_panel_update_areacount(panel, new_area);
}
