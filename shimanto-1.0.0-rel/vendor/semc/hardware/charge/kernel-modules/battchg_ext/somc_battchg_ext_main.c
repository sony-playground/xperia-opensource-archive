...
#define FELS_IF_SUPPORTED

#include "somc_battchg_ext.h"
#ifdef FELS_IF_SUPPORTED
				...
				struct glink_oem_notify_msg *notify_msg)
{
	...
	return kobject_uevent_env(&bcext_dev->vdev->kobj, KOBJ_CHANGE, envp);
}

#ifdef FELS_IF_SUPPORTED
static int somc_bcext_handle_glink_fels_notification(
				struct somc_bcext_dev *bcext_dev,
				struct glink_oem_notify_msg *notify_msg)
{
	switch (notify_msg->notification) {
	case NOTI_FELS_DETECT_INVALID_BATTERY:
		fels_debug_log(FELS_CATEGORY_CHARGER,FELS_LOG_LEVEL_ERROR,
					"detected invalid battery");
		fels_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			FELS_ERROR_CODE_CHARGER_DETECT_INVALID_BATTERY,
			1, notify_msg->extra_data[0], 0, 0);
		break;
	case NOTI_FELS_DETECT_PD_HARD_RESET:
		fels_debug_log(FELS_CATEGORY_CHARGER,FELS_LOG_LEVEL_ERROR,
			"detected PD Hard Reset");
		fels_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			FELS_ERROR_CODE_CHARGER_DETECT_PD_HARD_RESET,
			1, 0, 0, 0);
		break;
	case NOTI_FELS_DETECT_SOC_FREEZE_WARNING:
		fels_debug_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_WARNING,
			"warning:detected SoC freeze");
		fels_log(FELS_CATEGORY_CHARGER,	FELS_LOG_LEVEL_WARNING,
			FELS_ERROR_CODE_CHARGER_DETECT_SOC_FREEZE,
			(int)notify_msg->extra_data[0],
			(int)notify_msg->extra_data[1],
			(int)notify_msg->extra_data[2],
			(int)notify_msg->extra_data[3]);
		break;
	case NOTI_FELS_DETECT_SOC_FREEZE_ERROR:
		fels_debug_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			"error:detected SoC freeze");
		fels_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			FELS_ERROR_CODE_CHARGER_DETECT_SOC_FREEZE,
			(int)notify_msg->extra_data[0],
			(int)notify_msg->extra_data[1],
			...
			(int)notify_msg->extra_data[3]);
		break;
	case NOTI_FELS_DETECT_WLC_FW_UPDATE:
		fels_debug_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			"detected wlc fw update error");
		fels_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			FELS_ERROR_CODE_CHARGER_DETECT_WLC_FW_UPDATE,
			notify_msg->extra_data[0], notify_msg->extra_data[1],
			notify_msg->extra_data[2], 0);
		break;
	case NOTI_FELS_DETECT_WLC_MANY_ISR:
		fels_debug_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			"detected wlc many ISR");
		fels_log(FELS_CATEGORY_CHARGER, FELS_LOG_LEVEL_ERROR,
			FELS_ERROR_CODE_CHARGER_DETECT_WLC_MANY_ISR,
			1, 0, 0, 0);
		break;
	default:
		return -EINVAL;
				...
				struct glink_oem_notify_msg *notify_msg)
{
	...
	bcext_info("notification: %d\n", notify_msg->notification);

	if (notify_msg->hdr.opcode != MSG_OPCODE_OEM_NOTIFY)
		return -EINVAL;

	if (IS_NOTI_WLS(notify_msg->notification))
		rc = somc_bcext_handle_glink_wls_notification(bcext_dev,
								notify_msg);
#ifdef FELS_IF_SUPPORTED
	else if (IS_NOTI_FELS(notify_msg->notification))
		rc = somc_bcext_handle_glink_fels_notification(bcext_dev,
								notify_msg);
#endif
	else
		return -EINVAL;
...
static int somc_bcext_glink_cb(void *priv, void *data, size_t len)
{
	...
	case MSG_TYPE_NOTIFICATION:
		somc_bcext_handle_glink_notification(bcext_dev,
					(struct glink_oem_notify_msg *)hdr);
		break;
	default:
		break;
	}
				...
				const struct class_attribute *attr, char *buf)
{
	struct somc_bcext_dev *bcext_dev = container_of(c,
					struct somc_bcext_dev, bcext_class);
	u32 type;
	static const char * const str_mode[] = {"Unknown", "BackPower", "TX", "RX", "EPP", "BPP"};
