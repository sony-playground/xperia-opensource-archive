...
enum sony_camera_irq_type {
	...
	SONY_CAMERA_IRQ_EXTERNAL_EVENT				= 3,
	SONY_CAMERA_IRQ_IDLE_ASSERT_EVENT			= 4,
	SONY_CAMERA_IRQ_IDLE_DEASSERT_EVENT			= 5,
	SONY_CAMERA_IRQ_EVENT_MAX					= 6,
};
...
static irqreturn_t sony_camera_irq_handler(int irq, void *info)
{
	...
	if (desc != NULL) {
		if (!strcmp(desc->action->name, SONY_CAMERA_GPIO_IRQ_EXTERNAL)) {
...
static int sony_camera_power_up(struct sony_camera_data *data)
{
	...
	LOGI("id = %d, use_spi %d %s",
		data->id, data->qup_spi_dev[SONY_CAMERA_SUB_DEVICE_TYPE_EXTERNAL_ISP].enabled,
	...
	unsigned int cmd, unsigned long parm, bool compat)
{
	...
	case SONY_CAMERA_CMD_I2C_READ:
		...
		for (uint32_t device_type = 0; device_type < SONY_CAMERA_SUB_DEVICE_TYPE_MAX; device_type++) {
			if (data->qup_i2c_dev[device_type].enabled &&
				setting.slave_addr == (data->qup_i2c_dev[device_type].client->addr << 1)) {
				is_qup_device = true;
	...
	if (camera_data[id].thremal_enable) {
		...
		for (n = 0; n < camera_data[id].thermal_cnt; n++) {
			camera_data[id].thermal_zone_dev[n] =
				thermal_tripless_zone_device_register(camera_data[id].thermal_name[n],
					0, &sony_camera_thermal_ops, 0);
...
static int __init sony_camera_init_module(void)
{
	...
	g_camera_device_class = class_create(SONY_CAMERA_DEV_NAME);
