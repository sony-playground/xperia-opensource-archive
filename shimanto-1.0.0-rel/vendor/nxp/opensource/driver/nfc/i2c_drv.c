...
#endif
{
	...
	ret = configure_gpio(nfc_gpio->esimdata, GPIO_OUTPUT);
	if (ret) {
		pr_err("%s: unable to request esimdata gpio [%d]\n", __func__,
		       nfc_gpio->esimdata);
	} else {
		 gpio_set_value(nfc_gpio->esimdata, 0);
	}
	printk(KERN_ALERT"esimdata gpio value: %d\n", gpio_get_value(nfc_gpio->esimdata));

	ret = configure_gpio(nfc_gpio->esimclk, GPIO_OUTPUT);
	if (ret) {
		pr_err("%s: unable to request esimclk gpio [%d]\n", __func__,
		       nfc_gpio->esimclk);
	} else {
		 gpio_set_value(nfc_gpio->esimclk, 0);
	}
	printk(KERN_ALERT"esimclk gpio value: %d\n", gpio_get_value(nfc_gpio->esimclk));

	ret = configure_gpio(nfc_gpio->esimrst, GPIO_OUTPUT);
	if (ret) {
		pr_err("%s: unable to request esimrst gpio [%d]\n", __func__,
		       nfc_gpio->esimrst);
	} else {
		 gpio_set_value(nfc_gpio->esimrst, 0);
	}
	printk(KERN_ALERT"esimrst gpio value: %d\n", gpio_get_value(nfc_gpio->esimrst));
