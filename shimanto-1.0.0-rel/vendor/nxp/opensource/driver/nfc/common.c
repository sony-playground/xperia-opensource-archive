		 ...
		 uint8_t interface)
{
	...
	nfc_gpio->ven = -EINVAL;
	nfc_gpio->esimdata = -EINVAL;
	nfc_gpio->esimclk = -EINVAL;
	nfc_gpio->esimrst = -EINVAL;
	...
	nfc_gpio->esimdata = of_get_named_gpio(np, DTS_ESIMDATA_GPIO_STR, 0);
	if ((!gpio_is_valid(nfc_gpio->esimdata))) {
		pr_err("%s: euicc gpio invalid %d\n", __func__, nfc_gpio->esimdata);
	}
	nfc_gpio->esimclk = of_get_named_gpio(np, DTS_ESIMCLK_GPIO_STR, 0);
	if ((!gpio_is_valid(nfc_gpio->esimclk))) {
		pr_err("%s: euicc gpio invalid %d\n", __func__, nfc_gpio->esimclk);
	}
	nfc_gpio->esimrst = of_get_named_gpio(np, DTS_ESIMRST_GPIO_STR, 0);
	if ((!gpio_is_valid(nfc_gpio->esimrst))) {
		pr_err("%s: euicc gpio invalid %d\n", __func__, nfc_gpio->esimrst);
	}
