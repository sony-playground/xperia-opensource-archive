...
out:
    // Handle less than 8 byte case
    if (jj)
    {
      char last_buff[25];
      uint32_t written = 0;
      int retVal = -1;
      memset(last_buff, 0, 25);
      for (ii = 0; ii < jj &&  written < 25; ii++)
      {
        retVal = snprintf(last_buff + written, 25 - written, "%02x ", tmp[ii]);
        written += retVal;
      }
      __android_log_print(ANDROID_LOG_ERROR, TAG,"msg %s", last_buff);
    }
  }
} // hexDump
    ...
    sns_std_suid *suid, qsh_wifi_scan_db *scan_db)
{
  ...
  ret = qsh_wifi_timeout_block(CMD_TIMEOUT_IN_MS * 3, &cond);
  if (ret != 0) {
    pthread_mutex_lock(&scan_mutex);
    ...
    uint32_t list_count, bool on_console)
{
  ...
  for(evt_cnt = 0; evt_cnt < list_count; evt_cnt++)
  {
    ...
    for (ap_cnt = 0; ap_cnt < msg->result_count; ap_cnt++)
    {
      ...
      {
        __android_log_print(ANDROID_LOG_ERROR, TAG,
          "lat(%s %lld) long (%s %lld) altitude(%s %i) alt_type(%s %d)  lat_unc(%s %u) long_unc(%s %u) alt_unc(%s %u)",
