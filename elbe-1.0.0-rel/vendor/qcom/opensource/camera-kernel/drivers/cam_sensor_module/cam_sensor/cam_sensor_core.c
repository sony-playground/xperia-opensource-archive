...
#define IMX766_FAB_ID_REG 0X0018
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (chipid == 0x0766) {
		rc = camera_io_dev_read(&(s_ctrl->io_master_info), IMX766_FAB_ID_REG,
			&imx766_fab_id, CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "read imx766 fab id failed: rc = %d", rc);
			return rc;
		}
		if (imx766_fab_id == 0x11) {
			sensor_module_name = IMX766_FAB1_MODULE_NAME;
		} else if (imx766_fab_id == 0x30) {
			sensor_module_name = IMX766_FAB3_MODULE_NAME;
		}
		CAM_INFO(CAM_SENSOR, "sensor_module_name = %s, expect sensor_name = %s", sensor_module_name, s_ctrl->sensor_name);
