...
Copyright (c) 2012-2013,2016,2018, 2020, 2021 Qualcomm Technologies, Inc. All Rights Reserved.
...
EFI_EVENT DisplaySetModeEvent;                    // Event dispacted on a SetMode()

// Qcom uses this instead of hardcoded mGraphicsConsoleModeData 
GRAPHICS_CONSOLE_MODE_DATA  *mGraphicsGOPConsoleModeBuffer = NULL;
#endif
  ...
  )
{
...
#if defined (QCOM_EDK2_PATCH)
  // The total number of mode that graphics console supports
  // This is the max mode
  Count = *TextModeCount;
#else
  ...
  //
#if defined (QCOM_EDK2_PATCH)
  // QC diverges from UEFI spec, supporting lower resolution
  // modes including 480x640 and 480x800.  So we do not need to assert this requirement
#else
  ...
  //
#if defined (QCOM_EDK2_PATCH)
  /*mGraphicsGOPConsoleModeBuffer is expected to contain
  the columns and rows respective to each mode of
  horizontal and vertical resolution in Display driver
  If mGraphicsGOPConsoleModeBuffer remains NULL, use existing logic
  */

  if(mGraphicsGOPConsoleModeBuffer == NULL) {
    ModeBuffer = mGraphicsConsoleModeData;
  }
  else {
    ModeBuffer = mGraphicsGOPConsoleModeBuffer; 
  }
#else
  ModeBuffer = mGraphicsConsoleModeData;
#endif
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    if (ValidIndex == ValidCount) {
      ...
      NewModeBuffer[ValidCount].GopHeight     = VerticalResolution;
#if defined (QCOM_EDK2_PATCH)
      //Setting to Index to match GOP modes in Display driver and distinguish each mode
      NewModeBuffer[ValidCount].GopModeNumber = Index;
  ...
  )
{
...
#if defined (QCOM_EDK2_PATCH) 
  // These variables are used for Setmode operation
#else
  INT32                                PreferMode;
  INT32                                Index;
  UINTN                                Column;
  ...
  if (Private->GraphicsOutput != NULL) {
    ...
    if ((HorizontalResolution == 0x0) || (VerticalResolution == 0x0)) {
...
#if defined(QCOM_EDK2_PATCH)
      //Allocating a size with max mode supported by Display driver
      mGraphicsGOPConsoleModeBuffer = AllocateZeroPool (sizeof (GRAPHICS_CONSOLE_MODE_DATA) * MaxMode);
#endif
	  ...
	  if(mGraphicsGOPConsoleModeBuffer != NULL)
	  {
	    mGraphicsGOPConsoleModeBuffer[ModeIndex].Columns = Info->HorizontalResolution / EFI_GLYPH_WIDTH;
	    mGraphicsGOPConsoleModeBuffer[ModeIndex].Rows    = Info->VerticalResolution / EFI_GLYPH_HEIGHT;
	  }
#endif
  ...
  )
{
  ...
  // Commenting this and having it as a placeholder for now
  // Mode is now set to the number passed to this function
  ...
  if (Private->LineBuffer != NULL) {
    ...
    if ((INT32) ModeNumber == This->Mode->Mode) {
      ...
      //
#if defined(QCOM_EDK2_PATCH)
  // ClearScreen already done by SetMode for QC usecases
#else
