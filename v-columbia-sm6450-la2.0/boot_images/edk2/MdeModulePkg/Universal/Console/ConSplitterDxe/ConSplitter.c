  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    //default value of BaseMode is 0xFF and is updated only of col and
    //row is 80x25 as in line 3081.If there is no 80x25, setting Basemode
    //to be 0 which is the maximum resolution in display driver
    BaseMode = 0;
#endif
    //
  ...
  )
{
  ...
  UINTN Maxmode;
  UINTN Cols = 0, Rows = 0;
  UINTN ConsoleCols = 0, ConsoleRows = 0;
...
#if defined(QCOM_EDK2_PATCH)
      Status = This->QueryMode(This, ModeNumber, &Cols, &Rows);
      if (EFI_ERROR(Status))
        return EFI_UNSUPPORTED;

      Maxmode = Private->TextOutList[Index].TextOut->Mode->MaxMode;
      for(UINTN mode = 0; mode < Maxmode; mode++)
      {
        Status = Private->TextOutList[Index].TextOut->QueryMode(Private->TextOutList[Index].TextOut, mode, &ConsoleCols, &ConsoleRows);
        if (EFI_ERROR(Status))
          return EFI_UNSUPPORTED;
        if(Cols == ConsoleCols && Rows == ConsoleRows) {
          ModeFound = TRUE;
