...
#
# Copyright 2014-2017, 2020 by QUALCOMM Technologies, Incorporated.
# All Rights Reserved.
...
# --------   ---       ------------------------------------------------------
# 12/21/21   rjain     Make segments align to p_align for current segment.
# 07/1/20    rrwt      Added segment overlap check
# 06/30/20   yps       Support python3.8.3
# 04/17/20   cm        Fixed Zero division error
# 04/14/20   yps       Removed sectools hardcode path
# 02/27/20   yps       Make scripts compatible with python2 and python3
# 10/15/17   dp        Fixed equation "phdr.p_vaddr + phdr.p_memsz" 
#                      with "phdr.p_vaddr + phdr.p_memsz - 1". 
...
##############################################################################
# checks overlap in segments
##############################################################################
def check_segment_overlap(elf_out_file_name):

  prev_segment_end, prev_base, prev_size = 0,0,0
  elf_segments_list = []

  # msg to show on overlap
  error_msg = 'Segments are overlapping in ' + str(elf_out_file_name) + '\n\tBase\tSize\t\tBase+Size\n'

  elf_outfile_header, phdr_out_file_table = mbn_tools.preprocess_elf_file(elf_out_file_name)
  for i in range(elf_outfile_header.e_phnum):
    curr_hdr = phdr_out_file_table[i]
    elf_segments_list.append((curr_hdr.p_paddr, curr_hdr.p_memsz))

  elf_segments_list = list(sorted(elf_segments_list))

  # check the segment overlap and raise the error
  for base, size in elf_segments_list:
    if prev_segment_end > base:
      error_msg += str(hex(prev_base)) + "\t" + str(hex(prev_size)) + "\t\t" + str(hex(prev_base + prev_size)) + '\n'
      error_msg += str(hex(base)) + "\t" + str(hex(size)) + "\t\t" + str(hex(base + size)) + '\n'
      raise Exception(error_msg)
      sys.exit(0)
    prev_base = base
    prev_size = size
    prev_segment_end = base + size
...
def merge_elfs(env, 
  ...
  for i in range(elf_header1.e_phnum):
    ...
    if new_phdr.p_align == 0: # 0 and 1 mean un-alignment
        new_phdr.p_align = SEGMENT_ALIGN_4K  # to avoid zero division error
    # Correct offset to current phdr's alignment if not already aligned
    if segment_offset % new_phdr.p_align != 0:
        segment_offset = roundup(segment_offset,new_phdr.p_align)
        new_phdr.p_offset = segment_offset
    # update data segment offset to be aligned after current segment
    segment_offset += roundup(new_phdr.p_filesz,new_phdr.p_align)
...
#     print curr_phdr.p_filesz
  ...
  elf_out_fp.close()
  # check overlap in merged image
  check_segment_overlap(elf_out_file_name)
