...
class packit(object):
  ...
  def create_binary_deliverable(self,
    ...
    for path in tools_paths:
      tool_path = self.replace_arguments(os.path.join(pack_directory,path))
      dest_tool_path  = self.replace_arguments (os.path.join(binary_directory,path))
      ctool_path = self.correct_case_of_path(tool_path)
      if ctool_path != None:
        try:
          shutil.copytree(ctool_path,dest_tool_path)
          logger.debug("[%s] Copying %s to %s.",self.task_name,ctool_path,dest_tool_path)
        except:
          logger.info("[%s] Error encountered while copying tools from %s to %s.", self.task_name,ctool_path,dest_tool_path)
          logger.info("[%s] Error info:%s", self.task_name, sys.exc_info()[0])
          sys.exit()
      else:
