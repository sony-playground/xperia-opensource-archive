...
def create_new_cfg_item(input_cfg_item, base_directory):
  ...
  if CONFIG_NAME in input_cfg_item:
    ...
    config_file_size = int(cfg_entry[ITEM_SIZE][VALUE],16) % int(ITEM_SIZE_ALIGN,16)
    if(config_file_size):
      ...
      try:
        ...
        print("Appending 0s to: " + config_file + " to make the file " + ITEM_SIZE_ALIGN + " bytes aligned")
        with open(config_file, 'ab') as file:
            file.write(b'\x00'*(int(ITEM_SIZE_ALIGN,16) - config_file_size))
            file.close()
      except:
        print_error_exit("/nERROR: Input file:\"" + config_file + "\" is not " + ITEM_SIZE_ALIGN + " aligned and RO. Please update input file size to be " + ITEM_SIZE_ALIGN + " bytes aligned")
...
def create_meta_data_table(json_input_cfg_items, xbl_cfg_value, fp_cfg_meta_file, base_directory):
...
def generate_xbl_config_metadata(json_config_path, base_directory, output_directory):
  ...
  for xcfg_meta_item in input_json_xbl_cfgs:
    ...
    create_meta_data_table(input_json_xbl_cfgs[xcfg_meta_item], str(xcfg_meta_item), fp_cfg_meta_file, base_directory)
