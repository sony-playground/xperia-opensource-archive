...
def processArgs ():
    ...
    if options.inputs is None:
        ...
        options.inputs = [file for file in options.inputs if file.strip() not in ["","-i"]]
...
def PrintConfig(ModName, CONFIG, devcfgdatafilename, ConfigType, structSize):
    ...
    ranges = {}
    ranges["SegmentOffset"] = range(len(SegmentOffset))
    ranges["NameSection"] = range(len(SegmentOffset), len(NameSection))
    ranges["StringSection"] = range(len(SegmentOffset+NameSection), len(SegmentOffset+NameSection)+len(StringSection))
    ranges["ByteSection"] = range(len(SegmentOffset+NameSection+StringSection), 
                                  len(SegmentOffset+NameSection+StringSection)+len(ByteSection))
    ranges["UintSection"] = range(len(SegmentOffset+NameSection+StringSection+ByteSection), 
                                  len(SegmentOffset+NameSection+StringSection+ByteSection)+len(UintSection))
    ranges["DevSection"] = range(len(SegmentOffset+NameSection+StringSection+ByteSection+UintSection), 
                                 len(SegmentOffset+NameSection+StringSection+ByteSection+UintSection)+len(DevSection))
    ...
    if len(StructIncFiles) == 0:
        #struct ptrs
        CONFIG.write ('\n')
        for struct_ptr in list(StructPtrs.keys()):
    ...
    if structSize:
        CONFIG.write ("\nconst DALSYSPropStructTblType " + dalprop_structptrs_str + "[" + str(len(list(StructPtrs.keys()))+1) + "] =  {\n")
    else:
        CONFIG.write ("\nconst void * " + dalprop_structptrs_str + "[" + str(len(list(StructPtrs.keys()))+1) + "] =  {\n")
    ...
    for i in range(0, len(PropBin), 4):
        if i%20 == 0:
            CONFIG.write ("\n\t\t\t")
        if type(PropBin[i]) is type(b''):
            CONFIG.write ("%s" % (PropBin[i]))
        elif type(PropBin[i]) is type(u'') and i not in ranges["StringSection"]:
            CONFIG.write ("%s" %PropBin[i])
        else:
            if i+3<len(PropBin):
                writeval = 0
    ...
    if len(ConfigStructList)!=0:
        ...
        CONFIG.write("{\n\t\t\t")
        for index in range(len(ConfigStructList)):
            CONFIG.write(ConfigStructList[index])
            if(index <len(ConfigStructList)-1):
                CONFIG.write (",\n\t\t\t")
        CONFIG.write ("\n};\n\n")
...
def PackData(value, type1):
   if type1 == 1:    #String
      ...
      value=list(value)
      for index in range (len(value)):
         if ((type(value[index]) is type(b'')) or (type(value[index]) is type(u''))):
...
def GenerateGlobalDef(pDriver):
   ...
   for global_defs in pDriver.getElementsByTagName("global_def"):
      #removing comments before normalize is called to make sure the correct input is normalized
      removeComments(global_defs)
...
#Function that parses the xml tree from the root passed as parameter and removes all the comment nodes
def removeComments (root):
    for node in root.childNodes[:]:
        if node.nodeType == node.COMMENT_NODE:
            print("removing comment" + node.nodeValue)
...
def GenerateArray(DriverName, node, GlobalDef, GlobalOffset, remove_dup_names, memory_optimised_props):
   ...
   if nodeid:
      ...
      if node.getAttribute("name") in StringDeviceNames:
     ...
     #Check for Hash Collisions    
      if (not remove_dup_names) or (remove_dup_names and (DriverName.lower() not in "NULL".lower())):
          ...
          if hash in HashCollisionOffset:
   ...
   for Props in node.getElementsByTagName("props"):
      ...
      #(inline struct config props in XML)
         ...
         value = list(StructPtrs.keys()).index(char_data)
      ...
      # Its a global property
         ...
         if char_data in GlobalOffset:
            ...
            #DALPROP_ATTR_TYPE_STRING_PTR
               ...
               charDataList = GlobalDef[char_data]
               if isPy3:
                  for i,char in enumerate(GlobalDef[char_data]):
                     charDataList[i] = ord(str(char))
                     if charDataList[i] == 48:
                        charDataList[i] = 0
               StringSection.extend(charDataList)
   ...
   if('common' in DeviceVersionDict):
   ...
   for version_rev in list(DeviceVersionDict.keys()) :
      offset_adjustment = 0
   ...
   if(num_of_rev) :
      ...
      for version_rev in list(VersionOffset.keys()) :
...
def writeStructHeader (filename, ConfigType):
    ...
    for structPtr in list(StructPtrs.keys()):
