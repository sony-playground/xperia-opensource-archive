...
#####################################################################
def main(path_boot_config_h, path_c_code, table_name, outputdir):
  ...
  loadcancel_index = get_struct_member_index(struct_members, r'boot_logical_func_type\s*boot_load_cancel')
...
#####################################################################
def create_flattened_c_file(code, list, table_name, root_func_name, outputdir):
  ...
  for (type,arrayname) in list:
    if type == 'procs':
      ...
      code = re.sub(r'boot_function_table_type\s*'+arrayname+'*[^;]*',
             r'static void '+arrayname+'(bl_shared_data_type *bl_shared_data)\n{\n'
             +contents+'}\n',
  ...
  for (type,name) in list:
    if same_img:
      if type == 'img':
        ...
        func += '\t'+name+'(bl_shared_data);\n'
        single_img_additional_code += '\t'+name+'(bl_shared_data);\n'
    if type == 'img':
      parentfuncname = find_and_convert_img_name(code, name)
      contents += parentfuncname+'(bl_shared_data);\n'
      imgcount += 1
      func = 'static void '+parentfuncname+'(bl_shared_data_type *bl_shared_data)\n{\n'
      same_img = True
  func += '\n}\n'
  multi_img_additional_code += func

  filepath = os.path.join(outputdir, root_func_name+'.c')
  ...
  if imgcount > 1:
    ...
    code = re.sub(r'boot_configuration_table_entry\s*'+table_name+'\s*\[\s*\]\s*=\s*{\s*((?:(?!}\s*;).)*)\s*}\s*;',
                  r'static void '+root_func_name+'(bl_shared_data_type *bl_shared_data)\n{\n'+contents+'}\n',
                  code, flags=re.DOTALL | re.MULTILINE)
...
def find_and_convert_procs_array(code, name):
  ...
  if matchobj:
    return re.sub(r'\"(.*?)\"','(bl_shared_data);',matchobj.group(1).replace('NULL','').replace(',','').replace('}','').replace('{',''))
  else:
