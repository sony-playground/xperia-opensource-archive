...
if platform.system() == 'Windows':
  ...
  BUILDEX_BASE_CMD = ['python', os.path.join(u.QCOMPKG_DIR,'..','..','boot_tools','spy_buildex.py')]
  ...
  COMPILER_NAME = 'clang'
  BUILDEX_BASE_CMD = [os.path.join(u.QCOMPKG_DIR,'..','..','boot_tools','spy_buildex.py')]
...
#####################################################################
def buildspy(verbose, database, buildex_full_cmd, buildex_comp_cmd):
  LOGGER.debug('doing component cleanall build')
  real_toolchain_path = run_command_with_stdout(buildex_comp_cmd + ['-b', 'cleanall'], verbose, ignore_exit_code=False, yes_print=False)
...
def analyze_database(database, verbose, addfile=None, removefile=None):
  ...
  add_cmd = und_base_cmd + ['add', addfile]
  ...
  if not addfile and not removefile:
...
def main(verbose,
  ...
  buildex_comp_cmd = buildex_full_cmd + ['--components', component]
  try:
    buildspy(verbose, database_path, buildex_full_cmd, buildex_comp_cmd)
  finally:
    LOGGER.info('RUNNING CLEANUP. Doing component cleanall build.')
    run_command_with_stdout(buildex_comp_cmd + ['-b', 'cleanall'], verbose, ignore_exit_code=False, yes_print=False)
