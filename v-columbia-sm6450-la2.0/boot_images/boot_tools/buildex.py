...
#
# Copyright 2017 - 2021 by Qualcomm Technologies, Inc.
#                 All Rights Reserved.
                    ...
                    if build_config_json1[k][j]["Name"]==build_config_json2[k][i]["Name"]:
                        for config_k in list(build_config_json2[k][i].keys()):
                            ...
                            if config_k != "Process" and config_k in build_config_json1[k][j].keys():
                                print("[buildex.py] ERROR:object [",build_config_json1[k][j]["Name"],"] has confilict item [",config_k,"] in json files")
                                print("[buildex.py] The same item of object can't exsit in two json files, please resolve it.")
                                sys.exit(1)
...
def gen_build_config(options,target_path,variant):
  ...
  if json_path == "":
    print("Error:Can't find buildconfig.json in target folder!!")
    sys.exit(0)
  ...
  for key in list(buildconfig_json_files.keys()): 
    if os.path.exists(buildconfig_json_files[key]):
      ...
      print("Error:",buildconfig_json_files[key],"is not exist!!!")
...
def main():
  ...
  if options.target == "":
    print("Error: Buildex.py requires -t option, please input target name, like Waipio")
    sys.exit(0)
  try:
  ...
  for i in range(len(build_config['CreateImages'])):
    if build_config['CreateImages'][i]['Name'] == str(variant_flavor.image):
      ...
      for j in range(len(build_config['CreateImages'][i]['Process'])):
        ...
        try:
          print(tool_cmds,cmd_params)
          subprocess.check_call(tool_cmds + cmd_params)
        except Exception as error:
         print (error)
         raise NameError("ERROR: buildex::excute command:",tool_cmds ,cmd_params,"failed with error: \n")
      #break;
  return
#####################################################################
...
def component_dependency_check(up_level_dependency, dependencies):
  ...
  if not os.path.exists(dependency_pkg_path):
    print( "Error: Dependency pkg path ",dependency_pkg_path, " does not exist!!!")
    sys.exit(-1)
  ...
  if not os.path.exists(dependency_pkg_buildconfig_path): 
    print( "Error: Dependency pkg ",dependency_pkg_buildconfig_path, " does not exist!!!")
    sys.exit(-1)
  ...
  for component in cur_dep_pkg_buildconfigconfig['Components']:
   if "Dependency" in component:
     if component["Dependency"]['Pkg'] in dependencies:
       print( "Error:",component["Dependency"]['Pkg']," and ", up_level_dependency['Pkg']," depend on each other!!! Please check these two pkg buildconfig.json files")
       print( "Note:PKG can't depend on each other!!!")
       sys.exit(-1)
     else:
...
def process_component(options,variant_flavor,component,process_config,build_config,tools_config,merge_dict):
  ...
  if component_config is None:
    ...
    raise NameError("ERROR: buildex::build_component: " + \
  ...
  if "Binary" in component_config and "Process" not in component_config:
    ...
    if os.path.exists(Binary_path):
      ...
      sys.exit( ("[buildex.py] Component "+component_config['Name']+"\'s binary:"+Binary_path+" doesn't exist!!!"))
  ...
  if needs_compile:
    for i in range(len(component_config['Process'])):
      ...
      if "Createfolders" in cprocess_config:
        print("create folder ",cprocess_config["Createfolders"] )
      ...
      #print(build_cmd[0].split(os.sep))
      if build_cmd[0] == "build" or "build.py" in list(build_cmd[0].split(os.sep)) or "build.py" == build_cmd[0]:
        ...
        if ((options.components and component_config['Name'].upper() in options.components.upper()) or options.components == "") and not check_components_status(build_cmd):
          ...
          #print(build_cmd[0].split('.'))
          if "py" in build_cmd[0].split('.'):
            build_cmd.insert(0,'python')
          try:
             subprocess.check_call(build_cmd)
          except subprocess.CalledProcessError as cperror:
             print("ERROR: buildex::Run ",build_cmd,"Build Subprocess failed with error... ")
             print(cperror.output)
             sys.exit(1)
  ...
  if matches:
    for match in matches:
      ...
      if arg in ARGUMENTS:
        ...
        raise NameError("ERROR::Buidex: argument "+arg+" doesn't exist!!!" )
  if platform.system() == 'Windows':
...
def get_python_path(version):
  if platform.system() == "Windows":
    ...
    if python_path==b'':
     ...
     python_path = 'python'+str(version).encode()
  ...
  if 'Path' in tool_config:
      ...
      if isinstance(tool_config['Path'],type([])):
        ...
        if 'arm-elfcopy' in tool_config['Path']:
          if os.getenv("CLANG1203LINUX_ELFCOPY") and os.path.exists(os.getenv("CLANG1203LINUX_ELFCOPY")):
            tool_config['Path'] = os.getenv("CLANG1203LINUX_ELFCOPY")
...
def init_work_path(imgrootpath):
  ...
  if imgrootpath == "":
    ...
    if os.path.split(imgrootpath)[1]!="boot_images":
      print( "ERROR: imgrootpath is not boot_images path")
      sys.exit(0)
  ...
  for i in range(2):
    ...
    if "boot_images" in dirs:
      if dirs.count('boot_images') > 1:
         print( "There are more then one boot_images folder under current path")
         sys.exit(1)
      else:
  ...
  if build_root_path == "":
    print( "Can't find image root path")
    sys.exit(0)
...
def init_conf_folder(imgrootpath):
  ...
  if Conf_def_path != '':
    for conf_file in conf_files:
     ...
     print( "ERROR: can't find BaseTools!!!")
...
def setup_environment(imgrootpath):
  ...
  if edksetuppath =='':
    print( "ERROR: can't find edksetup.bat")
    sys.exit(0)
  ...
  if len(tools_config)==0:
    sys.exit("ERROR: Can't find toolchainconfig.json in all components folder!!!")
    
  # Export the OS environment variables
  operation_name = platform.system()

  if os.getenv("CLANG1203LINUX_BIN") and os.path.exists(os.getenv("CLANG1203LINUX_BIN")):
    tools_config['ToolChain']['CLANG120']['Linux']['Export']['CLANG120'] = os.getenv("CLANG1203LINUX_BIN")

  tools_dtc_path = os.path.join(build_root_path, '../tools/dtc/1.7.0')
  if os.path.exists(tools_dtc_path):
    tools_config['ToolChain']['DTC']['Linux']['Export']['DTC'] = tools_dtc_path
...
def set_compiler_path(tool_config,tool_name):
   ...
   if os.getenv("CLANG1203LINUX_BIN") and os.path.exists(os.getenv("CLANG1203LINUX_BIN")):
     os.environ[COMPILER_BIN_NAME] = os.getenv("CLANG1203LINUX_BIN") + "/"
   elif os.path.exists(tool_config['Path']):
     os.environ[COMPILER_BIN_NAME] = tool_config['Path']
   else:
     if os.getenv("CLANG1203LINUX_BIN"):
       print(( os.getenv("CLANG1203LINUX_BIN") +"does not exist"))
     else:
       print(( tool_config['Path'] +"does not exist"))
     raise NameError("ERROR: buildex::build_single_flavor: " + \
                        "Missing LLVM tool chain." + \
                        "Expected location " + os.environ[COMPILER_BIN_NAME])
     sys.exit(-1)
     
   if os.getenv("CLANG1203LINUX_TOOLS_BIN") and os.path.exists(os.getenv("CLANG1203LINUX_TOOLS_BIN")):
     os.environ[COMPILER_PREFIX] = os.getenv("CLANG1203LINUX_TOOLS_BIN") + "/"
   elif os.path.exists(tool_config['Tool_path']):
     os.environ[COMPILER_PREFIX] = tool_config['Tool_path']
   else:
     if os.getenv("CLANG1203LINUX_TOOLS_BIN"):
       print(( os.getenv("CLANG1203LINUX_TOOLS_BIN") + " does not exist"))
     else:
       print(( tool_config['Tool_path'] + " does not exist"))
     raise NameError("ERROR: buildex::build_single_flavor: " + \
                        "Missing LLVM tool chain." + \
...
def create_firehose_version(firehose_version):
  ...
  if firehose_version == "":
    firehose_version = "local_build"

  if os.path.isfile(firehose_version_file) and firehose_version == "0":
    print( "[buildex.py] QcomPkg/Library/DevPrgSomcLib/FirehoseVersion.h already exists.")
    print( "[buildex.py] No build version specified. Will use existing version.")
    return

  print(( "[buildex.py] Creating QcomPkg/Library/DevPrgSomcLib/FirehoseVersion.h with version: " + firehose_version))
  try:
    ...
    raise NameError("ERROR: buildex::create_firehose_version: " + \
                    "Unable to build firehose version file")
...
def find_components(image,json_file):
  ...
  if len(components) == 0:
    raise NameError("ERROR: buildex::find_components: "\
