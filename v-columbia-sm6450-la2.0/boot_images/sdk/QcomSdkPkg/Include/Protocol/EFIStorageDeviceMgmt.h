...
/*=============================================================================
 ...
 --------    ---     -----------------------------------------------------------
 02/24/23    cm      Add partition name in Error stats Queue
 01/11/23    cm      Added support for BB in Error Stats 
 11/07/22    sh      Added support for Error Stats 
 07/25/22    cm      Add Nand Devinfo support
 03/21/22    cm      Get information if turbo mode is supported
 02/18/22    cm      Add Level shifter support for quest
...
/** @ingroup EFI_STORAGE_DEV_MGMT_PROTOCOL_data
    Protocol version.
*/
#define EFI_STORAGE_DEV_MGMT_PROTOCOL_REVISION      0x0000000000010000
...
/**
  NAND bit error statistics
 */
typedef struct {
  UINT32 PageNum;
  /* Error Page number */
  CHAR8 PartitionName[30];
  /* Partition Name */
  UINT8 Error_ID;
  /* Identifies Error  */
  UINT8 TestID;
  ...
  UINT8  UECC_Error;
  /* Captures erase counter */
  UINT32 EraseCounter;
  /* capture error for each CW as bitmask  */
  UINT8 NumBitErrCnt[8];
  /* capture error for each CW  */
}EFI_NAND_ERR_INFO;

/**
  NAND specific error statistics
*/
typedef struct {
  UINT32 TotalFailureCnt;
  /**< Total number of failures since init */
  UINT32 TotalECCCnt;
  /**< Total Number of pages with UECC Errors */
  UINT32 TotalMPUErrCnt;
  /**< Total Number of MPU Error encountered */
  UINT32 TotalBAMErrCnt;
  /**< Total number of BAM errors encountered */
  UINT32 TotalTimeoutCnt;
  /**< Total Number of timeouts encountered */
  UINT32 ItrFailureCnt;
  /**< Total number of failures encountered during last iteration */
  UINT32 TestBBgenCnt;
  /**< Total number of bad block generated during test run */
  EFI_NAND_ERR_INFO *err_info;
  /**< Identifies error and provide additional info */
} EFI_NAND_ERR_STATS;
...
typedef union {
   ...
   EFI_NAND_ERR_STATS   **NandErrorStats;
} EFI_STORAGE_ERR_STATS;

/** @ingroup EFI_STORAGE_DEV_MGMT_PROTOCOL_data
  UFS/SD Device Information
*/
typedef struct {
  UINT32 SpecVersion; 
...
} EFI_MNAND_STORAGE_DEV_INFO; 

/** @ingroup EFI_STORAGE_DEV_MGMT_PROTOCOL_data
 NAND Storage Device Information
*/
typedef struct {  
  UINT8   *device_name;                     /**< Device name string */
...
} EFI_NAND_STORAGE_DEV_INFO;

/** @ingroup EFI_STORAGE_DEV_MGMT_PROTOCOL_data
  Storage Device Information
*/
typedef union {
  EFI_MNAND_STORAGE_DEV_INFO st_info;
  EFI_NAND_STORAGE_DEV_INFO nd_info;	
