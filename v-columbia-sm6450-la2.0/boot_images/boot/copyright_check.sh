...
grep  -e "Copyright.*Qualcomm.*Incorporated$" -e "Copyright.*Qualcomm.*Incorporated.$" -e "Copyright.*Qualcomm.*Incorporated"  * $grep_common_flags >copyright_check.log 
...
if [ "$value" -gt "$threshold" ]; then
  ...
  echo "[PASS] Current invalid copyright count value is $value and threshold is $threshold" >copyright_check_pass.log
fi
echo "------------------------------------------------------------------------------"
