...
Copyright 2010-2020 by Qualcomm Technologies Inc.  All Rights Reserved.
=============================================================================*/
...
*
*/
static bl_error_boot_type boot_log_message_raw2(char *message,
                          uint32 timestamp,
                          char log_type,
                          char *optional_data)
{
...
//    boot_log_meta_info *log_meta_info_ptr = logger_ptr->log_data->meta_info_start;

//    char *init_buffer_ptr = (char *)log_meta_info_ptr->log_buf_ptr;
    char *init_buffer_ptr = NULL;
    uint32 data_len = 0;

    return_status = boot_log_message_ram2(&(logger_ptr->log_meta_info), message, timestamp, log_type, optional_data, &init_buffer_ptr, &data_len);

//    uint32 data_len = (char *)log_meta_info_ptr->log_buf_ptr - init_buffer_ptr;
...
*
*/
static bl_error_boot_type boot_log_message_optional_data2(char *message, char *optional_data)
{
  bl_error_boot_type return_status = BL_ERR_NONE;
  ...
  do
  {
    ...
    return_status = boot_log_message_raw2(message, current_timestamp, LOG_MSG_TYPE_BOOT, optional_data);
    if(return_status != BL_ERR_NONE)
    {
      break;
    }
...
*
*/
static bl_error_boot_type boot_log_message2(char *message)
{
  bl_error_boot_type return_status = BL_ERR_NONE;
  /* Call boot_log_message_optional_data so two functions do not have
     redundant logic. */
  return_status = boot_log_message_optional_data2(message, NULL);
...
static bl_error_boot_type boot_log_message_delta_time_optional_data(uint32 old_time, char *message, char *optional_data)
{
  ...
  do
  {
    ...
    return_status = boot_log_message_raw2(message, delta_time, LOG_MSG_TYPE_DELTA, optional_data);
    if(return_status != BL_ERR_NONE)
    {
      break;
    }
...
bl_error_boot_type boot_logger_get_interface(void *ptr)
{
  ...
  do
  {
    ...
    logger_if->log_message = boot_log_message2;
    logger_if->log_message_optional = boot_log_message_optional_data2;
...
void boot_log_message_raw(char *message,
                          ...
                          char *optional_data)
{
  (void)boot_log_message_raw2(message, timestamp, log_type, optional_data);
}
...
void boot_log_message_optional_data(char *message, char *optional_data)
{
  ...
  (void)logger->log_message_optional(message, optional_data);
}
...
// Just temporary, to avoid linking issues.
// We should npot call a particular logger API
// TBD
