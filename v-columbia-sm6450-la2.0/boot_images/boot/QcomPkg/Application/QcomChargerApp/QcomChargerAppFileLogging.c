...
#define QCOM_CHARGER_LOGFILE_SIZE   0x400000 // 4MB
...
EFI_STATUS QcomChargerAppFileLog_LogParam(EFI_QCOM_CHARGER_ACTION_INFO *ChargerActionInfo)
{
  ...
  CHARGERAPP_FILE_UART_DEBUG((EFI_D_ERROR, "QcomChargerApp:: Battery Status %d,%d,%d,%d \r\n", ChargerActionInfo->BatteryStatusInfo.StateOfCharge,
...
EFI_STATUS QcomChargerAppFileLog_InitFileLog(VOID )
{
  ...
  UINT32           Value      = 0;
  UINTN            MaxLogFileSize = QCOM_CHARGER_LOGFILE_SIZE; // Target-specific maximum log size
  EFI_STATUS       Status     = EFI_SUCCESS;
  ...
  if(NULL != FileLogInfo.gChargerLogHandle)
  {
    ...
    if ((Status == EFI_SUCCESS) && (Value > 0)) {
      DEBUG((EFI_D_WARN, "QcomChargerApp:: %a InitFileLog SUCCESS MaxLogFileSize = (%x)\r\n",  __FUNCTION__, Value));
      MaxLogFileSize = Value;
    }
    result = ULogFile_Open(&(FileLogInfo.gChargerLogHandle), QCOM_CHARGER_LOGFILE_IN_EFS, MaxLogFileSize);
    if (0 == result)
    {
