...
--------   ---     ------------------------------------------------------------ 
03/19/18   ts      Updated with SMMU configuration and latest API changes
...
void gpi_test_uart_cb(gpi_result_type *result)
{
   if (result != NULL && result->user_data != NULL)
   {
      ...
      // copy result & return
      memsmove(&test_result[evt], sizeof(gpi_result_type)*(NUM_CMDS-evt),result, sizeof(gpi_result_type));
   }
...
void gpi_test_reg_gpii(void)
{
   ...
   gpii_params.chan[GPI_INBOUND_CHAN].ring_base_va = (U_LONG)(&in_transf_ring_elem[0]);
   gpii_params.chan[GPI_INBOUND_CHAN].ring_base_pa = (U_LONG)(&in_transf_ring_elem[0]);
   gpii_params.chan[GPI_OUTBOUND_CHAN].tre_ring_size = MAX_NUM_GPI_RING_ELEM;
   gpii_params.chan[GPI_OUTBOUND_CHAN].ring_base_va = (U_LONG)&out_transf_ring_elem[0];
   gpii_params.chan[GPI_OUTBOUND_CHAN].ring_base_pa = (U_LONG)(&out_transf_ring_elem[0]);
   gpii_params.irq_mode = FALSE;
...
void gpi_test_uart_in_transf(void)
{
   ...
   in_transf_ring_elem[0].dword_1 = 0x020000A0;
   in_transf_ring_elem[0].dword_2 = 0x00000002;
   ...
   in_transf_ring_elem[2].ctrl = 0x00100300;
...
void gpi_test_uart_out_transf(void)
{
   ...
   out_transf_ring_elem[0].dword_2 = 0x00000002;
  ...
  )
{
  ...
  *loopback_cfg_addr = 0x3;
  ...
  EFI_Status = gBS->LocateProtocol(&gEfiHalIommuProtocolGuid,
                               NULL,
  ...
  if(HAL_IOMMU_ERR_OK != fp_table->config_bypass_domain_func(p_pcie_domain_hdl, HAL_IOMMU_AARCH64_NON_CCA_DOMAIN_TYPE)) {
    DEBUG(( EFI_D_WARN, "%a: IOMMU domain configure failed\r\n", __func__));
    return EFI_UNSUPPORTED;
  }

  if (HAL_IOMMU_ERR_OK != fp_table->domain_attach_func(p_pcie_domain_hdl,
                                                       (void *)"\\_SB_.QGP1", 0x0, 0x0))
  {
    return EFI_DEVICE_ERROR;
  }

  if (HAL_IOMMU_ERR_OK != fp_table->domain_attach_func(p_pcie_domain_hdl,
                                                       (void *)"\\_SB_.QGP1", 0x1, 0x0))
  {
    return EFI_DEVICE_ERROR;
  }
  ...
  if (HAL_IOMMU_ERR_OK != fp_table->domain_detach_func(p_pcie_domain_hdl,
                                                       (void *)"\\_SB_.QGP1", 0x0, 0x0))
  {
    return EFI_DEVICE_ERROR;
  }
