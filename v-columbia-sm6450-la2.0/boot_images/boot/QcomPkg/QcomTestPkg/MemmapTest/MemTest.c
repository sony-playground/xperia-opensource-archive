...
/*=============================================================================
 ...
 02/01/19   am      Skipping Non-UEFI reserved memory regions in pagecount calculation
 10/30/18   kpa     Increase MAX_LIMIT_RT_RESERVED_ACPI to 360 MB
 09/18/17   mt      Added tests for UncachedAlignedRuntimePool and Pages
 04/28/17   pawans  Added Uefi memory use threshold
...
#define MAX_NUM_LOOP   48                 // Reduced the MAX_NUM_LOOP from 100 to 75 due to reduction in DXE Heap size due to carve out to Scheduler Heap
#define MAX_LIMIT_RT_RESERVED_ACPI 2000
  ...
  )
{
  ...
  for (Index=1; Index<=MAX_NUM_LOOP; Index++)
  {
    ...
    else if( ( (int)(uintptr_t)pMemBuffer & (SIZE_4KB-1) ) != 0x0 )
    {
      Status = EFI_OUT_OF_RESOURCES;
  ...
  for (Index=1; Index<=MAX_NUM_LOOP; Index++)
  {
    ...
    else if( ( (int)(uintptr_t)pMemBuffer & (SIZE_8KB-1) ) != 0x0 )
    {
      Status = EFI_OUT_OF_RESOURCES;
...
VOID PrintMemoryAttributesTable (
  VOID 
  )
{
  UINTN                          Index;
  EFI_MEMORY_ATTRIBUTES_TABLE    *MemoryAttributesTable = NULL;  
  EFI_MEMORY_DESCRIPTOR          *MemoryAttributesEntry = NULL;
  ...
  if((Index  == gST->NumberOfTableEntries ) || (MemoryAttributesTable == NULL)) {
    AsciiPrint( "MemoryAttributesTable  NOT FOUND:\n");  
    return ;
  }
  
  AsciiPrint( "MemoryAttributesTable:\n");
  AsciiPrint( "Version              - 0x%08x\n", MemoryAttributesTable->Version);
  AsciiPrint( "NumberOfEntries      - 0x%08x\n", MemoryAttributesTable->NumberOfEntries);
  AsciiPrint( "DescriptorSize       - 0x%08x\n", MemoryAttributesTable->DescriptorSize);
  
  MemoryAttributesEntry = (EFI_MEMORY_DESCRIPTOR *)(MemoryAttributesTable + 1); 
  if(MemoryAttributesEntry == NULL) {
    AsciiPrint( "MemoryAttributesTable is empty \n");  
    return ;
  }  

  for (Index = 0; Index < MemoryAttributesTable->NumberOfEntries; Index++) {  
    switch (MemoryAttributesEntry->Type) {
    case EfiRuntimeServicesCode:
    case EfiRuntimeServicesData:
      MemoryAttributesEntry->Attribute &= (EFI_MEMORY_RO|EFI_MEMORY_XP|EFI_MEMORY_RUNTIME);
      AsciiPrint( "Entry (0x%x)\n", MemoryAttributesEntry);
      AsciiPrint( "  Type              - 0x%x\n", MemoryAttributesEntry->Type);
      AsciiPrint( "  PhysicalStart     - 0x%016lx\n", MemoryAttributesEntry->PhysicalStart);
      AsciiPrint( "  VirtualStart      - 0x%016lx\n", MemoryAttributesEntry->VirtualStart);
      AsciiPrint( "  NumberOfPages     - 0x%016lx\n", MemoryAttributesEntry->NumberOfPages);
      AsciiPrint( "  Attribute         - 0x%016lx\n", MemoryAttributesEntry->Attribute);
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
     ...
     */
     for (Index = 3, TotalPages = 0; Index <= 6; Index++) {
      ...
      AsciiPrint("\nBS and RT (Data & Code) Total Pages = %llu and Total Memory = %d MB\n", TotalPages, PAGES_TO_MEGABYTES(TotalPages));
