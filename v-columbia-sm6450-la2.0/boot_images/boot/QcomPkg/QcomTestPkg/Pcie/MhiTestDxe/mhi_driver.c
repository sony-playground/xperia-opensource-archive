...
static void mhi_drv_process_closed_clients(void)
{
   ...
   mhi_notify_el_t *notify_el = NULL;
   mhi_notify_el_t *notify_el_next = NULL;
   uint32 client_count = 0;
   uint32 notify_count = 0;
   mhi_cb_payload_t data;
   //mhi_dev_t *dev = NULL;
   ...
   list_for_each_item (&mhi_drv->device_list, mhi_dev_el_t, link, dev_el, dev_next)
   {
	 ...
	 /* TODO : Adding a hack to remove all the devices from notifylist except wlan */
	 notify_count = 0;
	 list_for_each_item (&mhi_drv->notify_list, mhi_notify_el_t, link, notify_el, notify_el_next)
	 {
	    /* Assuming 1st element to be wlan and not removing it */
	    if (notify_count != 0) {
	       list_del(&notify_el->link);
	    }
	    notify_count++;
