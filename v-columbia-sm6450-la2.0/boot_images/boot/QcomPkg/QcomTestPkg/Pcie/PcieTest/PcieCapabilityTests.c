...
{
   uint32 val, host_L1ss_cap, ep_L1ss_cap, option_bits = 0;
   ...
   if (aspm_type == ASPM_TYPE_L11s) option_bits = L1_1_SS_BITS;
   if (aspm_type == ASPM_TYPE_L12s) option_bits = L1_2_SS_BITS;
...
pcie_status_t set_ASPM (UINTN CoreIdx, volatile uint32* ep_cfg_addr, boolean Enable, uint32 aspm_type)
{
   ...
   pcie_status_t pstatus;
   uint32 aspm_state = 0, repeat_cnt;
   pcie_link_status_t link_status_info;

   if (aspm_type <= 2) aspm_state = 0x14;  // TODO: Publish LTSSM states in a common header
   if (aspm_type == 0) aspm_state = 0x12;
   ...
   if (aspm_type == 0)
   {
      enable_disable_L1ss (host_cfg_addr, host_pcie_cap, ep_cfg_addr, ep_pcie_cap, 0, ASPM_TYPE_L11s);
      enable_disable_L1ss (host_cfg_addr, host_pcie_cap, ep_cfg_addr, ep_pcie_cap, 0, ASPM_TYPE_L12s);
   ...
   do
   {
      ...
      if (Enable == 0)
      {
         if (link_status_info.link_state != 0x11)  // TODO: Publish LTSSM states in a common header
            AsciiPrint ("Link is not in L0 after ASPM Disabled 0x%X\n", link_status_info.link_state);
         return PCIE_SUCCESS;
      }

      if (link_status_info.link_state == aspm_state)
      {
         AsciiPrint ("Entered into link LPM state 0x%X : 0x%X\n", link_status_info.link_state, (&link_status_info.L0s_counter)[aspm_type]);
         return PCIE_SUCCESS;
      }
...
EFI_STATUS TestASPM (UINTN CoreIdx, volatile uint32* ep_cfg_addr, uint32 aspm_type)
{
   ...
   aspm_counters = &link_status_info.L0s_counter;

   if (aspm_type <= 2) aspm_state = 0x14;
   if (aspm_type == 0) aspm_state = 0x12;
   ...
   for (i = 0; i < 0x10; ++i)
   {
      ...
      last_cnt = aspm_counters[aspm_type];

      if (link_status_info.link_state != aspm_state)
      {
         AsciiPrint ("Not in L0s 0x%x, 0x%X 0x%X\n", link_status_info.link_state, last_cnt, aspm_counters[aspm_type]);
         gBS->Stall (100); // wait to enter into L0s/L1ss
      }
      else
      {
         ...
         if ((last_cnt + 1) < aspm_counters[aspm_type])
            AsciiPrint ("Wakeup multiple times in L0s 0x%x, 0x%X 0x%X\n", link_status_info.link_state, last_cnt, aspm_counters[aspm_type]);
		
		if (last_cnt == aspm_counters[aspm_type])
        {
            AsciiPrint ("Test failed L0s counter didn't increment 0x%X 0x%X\n", last_cnt, aspm_counters[aspm_type]);
            pcie_print_aer_logs (CoreIdx, ALL_BDF_VAL);         
            return EFI_DEVICE_ERROR;
        }
        break;
