            ...
            )
{
    ...
    if((result = SpmiBus_ReadLong( bid, sid, prio, readAddr, ucData, 2, &bytesRead )) != SPMI_SUCCESS) {
    ...
    {
        ucData[0] = 0x03;
        if((result = SpmiBus_WriteLong( bid, sid, prio, writeAddr, &ucData[0], 1 )) != SPMI_SUCCESS) {
            DebugPrint( DEBUG_ERROR, "SPMI write failed: %d\n", result );
            goto exit;
        }
        
        ucData[1] = 0;
        if((result = SpmiBus_ReadLong( bid, sid, prio, writeAddr, &ucData[1], 1, &bytesRead )) != SPMI_SUCCESS) {
            DebugPrint( DEBUG_ERROR, "SPMI write failed: %d\n", result );
            goto exit;
        }
        ...
        ucData[1] = 0xFC;
        ucData[2] = 0x1E;
        if((result = SpmiBus_ReadModifyWriteLongByte( bid, sid, prio, writeAddr, 
