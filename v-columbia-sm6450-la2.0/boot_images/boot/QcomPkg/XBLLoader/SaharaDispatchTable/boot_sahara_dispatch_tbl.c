...
--------    ---      ----------------------------------------------------------
07/08/21    ck       Updated xbl_pcie_dump_enabled to xbl_pcie_fast_boot_enabled
...
extern boolean xbl_pcie_fast_boot_enabled(void);
...
bl_error_boot_type boot_sahara_dispatch_table_init(const boot_handle config_context_handle)
{
    ...
    if(boot_device == PCIE_FLASHLESS || nand_pcie_dump)
    {
        ...
        sahara_dispatch_tbl_ptr->get_max_packet_size =   mhi_if->get_max_packet_size;
    ...
    else
    {
        ...
        sahara_dispatch_tbl_ptr->get_max_packet_size =   qusb_if->boot_usb_al_bulk_get_max_packet_size;
    }
