...
static void boot_sahara_handle_hello_resp( boot_sahara_info * const, 
                                           uint32 length );
static uint32 boot_sahara_get_adjusted_size(uint32, uint32 );
...
static boot_boolean boot_sahara_mem_debug_copy_restricted(boot_sahara_info *, 
                                                            uintnt, uintnt);

/*static boot_sahara_data*                _get_boot_sahara_data(
                                                    boot_sahara_info * const);
static boot_sahara_callback_parameters* _get_boot_sahara_callback_parameters(
                                                    boot_sahara_info * const);
static boot_sahara_command_handler_if*  _get_boot_sahara_command_handler_if(
                                                    boot_sahara_info * const);
static boot_sahara_cmd_exec_data*       _get_boot_sahara_cmd_exec_data(
                                                    boot_sahara_info * const);
static boot_sahara_dispatch_tbl*        _get_boot_sahara_dispatch_tbl(
                                                    boot_sahara_info * const);
static ramdump_table_if*                _get_boot_sahara_mem_debug_tbl(
                                                    boot_sahara_info * const);*/
...
static void* _get_boot_sahara_ramdump_dload_handle(
                                                    boot_sahara_info *boot_sahara_info_ptr)
{
    return (boot_sahara_info_ptr->mem_debug_handle).ramdump_table_handle;
}

void boot_sahara_unrecoverable_error_handler(boot_sahara_info *boot_sahara_info_ptr)
{
    /* Get Dispatch table from boot sahara info */
    boot_sahara_dispatch_tbl* sahara_dispatch_tbl_ptr = 
                        _get_boot_sahara_dispatch_tbl(boot_sahara_info_ptr);
    
    /* Get Mem debug table pointer from boot sahara info */
    ramdump_table_if* boot_sahara_mem_debug_tbl_ptr = 
                            _get_boot_sahara_mem_debug_tbl(boot_sahara_info_ptr);
    
    /* Shutdown the Transport layer */
    sahara_dispatch_tbl_ptr->shutdown();
    
    boot_sahara_mem_debug_tbl_ptr->unrecoverable_error_handler();
}
/* ============================================================================
...
)
{
    ...
    if(callback_parameters_ptr->sahara_rx_err_code == BULK_ERR_CABLE_DISCONNECT && 
        boot_sahara_data_ptr->sahara_mode == SAHARA_MODE_MEMORY_DEBUG)
    {
        boot_sahara_unrecoverable_error_handler(boot_sahara_info_ptr);
    }
}
...
)
{
    ...
    /* Call unrecoverable_error_handler if
        BULK_ERR_CABLE_DISCONNECT error is returned in ram dump mode*/  
    if(callback_parameters_ptr->sahara_tx_err_code == BULK_ERR_CABLE_DISCONNECT &&
       boot_sahara_data_ptr->sahara_mode == SAHARA_MODE_MEMORY_DEBUG)
    {
        boot_sahara_unrecoverable_error_handler(boot_sahara_info_ptr);
    }
...
{
#ifdef BOOT_SAHARA_ENABLE_LOGGING
...
#endif /* BOOT_SAHARA_ENABLE_LOGGING  */
}
...
static uint32 boot_sahara_get_adjusted_size(uint32 size, uint32 buf_size)
{
...
)
{
    ...
    // Max packet size needs to be removed when USB starts handling max packet size
    uint32 max_packet_size = sahara_dispatch_tbl->get_max_packet_size();

    uint32 adjusted_length = boot_sahara_get_adjusted_size(length, max_packet_size);
    
    /* Check for integer overflow or false adjusted length */
        ...
        else
        {
            ...
            if(verified_address == TRUE)
            {
                ...
                while((status == SAHARA_STATUS_SUCCESS) && (memory_transfer_size_left != 0))
                {
                    ...
                    if(!boot_sahara_mem_debug_copy_restricted(boot_sahara_info_ptr, 
...
static boot_boolean boot_sahara_mem_debug_copy_restricted(boot_sahara_info * const boot_sahara_info_ptr, 
                                                    const uintnt addr,
                                                    const uintnt len)
{
    ...
    status = ramdump_table_copy_restricted_region ((uint8*)((boot_sahara_info_ptr->mem_debug_handle).sahara_mem_debug_buffer), addr,len);
...
static void boot_sahara_handle_memory_debug( boot_sahara_info * const boot_sahara_info_ptr )
{
    ...
    //#ifndef BOOT_MEMORY_DEBUG_64BIT... Need to include 64 bit RD too
            ...
            enum boot_sahara_context_flag context_flag )
{
  ...
  boot_sahara_status rx_status = SAHARA_STATUS_SUCCESS;
  ...
  while (!(boot_sahara_data_ptr->sahara_done))
  {
    ...
    rx_status = SAHARA_STATUS_SUCCESS;
...
)
{
    ...
    uint32 image_offset = src_offset;
           ...
           image_transfer_size_left != 0)
    {
        ...
        if (BULK_ERR_TIMEOUT == error)
        {
        // ERROR: Timeout while receiving data
        status = SAHARA_NAK_TIMEOUT_RX;
        }
        else if (BULK_SUCCESS != error)
        {
        // ERROR: General transmission or reception error
        status = SAHARA_NAK_GENERAL_TX_RX_ERROR;
        }
