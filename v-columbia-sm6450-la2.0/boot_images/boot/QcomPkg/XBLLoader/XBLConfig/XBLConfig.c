...
Copyright 2016,2017,2020-2021 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
/*===========================================================================
...
)
{
  ...
  xblconfig_status_type status = XBLCONFIG_OPEN_ERR;
  uintnt index;
  char hwinfo_buf[HWINFO_BUF_SIZE];
  ...
  do
  {
    ...
    {
      item_entry_ptr = xbl_config_data->config_entry_list[index].entry_ptr;
	  if(chipinfo != item_entry_ptr->chipinfo)
	  	continue;
	  if(platforminfo != item_entry_ptr->platforminfo)
	  	continue;
    ...
    if(index == xbl_config_data->max_config_entries)
    {
      status = XBLCONFIG_OPEN_ERR;
		...
		if (config_name_starts_with != NULL)
			snprintf(hwinfo_buf, HWINFO_BUF_SIZE, "Best match [%s] DTB found for HW chipinfo:[%llx] and platforminfo:[%llx]",
				config_name_starts_with, chipinfo, platforminfo);
		else
			snprintf(hwinfo_buf, HWINFO_BUF_SIZE, "Best match DTB found for HW chipinfo:[%llx] and platforminfo:[%llx]",
				chipinfo, platforminfo);
		boot_log_message(hwinfo_buf);
	}
  }
  while(0);
