...
--------   ---          -------------------------------------------------------
10/21/21   rama         invalidate cache before SW access in boot_block_dev_read
...
static uint8 block_dev_buffer_unalign[MAX_SECTOR_SIZE] __attribute__ ((aligned (CACHE_LINE_LENGTH_BYTES)));
...
static bl_error_boot_type boot_block_dev_read(const boot_handle handle, void *ram_addr, uint64 src_offset, uint32 bytes_to_read)
{
  ...
  uint8* block_dev_unaligned_rw_buffer = NULL;
  ...
  {
    return GET_ERROR_CODE(BL_ERROR_GROUP_BLOCKDEV, BL_ERR_INCORRECT_SECTOR_SIZE);
  }
  ...
  if ((leading_pad > 0) || (trailing_length > 0))
  {
    // error = (((block_device_handle *)handle)->mem_if)->memalign(MAX_SECTOR_SIZE, CACHE_LINE_LENGTH_BYTES, (void *)&block_dev_unaligned_rw_buffer);
    // if(error != BL_ERR_NONE)
    // {
    // return error;
    // }
    // if(block_dev_unaligned_rw_buffer == NULL)
    // {
    // return GET_ERROR_CODE(BL_ERROR_GROUP_BLOCKDEV, BL_ERR_MEMORY_ALLOC_FAIL);
    // }
    block_dev_unaligned_rw_buffer = (uint8 *)(block_dev_buffer_unalign);
  }
  ...
  if (leading_pad > 0 )
  {
    ...
    {
      return GET_ERROR_CODE(BL_ERROR_GROUP_BLOCKDEV, BL_ERR_COLDPLUG_READ_FAILURE);
    }
...
static bl_error_boot_type boot_block_dev_write(const boot_handle handle, void *ram_addr, uint64 dst_offset, uint32 bytes_to_write)
{
  ...
  {
    //error = (((block_device_handle *)handle)->mem_if)->memalign(MAX_SECTOR_SIZE, CACHE_LINE_LENGTH_BYTES, (void *)&block_dev_unaligned_rw_buffer);
    //if(error != BL_ERR_NONE || block_dev_unaligned_rw_buffer == NULL)
    //{
    //return error;
    //}
    block_dev_unaligned_rw_buffer = (uint8 *)(block_dev_buffer_unalign);
  ...
  if (leading_length > 0)
  {
    ...
    if (end_sector == start_sector)
    {
      ...
      {
        return GET_ERROR_CODE(BL_ERROR_GROUP_BLOCKDEV, BL_ERR_BUFFER_OVERFLOW);
      }
      ...
      {
        return GET_ERROR_CODE(BL_ERROR_GROUP_BLOCKDEV, BL_ERR_COLDPLUG_WRITE_FAILURE);
      }
      return error;
    }
