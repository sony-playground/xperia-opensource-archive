...
Copyright 2010-2011, 2013, 2015, 2017, 2020, 2021 by Qualcomm Technologies Inc.  All Rights Reserved.
=============================================================================*/
...
)
{
  ...
  max_ddr_channels = ddr_if->get_max_channels();
  ...
  for(i=0; i<shared_dev_info->noofddr; i++)
  {
      ...
      if(status != BL_ERR_NONE) { goto exit; }
...
bl_error_boot_type boot_populate_ddr_details_shared_table(boot_handle config_context_handle)
{
  ...
  do
  {
    ...
    ddr_details_entry_ptr = (ddr_details_entry_ptr_type)smem_if->alloc(DDRINFO_UEFI_SMEM_ID, (sizeof(ddr_details_entry) + sizeof(DDR_REGIONS_ENTRY)+ sizeof(shub_freq_plan_entry)));
    ...
    if((((uintnt)ddr_details_entry_ptr) > SCL_SHARED_RAM_BASE) &&
      (((uintnt)ddr_details_entry_ptr + sizeof(ddr_details_entry) + sizeof(DDR_REGIONS_ENTRY) + sizeof(shub_freq_plan_entry))
