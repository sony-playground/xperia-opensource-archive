...
Copyright 2010-2011, 2013-2015, 2017, 2018, 2020 by Qualcomm Technologies Inc.  All Rights Reserved.
=============================================================================*/
...
* 
*/
bl_error_boot_type boot_shared_imem_init(void)
{
  bl_error_boot_type return_status = BL_ERR_NONE;
  boot_pbl_shared_data_if_type pbl_shared_data_if;
  uintnt boot_media = (uintnt)MEDIA_TYPE_MAX;
  ...
  /*First check to see if magic number is present*/
        ...
        BOOT_SHARED_IMEM_MAGIC_NUM)
	{
      /*If magic number is not there then 
        ...
        return_status = boot_pbl_shared_data_get_interface(&pbl_shared_data_if);
		if(return_status != BL_ERR_NONE)
		{
		  break;
		}
		return_status = pbl_shared_data_if.get_value(PBL_APPS_XBL_SHARED_DATA_PARAM_ID_BOOT_MEDIA_TYPE, &boot_media);
        if(return_status != BL_ERR_NONE)
		{
		  break;
		}
		boot_shared_imem_cookie_ptr->boot_device_type = (boot_media_type)boot_media;
