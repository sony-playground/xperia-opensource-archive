...
typedef enum
{
  PM_TYPEC_CONNECTOR_TYPE_TYPEC,
  PM_TYPEC_CONNECTOR_TYPE_uUSB,
  PM_TYPEC_CONNECTOR_TYPE_INVALID
} pm_typec_connector_type;
...
typedef enum
{
  /* 10A ICHG full-scale current and 10A total QBG IBAT full-scale current */
  PM_CHG_FS_10A,
  /* 20A ICHG full-scale current and 20A total QBG IBAT full-scale current */
  PM_CHG_FS_20A,
...
/*
 * Get if system is powered by battery, USBIN or wireless input. 
 * @param typec_connector_type [out]: USB connector type uUSB/typec.
 * @return PM_ERR_FLAG_SUCCESS if success, otherwise error code.
 */
pm_err_flag_type
