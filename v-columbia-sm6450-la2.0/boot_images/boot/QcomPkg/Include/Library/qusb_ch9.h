...
/**
 * SuperSpeedPlus USB Capability descriptor: Set of SuperSpeedPlus USB
 * specific device level capabilities
 */
PACKED typedef struct usb_ssp_cap_desc {
  uint8  bLength;
  uint8  bDescriptorType;
  uint8  bDevCapabilityType;
  uint8  bReserved;
  uint32 bmAttributes;
  uint16 wFunctionalitySupport;
  uint16 wReserved;
  uint32 bmSublinkSpeedAttr0;
  uint32 bmSublinkSpeedAttr1;  
} PACKED_POST usb_ssp_cap_desc_t;
