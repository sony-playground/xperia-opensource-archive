...
/*=====================================================================================
 ...
 --------     ---     -----------------------------------------------------------------
 11/08/21     faizali Added check for supported charger while working for NO PMI support
...
)
{
  ...
  if (AsciiStriCmp ((CHAR8*)Section, "PlatformCommon") == 0) //Common
  {
    ...
    if (AsciiStriCmp ((CHAR8*)Key, "EmergencyShutdownVbatt") == 0)
    {
      ...
      if(gIs_2S_Battery)
      {
        gChargerPlatformCfgData.ChargerLibCfgData.emergency_shutdown_vbatt *=2;
      }
      return;
    }
...
EFI_STATUS ChargerPlatform_CpdlogTimer_Exit( VOID ){
  ...
  if(ReadChargerpdEvt != NULL){
    ...
    if(LoggingCategories==0){
      // If log categories read from SDAM is 0, set it to 0x1FFFFFFFFF in ChargerFw for complete logs when boot to hlos. This overwrites selfhost log category (set by uefi) in SDAM.
...
STATIC EFI_STATUS IsChargingSupported(VOID)
{
  ...
  //Adding RUMI platform check which doesn't have PMIC
   if((EFI_PLATFORMINFO_TYPE_RUMI == PlatformType))
  {
    CHARGER_DEBUG((EFI_D_INFO, " %a RUMI platform detected  \r\n", __FUNCTION__));
    return EFI_UNSUPPORTED;
  }
  ...
  Status = PmicVersionProtocol->GetPmicInfo(ChgPmicIndex, &PmicDeviceInfo);
  if(EFI_SUCCESS != Status)
  {
