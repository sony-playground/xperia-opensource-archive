    ...
    for (j = 0; j < NumIfc; j++) {
      ...
      for (k = 0; k < NumEp; k++){
        if (((EpDesc->Attributes) & USB_ENDPOINT_TYPE_MASK) == TransferType) {
		  ...
		  if(usb_shared_is_eud_bulk_out_ep_limit_required(EpDesc->EndpointAddress, EpDesc->Attributes, MaxPktSize) == TRUE) {
		    MaxPktSize = 128;
		  }
		  #endif
          EpDesc->MaxPacketSize = MaxPktSize;
		  DBG(EFI_D_INFO, "EndpointAddr 0x%x: PktSize_now %d, EpAtrr = 0x%x",
              EpDesc->EndpointAddress,
              EpDesc->MaxPacketSize,
			  EpDesc->Attributes
              );
...
)
{
  ...
  if (USB_SETUP_RT_RCP(Req->RequestType)  ==  USB_SETUP_RT_RCP_DEV){
    ...
    // Stall U1/U2 when operating in SuperSpeedPlus
    if ((UsbBusSpeedSuper == DevSm->BusSpeed) || (UsbBusSpeedSuperPlus == DevSm->BusSpeed)){
  ...
  *NextState = DEV_STATE_SAME;
  ...
  switch (Req->Request) {
    ...
    case USB_REQ_GET_DESCRIPTOR: {
      ...
      switch (DescType) {
        ...
        case USB_DESC_TYPE_DEVICE:
          if ((DevSm->BusSpeed == UsbBusSpeedSuper) || (DevSm->BusSpeed == UsbBusSpeedSuperPlus)) {
        ...
        case USB_DESC_TYPE_CONFIG:
          if ((DevSm->BusSpeed == UsbBusSpeedSuper) || (DevSm->BusSpeed == UsbBusSpeedSuperPlus)) {
            ...
            //Hardcode the HS usb to draw max power of 100mA. This is the workaround over power error,
            //when EUD is enable as self power hub. (0x32*2mA=100mA)
            ((EFI_USB_CONFIG_DESCRIPTOR *)DevSm->DescriptorSet.Descriptors[0])->MaxPower = 0x32;             
    ...
    case USB_REQ_SET_CONFIG:
    {
      ...
      NumConfig = ((DevSm->BusSpeed == UsbBusSpeedSuper) || (DevSm->BusSpeed == UsbBusSpeedSuperPlus)) ?
  ...
  IN EFI_USBFN_MESSAGE_PAYLOAD  *Payload )
{
  ...
  DBG(EFI_D_ERROR, "Dev Bus Speed: %a, state %d", PrintBusSpeed(DevSm->BusSpeed), DevSm->CurrentState);
  ...
  if (EFI_ERROR(Status) || (DevSm->BusSpeed == UsbBusSpeedUnknown)) {
    DBG(EFI_D_ERROR, "GetEndpointMaxPacketSize failed: %r", Status);
  } else if ((DevSm->BusSpeed != UsbBusSpeedSuper) && (DevSm->BusSpeed != UsbBusSpeedSuperPlus)){
    UpdateEpMaxPktSize(&DevSm->DescriptorSet, USB_ENDPOINT_BULK, MaxPacketSize);
  } else {
  ...
  if(DevSm->PortType == EfiUsbChargingDownstreamPort)
  {
	if ((DevSm->BusSpeed == UsbBusSpeedSuper) || (DevSm->BusSpeed == UsbBusSpeedSuperPlus))
    {
	  usb_shared_dp_toggle(200);
	  DBG(EFI_D_ERROR, "DP toggled for SS CDP");
    }  
