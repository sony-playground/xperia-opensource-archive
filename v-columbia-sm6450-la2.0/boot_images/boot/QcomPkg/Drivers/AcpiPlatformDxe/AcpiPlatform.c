/** @file
  ...
  Copyright (c) 2011-2020, Qualcomm Technologies Inc. All rights
  reserved. 
...
* EFI_STATUS.
*/
EFI_STATUS ValidateElfHeader(Elf32_Ehdr *Ehdr)
{
  ...
  if(NULL == Ehdr)
  {
    ...
    DEBUG((EFI_D_ERROR, "Null elf header was passed\r\n"));
    goto exit;
  }
  /*
  * Validate that the first four bytes in the ELF file are valid magic bytes.
  */
  if (Ehdr->e_ident[EI_MAG0] != ELFMAG0VALUE ||
    Ehdr->e_ident[EI_MAG1] != ELFMAG1VALUE ||
    Ehdr->e_ident[EI_MAG2] != ELFMAG2VALUE ||
    Ehdr->e_ident[EI_MAG3] != ELFMAG3VALUE)
  {

    DEBUG((EFI_D_ERROR, "Invalid ELF header magic bytes detected.\r\n"));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }

  /*
  * Validate that a 32 bit ELF file is present.  64 bit ELF's are currently
  * not supported.
  */
  if (Ehdr->e_ident[EI_CLASS] != ELFCLASS32)
  {
    DEBUG((EFI_D_ERROR, "Error: Invalid ELF class detected (must be 32 bit): %d.\n",
  ...
  /*
  * Validate that the ELF data type value is in a known range.
  */
  if (Ehdr->e_ident[EI_DATA] != ELFDATA2LSB &&
    Ehdr->e_ident[EI_DATA] != ELFDATA2MSB)
  {
    DEBUG((EFI_D_ERROR, "Error: Invalid ELF data encoding detected: %d.\n", Ehdr->e_ident[EI_DATA]));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }

  /*
  * Validate that the current ELF header version is the supported version.
  */
  if (Ehdr->e_ident[EI_VERSION] != EV_CURRENT)
  {
    DEBUG((EFI_D_ERROR, "Error: Invalid ELF file version detected: %d.\n", Ehdr->e_ident[EI_VERSION]));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }
...
/*!
* INTERNAL FUNCTION
* Validate the fields in the program header.
* @param Phdr
* Pointer to the program header.
* @return
* EFI_STATUS.
*/
EFI_STATUS ValidateProgramHeader(Elf32_Phdr *Phdr)
{
  EFI_STATUS Status = EFI_SUCCESS;

  if(NULL == Phdr)
  {
    DEBUG((EFI_D_ERROR, "Null program header was passed.\n"));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }

  /*
  * Verify that the type of the program header is in range.
  */
  if(Phdr->p_type != PT_DYNAMIC &&
    Phdr->p_type != PT_LOAD &&
    Phdr->p_type != PT_NULL)
  {
    DEBUG((EFI_D_ERROR, "Invalid Program Header type was detected: %d.\n", Phdr->p_type));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }
  ...
  if(Phdr->p_type == PT_LOAD)
    {
      ...
      {
        DEBUG((EFI_D_ERROR, "The file size of the program segment (%d) was greater than the size in memory (%d).\n", Phdr->p_filesz, Phdr->p_memsz));
        Status = EFI_INVALID_PARAMETER;
        goto exit;
      }
    }
...
/*!
* INTERNAL FUNCTION
* Validate Elf File's Elf Header and Program Headers.
* @param AcpiElf
* Pointer to the Acpi Elf file.
* @param ElfSize
* size of elf file
* @return
* EFI_STATUS 
*/
EFI_STATUS ValidateElf(void * AcpiElf, UINTN ElfSize)
{
  EFI_STATUS Status = EFI_SUCCESS;

  if((NULL == AcpiElf) || (0x0 == ElfSize ))
  {
    Status = EFI_INVALID_PARAMETER;
    DEBUG((EFI_D_ERROR, "Invalid Elf.\n"));
    goto exit;
  }

  Elf32_Ehdr *Ehdr = AcpiElf;
  VOID * ElfEnd = AcpiElf + ElfSize;
  Elf32_Phdr * pHdrList = AcpiElf + Ehdr->e_phoff;
  UINTN PhdrSize = sizeof(Elf32_Phdr);

  if((void *)pHdrList > ElfEnd)
  {
    Status = EFI_INVALID_PARAMETER;
    DEBUG((EFI_D_ERROR, "Invalid Phdr Offset.\n"));
    goto exit;
  }
  Status = ValidateElfHeader(AcpiElf);
  if(EFI_ERROR(Status))
  {
    Status = EFI_INVALID_PARAMETER;
    DEBUG((EFI_D_ERROR, "Error: Invalid Elf Header.\n"));
    goto exit;
  }

  for(int i = 0; i < Ehdr->e_phnum; i++)
  {
    Status =ValidateProgramHeader((Elf32_Phdr * )(pHdrList+(i*PhdrSize)));
    if(EFI_ERROR(Status))
    {
      Status = EFI_INVALID_PARAMETER;
      DEBUG((EFI_D_ERROR, "Error: Invalid Elf Header.\n"));
...
/*!
* 
* Get ACPI Elf File information required for authentication.
* @param AcpiElf
* Pointer to the elf file.
* @param AcpiInfo
* acpi information required for authentication.
* @param elfsize
* size of elf file
* @return
* EFI_STATUS 
*/
EFI_STATUS GetAcpiInfo(void * AcpiElf, UINTN ElfSize, ACPIImageInfo * AcpiInfo)
{
  EFI_STATUS           Status = EFI_INVALID_PARAMETER;
  Elf32_Phdr         * Phdr = NULL;
  UINT32               HashSegSize = 0x0;
  UINT32               HashSegOffset = 0x0;
  VOID               * HashSegOrgAddr = NULL;
  VOID               * HashSegNewAddr = NULL;
  EFI_PHYSICAL_ADDRESS PhysicalAddr = 0x0;

  if ((NULL == AcpiElf) || (ElfSize == 0x0) || (AcpiInfo == NULL))
  {
    Status = EFI_INVALID_PARAMETER;
    DEBUG((EFI_D_ERROR, "Error: Invalid Args.\n"));
    goto exit;
  }
  ...
  AcpiInfo->TzProcessorId = TZ_PIL_AUTH_ACPI;

  UINTN metadata_size = sizeof(Elf32_Ehdr)+(Ehdr->e_phnum * Ehdr->e_phentsize);

  HashSegNewAddr = AcpiElf + metadata_size;

  if(HashSegNewAddr > ElfEnd)
  {
    Status = EFI_INVALID_PARAMETER;
    DEBUG((EFI_D_ERROR, "Error: Invalid Number of Program Headers.\n"));
    goto exit;
  }
  ...
  for(int i = 0; i < Ehdr->e_phnum; i++)
  {
    ...
    {
      Status = EFI_INVALID_PARAMETER;
      DEBUG((EFI_D_ERROR, "Error: Invalid Program Header.\n"));
      goto exit;
    }

    if(((PT_LOAD == Phdr->p_type) || (Phdr->p_type == PT_NULL)) &&
      PF_MASKOS_IS_SEGMENT_TYPE(Phdr->p_flags, PF_MASKOS_SEGMENT_TYPE_VALUE_HASH_SEGMENT) &&
      (Phdr->p_memsz > 0x0))
    {
      HashSegSize = Phdr->p_memsz;
      HashSegOffset = Phdr->p_offset;
      HashSegOrgAddr = AcpiElf + HashSegOffset;

      if((HashSegSize > ElfSize) || (HashSegOrgAddr > ElfEnd))
      {
        Status = EFI_INVALID_PARAMETER;
        DEBUG((EFI_D_ERROR, "Invalid Hash Segment.\n"));
        goto exit;
      }
    }

    if (Phdr->p_type != PT_LOAD || PF_MASKOS_IS_SEGMENT_TYPE(Phdr->p_flags, PF_MASKOS_SEGMENT_TYPE_VALUE_HASH_SEGMENT) ||
      Phdr->p_memsz == 0)
    {
      continue;
    }
    ...
    if (Phdr->p_type == PT_LOAD)
    {
      VOID * SegmentAddr = AcpiElf + Phdr->p_offset;

      if ((Phdr->p_memsz > ElfSize) || (SegmentAddr > ElfEnd) || (NULL == SegmentAddr))
      {
        Status = EFI_INVALID_PARAMETER;
        DEBUG((EFI_D_ERROR, "Invalid Load Segment.\n"));
        goto exit;
      }

      ACPI_INFO_HEADER * AcpiInfoHeader = (ACPI_INFO_HEADER *)SegmentAddr;

      if (AcpiInfoHeader->signature == ACPI_INFO_HEADER_SIGNATURE)
      {
        PhysicalAddr = ConvertToPhysicalAddress(AcpiElf + Phdr->p_offset);
        AcpiInfo->ImagePhysicalBaseAddress = PhysicalAddr;
        AcpiInfo->ImageMemorySize = Phdr->p_memsz;
        Status = EFI_SUCCESS;
        break;
      }
    }
  ...
  if (0x0 == HashSegSize)
  {
    DEBUG((EFI_D_ERROR, "Error: Hash Segment Not found.\n"));
    Status = EFI_INVALID_PARAMETER;
  }
  else
  {
    CopyMem(HashSegNewAddr, HashSegOrgAddr, HashSegSize);

    PhysicalAddr = ConvertToPhysicalAddress(AcpiElf);
    AcpiInfo->ElfPhysicalAddress = PhysicalAddr;

    // flush cache
    WriteBackInvalidateDataCacheRange((VOID *)AcpiElf, ElfSize);
  }
...
/*!
* 
* Function make SMC call to TZ for specific CommandId.
* @param ImageInfo
* Pointer to Image info structure.
* @param CommandId
* SMC ID.
* @return
* EFI_STATUS 
*/
EFI_STATUS
ACPIAuthTZSysCall(ACPIImageInfo * ImageInfo, UINT32 CommandId)
{
  EFI_STATUS        Status = EFI_SUCCESS;
  ...
  if (ImageInfo == NULL)
  {
    DEBUG((EFI_D_ERROR, " Invalid Image Info \r\n"));
    Status = EFI_INVALID_PARAMETER;
    goto ErrorExit;
  }

  if (TZ_PIL_AUTH_ACPI != ImageInfo->TzProcessorId)
  {
    DEBUG((EFI_D_ERROR, " Invalid TzProcessorId: 0x%x \r\n", ImageInfo->TzProcessorId));
    Status = EFI_INVALID_PARAMETER;
    goto ErrorExit;
  }
  ...
  switch (CommandId)
  {
    case TZ_PIL_INIT_ID:
    {
       tz_pil_init_req_t *SysCallReq = (tz_pil_init_req_t*)Parameters;
       if (NULL == (VOID *)ImageInfo->ElfPhysicalAddress)
       {
         DEBUG((EFI_D_ERROR, " Invalid Image Physical Address \r\n"));
         Status = EFI_INVALID_PARAMETER;
         goto ErrorExit;
       }
       //Adding this check as HYP and TZ expect page alignment.
       if (ALIGN_TZ(ImageInfo->ElfPhysicalAddress, PIL_TZ_ALIGNMENT) != ImageInfo->ElfPhysicalAddress)
       {
         DEBUG((EFI_D_ERROR, " Start Address is not aligned \r\n"));
         Status = EFI_INVALID_PARAMETER;
         goto ErrorExit;
       }
       else
       {
         /* Tree Command ID */
         SmcId = TZ_PIL_INIT_ID;

         /* Tree Parameter ID */
         ParamId = TZ_PIL_INIT_ID_PARAM_ID;

         /* Input parameters to SCM command */
         SysCallReq->proc = ImageInfo->TzProcessorId;
         SysCallReq->image_hdr_ptr = ImageInfo->ElfPhysicalAddress;
       }
       break;
    }

    case TZ_PIL_UNLOCK_XPU_ID:
    {
       tz_pil_unlock_req_t *SysCallReq = (tz_pil_unlock_req_t*)Parameters;

       /* Tree Command ID */
       SmcId = TZ_PIL_UNLOCK_XPU_ID;

       /* Tree Parameter ID */
       ParamId = TZ_PIL_UNLOCK_XPU_ID_PARAM_ID;

       /* Input parameters to SCM command */
       SysCallReq->proc = ImageInfo->TzProcessorId;
       break;
    }
    case TZ_PIL_AUTH_RESET_ID:
    {
       tz_pil_auth_reset_req_t *SysCallReq = (tz_pil_auth_reset_req_t*)Parameters;

       /* Tree Command ID */
       SmcId = TZ_PIL_AUTH_RESET_ID;

       /* Tree Parameter ID */
       ParamId = TZ_PIL_AUTH_RESET_ID_PARAM_ID;
    ...
    case TZ_PIL_MEM_ID:
    {
        tz_pil_mem_req_t *SysCallReq = (tz_pil_mem_req_t*)Parameters;

        if ((NULL == (VOID *)ImageInfo->ImagePhysicalBaseAddress) || (0x0 == ImageInfo->ImageMemorySize))
        {
          DEBUG((EFI_D_ERROR, " Invalid Image Address or size \r\n"));
          goto ErrorExit;
        }

        if((ALIGN_TZ(ImageInfo->ImagePhysicalBaseAddress,PIL_TZ_ALIGNMENT) != ImageInfo->ImagePhysicalBaseAddress) ||
          (ALIGN_TZ(ImageInfo->ImageMemorySize,PIL_TZ_ALIGNMENT) != ImageInfo->ImageMemorySize))
        {
              DEBUG(( EFI_D_ERROR, "Address or Size not page aligned \r\n"));
              Status = EFI_INVALID_PARAMETER;
              goto ErrorExit;
        }
        else
        {
          ...
          /* Tree Parameter ID */
          ParamId = TZ_PIL_MEM_ID_PARAM_ID;

          /* Input parameters to SCM command */
          SysCallReq->proc = ImageInfo->TzProcessorId;
          SysCallReq->start_addr = ImageInfo->ImagePhysicalBaseAddress;
          SysCallReq->image_len = ImageInfo->ImageMemorySize;
        }
        break;
    }
    default:
      DEBUG((EFI_D_ERROR, " Unsupported Command ID (0x%x) received\r\n", CommandId));
      Status = EFI_INVALID_PARAMETER;
      goto ErrorExit;

  }

  if((0x0 == SmcId) || (0x0 == ParamId))
  {
    DEBUG((EFI_D_ERROR, " Unsupported CommandID or ParamID received\r\n"));
    Status = EFI_INVALID_PARAMETER;
    goto ErrorExit;
  }
  ...
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "ScmSysCall() failed, Status = (0x%x)\r\n", Status));
    goto ErrorExit;
  }
...
/*!
* 
* Parse ACPI Tables from elf and store to PreFixArray.
...
*   -----------------------------
* @param AcpiElf
* Pointer acpi elf file.
* @param ElfSize
* size of elf file.
* @return
* EFI_STATUS 
*/
EFI_STATUS ParseAcpiTables(void * AcpiElf, UINTN ElfSize)
{
  Elf32_Phdr *PhdrAcpi = NULL;
  ...
  Elf32_Ehdr *Ehdr = (Elf32_Ehdr *)AcpiElf;
  Elf32_Phdr *Phdr = (Elf32_Phdr *)(AcpiElf + Ehdr->e_phoff);
  VOID *     ElfEnd = AcpiElf + ElfSize;

  if(NULL == Phdr)
  {
    DEBUG((EFI_D_ERROR, "Invalid Program Header \n"));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }
  ...
  for(int i = 0; i < Ehdr->e_phnum; i++)
  {
    ...
    if(ElfEnd < (VOID *)PhdrAcpi)
    {
      DEBUG((EFI_D_ERROR, "Phdr address falls beyond the end of the elf file \n"));
      Status = EFI_INVALID_PARAMETER;
      goto exit;
    }

    if(PhdrAcpi->p_type == PT_LOAD)
    {
      AcpiInfoHeader = AcpiElf + PhdrAcpi->p_offset;
      AcpiTableInfoList = (ACPI_TABLE_INFO *)((VOID *)AcpiInfoHeader + sizeof(ACPI_INFO_HEADER));
      break;
    }
  }

  if((NULL == AcpiTableInfoList) || (NULL == AcpiInfoHeader) || ((VOID *)AcpiTableInfoList > ElfEnd))
  {
  ...
  for(int j = 0; j < AcpiInfoHeader->number_of_tables; j++)
  {
    ...
    if((AcpiTableBuffer > ElfEnd) || (NULL == AcpiTableBuffer))
    {
...
/*!
* 
* Authenticate ACPI elf file and Parse ACPI files from elf.
* @param AcpiElf
* Pointer acpi elf file.
* @param ElfSize
* size of elf file.
...
* EFI_STATUS 
*/
EFI_STATUS AuthenticateAndParseAcpi(VOID * AcpiElf, UINTN ElfSize)
{

  EFI_STATUS Status = EFI_SUCCESS;
  ACPIImageInfo  AcpiInfo;

  if((NULL == AcpiElf) || (0x0 == ElfSize))
  {
    Status = EFI_INVALID_PARAMETER;
    DEBUG((EFI_D_ERROR, "Invalid Parameters !!\r\n"));
    goto AuthenticateAndParseAcpiExit;
  }

  Status = ValidateElf(AcpiElf, ElfSize);
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "Elf Validation Failed = (0x%x)\r\n", Status));
    goto AuthenticateAndParseAcpiExit;
  }
  
  Status = GetAcpiInfo(AcpiElf, ElfSize, &AcpiInfo);

  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "GetAcpiInfo failed, Status = (0x%x)\r\n", Status));
    goto AuthenticateAndParseAcpiExit;
  }

  Status = ACPIAuthTZSysCall(&AcpiInfo, TZ_PIL_INIT_ID);
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "TZ_PIL_INIT_ID TZ call Failed, Status = (0x%x)\r\n", Status));
    goto AuthenticateAndParseAcpiExit;
  }
  DEBUG((EFI_D_INFO, "TZ_PIL_INIT_ID TZ call Pass, Status = (0x%x)\r\n", Status));
  Status = ACPIAuthTZSysCall(&AcpiInfo, TZ_PIL_MEM_ID);

  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "TZ_PIL_MEM_ID TZ call Failed, Status = (0x%x)\r\n", Status));
    goto AuthenticateAndParseAcpiExit;
  }
  DEBUG((EFI_D_INFO, "TZ_PIL_MEM_ID TZ call Pass, Status = (0x%x)\r\n", Status));
  Status = ACPIAuthTZSysCall(&AcpiInfo, TZ_PIL_AUTH_RESET_ID);

  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "TZ_PIL_AUTH_RESET_ID TZ call Failed, Status = (0x%x)\r\n", Status));
    goto AuthenticateAndParseAcpiExit;
  }
  DEBUG((EFI_D_INFO, "TZ_PIL_AUTH_RESET_ID TZ call Pass, Status = (0x%x)\r\n", Status));

  Status = ParseAcpiTables(AcpiElf, ElfSize);
  if(EFI_ERROR(Status))
  {
    DEBUG((EFI_D_ERROR, "Acpi Parsing Failed, Status = (0x%x)\r\n", Status));
    goto AuthenticateAndParseAcpiExit;
  }
  DEBUG((EFI_D_INFO, "Acpi Parsing Pass, Status = (0x%x)\r\n", Status));

AuthenticateAndParseAcpiExit:
...
)
{
  ...
  UINT32                            Attrib = 0;
  UINTN                             Pages = 0;
  CONST CHAR8                      *AcpiFileName = ACPI_ELF_NAME;
  ...
  while (1) {
    ...
    // if acpi.elf is found , we authenticate and process the elf and store ACPI tables 
    // present in acpi.elf to PreFixArray.
    // If acpi.elf not present, we store individual ACPI tables to PreFixArray.

    if(!AsciiStrCmp(AcpiFileName,pFileName))
    {

      Status = AuthenticateAndParseAcpi(AcpiFileBuffer, AcpiFileBufferSize);
      if (EFI_ERROR (Status)) 
      {
        DEBUG(( EFI_D_WARN, "Acpi Authentication Failed!!. Status: %d.\r\n",
          Status));
        goto Done;
      }
    }
    else if(EnableACPIFallback)
    {
        // Add the table
    ...
    )
{
    ...
    {
        Status = EFI_NOT_FOUND;
        DEBUG((EFI_D_ERROR, "ACPIPlatformDXE : UpdateSTA: Unexpected data found\n"));
        goto Error;
    }
    ...
    for ( i = 0; i < NumDevices; i++)
    {
            ...
            DeviceNameLengthList[i] > ACPI_MAX_DEVICE_NAME_SIZE)
        {
            DEBUG(( EFI_D_WARN, 
               ...
               "DeviceNameList[%d] or DeviceNameLengthList[%d] is more than "
               "MAX_SIZE(%d)\r\n", i, i, ACPI_MAX_DEVICE_NAME_SIZE));
            return EFI_INVALID_PARAMETER;
        }
