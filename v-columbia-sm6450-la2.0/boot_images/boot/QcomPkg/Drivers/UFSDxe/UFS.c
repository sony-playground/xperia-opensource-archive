...
2022/07/25      cm    Update structures to get devinfo
2022-07-27      vm    Correct RPMB partition size
2022-04-14      rn    Removed debug logs
2022-03-21      cm    Add API to return turbo mode support info
2022-03-07      ssb   Added Check based conversion of unicode to ASCII 
...
static const EFI_STORAGE_DEV_MGMT_PROTOCOL gUfsDeviceMgmtProtocol = {
   EFI_STORAGE_DEV_MGMT_PROTOCOL_REVISION3,   /* Revision */
...
{
/* This Function is NA for UFS, but required to make changes inline for Quest. */
	return EFI_UNSUPPORTED; 
}
/** 
...
VOID EFIAPI UFSDxeExitBSCallback (
            IN EFI_EVENT        Event,
            IN VOID             *Context)
{
   INT32 rc = 0;
   rc = ufs_set_device_mode((struct ufs_handle *)Context, UFS_POWER_DOWN);
   if (UFS_EOK != rc) {
...
// TODO: Remove Timer Profiling Code
   ...
   if (0 != AsciiStrCmp("WP", OsTypeString))
   {
      Status = gBS->CreateEventEx (
                       ...
                       (VOID *)gUfsWlunDevice.DeviceHandle,
                       &gEfiEventExitBootServicesGuid,
                       &ExitBootServicesEvent
                       );
      if (EFI_ERROR (Status))
      {
         DEBUG ((EFI_D_ERROR, "UFS Exit Boot Service callback registration failed, Status = 0x%lx\n", Status));
         return Status;
      }
   }
            ...
            IN VOID                     *CryptoIvInput)
{
       ...
       gUfsTransferInfo.TransferType == BLOCKIO_WRITE)
   {
      ...
      if (pReqList->CurReq->IsStart) {
         Status = BlkIoTakeOverTransfer(pThis, MediaId, Lba, BufferSize, Buffer, CryptoIndex, CryptoIvInput);
      }
   ...
   else
   {
      ...
      // Validate passed-in pointers are not NULL
      if ((NULL == Buffer) || (NULL == pThis) || (NULL == Token)) {
      ...
      pReqList->LastReq->Index = CryptoIndex; 
      pReqList->LastReq->CryptoIvInput = CryptoIvInput;
            ...
            OUT VOID                  *Buffer)
{
   ...
   Status = UFSIoWrapper (This, MediaId, Lba, NULL, BufferSize, Buffer, NULL, NULL);
   ReleaseLock (UfsLockPtr);
            ...
            IN VOID                          *CryptoIvInput)
{
   ...
   Status = UFSIoWrapper (This, MediaId, Lba, Token, BufferSize, Buffer, Index, CryptoIvInput);
   ReleaseLock (UfsLockPtr);
            ...
            OUT VOID                      *Buffer)
{
   ...
   gUfsTransferInfo.TransferType = BLOCKIO2_READ;
   Status = UFSIoWrapper(This, MediaId, Lba, Token, BufferSize, Buffer, NULL, NULL);
