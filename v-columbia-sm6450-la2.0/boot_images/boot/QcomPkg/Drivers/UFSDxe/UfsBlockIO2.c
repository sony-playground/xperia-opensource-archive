...
static VOID BlkIo2CleanUp()
{
   ...
   if (gUfsTransferInfo.EventPending == TRUE)
   {
      ...
      if (gUfsTransferInfo.CryptoEnabledEventPending)
      {
         gUfsTransferInfo.CryptoEnabledEventPending = FALSE; 
         gBS->SignalEvent(((EFI_BLOCK_IO_CRYPTO_TOKEN *)gUfsTransferInfo.Token)->Event);
      }
      else
      {
         gBS->SignalEvent(((EFI_BLOCK_IO2_TOKEN *)gUfsTransferInfo.Token)->Event);
      }
   }
} 
   ...
   )
{
   ...
   {
      if ((pReqList->CurReq->Index != NULL) || (pReqList->CurReq->CryptoIvInput != NULL)) {
         EFI_BLOCK_IO_CRYPTO_TOKEN *Token = pReqList->CurReq->Token;
         Token->TransactionStatus = status;
         gBS->SignalEvent(Token->Event);
      }
      else {
         EFI_BLOCK_IO2_TOKEN *Token = pReqList->CurReq->Token;
         Token->TransactionStatus = status;
         gBS->SignalEvent(Token->Event);
      }
                        ...
                        IN VOID                    *CryptoIvInput)
{
   ...
   {  
      ...
      if (rc == UFS_EOK) {
         ...
         {
            CurBuffer = (UINT8 *)((UINT8 *)pReqList->CurReq->Buffer - pReqList->CurReq->CurTxSizeInBytes); 
            InvalidateDataCacheRange((VOID *)CurBuffer, pReqList->CurReq->CurTxSizeInBytes);
         }
         break; 
      }
   ...
   { 
      ...
      if ((NULL != pReqList->CurReq->Index) && (NULL != pReqList->CurReq->CryptoIvInput)) 
      {
         ((EFI_BLOCK_IO_CRYPTO_TOKEN *)pReqList->CurReq->Token)->TransactionStatus = EFI_DEVICE_ERROR;
         gUfsTransferInfo.CryptoEnabledEventPending = TRUE;
      }
      else
      {
         ((EFI_BLOCK_IO2_TOKEN *)pReqList->CurReq->Token)->TransactionStatus = EFI_DEVICE_ERROR;
      }
   ...
   if (gUfsTransferInfo.TransferType == BLOCKIO_READ)
      status = UFSReadBlocks (This, MediaId, Lba, BufferSize, Buffer, CryptoIndex, CryptoIvInput);
   else 
      status = UFSWriteBlocks (This, MediaId, Lba, BufferSize, Buffer, CryptoIndex, CryptoIvInput);
   ...
   if (status != EFI_SUCCESS)
   {
      ...
      if (pReqList->CurReq->BufferSize == 0)
      {
         if ((NULL != pReqList->CurReq->Index) && (NULL != pReqList->CurReq->CryptoIvInput)) 
         {
            ((EFI_BLOCK_IO_CRYPTO_TOKEN *)pReqList->CurReq->Token)->TransactionStatus = EFI_SUCCESS;
         }
         else
         {
            ((EFI_BLOCK_IO2_TOKEN *)pReqList->CurReq->Token)->TransactionStatus = EFI_SUCCESS;
         }
      }
...
{ 
   ...
   struct ufs_xfr_context xfr_ctxt; 
   UINT64       *Index = NULL; 
   ...
   Index = pReqList->CurReq->Index;
   CryptoIvInput = pReqList->CurReq->CryptoIvInput;

   if ((Index != NULL) && (CryptoIvInput != NULL)) {
      struct ufs_crypto_context crypto; 
      UfsDevice = UFS_DEV_FROM_BLOCKIO_CRYPTO (pReqList->CurReq->This); 
   ...
   pReqList->CurReq->CurTxSizeInBytes = (transfer_size * block_size); 
      
   if (pReqList->CurReq->Type == BLOCKIO2_READ) {
      rc = ufs_read_ex (hUFS, temp_buffer, Lba, transfer_size, &xfr_ctxt, crypto_ctxt);
   }
   else if (pReqList->CurReq->Type == BLOCKIO2_WRITE) {
      rc = ufs_write_ex (hUFS, temp_buffer, Lba, transfer_size, &xfr_ctxt, crypto_ctxt);
   }
   else {
...
{  
   ...
   if (rc == -UFS_ENOENT) {
      ...
      if (crypto_enabled) {
         ((EFI_BLOCK_IO_CRYPTO_TOKEN *)temp_token)->TransactionStatus = EFI_SUCCESS;
         gBS->SignalEvent(((EFI_BLOCK_IO_CRYPTO_TOKEN *)temp_token)->Event);
      }
      else
      {
         ((EFI_BLOCK_IO2_TOKEN *)temp_token)->TransactionStatus = EFI_SUCCESS;
         gBS->SignalEvent(((EFI_BLOCK_IO2_TOKEN *)temp_token)->Event);
      } 
   }
   else {
