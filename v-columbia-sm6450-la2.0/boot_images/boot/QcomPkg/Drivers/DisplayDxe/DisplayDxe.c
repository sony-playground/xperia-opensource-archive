  ...
  )
{
  ...
  else 
  {
    ...
    uFrameBufferLength                  = (UINTN)pMode->HorizontalResolution * pMode->VerticalResolution * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
    ...
    for (eDisplayId = MDP_DISPLAY_PRIMARY; eDisplayId <= MDP_DISPLAY_EXTERNAL; eDisplayId++)
    {
      ...
      else
      {
        ...
        aFrameBuffers[uNumDisplays].uFrameHeight = pDisplayMode->VerticalResolution;
        aFrameBuffers[uNumDisplays].uStride      = pDisplayMode->HorizontalResolution * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
        uNumDisplays ++;
      }
    }
    ...
    for (eDisplayId = MDP_DISPLAY_PRIMARY; eDisplayId <= MDP_DISPLAY_EXTERNAL; eDisplayId++)
    {
      ...
      {
        EFI_GRAPHICS_OUTPUT_MODE_INFORMATION *pDisplayMode = &gDxeModeInfo.aDxeModeList[eDisplayId][ModeNumber];
        MDP_Display_IDType                    eBufferId    = (eDisplayId < FRAMEBUFFER_MAX_NUMBER) ? eDisplayId : MDP_DISPLAY_PRIMARY;
        ...
        sModeParams.sSurfaceInfo.pPlane0Offset = aFrameBuffers[eBufferId].pFrameBufferStart;
        sModeParams.sSurfaceInfo.uPlane0Stride = pDisplayMode->HorizontalResolution * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
  ...
  IN  UINTN                                   Delta)
{
    ...
    else
    {
      ...
      if (EFI_SUCCESS != GetFrameBufferInfo(eDisplayId, &sFrameBuffer))
      {
        ...
        sFrameBuffer.uFrameHeight      = gDxeModeInfo.sCurrentModeInfo.VerticalResolution;
        sFrameBuffer.uStride           = gDxeModeInfo.sCurrentModeInfo.HorizontalResolution * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
      }
...
static VOID CalculateFrameBufferOffsets(UINT64 uFrameBufferBase, UINT64 uFrameBufferSize, FrameBufferOffsetType *pFrameBuffer, UINT32 uNumBuffers)
{
  ...
  else if (1 == uNumBuffers)
  {
    ...
    pFrameBuffer[0].uFrameBufferLength = (UINT64)pFrameBuffer[0].uFrameWidth * pFrameBuffer[0].uFrameHeight * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
  }
  else
  {
    ...
    {
      pFrameBuffer[uBufferIndex].uFrameBufferLength  = ((pFrameBuffer[uBufferIndex].uFrameWidth) * (pFrameBuffer[uBufferIndex].uFrameHeight)) * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
    ...
    else
    {
      for(uBufferIndex = 0; uBufferIndex < uNumBuffers-1; uBufferIndex++)
      {
        // Calculate unaligned start address for uBufferIndex + 1 buffer based on uBufferIndex buffer start address and its buffer length.
        uUnalignedFrameBufferBase                        = (UINT64)pFrameBuffer[uBufferIndex].pFrameBufferStart +
                                                           pFrameBuffer[uBufferIndex].uFrameWidth * pFrameBuffer[uBufferIndex].uFrameHeight * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
        // Align start address for uBufferIndex + 1 buffer.
        pFrameBuffer[uBufferIndex + 1].pFrameBufferStart = (VOID *) (MDP_DISPLAY_ALIGN(uUnalignedFrameBufferBase, MDP_ALIGN_256));
        ...
        if ((uNumBuffers - 2) == uBufferIndex)
        {
          pFrameBuffer[uNumBuffers - 1].uFrameBufferLength   = pFrameBuffer[uNumBuffers - 1].uFrameWidth * pFrameBuffer[uNumBuffers - 1].uFrameHeight * DISPLAYDXE_DEFAULT_BYTES_PER_PIXEL;
