/*===============================================================================
  ...
  --------   ---         --------------------------------------------------------
  05/05/23   pk         Checking Phdr against metabuffersize inner bound and OOB read  
  02/09/23   ac         Assigning MetaData Size and check against OOB read
  08/10/21   var        Added error logging support for PIL authentication
...
PIL_STATUS PilValidateHdr(PIL_SUBSYS_CTX_TYPE* Ctx, UINT8* fwbuf) 
{
   ...
   // to make sure starting address of Phdr is not greater than metabuffer end
    ASSERT(Phdr < (Elf32_Phdr*)((uint8*)fwbuf + Priv->MdtBufSize));
    ASSERT_FAIL_PIL_RETURN(Phdr < (Elf32_Phdr*)((uint8*)fwbuf + Priv->MdtBufSize), PIL_INVALID_PARAM, PIL_INVALID_PARAM);		
	
    // to make sure starting address of Phdr is not lesser than metabuffer start
    ASSERT(Phdr > (Elf32_Phdr*)((uint8*)fwbuf));
    ASSERT_FAIL_PIL_RETURN(Phdr > (Elf32_Phdr*)((uint8*)fwbuf), PIL_INVALID_PARAM, PIL_INVALID_PARAM);

    // to make sure end address of Phdr is not greater than metabuffer end        
    ASSERT((Elf32_Phdr*)((uint8*)Phdr+(Ehdr->e_phnum * sizeof(Elf32_Phdr))) < (Elf32_Phdr*)((uint8*)fwbuf + Priv->MdtBufSize));
    ASSERT_FAIL_PIL_RETURN((Elf32_Phdr*)((uint8*)Phdr+(Ehdr->e_phnum * sizeof(Elf32_Phdr))) < (Elf32_Phdr*)((uint8*)fwbuf + Priv->MdtBufSize), PIL_INVALID_PARAM, PIL_INVALID_PARAM);
    
    // to make sure end address of Phdr is not lesser than metabuffer start
    ASSERT((Elf32_Phdr*)((uint8*)Phdr+(Ehdr->e_phnum * sizeof(Elf32_Phdr))) > (Elf32_Phdr*)((uint8*)fwbuf));
    ASSERT_FAIL_PIL_RETURN((Elf32_Phdr*)((uint8*)Phdr+(Ehdr->e_phnum * sizeof(Elf32_Phdr))) > (Elf32_Phdr*)((uint8*)fwbuf), PIL_INVALID_PARAM, PIL_INVALID_PARAM);
