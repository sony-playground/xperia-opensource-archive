/*===============================================================================
  ...
  --------   ---         --------------------------------------------------------
  02/09/23   ac         Added MetaData size to avoid OOB read
  09/10/20   yw         add func to return cfg ptr
...
typedef struct _PIL_SUBSYS_PRIV_TYPE_
{
    ...
    UINT8*    MdtBuf;                     /* buffer of meta data for PIL use */
    UINT32    MdtBufSize;                 /*Size of Meta Data Buffer in bytes for PIL use*/
    UINT8*    UncachedMdtBuf;              /* aligned & uncached metadata buffer used by hyp/tz */
...
PIL_STATUS GetLoadedSubsysCfgById(UINT32, PIL_SUBSYS_CFG_TYPE**);
