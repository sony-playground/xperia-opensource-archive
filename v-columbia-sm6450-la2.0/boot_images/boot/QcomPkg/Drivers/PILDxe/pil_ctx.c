/*===============================================================================
  ...
  --------   ---         --------------------------------------------------------
  02/09/23   ac         Updating MetaData size to 0 after freeing MetaData buffer
  09/10/20   yw         add func to return subsystem cfg ptr
...
/**
  ...
  @retval PIL_SUBSYS_EXIST                ctx exists and subsys is in shutdown status
...
/* 
    search for cfg from ctx list with given subsysid, 
    assuming there could be multiple cfg with same subsysid 
    in the cfg list, but only one in ctx list

  @param[in] SubsysId       subsystem ID to look for
  @param[out]CfgPtr         ptr to return the subsystem cfg  
  
  @retval PIL_SUCCESS       success
  @retval other             failurel
*/
PIL_STATUS GetLoadedSubsysCfgById(UINT32 SubsysId, PIL_SUBSYS_CFG_TYPE** CfgPtr)
{
    UINT32 Idx;
    UINT32 FoundNum=0;
    PIL_SUBSYS_CFG_TYPE* TmpCfg;
    
    if (CfgPtr==NULL)
        return PIL_INVALID_PARAM;
    
    for (Idx=0; Idx<PIL_SUBSYS_MAX_NUM; Idx++) 
    {
        TmpCfg = PilSubsysCtxList[Idx].Cfg;
        if (TmpCfg!=NULL && TmpCfg->SubsysID == SubsysId) {
            FoundNum++;
            *CfgPtr = TmpCfg;
        }
    }
    ...
    return (FoundNum==0)?PIL_SUBSYS_NOT_FOUND:PIL_SUCCESS;
}
