...
#define CLIENT_RID_MASK             0x0000FFFF
...
extern HAL_IOMMUResult_t arch_mmu_get_aspace_ttbr(void *p_aspace, uint64_t **pt_ttbaseaddr);
...
void hal_iommu_invalidate_tlb_all(struct hal_iommu_list_obj *p_hal_iommu_obj)
{
    if(p_hal_iommu_obj->arch == HAL_IOMMU_ARCH_MMU500 || p_hal_iommu_obj->arch == HAL_IOMMU_ARCH_QSMMU) {
        HAL_IOMMU_InvalidateCBTLBAll((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx);
    } else {
...
void hal_iommu_invalidate_tlb_va_list(struct hal_iommu_list_obj *p_hal_iommu_obj, uint32_t number, uint64_t *va_list)
{
    ...
    for(i = 0; i < number; i++)
    {
        HAL_IOMMU_InvalidateCBTLBVA((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, va_list[i]);
    }
...
static HAL_IOMMUResult_t program_smr_s2cr_to_cb(struct hal_iommu_list_obj *p_hal_iommu_obj, uint32_t stream_id)
{
    ...
    HAL_IOMMU_GetSecureIDR0((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, &idr0);
    
    for(i = 0; i < idr0.formatted.NUMSMRG - 1; i++) {
        HAL_IOMMU_GetSMRn((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, i, &smr);
        
        if(smr.formatted.smr_type.type_exid0.VALID)
            continue;
        
        smr.raw = stream_id | HAL_IOMMU_VALID_MASK;
        HAL_IOMMU_SetSMRn_raw((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, i, smr);
        
        s2cr.raw = 0;
        s2cr.formatted.TYPE = HAL_IOMMU_S2CR_TYPE_TRANSLATION_CONTEXT;
        s2cr.formatted.FIELD = p_hal_iommu_obj->cb_idx;
        HAL_IOMMU_SetS2CRn((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, i, s2cr);
...
static void get_free_cb(void *base_addr, enum hal_iommu_vmsa pt_type, uint32_t *cb_idx)
{
    ...
    // read NUMCB from IDR
    HAL_IOMMU_GetSecureIDR1((uint32_t)(uintptr_t)base_addr, &idr1);
    
    for(i = 0; i < idr1.formatted.NUMCB; i++) {
        // get CBARi
        HAL_IOMMU_GetCBARn((uint32_t)(uintptr_t)base_addr, i, &cbar);
        
        // check if CBAR is used or in POR state
        if( cbar.formatted.cbar_type.type_s1.TYPE == 0 || cbar.formatted.cbar_type.type_s1.TYPE == 2 ||  
            ...
            cbar.formatted.cbar_type.type_s1.MEMATTR = 0xF;
            HAL_IOMMU_SetCBARn((uint32_t)(uintptr_t)base_addr, i, cbar);
            HAL_IOMMU_GetCBA2Rn((uint32_t)(uintptr_t)base_addr, i, &cba2r);
            cba2r.formatted.VA64 = (pt_type == HAL_IOMMU_ARM_ARCH_AARCH64) ? 1 : 0;
            HAL_IOMMU_SetCBA2Rn((uint32_t)(uintptr_t)base_addr, i, cba2r);
            *cb_idx = i;
            return;
        }
    }
...
static void release_cb_idx(struct hal_iommu_list_obj *p_hal_iommu_obj)
{
    ...
    HAL_IOMMU_SetCBARn((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, cbar);
    ...
    while(curr_iommu != NULL) {
        if(curr_iommu->dev_list_head && (curr_iommu->cb_idx != HAL_IOMMU_INVALID_CB_IDX)) {
            ...
            if(p_domain_local->pt_type == HAL_IOMMU_ARM_ARCH_AARCH64) {
                ...
                tcr2.raw = p_domain_local->config.domain_cfg64.TCR2;
                HAL_IOMMU_SetSCTLR((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, sctlr);
                HAL_IOMMU_SetTTBR0((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, p_domain_local->config.domain_cfg64.TTBR0);
                HAL_IOMMU_SetTTBR1((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, p_domain_local->config.domain_cfg64.TTBR1);
                HAL_IOMMU_SetMAIR0((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, p_domain_local->config.domain_cfg64.MAIR0);
                HAL_IOMMU_SetMAIR1((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, p_domain_local->config.domain_cfg64.MAIR1);
                HAL_IOMMU_SetTTBCR((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, tcr);    
                HAL_IOMMU_SetTCR2((uint32_t)(uintptr_t)curr_iommu->base_addr, curr_iommu->cb_idx, tcr2);
            } else {
...
HAL_IOMMUResult_t hal_iommu_domain_attach_device(void *p_domain, void *client_handle, uint32_t arid, uint32_t mask)
{
...
#if 0 // CCA setting in IORT has to be hard coded and really this is an implementation decision by the device drivers    
    ...
    for(i = 0; i < named_component_info.NumIDMappings; i++) {
        if((p_id_mappings[i].InputBase & CLIENT_RID_MASK) == (arid & CLIENT_RID_MASK)) {
    ...
    if(p_hal_iommu_obj->cb_idx == HAL_IOMMU_INVALID_CB_IDX) {
        ...
        if(p_domain_local->pt_type == HAL_IOMMU_ARM_ARCH_AARCH64) {
            ...
            tcr.raw = p_domain_local->config.domain_cfg64.TCR;
            tcr2.raw = p_domain_local->config.domain_cfg64.TCR2;
            HAL_IOMMU_SetSCTLR((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, sctlr);
            HAL_IOMMU_SetTTBR0((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, p_domain_local->config.domain_cfg64.TTBR0);
            HAL_IOMMU_SetTTBR1((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, p_domain_local->config.domain_cfg64.TTBR1);
            HAL_IOMMU_SetMAIR0((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, p_domain_local->config.domain_cfg64.MAIR0);
            HAL_IOMMU_SetMAIR1((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, p_domain_local->config.domain_cfg64.MAIR1);
            HAL_IOMMU_SetTTBCR((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, tcr);
            HAL_IOMMU_SetTCR2((uint32_t)(uintptr_t)p_hal_iommu_obj->base_addr, p_hal_iommu_obj->cb_idx, tcr2);
        } else {
            // No support for AARCH32 at this time
            FreePool(p_id_mappings);
...
/** 
 * Function: hal_iommu_domain_update_aspace
 *
 * Description:
 *
 * Input:
 ...
 * Returns:
 */
HAL_IOMMUResult_t hal_iommu_domain_update_aspace(struct hal_iommu_domain_config *p_domain_cfg, void *p_aspace)
{    
    struct arch_aspace *p_aspace_local = (struct arch_aspace *)p_aspace;
    //HAL_IOMMU_CBn_SCTLR_t sctlr;

    /* Input Validation */
    if(!p_domain_cfg || !p_aspace) {
        return HAL_IOMMU_ERR_NULL_POINTER;
    }
    
    /* Populate domain_config members */
    if(p_domain_cfg->vmsa == HAL_IOMMU_ARM_ARCH_AARCH64) {
        p_domain_cfg->config.domain_cfg64.TTBR0 = (uint64_t)p_aspace_local->tt_virt;
        p_domain_cfg->config.domain_cfg64.TTBR1 = 0x0; //Right now we are not using TTBR1. We can extend this on need basis
        p_domain_cfg->config.domain_cfg64.MAIR0 = (uint32_t) MMU_MAIR_VAL;
        p_domain_cfg->config.domain_cfg64.MAIR1 = (uint32_t) (MMU_MAIR_VAL>>32);
        ...
        [    0]   M                                        =  1
        */
        p_domain_cfg->config.domain_cfg64.SCTLR =  0x00C000E7;

        // set up TCR, preferbly use MACROs . 
        //But need to clean the KERNELBASE ADDR stuff for that
        //TODO
        p_domain_cfg->config.domain_cfg64.TCR = 0x00802010; 
        ...
        BITS
        [   30]   NSCFG1                                   = 0
        [17:15]   SEP                                      =  7 //Leave it to upstream bus
        [   14]   NSCFG0                                   =  1
        [    6]   TBI1                                     =  1
        [    5]   TBI0                                     =  1
        [    4]   AS                                       =  1
        [ 2: 0]   PASIZE                                   =  1
        */
        p_domain_cfg->config.domain_cfg64.TCR2 = 0x0003C071 ; 
    }
    else {
...
HAL_IOMMUResult_t hal_iommu_domain_configure(void *p_domain, struct hal_iommu_domain_config *p_domain_cfg)
{
    ...
    if(p_domain_cfg->vmsa == HAL_IOMMU_ARM_ARCH_AARCH64) {
        ...
        p_domain_local->config.domain_cfg64.TCR2 = p_domain_cfg->config.domain_cfg64.TCR2;
    }
    else {
        // AARCH32 not supported at this time
        return HAL_IOMMU_ERR_UNSUPPORTED_INSTANCE_FOR_TARGET;
