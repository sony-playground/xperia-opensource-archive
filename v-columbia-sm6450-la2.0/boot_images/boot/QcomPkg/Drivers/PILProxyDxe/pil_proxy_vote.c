...
{
	PIL_PROXY_VOTE_CFG_TYPE* pProxyVoteCfg = NULL;
	UINT32 ConfigIndx = 0;
	
	pProxyVoteCfg = LoadPilProxyVoteConfigFromDT(SubsysId);
	if (pProxyVoteCfg == NULL)
	{
		DEBUG(( EFI_D_ERROR, "PilProxyDxe dt is not loaded successfully\n"));
		return EFI_UNSUPPORTED;
	}
	else
	{
