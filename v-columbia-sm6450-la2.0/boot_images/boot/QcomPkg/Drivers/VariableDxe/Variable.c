    ...
    )
{
  ...
  *RootPartition = NULL;
  Status = EFI_UNSUPPORTED;
  DEBUG((EFI_D_ERROR, "%a(%d): QseeStorId=%d\n", __FILE__, __LINE__,QseeStorId));
  ...
  switch (QseeStorId)
  {
    ...
    case QSEE_STOR_SPINOR_ALL:
      ...
      DEBUG ((EFI_D_ERROR, "%a(%d): deferring varstore to secure world\n", __FILE__, __LINE__));
      Status = EFI_SUCCESS;
      break;
    ...
    case QSEE_STOR_EMMC_USER:
      ...
      {
	DEBUG ((EFI_D_ERROR, "%a(%d):boot gEfiUfsLU0Guid \n", __FILE__, __LINE__));
    ...
    case QSEE_STOR_EMMC_GPP1:
      ...
      {
	 DEBUG ((EFI_D_ERROR, "%a(%d):boot gEfiUfsLU4Guid\n", __FILE__, __LINE__));
#if 1 /* SOMC device */
        *RootPartition = &gEfiUfsLU0Guid;
#else
...
SanitizeVarStores (VOID)
{
  ...
  gVarStoreQseeStorId = QSEE_STOR_EMMC_USER;
