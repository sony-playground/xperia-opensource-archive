...
)
{
   ...
   UINT32 ChargerPmic = 0;

   err_flag = pm_chg_get_primary_charger_pmic_index(&ChargerPmic);

   if((err_flag == PM_ERR_FLAG_SUCCESS) && (NULL != ChargerPmicIndex))
   {
      *ChargerPmicIndex = ChargerPmic;
   }
   else
   {
...
EFI_QCOM_PMIC_USB_PROTOCOL PmicUsbProtocolImplementation = 
{
  ...
  .GetChargerPmicIndex    = EFI_PmicGetChargerPmicIndex
};
