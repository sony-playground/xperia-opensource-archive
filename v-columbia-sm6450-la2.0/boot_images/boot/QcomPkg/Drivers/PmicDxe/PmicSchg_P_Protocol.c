...
)
{
  ...
  errFlag = pm_schg_usb_irq_status(PmicDeviceIndex, PM_SCHG_USB_IRQ_USBIN_PLUGIN, PM_IRQ_STATUS_RT, Valid);
...
)
{
  ...
  errFlag = pm_schg_chgr_get_chgr_status(PmicDeviceIndex, &chargerStatus);
  if(errFlag == PM_ERR_FLAG_SUCCESS)
  {
    if(chargerStatus.status_5.valid_input_power_source)
    {
...
)
{
  ...
  *PortType = EFI_PM_SCHG_CHG_PORT_INVALID;
  do
  {
    ...
    {
      errFlag =  pm_schg_usb_get_apsd_result_status(PmicDeviceIndex, (pm_chg_charger_port_type*) PortType);
      if(PM_ERR_FLAG_SUCCESS != errFlag)
      {
        break;
      }
...
)
{
 ...
 //
 //err_flag = pm_scpq_misc_get_config_wdog(PmicDeviceIndex, &wdog_cfg);
 //
 ...
 //
 //err_flag |= pm_scpq_misc_config_wdog(PmicDeviceIndex, wdog_cfg);
...
)
{
  ...
  //err_flag |= pm_scpq_misc_bark_bite_wdog_pet(PmicDeviceIndex);
...
)
{
  ...
  if(errFlag == PM_ERR_FLAG_SUCCESS)
  {
    ...
    pChargingStatus->bChargingEnabled = chargerStatus.status_5.charging_enable;  
    return EFI_SUCCESS;
  }
...
{
  pm_chg_get_primary_charger_pmic_index(&(SchgPmicInfo->PmicIndex));
  SchgPmicInfo->SlaveIndex = SchgPmicInfo->PmicIndex;
  return EFI_SUCCESS;
}
...
)
{
  ...
  else
  {
    *active_pmic_index = 0;
    ...
    {
      errFlag = pm_schg_chgr_get_schg_pmic_info(gSchg_P_PmicIndex, gSchg_P_SlaveIndex, &gCharger_P_Count, MAX_CHARGING_PORT);
      if (PM_ERR_FLAG_SUCCESS != errFlag)
      {
        return EFI_DEVICE_ERROR;
      }
...
EFI_STATUS
EFIAPI
EFI_PmicSchg_P_GetUsbConnectorType
(
  IN  UINT32 PmicDeviceIndex,
...
)
{
  ...
  pm_typec_connector_type  UsbConnectorType;
  ...
  errFlag = pm_chg_get_typec_connector_type(&UsbConnectorType );
...
*/
EFI_QCOM_PMIC_SCHG_PROTOCOL PmicSchg_P_ProtocolImplementation = 
{
  PMIC_SCHG_REVISION,
  EFI_PmicSchgInit,
  EFI_PmicSchgEnableCharger,
