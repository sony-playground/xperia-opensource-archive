...
)
{
  ...
  case PM_VREG_RAIL_TYPE_SMPS:
    err_flag = pm_smps_sw_enable(PmicDeviceIndex, resource_index,(pm_on_off_type)Enable, WAIT_FOR_SETTLE);
    break;
  case PM_VREG_RAIL_TYPE_LDO:
    err_flag = pm_ldo_sw_enable(PmicDeviceIndex, resource_index,(pm_on_off_type) Enable, WAIT_FOR_SETTLE);
    break;
...
)
{
  ...
  {
  case PM_VREG_RAIL_TYPE_SMPS:
    err_flag = pm_smps_volt_level(PmicDeviceIndex, resource_index, VoltageInUv, WAIT_FOR_SETTLE);
    break;
  case PM_VREG_RAIL_TYPE_LDO:
    err_flag = pm_ldo_volt_level(PmicDeviceIndex, resource_index, VoltageInUv, WAIT_FOR_SETTLE);
    break;
  case PM_VREG_RAIL_TYPE_VS:
  default:
    err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
...
)
{
  ...
  if (PM_ERR_FLAG_SUCCESS == err_flag && Status == EFI_SUCCESS)
  {
  ...
  else if (PM_ERR_FLAG_SUCCESS != err_flag)
  {
