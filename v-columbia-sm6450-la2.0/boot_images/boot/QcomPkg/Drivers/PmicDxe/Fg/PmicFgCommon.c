...
#define TOKEI_PMIC_INDEX 		0
#define NEBULA_PMIC_INDEX 		2
...
{
  UINT32 ChargerPmicIndex = PMIC_INDEX_MAX;
  pm_err_flag_type errFlag = PM_ERR_FLAG_SUCCESS;
  if(FgPmicInfo == NULL)
  {
    return EFI_INVALID_PARAMETER;
  }
  errFlag = pm_chg_get_primary_charger_pmic_index(&ChargerPmicIndex);
  if (errFlag != PM_ERR_FLAG_SUCCESS)
  {
    return errFlag;
  }
  FgPmicInfo->PmicIndex = ChargerPmicIndex;
  FgPmicInfo->SlaveIndex = ChargerPmicIndex;
  return EFI_SUCCESS;
}
...
)
{
  ...
  errFlag = pm_chg_is_battery_present(isBatteryPresent);
  Status = (errFlag == PM_ERR_FLAG_SUCCESS) ? EFI_SUCCESS: EFI_DEVICE_ERROR;
  return Status;
}
...
)
{
  ...
  if(pmic_index == PMIC_D)
  {
    *SdamPmicIndex = TOKEI_PMIC_INDEX;
	*SdamIndex = PM_SDAM_24;
	return EFI_SUCCESS;
  }
  else if(pmic_index == PMIC_I || pmic_index == PMIC_C)
  {
    if(pmic_index == PMIC_I)
	{
	  *SdamPmicIndex = TIOMAN_PMIC_INDEX;
	}
	else if(pmic_index == PMIC_C)
	{
	  *SdamPmicIndex = NEBULA_PMIC_INDEX;
	}
	*SdamIndex = PM_SDAM_2;
    return EFI_SUCCESS;
  }
