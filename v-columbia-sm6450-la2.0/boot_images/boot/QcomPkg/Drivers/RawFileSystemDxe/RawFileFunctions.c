...
Copyright (c) 2018 - 2021 Qualcomm Technologies, Inc. All rights
reserved.
...
)
{
    ...
    {
        DEBUG((EFI_D_WARN, "PrepareAddFileWriteBlocks:Using existing empty entry for writing current file\r\n"));
        status = EFI_SUCCESS;
    }
    else if (EFI_ERROR(status))
    {
        DEBUG((EFI_D_ERROR, "PrepareAddFileWriteBlocks:GetNewFilePlacementInformation failed.Status: 0x%x\n", status));
        goto Exit;
    }
