      ...
      switch (DescType) {
        ...
        case ((UINT8) USB_DESC_TYPE_DEVICE):
          if (DevSm->BusSpeed >= UsbBusSpeedSuper) {
            Data = (VOID *) &SSDeviceDescriptor;
            DataSize = SSDeviceDescriptor.Length;
          }
          else {
        ...
        case ((UINT8) USB_DESC_TYPE_BOS):
          Data = UsbMsdSSDeviceInfo.BosDescriptor;
          DataSize = UsbMsdSSDeviceInfo.BosDescriptor->TotalLength;
          break;

        case ((UINT8) USB_DESC_TYPE_CONFIG):
          if (DevSm->BusSpeed >= UsbBusSpeedSuper) {
            Data = &TotalSSConfigDescriptor.ConfigDescriptor;
            DataSize = TotalSSConfigDescriptor.ConfigDescriptor.TotalLength;
          }
          else {
  ...
  IN EFI_USBFN_MESSAGE_PAYLOAD  *Payload )
{
  ...
  DBG(EFI_D_ERROR, "MSD Bus Speed: %a, state %d", PrintBusSpeed(DevSm->BusSpeed), DevSm->CurrentState);
