  ...
  )
{
  ...
  // QC Fix: Mark all USB Mass Storage Device (MSD) as Removable to avoid fix up GPT that can break the partition.
  // Note that some of the MSD device reports as non-removable even though it is removable.
...
ON_ERROR:
  ...
  if ((Media->MediaId != OldMedia.MediaId) ||
    ...
    if (EfiGetCurrentTpl () != TPL_CALLBACK){
      ...
      Status = EFI_INVALID_PARAMETER;
      USB_ASSERT_GOTO ((EfiGetCurrentTpl () == TPL_CALLBACK),ON_EXIT);
    }
    gBS->RaiseTPL (OldTpl);
