/** @file ScmArmV7.c
  ...
  Copyright (c) 2014, 2019, 2020, Copyright Qualcomm Technologies, Inc.  
  All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/09/20   shl     Added event for calling exit boot sevice handler at last.    
 09/19/19   shl     Ported some changes from other target and reorg the code
...
static BOOLEAN  ScmCallInProgress = FALSE;
...
EFI_EVENT  ExitBootServicesScmDxeV7Event;

EFI_STATUS ScmArmV7ExitBootServicesHandler( IN QCOM_SCM_PROTOCOL  *This );

VOID EFIAPI ExitBootServicesScmDxeV7EventHandler
(
  IN EFI_EVENT  Event,
  IN VOID      *Context
)
{
   EFI_STATUS  Status = ScmArmV7ExitBootServicesHandler( NULL );
   if ( EFI_ERROR(Status))
      DEBUG(( EFI_D_INFO, "ScmArmV7ExitBootServicesHandler failed 0x%x \r\n", Status));
...
ScmArmV7Initialize(VOID)
{
   return gBS->CreateEventEx ( EVT_NOTIFY_SIGNAL,
                               TPL_CALLBACK,
                               ExitBootServicesScmDxeV7EventHandler,
                               NULL,
                              &gEfiEventExitBootServicesSuccessGuid,
                              &ExitBootServicesScmDxeV7Event);
}
    ...
    )
{
    ...
    do
    {
        ...
        if (SyscallQseeResponse->result == TZOS_RESULT_INCOMPLETE  &&  SyscallQseeResponse->resp_type == QSEE_LISTENER_ID)
        {
    ...
    )
{
    ...
    // always deregister high index first
    for( Index = 0; Index < MaxTableSize; Index++ )
    {
        AppDeregisterListenerSyscall->req.listener_id = ListenerGetId( Index );

        if( AppDeregisterListenerSyscall->req.listener_id == 0 )
           continue;
