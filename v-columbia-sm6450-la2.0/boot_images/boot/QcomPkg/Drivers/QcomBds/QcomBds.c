...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/06/22 ? ps ? ? ?use ConvertDevicePathToText in place of undefined DevicePathToStr
...
GetValidUsbDevicePath(BDS_LOAD_OPTION * BootOption){
  ...
  while ((Progress < DelayValue) && (!DevicePathSet))
  {
    ...
    DEBUG((EFI_D_INFO, "BootOption DevPath: %s\n", ConvertDevicePathToText(BootOption->FilePathList, TRUE, TRUE)));
    if (DevicePathSet)
      return EFI_SUCCESS;
  ...
  )
{
  ...
  {
    Status = ProcessFvLoadingForFlashless(RAM_PARTITION_APPSBL_MEMORY); 
    if (Status != EFI_SUCCESS)
    {
      CHAR8      AppToLoad[APP_STR_LEN];
      UINTN      Size = APP_STR_LEN;
  ...
  )
{
  ...
  Status = MountDecompressedFv (&gCompressedAuxFvGuid);
  if (Status != EFI_SUCCESS)
     DEBUG ((EFI_D_ERROR, "MountDecompressedFv failed, Status: %d\r\n", Status));
