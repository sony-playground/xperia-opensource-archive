  ...
  )
{
  ...
  */
  ...
  for (Index = 0; Index < Loop; Index++) {
    ...
    if (Finished) {
      if (PollUsbMsdEvt) {      
        DEBUG ((EFI_D_INFO, "XhcExecTransfer: Transfer Complete, Dir = %d, Length = %d \n", 
          Urb->Ep.Direction, Urb->Completed));
  ...
  )
{
  ...
  USB_ASSERT_RETURN ((EvtTrb->SlotId <= Xhc->MaxSlotsEn), EFI_INVALID_PARAMETER);
  DEBUG ((EFI_D_ERROR, "Enable Slot Successfully, The Slot ID = 0x%x, dev_spd %d\n", EvtTrb->SlotId, DeviceSpeed));
  SlotId = (UINT8)EvtTrb->SlotId;
  USB_ASSERT_RETURN ((SlotId != 0), EFI_INVALID_PARAMETER);
