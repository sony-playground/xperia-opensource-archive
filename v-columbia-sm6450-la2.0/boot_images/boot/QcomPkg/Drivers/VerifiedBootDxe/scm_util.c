/** @file  
  ...
  Copyright (c) 2016, 2020 Copyright Qualcomm Technologies, Inc.
  All Rights Reserved.
...
/*=============================================================================
 ...
 --------   ---     ----------------------------------------------------------
 09/29/20   shl    Added more bit check per platform team request
...
/*
tzbsp_get_secure_state_legacy (TZ_INFO_GET_SECURE_STATE 0x02000604)

Below is the updated bits and description
   Bit 0: if secboot is enabled, set to 0 else set to 1
   Bit 1: if Sec HW key is not programmed, set to 0 else set to 1
   Bit 2: if debug is disabled, set to 0 else set to 1
   Bit 3: if Anti-rollback enabled, set to 0 else set to 1
   Bit 4: if fuse config is enabled, set to 0 else set to 1
   Bit 5: if RPMB provisioned , set to 0 else set to 1
   Bit 6: APPS secure debug enabled, set to 0 else set to 1

   Debug Rearch Bits
   Bit 7: RSVD
   Bit 8: tz inv and non-inv debug disabled, set to 0 else set to 1
   Bit 9: mss inv and non-inv debug disabled, set to 0 else set to 1
   Bit 10: cp inv and non-inv debug disabled, set to 0 else set to 1
   Bit 11: non-secure inv and non-inv debug disabled, set to 0 else set to 1
   Bit 12: tz invasive debug disabled, set to 0 else set to 1
   Bit 13: mss invasive debug disabled, set to 0 else set to 1
   Bit 14: cp invasive debug disabled, set to 0 else set to 1
   Bit 15: non-secure invasive debug disabled, set to 0 else set to 1
   Bit 16: tz non-invasive debug disabled, set to 0 else set to 1
   Bit 17: mss non-invasive debug disabled, set to 0 else set to 1
   Bit 18: cp non-invasive debug disabled, set to 0 else set to 1
   Bit 19: non-secure non-invasive debug disabled, set to 0 else set to 1
...
#define RSVD_FUSE                       7
#define TZ_INV_NONINV_DEBUG_FUSE        8
#define MSS_INV_NONINV_DEBUG_FUSE       9
#define CP_INV_NONINV_DEBUG_FUSE        10
#define NS_INV_NONINV_DEBUG_FUSE        11
#define TZ_INV_DEBUG_FUSE               12
#define MSS_INV_DEBUG_FUSE              13
#define CP_INV_DEBUG_FUSE               14
#define NS_INV_DEBUG_FUSE               15
#define TZ_NONINV_DEBUG_FUSE            16
#define MSS_NONINV_DEBUG_FUSE           17
#define CP_NONINV_DEBUG_FUSE            18
#define NS_NONINV_DEBUG_FUSE            19
...
EFI_STATUS is_secure_device(BOOLEAN *is_secure)
{
  ...
  *is_secure = FALSE;
  /*
     from Lahaina to check 0,1,5, 8, 9, 10, 12 to 19 bits (all debug should be disabled except NS NINV bit 19)
   */
       ...
       !CHECK_BIT(secure_state, NS_INV_DEBUG_FUSE  )    )
  {
    *is_secure = TRUE;
    ...
    DEBUG((EFI_D_ERROR, "VB: Non-secure device: Security State: (0x%x)\r\n", secure_state));
  }
  status = EFI_SUCCESS;
exit:
  return status;
...
EFI_STATUS is_secure_device_nocheck_rpmb(BOOLEAN *is_secure_no_rpmb)
{
  ...
  *is_secure_no_rpmb = FALSE;
  /*
     from Lahaina to check 0,1,5, 8, 9, 10, 12 to 18 bits (all debug should be disabled except NS NINV bit 19)
   */
  if ( !CHECK_BIT(secure_state, SECBOOT_FUSE )                &&
       !CHECK_BIT(secure_state, SHK_FUSE )                    &&
       !CHECK_BIT(secure_state, TZ_INV_NONINV_DEBUG_FUSE  )   &&
