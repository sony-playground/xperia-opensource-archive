...
/*=============================================================================
 ...
 10/09/19   shl     Replaced QseecomDxe with EFIScm and EFITzeLoader protocols
  ...
  */
  ...
  case WRITE_CONFIG:
    if (secure_device_nocheck_rpmb) {
      ...
      wread_req.data = (UINT32)(uintptr_t)info;
      ...
      if (status != EFI_SUCCESS || wread_rsp.status != 0) {
        ...
        */
        ...
        write_req.data = (UINT32)(uintptr_t)info;
  ...
  case READ_CONFIG:
    if (secure_device_nocheck_rpmb) {
      ...
      read_req.data = (UINT32)(uintptr_t)info;
...
*/
EFI_STATUS
QCOM_VB_IsKeymasterEnabled(IN  QCOM_VERIFIEDBOOT_PROTOCOL *This,
                           OUT BOOLEAN                    *KeymasterEnabled)
{
  ...
  if (!KeymasterEnabled) {
      Status = EFI_INVALID_PARAMETER;
      DEBUG ((EFI_D_ERROR, "VB: DEBUG: Inavlid Parameter passed\n"));
      goto Exit;
  }

  Status = GetConfigValue ("SecurityFlag", &SecurityFlag);
  if (Status != EFI_SUCCESS) {
      DEBUG ((EFI_D_ERROR, "VB: DEBUG: Unable to read the security flag\n"));
      goto Exit;
  }

  *KeymasterEnabled = ((SecurityFlag & KEYMASTER_LOAD_FLAG) ==  KEYMASTER_LOAD_FLAG);
