...
=============================================================================*/
/*=============================================================================
 $Header: //components/rel/core.boot/6.0/QcomPkg/Drivers/EnterpriseMgtDxe/EnterpriseMgtCore.c#1 $
                              EDIT HISTORY
...
{
    BOOLEAN status = TRUE;
    UINT32 RegReadValue = ENT_MGT_REG_READ(RegisterAddress);
    UINT32 FinalValue = (RegReadValue & (~Mask)) | (Mask & Value);
    ...
    )
{
    ...
    {
        DEBUG ((ENT_MGT_INFO, "EnterpriseMgt : Locked all EnterpriseMgtX registers.\n"));
    }
    ...
    )
{
    ...
    {              
        ...
        if (!Found)
        {
            ...
            memcpy(pPolicy->HlosDeviceList[pPolicy->NumberOfHlosDevices].DeviceName, 
...
/**
  ...
  @retval FALSE Failed to process this request.
**/
static BOOLEAN EnterpriseMgtProcessFeatureDisablePolicy (
    IN UINT32 FeatureType, 
...
EnterpriseMgtProcessClientPolicies (
    IN ENT_MGT_POLICY_STORE *pStore,
    IN UINT32 EnterpriseMgtLockRegAddr
    )
{
    EFI_STATUS status = EFI_SUCCESS;
    UINT16 i = 0;
    ...
    DEBUG((ENT_MGT_INFO, "EnterpriseMgt : Read configuration table with vendorId 0x%08X \n", pEntMgtCfg->VendorId));
    ...
    for (i = 0; i < pStore->NumFeatures; i++)
    {
        DEBUG((ENT_MGT_INFO, 
                "EnterpriseMgt : Processing disable policy for FeatureType[%d] : %d \n", 
                 i, pFeatureList[i]));
        if (EnterpriseMgtProcessFeatureDisablePolicy(
                pFeatureList[i], 
                pEntMgtCfg, 
                pPolicy) != TRUE)
        {
            DEBUG((ENT_MGT_ERRO, 
                "EnterpriseMgt : EnterpriseMgtProcessFeatureDisablePolicy returned failure.\n"));
