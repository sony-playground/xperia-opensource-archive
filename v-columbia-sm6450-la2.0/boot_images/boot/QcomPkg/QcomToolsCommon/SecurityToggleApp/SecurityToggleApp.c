...
/*=============================================================================
  ...
  ----------   ---     -----------------------------------------------------------
  2021-07-01   am      Moving secbbot file to tools.fv partition from PLAT partition.
...
STATIC CHAR16* SecBootKeysFiles[] = {
  L"KEK_SET.bin",
  L"DB_SET.bin",
  L"DBX_SET.bin",
  L"TEST_PK_SET.bin",
  ...
  )
{
  ...
  if (EFI_ERROR(Status))
  {
     Status = ReadFromFV(SecBootKeysFiles[ValueType], (VOID **) &DataBuffer, &DataSize);
	 if (EFI_ERROR(Status))
     { 
	     AsciiPrint ("\n Error: Reading secboot files failed with status 0x%x \n", Status); 
		 goto clean_up;
	 } 
  }  
