...
static void boot_write_ram_dump_headers(int32 dump_header_handle)
{
  ...
  boot_efs_write_file(dump_header_handle,
                      ...
                      (dload_mem_debug_num_ent() * SECTION_HEADER_SIZE) );
}
...
static int
boot_create_ramdump_header_file (void)
{
  ...
  {
    /* log error file name */
    BL_VERIFY((snprintf(filename, MAX_FILE_NAME, "%s", SD_DUMP_HEADER_NAME) < MAX_FILE_NAME), BL_ERR_INVALID_FILE_NAME|BL_ERROR_GROUP_BOOT);
    return -1;
  }
...
#define MEM_INDEX_RAWDUMP_HEADER_FILE 0

  baseaddr = dload_mem_debug_mem_base(MEM_INDEX_RAWDUMP_HEADER_FILE);
  length = dload_mem_debug_mem_length(MEM_INDEX_RAWDUMP_HEADER_FILE);

  boot_sd_rd_errdata.err_no = EFS_NO_ERR;

  /* update section header info for this dump section */
  boot_update_section_header_table_by_index(MEM_INDEX_RAWDUMP_HEADER_FILE);
...
static int
boot_update_ramdump_header_file (int index)
{
  int32 sd_dump_header_handle = INVALID_FILE_HANDLE;
  char filename[MAX_FILE_NAME];
  ...
  sd_dump_header_handle = boot_efs_open (filename, O_WRONLY, 0666);
  ...
  /* update section header info for this dump section */
  boot_update_section_header_table_by_index(index);
...
bl_error_boot_type rd_hfat_format_write (boot_handle raw_handle, char *filename, void *buffer, uintnt size)
{
  ...
  else
  {
    ...
    /* write a fresh copy of the headers */
    boot_update_ramdump_header_file(memregion_index);
  }
...
bl_error_boot_type rd_hfat_format_open (boot_handle  raw_handle)
{
  ...
  /* Make sure the number of sections we need to dump doesn't exceed the max we support */
  BL_VERIFY((dload_mem_debug_num_ent() <= MAX_CRASHDUMP_TABLE_ENTRIES), BL_ERR_RAM_DUMP_FAIL|BL_ERROR_GROUP_BOOT);
  ...
  if (boot_create_ramdump_header_file() < 0)
  {
    DL_LOG("Failed to create %s", SD_DUMP_HEADER_NAME);
    return BL_ERR_RD_UNSUPPORTED_MODE;
  }
...
bl_error_boot_type rd_hfat_format_close (boot_handle raw_handle)
{
  ...
  if (err_count == 0)
  {
    ...
    DL_LOG("Set dump header as valid");
