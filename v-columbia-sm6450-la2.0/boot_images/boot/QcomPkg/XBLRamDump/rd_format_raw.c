...
struct boot_raw_partition_dump_section_header ALIGN(4) raw_dump_section_header_table[MAX_RAW_DUMP_SECTION_NUM] SECTION(".bss.BOOT_UNCACHED_DDR_ZI_ZONE");
...
void boot_update_section_header_table_by_index(uint32 i)
{
  ...
  strlcpy((char *)&raw_dump_section_header_table[i].section_name, debug_filename, SECTION_NAME_LEN);
}
...
bl_error_boot_type rd_raw_format_write (boot_handle  media_if, char *filename, void *buffer, uintnt size)
{
  ...
  boot_update_section_header_table_by_index(curr_partition_index);
...
void boot_ram_dump_header_init()
{
  ...
  memset(&raw_dump_section_header_table[0], 0x0, sizeof(raw_dump_section_header_table));
...
bl_error_boot_type rd_raw_format_open (boot_handle  media_if)
{
  ...
  if (status != BL_ERR_NONE)
  {
    ...
    return BL_ERROR_GROUP_RAMDUMP|BL_ERROR_OPEN_GPT_PARTITION_FAIL;
  }

  /* Make sure the number of sections we need to dump doesn't exceed the max we support */
  if (dload_mem_debug_num_ent() > MAX_RAW_DUMP_SECTION_NUM)
  {
    return boot_raw_ram_dump_error_handler(CONFIG_CONTEXT_CRT_HANDLE);
  }
