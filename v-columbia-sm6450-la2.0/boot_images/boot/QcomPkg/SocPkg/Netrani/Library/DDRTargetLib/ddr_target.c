...
#define PMIC_VDDQ PMIC_B
#define PMIC_VDDQ_LDO PM_LDO_1
#define PMIC_VDD2H         PMIC_B
#define PMIC_VDD2H_SMPS    PM_SMPS_6
...
#define REG_OFFSET_MC5_CH(ch)            (SEQ_DDR_SS_MC0_MC5_OFFSET + (ch * 0x100000))
#define REG_OFFSET_CABO_CH(ch)            (SEQ_DDR_SS_MC0_CABO_OFFSET + (ch * 0x100000))
#define SEQ_DDR_SS_MC0_MC5_OFFSET                                    0x19260000 
#define SEQ_DDR_SS_MC0_CABO_OFFSET                                 0x19250000

#define GPIO06_DIG_PULL_CTL 0x8D42 
#define GPIO06_STATUS1  0x8D08
#define GPIO6_LEVEL_STATUS_LOW 0X80
/*==============================================================================
                                  DATA
==============================================================================*/
...
extern void Clock_GetDDRSHUB_Freqtable(void);
/*==============================================================================
                                  FUNCTIONS
==============================================================================*/
void DRAM_ROW_HAMMER_CHECK(void);
extern uint32 MC_MR_Read (uint32 _mc_inst_ , DDR_CHIPSELECT DDR_CS0, uint8 mr_num );
...
{
  uint32 bytes_read = 0xFFFFFFFF;
  uint32 chip_plat_ver_info;
  uint8 data = 0xFF;
  uint32 err=0xFFFFFFFF;
  ...
  uint8 gpio_status = 0;
  uint8 Gpio06_pd_30uA = 0x0;
  uint8 Gpio_pull_down_10UA = 0x4;
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
    ...
    if((g_boot_mode == 0 )||(g_boot_mode == BOOT_MODE_RAMDUMP)) //if (PcdGet32 (PcdBuildType) == 0)
    {
      ...
      err = pm_smps_sw_mode(1, PM_SMPS_1, PM_SW_MODE_NPM); // CX SMPS1B to PWM 
      CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);
      err = pm_smps_sw_mode(1, PM_SMPS_5, PM_SW_MODE_NPM); // VDDA_EBI SMPS5B to PWM
      CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);     
    }
  }
  ...
  ddr_shared_data->num_channel = 2;
  ddr_shared_data->enabled_chipselects = 3;
  ddr_shared_data->ddr_init_clk_idx = 0x1; //200MHz
  if (platform == DALPLATFORMINFO_TYPE_RUMI)
  {
	  ddr_shared_data->ddr_init_clk_idx = 0x1; //3.2 GHz for LP5 in D4N mode
	  ddr_shared_data->device_params.device_type = DDR_TYPE_LPDDR4X;
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
	...
	err = SpmiBus_ReadLong (0, PMIC_A , 0, DDR_TYPE_OFFSET , &data , 1 , &bytes_read);  //3nd is priority & 5th is the byte to read
    if(err != PM_ERR_FLAG_SUCCESS)
    {
      HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "pmic ddr detect failed");
	  HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "pmic read from spmi 0x%x & no.of bytes read 0x%x" ,data, bytes_read); 
	  ddr_abort();
    }
	
	    /* if MC4 fuse is set to 0x0 ; then it is LP4 device */
	if ( data == 1 && (HWIO_DDR_REG_FUSE_INPUTS_STATUS_INM(HWIO_DDR_REG_FUSE_INPUTS_STATUS_MC4_FUSE_STATUS_BMSK) ==0))
	{	
		HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "LP4 DDR detected"); 
		ddr_shared_data->device_params.device_type = DDR_TYPE_LPDDR4X;
      
	}
    /* if MC5 fuse is set to 0x0 ; then it is LP5 device */	
	else if ( data == 0 && (HWIO_DDR_REG_FUSE_INPUTS_STATUS_INM(HWIO_DDR_REG_FUSE_INPUTS_STATUS_MC5_FUSE_STATUS_BMSK)==0))
	{	
		HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "LP5 DDR detected");
	    ddr_shared_data->device_params.device_type = DDR_TYPE_LPDDR5;
	    if(HWIO_DDR_REG_FUSE_INPUTS_STATUS_INM(HWIO_DDR_REG_FUSE_INPUTS_STATUS_MC4_FUSE_STATUS_BMSK) == 0)
	    {
		  /* VDD2H init value to 1.080V. This overrides PSI configured voltage. Taken from Waipio, but is not required but keeping it here to have same changes on LP5 fillmore as that of waipio. */ 
		 err = pm_smps_volt_level(PMIC_VDD2H,PMIC_VDD2H_SMPS, 1052 * 1000, TRUE);
		 CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);
		 HWIO_OUTF(DDR_REG_FUSE_DATA_MUX_DFT_CTL, FUSE_DATA_MUX_CTL, 0x1); /*default mc is on for lp4 if lp4 fuse is not blown, in case of both lp5 &4 lp4 not blown force mc on for lp5*/
	    }
	}
	else 
	{
		HAL_EXT_CORE_SW_DDR_printf (DDR_ERROR, "Mismatch between fuse detect and pmic detect , PMIC offset read %x" , data );
        ddr_abort();
	}

  }
  
	/* Byteswap changes SKU3 Discreet Devices*/
	
	spmi_result = SpmiBus_WriteLong(0, slaveId, SPMI_ACCESS_PRIORITY_HIGH,GPIO06_DIG_PULL_CTL, &Gpio06_pd_30uA,1);
    if(spmi_result != SPMI_BUS_SUCCESS)
    {
        HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "pmic gpio pull selection failed");
        ddr_abort();
    }
	spmi_result = SpmiBus_ReadLong(0, slaveId, SPMI_ACCESS_PRIORITY_HIGH,GPIO06_STATUS1,&gpio_status,1, NULL); //GPIO6 to decide Byte Swap Enable vs Disable
    
	if (spmi_result != SPMI_BUS_SUCCESS)
    {
        HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "pmic gpio input level failed");
        ddr_abort();
    }
	/* Byteswap changes SKU3 Discreet Device condition to detect*/
	if((gpio_status & 0xFF) == GPIO6_LEVEL_STATUS_LOW)
    {
        ddr_shared_data->board_byte_swap = 1;
    }
	
	spmi_result = SpmiBus_WriteLong(0, slaveId, SPMI_ACCESS_PRIORITY_HIGH,GPIO06_DIG_PULL_CTL,&Gpio_pull_down_10UA,1);
    if (spmi_result != SPMI_BUS_SUCCESS)
    {
        HAL_EXT_CORE_SW_DDR_printf (DDR_BOOTLOG, "pmic gpio pull selection failed");
        ddr_abort();
...
boolean ddr_target_sdi_pass_init(uint32 clk_speed)
{
  ...
  */
            ...
            HWIO_INF(AOSS_CC_RESET_FSM_STATUS, FIRST_PASS_EXPIRE)||(!HWIO_INF(AOSS_CC_RESET_FSM_CTRL, FIRST_PASS_COMPLETE)))
  {
	...
	{
	   ddr_shared_data->device_params.device_type = DDR_TYPE_LPDDR5;	
	   pm_status = pm_ldo_volt_level(PMIC_VDDQ,  PMIC_VDDQ_LDO, 500000, TRUE);
...
boolean DDR_SetBIMCSpeed(uint32 nFreqKHz )
{
  ...
  {
    pm_status = pm_ldo_volt_level(PMIC_VDDQ, PMIC_VDDQ_LDO, pVddqCfg[nNewLvlIdx].vddq, TRUE);
    if (pm_status != PM_ERR_FLAG_SUCCESS)
    {
      return FALSE;
    }
...
boolean ddr_post_init()
{
  ...
  {
#if 0
	 /* VDDA has to remain TURBO (that was voted earlier in INIT) until we reach Training/Restore (required for RXCAL).
         Make sure no DDR clock switch in b/w ddr init and ddr training/restore that might change VDDA level. 
         In DDR training/restore, when DDR clock switches happen (after RXCAL), VDDA would scale according to freq plan.
      */
	...
	if ( (ddr_driver_struct->aop_params != NULL) && (ddr_driver_struct->aop_params->aop_pmic_settings != NULL) )
	{
	pm_status = pm_smps_volt_level(PMIC_VDD2H, PMIC_VDD2H_SMPS, ddr_driver_struct->aop_params->aop_pmic_settings[2].values[ddr_driver_struct->aop_params->mc_states[1].pmic_mode_cfg]*1000, TRUE);
		if (pm_status != PM_ERR_FLAG_SUCCESS)
			{
				snprintf(ddr_log_string, 200, "Not able to set VDD2H voltage, pm_status = %d", pm_status);
				boot_log_message(ddr_log_string);
				return FALSE;
			} 
	}
	...
	/* // VTMC architecture no GDSCR and LLCC
	  HWIO_OUTF(DPCC_MC5_GDSCR, SW_COLLAPSE, 0x1); //collpse mc5 & override llcc read hint in case of LP4 Dram
	  HWIO_OUTF(LLCC_BROADCAST_ORLLCC_FERC_CFG0, RDHINT_BYPASS_EN, 0x0);
	  HWIO_OUTF(LLCC_BROADCAST_ORLLCC_BEAC0_LLCC_BEAC_CMD_CREDIT_THRESHOLD0_CFG0, RDHINT_IF_THRESHOLD, 0x0);
	  HWIO_OUTF(LLCC_BROADCAST_ORLLCC_BEAC0_LLCC_BEAC_CMD_CREDIT_THRESHOLD1_CFG0, RDHINT_IF_THRESHOLD, 0x0);
	  HWIO_OUTF(LLCC_BROADCAST_ORLLCC_BEAC0_LLCC_BEAC_CMD_CREDIT_THRESHOLD2_CFG0, RDHINT_IF_THRESHOLD, 0x0);
	  HWIO_OUTF(LLCC_BROADCAST_ORLLCC_BEAC0_LLCC_BEAC_CMD_CREDIT_THRESHOLD3_CFG0, RDHINT_IF_THRESHOLD, 0x0); */
	}
	//Call IOTCTL API only if we read 0 from DT entry corresponding to MTE

	  disable_mask = DDR_FW_DISABLE_MTE;
	  value =HAL_DDR_FW_IOCTL(DDR_FW_IOW_Disable_Features, &disable_mask, sizeof(disable_mask));
DRAM_ROW_HAMMER_CHECK();
	return TRUE;
}
...
void print_ddr_frequency()
{
  ...
  if((ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5))
	...
	clk_period = HWIO_MC_BROADCAST_ORCABO_GLOBAL_DDR_CLK_PERIOD_CFG_INM(HWIO_MC_BROADCAST_ORCABO_GLOBAL_DDR_CLK_PERIOD_CFG_DDR_CLK_PERIOD_BMSK);	
...
boolean ddr_params_is_training_required( void )
{
      ...
      (platform != DALPLATFORMINFO_TYPE_VIRTIO))
  {
	if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
		...
		training_required = LP4_TRAINING_ENABLED;
  }
  else
  {
...
void ddr_do_phy_training_init( void )
{
  ...
  /* WA for Lp4 training Issue to make noncacheable */
  if (	ArmSetMemoryAttributes(DDR_MEM_BASE_1, DDR_MEM_SIZE_1, EFI_MEMORY_WC) != EFI_SUCCESS)
	{
		boot_log_message("WA: Error in DDR Rank 0 before training");
	}
	
	if ( ArmSetMemoryAttributes(DDR_MEM_BASE_2, DDR_MEM_SIZE_2, EFI_MEMORY_WC) != EFI_SUCCESS )
	{
		boot_log_message("WA: Error in DDR Rank 1 after training");
	}

	HAL_DDR_FW_Train();  
	/* WA for Lp4 training Issue to revert noncacheable to cacahable */
 
	if ( ArmSetMemoryAttributes(DDR_MEM_BASE_1, DDR_MEM_SIZE_1, EFI_MEMORY_UC) != EFI_SUCCESS)
	{
		boot_log_message("WA: Error in DDR Rank 0 before training");
	}
	
	if (	ArmSetMemoryAttributes(DDR_MEM_BASE_2, DDR_MEM_SIZE_2, EFI_MEMORY_UC) != EFI_SUCCESS)
	{
		boot_log_message("WA: Error in DDR Rank 1 after training");
	} 
...
void ddr_post_setup()
{
    ...
    if(g_boot_mode !=  BOOT_MODE_RAMDUMP ) //if (!boot_dload_is_dload_mode_set()) 
    {
        ...
        fill_aop_params();
        if(ddr_shared_data->target_silicon ==1)
		{
			if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
			{
					DDR_SetBIMCSpeed(2092800);
					//ddr_freq_set_min_max(5,5);
			}
			else
			{
					DDR_SetBIMCSpeed(1708000);
					//ddr_freq_set_min_max(5,5);
			}
		}
		else 
		{
			...
			if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
			{
					DDR_SetBIMCSpeed(3196800);
          			ddr_freq_set_min_max(12,12);
					
                   
			}
			else
			{
					DDR_SetBIMCSpeed(200000);
                    ddr_freq_set_min_max(1,1);
...
/*Clk team API to call for DFS bootup fix in Wildcat architecture  */
Clock_GetDDRSHUB_Freqtable();
...
bl_error_boot_type ddr_get_syndrome_regions(struct syndrome_struct *tag, uint32 *region_count)
{
    ...
    {    
        ...
        boot_log_message("MTE is DISABLED");
    }
...
/* =============================================================================
**  Function : DDR_Row_Hamming_print_abort
** =============================================================================
*/
/**
*   @brief
*   Print the channel & rank info if that combination doesn't have unlinited MAC value
*
*   @param Mac value , channel , rank
*
*   @retval  void
*
*   @dependencies
...
*
*   @sideeffects
*   will abort in case of Limited MAC value
*
*   @sa
*   None
*/
void DRAM_ROW_HAMMER_CHECK_print_abort(uint8 MR_b3 ,uint8 MR_b2_0, uint8 CH , uint8 CS)
{
	char ddr_log_string[250];
	if (MR_b3 == 1) //unlimited MAC is present
	{
		snprintf (ddr_log_string, 250,"Row Hammer Check : DRAM supports unlimited MAC Value : MR_RH[OP2:0 = %d] & MR_RH[OP3 = %d] for CH%d & CS%d",MR_b2_0, MR_b3, CH,CS,"/n");
		boot_log_message (ddr_log_string);
	}
	else
	{	
		snprintf (ddr_log_string, 250,"Row Hammer Check : DRAM doesn't support unlimited MAC Value : MR_RH[OP2:0 = %d] & MR_RH[OP3 = %d] for CH%d & CS%d ",MR_b2_0, MR_b3, CH,CS,"/n");
		boot_log_message (ddr_log_string);
		boot_log_message ("Aborting DDR");
		ddr_abort();
	}
} /* DRAM_ROW_HAMMER_CHECK_print_abort */


/* =============================================================================
**  Function : DRAM_ROW_HAMMER_CHECK
** =============================================================================
*/
/**
*   @brief
*   Check if the DRAM has unlimited MAC value
*
*   @param void
*
*   @retval  void
...
void DRAM_ROW_HAMMER_CHECK (void)
{
	...
	uint8 num_CH;
	char ddr_log_string[200];
	boot_log_message ("Row Hamming DDR");
	if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
		JEDEC_MR = 0x1B;
	else
        JEDEC_MR = 0x18;
	for (num_CH = 0 ; num_CH < ddr_shared_data->num_channel ; num_CH++)
	{		
		if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
		{			
			uint8  MR_b0 = 0 ;
			MR_value = MC_MR_Read  (REG_OFFSET_MC5_CH(num_CH), DDR_CS0 , JEDEC_MR);
			snprintf(ddr_log_string, 200, "LP5 MR_value =  0x%x",MR_value);
			boot_log_message(ddr_log_string);
			MR_b0 = MR_value & 1; //0th bit represents if RFM is required or not ; 0 - not required , 1 - required
			if (MR_b0){
				boot_log_message ("RFM is supported");
			}
			else{
				boot_log_message ("RFM is NOT supported");
				//ddr_abort();
			}
		}
		else
		{
			uint8  MR_b3 = 0 ;
			uint8  MR_b2_0 = 0 ;
			MR_value = MC_MR_Read  (REG_OFFSET_CABO_CH(num_CH), DDR_CS0 , JEDEC_MR);
			snprintf(ddr_log_string, 200, "LP4 MR_value =  0x%x",MR_value);
			boot_log_message(ddr_log_string);
			MR_b3 = (MR_value & 8) >> 3; //3rd bit incidates whether it is MAC value is limited
			MR_b2_0 = (MR_value & 7); //0-2 bit indicates the exact MAC value		
			DRAM_ROW_HAMMER_CHECK_print_abort (MR_b3, MR_b2_0, num_CH ,0); //call print based on channel & rank
			
			/* Considering Rank 0 is always present , adding check for rank1 only. */
			if (ddr_shared_data->ddr_size_info.ddr_cs1_mb[num_CH]) 
			{
				MR_value = MC_MR_Read  (REG_OFFSET_CABO_CH(num_CH), DDR_CS1 , JEDEC_MR);
				snprintf(ddr_log_string, 200, "LP4 MR_value =  0x%x",MR_value);
				boot_log_message(ddr_log_string);
				MR_b3 = (MR_value & 8)>>3; //3rd bit incidates whether it is MAC value is limited
				MR_b2_0 = (MR_value & 7); //0-2 bit indicates the exact MAC value	
				DRAM_ROW_HAMMER_CHECK_print_abort (MR_b3, MR_b2_0, num_CH ,1); //call print based on channel & rank
			}
		}
	}
