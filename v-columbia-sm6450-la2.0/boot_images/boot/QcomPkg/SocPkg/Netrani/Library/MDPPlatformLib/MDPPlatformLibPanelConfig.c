...
{
  {"samsung_amoled_fhd_cmd",             "Panel_samsung_amoled_fhd_cmd.xml",                      "qcom,mdss_dsi_samsung_fhd_amoled_cmd:",                       &sModeConfig[MDP_MODE_SINGLEDSI0], &sHwConfig[HW_CONFIG_DEFAULT_BRIGHT_V2]},
};
...
static PlatformDSIDetectParams uefiDphyPanelList[] = {
    // Panel #0 - samsung fhd plus Command Mode Panel (1080x2520 24bpp)
    {
      "samsung_amoled_fhd_cmd",
      0x06,                                                  // uCmdType
      0x05,                                                  // total number of retry on failures
      {
        {{0xA1, 0x00},                                       // address to read ID1
...
MDP_Status MDPPlatformGetPanelXmlConfig (MDP_Display_IDType eDisplayId, MDPPlatformPanelInfo *pPlatformPanel)
{
  ...
  DEBUG((EFI_D_ERROR, "Entry MDPPlatformGetPanelXmlConfig\n"));
                                               ...
                                               0))
  {
    ...
    DEBUG((EFI_D_ERROR, "MDPPlatformGetPanelXmlConfig: MDP_Display_LoadFile fail\n"));
  }
...
VOID MDPPlatformGetPanelList(MDPDisplayAutoDetectType eAutoDetect, PlatformDSIDetectParams **pPanelList, UINT32 *pListLen, UINT32 *pPanelIndex)
{
  ...
  DEBUG((EFI_D_ERROR, "MDPPlatformGetPanelList: entry pPanelList\n"));
  if ((NULL == pPanelList) ||
      (NULL == pListLen))
  {
...
static bool32 PanelCheckOverride(MDP_Display_IDType eDisplayId, MDPPlatformPanelInfo *pPlatformPanel, MDPPlatformInfo *pPlatformInfo)
{
  ...
  DEBUG((EFI_D_ERROR, "Entry PanelCheckOverride\n"));
  ...
  else if (NULL != pPanelParams->pPanelConfig)
  {
    ...
    {
      pPlatformInfo->bSWRender = TRUE;
      DEBUG((EFI_D_ERROR, "PanelCheckOverride: Panel override <%a> in skip mode\n", pPanelParams->shortName));
    }
    else
    {
      DEBUG((EFI_D_ERROR, "PanelCheckOverride: Panel override <%a>\n", pPanelParams->shortName));
    }
  }
  else if (NULL == pPanelParams->pPanelConfig)
  {
                                                    ...
                                                    MDPPlatformInfo      *pMDPPlatformInfo)
{
  ...
  DEBUG((EFI_D_ERROR, "Entry MDPPlatformSetupPanelConfig\n"));
  if ((NULL == pPlatformPanel) ||
      (NULL == pPlatformInfo)  ||
  ...
  else
  {
    ...
    else
    {
      ...
      case EFI_PLATFORMINFO_TYPE_IDP:
      {
        if ((0x10000 == pPlatformInfo->sEFIPlatformType.version)  && 
            ((2      == pPlatformInfo->sEFIPlatformType.subtype)  ||
             (3      == pPlatformInfo->sEFIPlatformType.subtype)  ||
             (4      == pPlatformInfo->sEFIPlatformType.subtype)))  
        {
      ...
      {
        uint32 uPanelId = 1;   //for qrd dphy panel

        //for qrd AB chip
        if (0x10 == (pPlatformInfo->sEFIPlatformType.version & 0xFF))
        {
          uPanelId = 2;
        }
        ...
        {
          // No override and it is not SW render - select default panel for power up and trigger dynamic detection
          MDPPlatformGetPanelList(eAutoDetect, &pPanelList, &uPanelCnt, &uPanelId);
          pPlatformPanel->eAutoDetect = eAutoDetect;
        }
        ...
        DEBUG((EFI_D_ERROR, "MDPPlatformSelectPanelConfig: pPlatformPanel->uPanelId=%d\n",pPlatformPanel->uPanelId));
        DEBUG((EFI_D_ERROR, "MDPPlatformSelectPanelConfig: pPanelList->szName=%s\n",pPanelList->szName));
        MDPPlatformSetPanelName(pPlatformPanel->szName, "samsung_amoled_fhd_cmd");
        break;
      }
...
PanelConfigType* MDPPlatformSelectPanelConfig(MDP_Display_IDType eDisplayId, CHAR8 *pPanelName, bool32 *bSWEnable)
{
  ...
  DEBUG((EFI_D_ERROR, "Entry MDPPlatformSelectPanelConfig\n"));
  ...
  case CONFIG_LOCATION_PHYSICAL_TABLE:
    {
      ...
      for (i = 0; i < ARRAY_SIZE(sPhysicalPanelConfigTable); i++)
      {
        ...
        {
          DEBUG((EFI_D_ERROR, "LookUpPanel : i=%d  pPanelName=%s,sPhysicalPanelConfigTable[i].szName=%s\n",i, pPanelName, sPhysicalPanelConfigTable[i].szName));
  ...
  case CONFIG_LOCATION_MAINFV:
    {
      ...
      {
        DEBUG((EFI_D_ERROR, "MDPPlatformSelectPanelConfig: fail to load %a\n", DISP_SIMPANEL_CONFIG_FILE_ASICII));
      }
      else
      {
        if (EFI_SUCCESS != (eStatus = GetPanelConfigFromFile(pPanelName, pPanelConfigFileBuf, uPanelConfigFileSize, pPanelConfig)))
        {
          DEBUG((EFI_D_ERROR, "MDPPlatformSelectPanelConfig: fail to get panel config from PanelConfig.csv at mainfv\n"));
        }
  ...
  case CONFIG_LOCATION_IMAGEFVB:
    {
      ...
      {
        DEBUG((EFI_D_ERROR, "MDPPlatformSelectPanelConfig: fail to read panel config data\n"));
      }
      else
      {
        if (EFI_SUCCESS != (eStatus = GetPanelConfigFromFile(pPanelName, pPanelConfigFileBuf, uPanelConfigFileSize, pPanelConfig)))
        {
          DEBUG((EFI_D_ERROR, "MDPPlatformSelectPanelConfig: fail to get panel config from PanelConfig.csv at %s\n", DISP_CONFIG_PART_NAME));
        }
