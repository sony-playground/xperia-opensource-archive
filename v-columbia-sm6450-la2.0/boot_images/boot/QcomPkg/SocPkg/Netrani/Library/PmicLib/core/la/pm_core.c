...
pm_err_flag_type pm_post_pmic_initialization(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  err_flag |= pm_pon_stage3_reset_source_cfg(0, PM_PON_RESET_SOURCE_RESIN_AND_KPDPWR, 32000);

  pm_comm_write_byte(0, 0x5444, 0x20);
  pm_comm_write_byte(0, 0x544D, 0x20);
...
static pm_err_flag_type pm_i2c_sid_config(void)
{
  ...
  Status |= PmicGpioProtocol->SetOutputLevel(PMIC_B, LEICA_1_RESET_GPIO, EFI_PM_GPIO_LEVEL_LOW); //GPIO3 on Bompoka
  Status |= PmicGpioProtocol->SetOutputLevel(PMIC_B, LEICA_2_RESET_GPIO, EFI_PM_GPIO_LEVEL_HIGH); //GPIO4 on Bompoka

  Status |= PmicGpioProtocol->Enable(PMIC_B, LEICA_1_RESET_GPIO, TRUE);
  Status |= PmicGpioProtocol->Enable(PMIC_B, LEICA_2_RESET_GPIO, TRUE);
