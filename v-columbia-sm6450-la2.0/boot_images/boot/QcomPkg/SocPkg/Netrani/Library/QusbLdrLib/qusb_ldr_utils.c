...
//============================================================================
/**
  Copyright (c) 2015-2019,2021-2023 Qualcomm Technologies, Inc. All rights reserved.
...
#define QUSB_DCI_PMI_INDEX                        (8)  /* For PMIC_I, Tioman PMIC is used for Netrani*/
...
static pm_chg_charger_port_type qusb_ldr_utils_get_chgr_type(void)
{
  ...
  uint32 pm_index_charger = PMIC_INDEX_MAX;
  pm_model_type pmic_model = PMIC_IS_INVALID;
 
  pm_err = pm_chg_get_primary_charger_pmic_index(&pm_index_charger);
  if (pm_err != PM_ERR_FLAG_SUCCESS) {
  	qusb_uart_log("Get PMIC index failed, Hence using default PMIC index", QUSB_DCI_PMI_INDEX);
    pmic_model = pm_get_pmic_model(QUSB_DCI_PMI_INDEX);	
  } else {
    pmic_model = pm_get_pmic_model(pm_index_charger);
  }
  dtb_status = usb_shared_xbl_dtb_node_init_phy("/sw/pmic/pm/charger");
  if(USB_DTB_STATUS_SUCCESS != dtb_status)
  {
	qusb_uart_log("chg_dtb_failed", dtb_status);
  }
  // if usb-phy DT property is set, go for usb-phy based charging detection
  ...
  //USB-Phy Dt property is not set, check for no-pmi
  if((pmic_model == PMIC_IS_UNKNOWN) || (pmic_model == PMIC_IS_INVALID))
  {
    //return SDP to allow FEDL on no-PMI devices
  ...
  while (--charger_det_retry_cnt)
  {
    ...
    {
      qusb_uart_log("qusb_ldr, pmi_not_detected", 0);
      usb_charger_type = PM_CHARGER_PORT_TYPE_INVALID;
      return usb_charger_type;
    }
...
void qusb_ldr_utils_update_serial(void)
{
  ...
  if (serial_num == 0)
  {
    ...
    serial_num = qusb_dci_get_chip_serial_number();
  }
...
void qusb_ldr_eud_init(void)
{
  ...
  eud_serial = qusb_dci_get_chip_serial_number();
  qusb_dci_eud_update_serial_number(eud_serial);
