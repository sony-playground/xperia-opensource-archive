 ...
 *
 * $DateTime: 2022/05/24 05:05:46 $
 * $Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Netrani/Library/CPRTargetLib/image/core/target/netrani/cpr_image_target.c#5 $
 * $Change: 37387097 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    CPR_ASSERT(cpr_settings_netrani);

    cpr_settings_netrani->railsCount = 1;
    cpr_settings_netrani->enablementsCount = 1;
    cpr_settings_netrani->controllersCount = 1;

    cpr_settings_netrani->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_netrani->railsCount);
    for (i = 0; i < cpr_settings_netrani->railsCount; i++) {
        cpr_settings_netrani->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_netrani->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_netrani->controllersCount);
    for (i = 0; i < cpr_settings_netrani->controllersCount; i++) {
        cpr_settings_netrani->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_netrani->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_netrani->railsCount);
    for (i = 0; i < cpr_settings_netrani->railsCount; i++) {
        cpr_settings_netrani->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_netrani->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_netrani->enablementsCount);
    for (i = 0; i < cpr_settings_netrani->enablementsCount; i++) {
        cpr_settings_netrani->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_netrani->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_netrani, /*idx*/ 0);

    return cpr_settings_netrani;
}
