...
//============================================================================
/**
  Copyright (c) 2015-2020,2022,2023 Qualcomm Technologies, Inc. All rights reserved.
...
#define QUSB_DCI_PMI_INDEX                    (8)   /*For PMIC_I, Netrani uses Tioman Pmic*/
...
QCOM_USB_CLOCK SSUsb1EnCoreClocks[] =
{
  ...
  {"gcc_eusb3_0_clkref_en",             0,                             1 },
  {"gcc_usb3_0_clkref_en",              0,                             1 }
...
QCOM_USB_CLOCK SSUsb1DisCoreClocks_EUD[] =
{
  {"gcc_cfg_noc_usb3_prim_axi_clk",     USB3_AXI_CLK_MIN_FREQ_HZ,      1 },
  {"gcc_aggre_usb3_prim_axi_clk",       USB3_AXI_CLK_MIN_FREQ_HZ,      1 },
  {"gcc_usb30_prim_master_clk",         USB3_MASTER_CLK_MIN_FREQ_HZ,   1 },
...
STATIC EFI_QCOM_PMIC_USB_PROTOCOL             *gPmicUsbProtocol = NULL;
  ...
  else
  {
    if (NULL == gPmicUsbProtocol) {
      ...
      if (EFI_ERROR(Status)) {
        ...
        WRN_CHK("Failed to open PMIC USB protocol Status (0x%x) \r\n", Status);
      }
    }

    if (gPmicUsbProtocol && gPmicUsbProtocol->GetChargerPmicIndex) {
      Status = gPmicUsbProtocol->GetChargerPmicIndex(&qUsbDciPmiIndex);
    }

    if (Status == EFI_SUCCESS) {
      Status = PmicVersionProtocol->GetPmicInfo(qUsbDciPmiIndex, &PmiDeviceInfo);
    } else {
  ...
  )
{
  ...
  QCOM_USB_CLOCK  *PrimaryDisClocks     = SSUsb1DisCoreClocks;
   UINTN           PrimaryDisClockCountEUD  = sizeof(SSUsb1DisCoreClocks_EUD) / sizeof(QCOM_USB_CLOCK);
  QCOM_USB_CLOCK  *PrimaryDisClocksEUD     = SSUsb1DisCoreClocks_EUD;
  ...
  {
    // Disable clocks for Primary Core controller
    Status = DisableUsbClocks (PrimaryDisClockCountEUD, PrimaryDisClocksEUD);
    WRN_CHK ("failed to disable USB clocks");
    UsbConfigLib_pmic_npa_issue_required_request(gPmicNpaClientSS1, PMIC_NPA_MODE_ID_GENERIC_OFF);
  }
  ...
  )
{
  ...
  else
  {
    ...
    QCOM_USB_CLOCK  *PrimaryDisClocksEUD     = SSUsb1DisCoreClocks_EUD;
    Status = DisableUsbClocks (PrimaryDisClockCountEUD, PrimaryDisClocksEUD);
    WRN_CHK ("failed to disable EUD USB clocks");
  }
...
//TBD, Registers not there in ipcat
#if 0
