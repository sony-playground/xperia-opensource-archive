...
//============================================================================
/**
  Copyright (c) 2015, 2017-2020,2022 Qualcomm Technologies, Inc. All rights reserved.
...
static uint8 qusb_dci_eud_reg_addr[QUSB_EUD_TUNE_TABLE_SIZE] =
{
  ...
  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_FSEL_SEL_ADDR                      ),  // 0x01,

  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_PARAMETER_OVERRIDE_X0_ADDR         ),  // 0x83,
  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_PARAMETER_OVERRIDE_X1_ADDR         ),  // 0xCB,
  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_PARAMETER_OVERRIDE_X2_ADDR         ),  // 0x1E,
  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_PARAMETER_OVERRIDE_X3_ADDR         ),  // 0x03,
  
  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_HS_PHY_CTRL2_ADDR                  ),  // 0x0E,
  USB2PHY_OFF( HWIO_USB2PHY_USB_PHY_HS_PHY_CTRL_COMMON0_ADDR           ),  // 0x88,
...
boolean qusb_dci_dp_gnd_check(boolean check_dp_high, uint32 check_dp_timeout_ms)
{
  ...
  if ((linestate == 0) && (ls_tries == USB_FEDL_MAX_DP_LOW_TIMEOUT_MS))
  {
    ...
    else
    {
      ...
      for(ls_tries = check_dp_timeout_ms; (linestate == 0) && (ls_tries > 0);  ls_tries--)
      {
        linestate = (HWIO_USB2PHY_USB_PHY_UTMI_RX_PORT_STATUS_INM(
          HWIO_USB2PHY_USB_PHY_UTMI_RX_PORT_STATUS_LINESTATE_BMSK) 
          >> HWIO_USB2PHY_USB_PHY_UTMI_RX_PORT_STATUS_LINESTATE_SHFT) & USB_FEDL_DP_BMSK;
        if(ls_tries%1000==0)
        {
          qusb_uart_log("wait unplug(dphigh), (ms):", ls_tries);
        }
