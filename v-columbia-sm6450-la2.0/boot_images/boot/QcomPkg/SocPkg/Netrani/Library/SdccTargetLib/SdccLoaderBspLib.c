...
2022-05-26   ssb     Updated PMIC api as per ipcat
...
sdcc_bsp_err_type sdcc_bsp_get_vreg_switch_status (UINT32 driveno)
{
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      ...
      if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_E, PM_LDO_6, PM_ON, TRUE))
      {
...
sdcc_bsp_err_type sdcc_bsp_lower_volt_level (UINT32 driveno)
{
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      ...
      if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_E, PM_LDO_6, 1800 * 1000, TRUE))
      {
         return SDCC_BSP_ERR_PMIC; 
      }
      else
...
// MakuaC   [PM8150]   E 1
#define LDO09_VSET_ADDR 0x14840 //SD Card Vdd Regulator register address
#define LDO09_EN_CTL_ADDR 0x14846 //LDO9 enable control register address
#define LDO09_VSET_LB 0x14840   //SD Card Vdd Regulator register address lower byte
#define LDO09_VSET_UB 0x14841   //SD Card Vdd Regulator register address upper byte
#define LDO09_VSET_SIZE 2       //SD Card Vdd Regulator register size
#define LDO09_EN_CTL_SIZE 1       //SD Card Vdd Regulator register size

#define LDO06_VSET_ADDR 0x14540 //SD Card Vdd_px2 Regulator register address
#define LDO06_EN_CTL_ADDR 0x14546 //LDO6 enable control register address
#define LDO06_VSET_LB 0x14540   //SD Card Vdd_px2 Regulator register address lower byte
#define LDO06_VSET_UB 0x14541   //SD Card Vdd_px2 Regulator register address upper byte
#define LDO06_VSET_SIZE 2       //SD Card Vdd_px2 Regulator register size
#define LDO06_EN_CTL_SIZE 1       //SD Card Vdd_px2 Regulator register size

       uint8_t data1[LDO09_EN_CTL_SIZE] = {0x0}; // disable LD09
       if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x4 /*MakuaC   [PM8150]   E 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO09_EN_CTL_ADDR, data1, LDO09_EN_CTL_SIZE))
       {
         return SDCC_BSP_ERR_PMIC;
       }
       else
       {
         uint8_t data[LDO09_VSET_SIZE] = {0x90,0x0b}; //2960 mili volt in hex
         if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x4 /*MakuaC   [PM8150]   E 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO09_VSET_ADDR, data, LDO09_VSET_SIZE))
         {
           return SDCC_BSP_ERR_PMIC;
         }

         uint8_t data2[LDO09_EN_CTL_SIZE] = {0x80}; // Enable LDO09
         if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x4 /*MakuaC   [PM8150]   E 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO09_EN_CTL_ADDR, data2, LDO09_EN_CTL_SIZE))
         {
            return SDCC_BSP_ERR_PMIC;
         }
       }
   
       uint8_t data3[LDO06_EN_CTL_SIZE] = {0x0}; // disable LDO06
       if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x4 /*MakuaC   [PM8150]   E 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO06_EN_CTL_ADDR, data3, LDO06_EN_CTL_SIZE))
       {
         return SDCC_BSP_ERR_PMIC;
       }
       else
       {
         uint8_t data4[LDO06_VSET_SIZE] = {0x86,0x0b}; //2950 mili volt in hex
         if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x4 /*MakuaC   [PM8150]   E 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO06_VSET_ADDR, data4, LDO06_VSET_SIZE))
         {
           return SDCC_BSP_ERR_PMIC;
         }

         uint8_t data5[LDO06_EN_CTL_SIZE] = {0x80}; // Enable LDO18
         if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x4 /*MakuaC   [PM8150]   E 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO06_EN_CTL_ADDR, data5, LDO06_EN_CTL_SIZE))
         {
            return SDCC_BSP_ERR_PMIC;
         }
       }
       ...
       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_E, PM_LDO_6, PM_OFF, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_E, PM_LDO_9, PM_OFF, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_E, PM_LDO_6, 2960 * 1000, TRUE))
       {
              return SDCC_BSP_ERR_PMIC;
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_E, PM_LDO_9, 2950 * 1000, TRUE))
       {
              return SDCC_BSP_ERR_PMIC;
       }
       ...
       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_E, PM_LDO_9, PM_ON, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }
