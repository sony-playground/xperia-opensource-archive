...
$Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Netrani/Settings/UFS/core/ufs_bsp_core.c#8 $
$DateTime: 2022/04/28 02:12:50 $
...
{
   {
      ...
      {"gcc_ufs_phy_ice_core_clk", 0, 0, 0, 75000000, 150000000, 300000000, 300000000}, 
...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_regulator_mode mode)
{
   ...
   if (mode == UFS_BSP_REGULATORS_ON) {
      ...
      // PMIC_B, PM_LDO_24 is Netrani target specific UFS Device Vcc regulator
     // Refer ipcat to update it for different Target.	 	 
     // BOMPOKA [PM6450]    B 1	 	 
	
      Status=PmicVRegProtocol->VregGetLevel(0x1 /*BOMPOKA [PM6450] B 1*/, EFI_PM_LDO_24, &voltage_req_rail);
      if (Status != EFI_SUCCESS) {
         return UFS_BSP_ERR_PMIC;  
      }
