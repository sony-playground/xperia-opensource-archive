...
bl_error_boot_type uncached_heap_memmap_init(uintnt base, uintnt size)
{
  ...
  do
  {
    status = boot_config_context_get_ptr(CONFIG_CONTEXT_CRT_HANDLE, CONFIG_CONTEXT_EDK2_MMU_INTERFACE, (void **)&edk2_mmu_if);
    ...
    if(edk2_mmu_if->ArmSetMemoryAttributes(current_entry->PhysicalBase, current_entry->Length, current_entry->Attributes) != EFI_SUCCESS)
    {
...
bl_error_boot_type ramdump_populate_mem_map(boot_handle config_context_handle)
{
  ...
  ARM_MEMORY_REGION_DESCRIPTOR ramdump_memory_map[]=
  {
    ...
    { (uint64)Image$$RAMDUMP_DATA_ZI$$Base,(uint64)Image$$RAMDUMP_DATA_ZI$$Base, (uint64)Image$$RAMDUMP_DATA_ZI$$Length,                             ARM_MEMORY_REGION_ATTRIBUTE_UNCACHED_UNBUFFERED_XN },
