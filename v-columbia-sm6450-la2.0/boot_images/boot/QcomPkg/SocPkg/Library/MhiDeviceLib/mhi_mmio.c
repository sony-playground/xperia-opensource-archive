...
{
   if((sizeof(erdbmask)/sizeof(erdbmask[0]))<=ernum/32)
   {
      mhi_osal_debug(MSG_FATAL, "Invalid ernum %d",ernum);
      return;
   }
...
mhi_status_t mhi_mmio_init(uint32 base, mhi_config_t *mhiConfig)
{
   ...
   mhiConfig->session_id = MMIO_IN(BHI_IMGTXDB);
...
/**
 * @brief      mhi_mmio_get_bhi_intvec
 *
 *             Get BHI_INTVEC. BHI INTVEC is the interrupt vector to be used to notify
 *             of EXECENV changes or when an error occurs and the primary event ring
 *             is not initialized
 *
 * @dependencies
 *             None
 ...
 * @sideeffect None
 */
uint32 mhi_mmio_get_bhi_intvec(void)
{
   return MMIO_IN(BHI_INTVEC);
}

/**
 * @brief      mhi_mmio_clear_chdb
 *
 *             Clear MHI DB channel
 *
 * @param[in]  cci MHI DB channel offset
 * @param[in]  val Value to be written
 *
 * @return     None
 */
void mhi_mmio_clear_chdb(uint32 cci, uint32 val)
{
    MMIO_OUTD(CHDB_LOWER_n(cci), val);
}

/**
 * @brief mhi_mmio_is_edl_mode
 *
 *        Is EDL mode enabled.
 *
 * @param[in]  base   MHI MMIO base address.
 * @param[in]  chnum  MHI DB channel offset.
 *
 * @return boolean    True if enabled else False.
 */
boolean mhi_mmio_is_edl_mode(uint32 base, uint32 chnum)
{
    uint32 val = 0;
    uint32 early_init = 0;
    ...
    val = mhi_mmio_get_chdb(chnum);
    //if (MHI_EDL_COOKIE == mhi_mmio_get_chdb(chnum))
    if (MHI_EDL_COOKIE == val)
    {
        // clear the cookie
        mhi_mmio_clear_chdb(chnum, 0x0);
        status = TRUE;
    }
    if (early_init)
    {
        // clear the MMIO base if set at early init
        mmio_base = 0;
    }
    return status;
}
