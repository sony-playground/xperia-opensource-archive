...
#define MAX_REQUESTS_PER_CH 20

//#define MHI_DB_EDL_CHAN 0x6E
/* MHI DB channel used for EDL mode*/
#define MHI_DB_EDL_CHAN  0x5B
...
/**
 * @brief      mhi_get_config
 *
 *             Return MHI Configuration. This API should be called after MHI is enumerated.
 *             It will return information such as number of event rings programmed by host,
 *             enumeration session ID, and other host related configuration.
 *
 * @param [in/out] cfg - MHI Configuration structure that will be populated
 *
 * @dependencies
 *             None
 *
 ...
 * @sideeffect None
 */
void mhi_get_config(mhi_cfg_t* cfg)
{
  if(NULL != cfg)
  {
     cfg->nEr=mhiCtx.mhiConfig.nEr;
     cfg->nCh=mhiCtx.mhiConfig.nCh;
     cfg->nHwch=mhiCtx.mhiConfig.nHwCh;
     cfg->nHwer=mhiCtx.mhiConfig.nHwEr;
     cfg->session_id=mhiCtx.mhiConfig.session_id;
     cfg->ver=mhiCtx.mhiConfig.version;
  }
  return;
}
...
mhi_status_t mhi_deinit(boolean reset)
{
   ...
   if(mhi_link_deinit())
   {
      mhi_osal_debug(MSG_ERR, "mhi_deinit: Link De-init failed");
   }
...
mhi_status_t mhi_channel_rw_multiple(uint32 chId, void** data_array, uint32* size_array, uint32 nElements, uint32 *nSubmitted, uint32* bytesrw, mhi_cb_t *cb, mhi_op_t op)
{
   ...
   if(NULL == nSubmitted || NULL == size_array || 0 == nElements)
   {
...
boolean mhi_is_edl_mode(void)
{
    ...
    if (NULL == mhiCtx.mhi_core_tgt_config)
    {
        //MHI context not up. Do light MMIO init.
        // Get target cfg to extract MMIO base
        mhiCtx.mhi_core_tgt_config = mhi_get_core_tgt_info();
        early_init = 1;
        if (NULL == mhiCtx.mhi_core_tgt_config)
        {
    ...
    // set MMIO base and get EDL cookie. Then clear the cookie and base if intialize here.
    status = mhi_mmio_is_edl_mode(mhiCtx.mhi_core_tgt_config->mmio_basephys, MHI_DB_EDL_CHAN);
    if (early_init)
    {
        // this is updated at early init, so clear it.
        mhiCtx.mhi_core_tgt_config = 0x0;
    }
    return status;
}
