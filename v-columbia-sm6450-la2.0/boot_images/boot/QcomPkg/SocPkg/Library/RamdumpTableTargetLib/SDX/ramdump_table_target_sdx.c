...
$Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Library/RamdumpTableTargetLib/SDX/ramdump_table_target_sdx.c#7 $
...
=============================================================================*/
#define TARGET_DEPENDENT_RAMDUMP_ENTRIES     9
...
static bl_error_boot_type _ramdump_add_aoss_cc_regs(boot_handle, ramdump_table_sdx_handle*);
...
static bl_error_boot_type _ramdump_add_ddr_dt_log_regions(boot_handle, ramdump_table_sm_handle*, uint32 dload_cookie)
                                       ...
                                       boot_dal_if_type *dal_if)
{
  ...
  do
  {
    ...
    /* Region: AOSS */
    result = dal_if->DALSYS_MallocType(sizeof(uintnt), (void **)&(handle->aoss_cc_fsm_status_register), DALHEAP_UNCACHED);

    if ((result != DAL_SUCCESS) || ((handle->aoss_cc_fsm_status_register) == NULL))
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }

    result = dal_if->DALSYS_MallocType(sizeof(uintnt), (void **)&(handle->aoss_cc_fsm_ctrl_register), DALHEAP_UNCACHED);

    if ((result != DAL_SUCCESS) || ((handle->aoss_cc_fsm_ctrl_register) == NULL))
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    /* Region: Reset Status Register */
    result = dal_if->DALSYS_MallocType(sizeof(uintnt), (void **)&(handle->reset_status_register), DALHEAP_UNCACHED);

    if ((result != DAL_SUCCESS) || ((handle->reset_status_register) == NULL))
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    /* Region: PMIC */
    result = dal_if->DALSYS_MallocType(sizeof(uint64), (void **)&(handle->pmic_power_on), DALHEAP_UNCACHED);

    if ((result != DAL_SUCCESS) || ((handle->pmic_power_on) == NULL))
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }

    result = dal_if->DALSYS_MallocType(sizeof(pm_pbs_pon_reason_history_data_type),
                                       (void **)&(handle->pm_pon_reason_history),
                                       DALHEAP_UNCACHED);

    if ((result != DAL_SUCCESS) || ((handle->pm_pon_reason_history) == NULL))
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }

    result = dal_if->DALSYS_MallocType(sizeof(pm_pbus_data_type), (void **)&(handle->pbus_log_data), DALHEAP_UNCACHED);

    if ((result != DAL_SUCCESS) || ((handle->pbus_log_data) == NULL))
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    boot_dal_if_type *dal_if)
{
  ...
  do
  {
    ...
    {
      /* Region: AOSS */
      result = dal_if->Free((void *)(handle->aoss_cc_fsm_status_register));

      if (result != DAL_SUCCESS)
      {
        return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_DEALLOC_FAIL);
        break;
      }
    }
    ...
    {
      result = dal_if->Free((void *)(handle->aoss_cc_fsm_ctrl_register));
    ...
    {
      /* Region: Reset Status Register */
      result = dal_if->Free((void *)(handle->reset_status_register));
    ...
    {
      /* Region: PMIC */
      result = dal_if->Free((void *)(handle->pmic_power_on));
    ...
    if (handle->pm_pon_reason_history)
    {
      result = dal_if->Free((void *)(handle->pm_pon_reason_history));
    ...
    handle->pm_pon_reason_history = NULL;

    if (handle->pbus_log_data)
    {
      result = dal_if->Free((void *)handle->pbus_log_data);
...
)
{
  ...
  {
    /* Allocate handle */
    return_status = boot_config_context_get_ptr(config_context_handle, CONFIG_CONTEXT_DAL_INTERFACE, (void **)&dal_if);
    ...
    result = dal_if->Malloc(sizeof(ramdump_table_sdx_handle), (void **)&handle);
    ...
    return_status = _init_handle(config_context_handle, handle, dal_if);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
    ...
    /* Add AOSS region */
    return_status = _ramdump_add_aoss_cc_regs(config_context_handle, handle);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
    ...
    return_status = _ramdump_add_ddr_dt_log_regions (config_context_handle, dload_cookie);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
...
)
{
  ...
  do
  {
    ...
    result = dal_if->Malloc(DLOAD_DEBUG_STRLEN_BYTES, (void **)&rst_stat_filename);

    if ((result != DAL_SUCCESS) || rst_stat_filename == NULL)
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    if (rst_stat_filename)
    {
      ...
      result = dal_if->Free((void *)rst_stat_filename);
...
)
{
  ...
  do
  {
    ...
    result = dal_if->Malloc(DLOAD_DEBUG_STRLEN_BYTES, (void **)&pmic_pon_filename);

    if ((result != DAL_SUCCESS) || pmic_pon_filename == NULL)
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    result = dal_if->Malloc(DLOAD_DEBUG_STRLEN_BYTES, (void **)&pmic_hist_filename);

    if ((result != DAL_SUCCESS) || pmic_hist_filename == NULL)
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    result = dal_if->Malloc(DLOAD_DEBUG_STRLEN_BYTES, (void **)&pmic_log_data_filename);

    if ((result != DAL_SUCCESS) || pmic_log_data_filename == NULL)
    {
      return_status = GET_ERROR_CODE(BL_ERROR_GROUP_DAL, BL_ERR_MEMORY_ALLOC_FAIL);
      break;
    }
    ...
    pmic_if->pm_pbs_get_pon_reason_history(handle->pm_pon_reason_history);
    ...
    if (dload_cookie & SBL_DLOAD_MODE_BIT_MASK)
    {
      ...
      return_status = ramdump_table_add_entry(handle->ramdump_table_handle_ptr, RAMDUMP_TABLE_OPTIONAL_DEF_SAVE, (uintnt)(handle->pm_pon_reason_history), (uintnt) sizeof(uintnt), "PM PON HIST ", pmic_hist_filename);
      if (return_status != BL_ERR_NONE)
      {
        break;
      }
    ...
    if (dload_cookie & SBL_MINIDUMP_MODE_BIT_MASK)
    {
      ...
      if(!(boot_add_minidump_region(MD_SS_BOOT, pmic_hist_filename, (uint64)(handle->pm_pon_reason_history), (uint64)(sizeof(*handle->pm_pon_reason_history)))))
      {
        return GET_ERROR_CODE(BL_ERROR_GROUP_RAMDUMP_TABLE, BL_ERR_MD_ADD_REGION_FAIL)
      }
    ...
    if (pmic_pon_filename)
    {
      ...
      result = dal_if->Free((void *)pmic_pon_filename);
    ...
    if (pmic_hist_filename)
    {
      ...
      result = dal_if->Free((void *)pmic_hist_filename);
    ...
    if (pmic_log_data_filename)
    {
      ...
      result = dal_if->Free((void *)pmic_log_data_filename);
...
static bl_error_boot_type _ramdump_add_ddr_dt_log_regions(boot_handle config_context_handle, ramdump_table_sm_handle* handle, uint32 dload_cookie)
{
  bl_error_boot_type return_status = BL_ERR_NONE;
...
)
{
  ...
  do
  {
    ...
    return_status = _deinit_handle(config_context_handle, handle, dal_if);
    if (return_status != BL_ERR_NONE)
    {
      break;
    }
