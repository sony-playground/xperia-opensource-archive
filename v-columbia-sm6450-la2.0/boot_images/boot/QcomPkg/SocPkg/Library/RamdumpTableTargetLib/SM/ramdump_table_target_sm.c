...
#
#  $Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Library/RamdumpTableTargetLib/SM/ramdump_table_target_sm.c#17 $
#
#  when       who     what, where, why
#  --------   ---     ----------------------------------------------------------
#  12/02/21   rama    Added logic to copy shared IMEM region to DDR before encrypting
...
=============================================================================*/
#define TARGET_DEPENDENT_RAMDUMP_ENTRIES    200
...
)
{
  ...
  do
  {
    ...
    {
      status = GET_ERROR_CODE(BL_ERROR_GROUP_RAMDUMP_TABLE, BL_ERR_MD_ADD_REGION_FAIL);
      break;
    }
...
bl_error_boot_type ramdump_table_target_unrecoverable_error_handler (void)
{
  ...
  uint8 *external_heap_base_addr = NULL;
  ...
  do
  {
    ...
    external_heap_base_addr = sbl1_get_external_heap_addr();
    edk2_cache_if->WriteBackInvalidateDataCacheRange((void *)external_heap_base_addr, (uintnt)sbl1_get_external_heap_size());
