...
//============================================================================
/**
  Copyright (c) 2015-2021,2022 Qualcomm Technologies, Inc. All rights reserved.
...
// ===========================================================================
/**
 * @function    pm_chgr_to_EFI_chgr
 * 
 * @brief   This function converts pm charger type enum to EFI_USB charger type.
 * 
 * @param  PMIC charger type
 * 
 * @return  EFI USB charger type
 * 
 */
// ===========================================================================
EFI_USB_PWR_CTRL_HSUSB_CHGPORT_TYPE pm_chgr_to_EFI_chgr(pm_chg_charger_port_type usb_charger_type)
{
  switch (usb_charger_type)
  {
    case PM_CHARGER_PORT_SDP_CHARGER:         return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_SDP;
    case PM_CHARGER_PORT_OCP_CHARGER:         return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_OCP;
    case PM_CHARGER_PORT_CDP_CHARGER:         return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_CDP;
    case PM_CHARGER_PORT_DCP_CHARGER:         return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_DCP;
    case PM_CHARGER_PORT_FLOAT_CHARGER:       return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_FLOAT_CHARGER;
    case PM_CHARGER_PORT_QC_2P0_CHARGER:      return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_QC_2P0;
    case PM_CHARGER_PORT_QC_3P0_CHARGER:      return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_QC_3P0;
    case PM_CHARGER_PORT_TYPE_INVALID:        return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_INVALID;
    default:                                  return EFI_USB_PWR_CTRL_HSUSB_CHGPORT_UNKNOWN;
  }
}
  ...
  OUT EFI_USB_PWR_CTRL_HSUSB_CHGPORT_TYPE *ChargerPortType)
{
  ...
  *ChargerPortType = EFI_USB_PWR_CTRL_HSUSB_CHGPORT_INVALID;
  ...
  // To get usb-phy charging value from Device tree node
	dtb_status = usb_shared_xbl_dtb_node_init_phy("/sw/pmic/pm/charger");
	if(USB_DTB_STATUS_SUCCESS != dtb_status)
	{
	  usb_shared_uart_log_error("/sw/pmic/pm/charger dtb inaccessible", dtb_status);
	}
	if (usb_shared_is_usbchg_enabled())
	{
	  pm_err = (pm_err_flag_type)usb_shared_detect_charger_type(&usb_charger_type);
	  if (PM_ERR_FLAG_SUCCESS != pm_err)
	  {
		ERR_CHK("usb_shared_detect_charger_type: Error - Charger port type error");
	  }
	  *ChargerPortType = pm_chgr_to_EFI_chgr(usb_charger_type);
	  goto ON_EXIT;
	}
  ...
  if (NULL == gPwrCtrlProtocol) {
	...
	{
	  // No PMI detected
	  DBG(EFI_D_INFO, "PwrCtrlProtocol unsupported, assume SDP", Status);
	  Status = EFI_SUCCESS;
  ...
  )
{
  ...
  // Check if Platform is QRD, limit speed to HIGH
  if (PlatformInfo.platform ==  EFI_PLATFORMINFO_TYPE_QRD) {
    gHighSpeedOnlyPrimary = TRUE;
    DBG(EFI_D_WARN, "usb force high speed mode for QRD");
  }  
  //Turn off NPA resources for all the Usb Cores.
  Status = UsbConfigLibCloseNPA();
  ...
  //Check DPBonded Primary
  UefiVar = 0;
  UefiVarSize = sizeof(UefiVar);
  Status = gRT->GetVariable(L"EnableDPBondedForPri", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
  if(!EFI_ERROR(Status)) {
    DBG(EFI_D_WARN, "Skip GCC Reset for DP Bonded Mode for Primary Port via Menu DPBonded=%d", UefiVar);
    if (UefiVar != 0) {
      gUSBPriDPBonded = TRUE;
    }
  } else if (FeaturePcdGet(USBPriDPBonded)) {
    DBG(EFI_D_WARN, "Skip GCC Reset for DP Bonded Mode for Primary Port via PCD");
    gUSBPriDPBonded = TRUE;
  }

#ifdef USB_SEC_PORT_FLAG
#ifdef USB_SEC_TYPEC
  //Check DPBonded Secondary
  UefiVar = 0;
  UefiVarSize = sizeof(UefiVar);
  Status = gRT->GetVariable(L"EnableDPBondedForSec", &gQcomTokenSpaceGuid, NULL, &UefiVarSize, &UefiVar);
  if(!EFI_ERROR(Status)) {
    DBG(EFI_D_WARN, "Skip GCC Reset for DP Bonded Mode for Secondary Port via Menu DPBonded=%d", UefiVar);
    if (UefiVar != 0) {
      gUSBSecDPBonded = TRUE;
    }
  } else if (FeaturePcdGet(USBSecDPBonded)) {
    DBG(EFI_D_WARN, "Skip GCC Reset for DP Bonded Mode for Secondary Port via PCD");
    gUSBSecDPBonded = TRUE;
  }
#endif
#endif
  ...
  if(gPwrCtrlProtocol != NULL && gPwrCtrlProtocol->GetHwInfo!=NULL) {
    ...
    DBG(EFI_D_INFO, "UsbPwrCtrl No. of Ports = %d\r\n", port_hw_info.NumOfPorts);
  }
