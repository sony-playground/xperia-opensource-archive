 ...
 * @ref         
 *
 *              Copyright (c) 2013-2015,2018-2020,2021 Qualcomm Technologies Incorporated.
 *              All Rights Reserved.
...
boolean qusb_vbus_detect(void)
{
    ...
    if (is_pm_err_detected)
    {
      ...
      if (qusb_dci_gpio_cc_out_get_ss_lane() != QUSB_TYPE_C_NO_LANE)
      {
        return TRUE;
      }
#endif
	...
	if (PM_ERR_FLAG_FEATURE_NOT_SUPPORTED == pm_status)
    {
      // No PMI detected, assume Vbus present
...
// TBD, Registers not presnet in ipcat
#if 0
// Following version of controller requires 1us delay for data to be written to prevent AXI reordering
#define USB3_VER_NUMBER_170A  (0x3137302a)
#define USB3_VER_TYPE_EA      (0x65613036)

  if ( (HWIO_USB3_PRI_USB31_VER_NUMBER_IN == USB3_VER_NUMBER_170A)
    || (HWIO_USB3_PRI_USB31_VER_TYPE_IN == USB3_VER_TYPE_EA) )
  {
    static boolean print_oncer = 1;

    if (print_oncer)
    {
      print_oncer=0;
      qusb_uart_log("AXI reordering", USB3_VER_TYPE_EA);
    }
    qusb_dci_delay_us(1);
  }
#endif
...
void qusb_dci_read_enum_cookie(uint32 *cookie)
{
  *cookie = (*(uint32 *)(QUSB_SINGLE_BOOT_ENUM_ADDR));
  // TODO: Clarence PBL did not write cookie, will fix in v2, and can be remove
  if (HWIO_USB3_PRI_DCTL_INM(HWIO_USB3_PRI_DCTL_RUN_STOP_BMSK))
  {
    *cookie = QUSB_SINGLE_BOOT_ENUM_MAGIC_DL;
  }
}
// ===========================================================================
/**
