...
//============================================================================
/**
  Copyright (c) 2013-2015, 2017-2020,2021,2022 Qualcomm Technologies, Inc. All rights reserved.
...
void qusb_dci_configure_device_mode(qusb_max_speed_required_t speed_required)
{
  ...
  switch(speed_required)
  {
    ...
    {
      // Clarence QRD didn’t support USB3 hence limit speed to HS
      if (DALPLATFORMINFO_TYPE_QRD == DalPlatformInfo_Platform())
      {
        qusb_uart_log("qrd_limit_usbhs", 0);
        connect_speed = QUSB_HIGH_SPEED;
      }
      else
      {
		// Clarence has Gen1 controller support, hence limit speed to SS
...
void qusb_dci_configure_usb30(qusb_max_speed_required_t speed_required)
{
  ...
  // TBD , Registers not present in ipcat
 #if 0
...
void qusb_dci_eud_recovery(void)
{
  ...
  if (is_eud_active)
  {
    // USB3p1 is L2B for this target
    // Toggling USB 3p1 will reset EUD
    usb_shared_uart_log_error("qusb_dci_eud_recovery", PM_LDO_2);
    if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_2, PM_OFF, TRUE))
    {
      usb_shared_uart_log_error("usb_eud_is_active, disable failed ", 0);
    }
    qusb_dci_delay_ms(10);

    if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_2, PM_ON, TRUE))
    {
      usb_shared_uart_log_error("usb_eud_is_active, enable failed ", 0);
    }
  }
}
...
void qusb_dci_handle_charger_cdp(void)
{
  ...
  for (loop_count = 0; loop_count < QUSB_DCI_CHGDET_TIMEOUT_LOOP_COUNT; loop_count++)
  {
	...
	if (PM_ERR_FLAG_FEATURE_NOT_SUPPORTED == pm_err)
    {
      //no PMI detected.
      qusb_uart_log("dci, no PMI", 0);
...
void qusb_dci_typec_lane_detect(void)
{
  ...
  while (0 == typec_status.vbus_sts)
  {
	...
	if (PM_ERR_FLAG_FEATURE_NOT_SUPPORTED == pm_err)
    {
      // No PMI, force Lane1 and assume Vbus present
      qusb_dci_ss_lane = 0;
      qusb_uart_log("No PMI: forced ss_lane_1st", 0);
      return;
    }	
    if (PM_ERR_FLAG_SUCCESS != pm_err)
    {
