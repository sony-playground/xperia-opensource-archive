...
#define DDR_OPTION_DECODE_PMIC_SLAVEID 0
...
#define PMIC_VDD2H_SMPS    PM_SMPS_7
...
void ddr_target_init(boot_mode_type boot_mode)
{
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
    ...
    if((g_boot_mode == 0 )||(g_boot_mode == BOOT_MODE_RAMDUMP)) //if (PcdGet32 (PcdBuildType) == 0)
    {
      ...
      err = pm_smps_sw_mode(1, PM_SMPS_5, PM_SW_MODE_NPM); // CX SMPS5B to PWM 
      CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);
      err = pm_smps_sw_mode(1, PM_SMPS_6, PM_SW_MODE_NPM); // VDDA_EBI SMPS6B to PWM
      CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);     
    }
  }
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
	...
	err = SpmiBus_ReadLong (0, DDR_OPTION_DECODE_PMIC_SLAVEID , 0, DDR_TYPE_OFFSET , &data , 1 , &bytes_read);  //3nd is priority & 5th is the byte to read
	...
	{	
	    ...
	    if(HWIO_DDR_REG_FUSE_INPUTS_STATUS_INM(HWIO_DDR_REG_FUSE_INPUTS_STATUS_MC4_FUSE_STATUS_BMSK) == 0)
	    {
		  ...
		  /*Core hang issue Bumpup from 1.08mV to 1.1mV)*/
		  err = pm_smps_volt_level(PMIC_VDD2H,PMIC_VDD2H_SMPS, 1100 * 1000, TRUE);
		  CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);
...
boolean ddr_post_init()
{
  ...
  DalPlatformInfoPlatformType platform = DalPlatformInfo_Platform();
  if (ddr_shared_data != NULL)
  {
	memset((void*)ddr_driver_struct, 0x0, sizeof(DDR_DRIVER_STRUCT));
  }
  else
  {
	  boot_log_message("ddr_driver_struct is NULL");
	  return FALSE;
  }
  ...
  /*snprintf (ddr_log_string, DDR_LOG_LEN, "DSF version = %03d.%02d.%02d",		
			ddr_shared_data->dsf_code_version.mainline,ddr_shared_data->dsf_code_version.major,ddr_shared_data->dsf_code_version.minor); */
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
	...
	if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
	{
		...
		//  HWIO_OUTF(DPCC_MC4_GDSCR, SW_COLLAPSE, 0x1);   //collpse mc4 & in case of LP5 Dram 
		if ( (ddr_driver_struct->aop_params != NULL) && (ddr_driver_struct->aop_params->aop_pmic_settings != NULL) )
		{
			pm_status = pm_smps_volt_level(PMIC_VDD2H,PMIC_VDD2H_SMPS, (ddr_driver_struct->aop_params->aop_pmic_settings[2].values[0]) * 1000, TRUE);
			if (pm_status != PM_ERR_FLAG_SUCCESS)
			{
		...
		{
			boot_log_message("AOP Params are NULL");
			return FALSE;
		}
	}
	else{
