 ...
 * $Header: //components/dev/core.boot/6.0/aravapal.core.boot.6.0.tip_dev/QcomPkg/SocPkg/Clarence/Library/CPRTargetLib/image/core/target/clarence/cpr_image_target.c#1 $
 * $Change: 36920166 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    CPR_ASSERT(cpr_settings_clarence);

    cpr_settings_clarence->railsCount = 1;
    cpr_settings_clarence->enablementsCount = 1;
    cpr_settings_clarence->controllersCount = 1;

    cpr_settings_clarence->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_clarence->railsCount);
    for (i = 0; i < cpr_settings_clarence->railsCount; i++) {
        cpr_settings_clarence->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_clarence->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_clarence->controllersCount);
    for (i = 0; i < cpr_settings_clarence->controllersCount; i++) {
        cpr_settings_clarence->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_clarence->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_clarence->railsCount);
    for (i = 0; i < cpr_settings_clarence->railsCount; i++) {
        cpr_settings_clarence->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_clarence->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_clarence->enablementsCount);
    for (i = 0; i < cpr_settings_clarence->enablementsCount; i++) {
        cpr_settings_clarence->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_clarence->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_clarence, /*idx*/ 0);

    return cpr_settings_clarence;
}
