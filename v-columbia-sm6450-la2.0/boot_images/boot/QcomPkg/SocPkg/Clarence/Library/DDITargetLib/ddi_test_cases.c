...
uint64 ddi_get_rank (uint64* addr)
{
  ...
  {
    if (((uint64)addr >= ddr_test_region[i].start)
    && ((uint64)addr < ddr_test_region[i].end))
    {
...
*   @sa None
*/
ddi_error ddr_setrand(uint64* start, uint64* end)
{
  uint64 *ptr = start;
  uint64 i;
...
*   @sa None
*/
ddi_error ddr_setrandstride(uint64* start, uint64* end, uint64 rand_stride, uint64 random_seed)
{
  uint64 *ptr = start;
  uint64 i, rand_idx=0;
...
*   @sa None
*/
ddi_error ddr_cmprand(uint64* start, uint64* end, ddi_cmp_params* cmp_params)
{
  ddi_error ret = DDI_CMD_NOT_COMPLETE;
  uint64 *ptr = start;
  uint64 i;
...
*   @sa None
*/
ddi_error ddr_cmprandstride(uint64* start, uint64* end, uint64 rand_stride, uint64 random_seed, ddi_cmp_params* cmp_params)
{
  ddi_error ret = DDI_CMD_NOT_COMPLETE;
  uint64 *ptr = start;
  uint64 i,rand_idx=0;
...
*   @sa None
*/
ddi_error ddr_op_rand(uint64* start, uint64* end, uint64 rand_stride, uint64 random_seed, ddi_cmp_params* cmp_params)
{
  if (cmp_params)
{
    if (rand_stride && (rand_stride < (end - start)))
    {
    return ddr_cmprandstride(start, end, rand_stride, random_seed, cmp_params);
    }
    else
    {
    return ddr_cmprand(start, end, cmp_params);
    }
  }
  else
  {
    if (rand_stride && (rand_stride < (end - start)))
    {
      return ddr_setrandstride(start, end, rand_stride, random_seed);
    }
    else
    {
      return ddr_setrand(start, end);
  }
  }
}
...
ddi_error ddr_op_read(uint64* start, uint64* end)
{
  ...
  while (ptr < end)
  {
    ...
    #if DDI_PRINT_ENABLE
    snprintf(ddi_log_string, sizeof(ddi_log_string), " read:addr=0x%016lx", (uint64)ptr);
...
*   @sa None
*/
ddi_error ddr_mem_op(uint64* start, uint64* end, boolean invert_row, boolean rand, uint64 rand_stride, uint64 random_seed, uint64* pattern, uint64 pattern_size, ddi_mem_op_type op_type, ddi_cmp_params* cmp_params)
{
  ...
  if (op_type == DDI_MEM_OP_RD) {
    ...
    return ddr_op_rand(start, end, rand_stride, random_seed, cmp_params);
  }
...
*   @sa None
*/
ddi_error ddi_mem_op(boolean invert_row, boolean random, uint64 random_stride, uint64 random_seed, uint64* pattern, uint64 pattern_size, ddi_mem_op_type op_type, ddi_cmp_params* cmp_params)
{
  uint8 i;
  ddi_error err = DDI_CMD_NOT_COMPLETE;
  for (i = 0; i < num_ddr_test_region; i++)
{
    err |= ddr_mem_op((uint64*)ddr_test_region[i].start, (uint64*)ddr_test_region[i].end, invert_row, random, random_stride, random_seed, pattern, pattern_size, op_type, cmp_params);
  }
  return err;
}
...
*   @sa None
*/
ddi_error ddi_mem_set(boolean invert_row, boolean random, uint64 random_stride, uint64 random_seed, uint64* pattern, uint64 pattern_size)
{
  return ddi_mem_op(invert_row, random, random_stride, random_seed, pattern, pattern_size, DDI_MEM_OP_WR_CMP, NULL);
}
...
*   @sa None
*/
ddi_error ddi_mem_cmp(boolean invert_row, boolean random, uint64 random_stride, uint64 random_seed, uint64* pattern, uint64 pattern_size, ddi_cmp_params* cmp_params)
{
  return ddi_mem_op(invert_row, random, random_stride, random_seed, pattern, pattern_size, DDI_MEM_OP_WR_CMP, cmp_params);
}
...
*   @sa None
*/
ddi_error ddi_mem_read(boolean random, uint64 random_stride, uint64* pattern, uint64 pattern_size)
{
  return ddi_mem_op(FALSE, random, random_stride, 0, pattern, pattern_size, DDI_MEM_OP_RD, NULL);
}
  ...
  *output_iter += sizeof(ddi_output_header);
  ...
  do
  {
    ...
    for (uint64 i = 0; i < repeat_count; i++)
    {
      ...
      boot_log_message(ddi_log_string);
      #endif
      ret |= ddi_mem_read(FALSE, 0, NULL, 0);
    }
  ...
  *output_iter += sizeof(ddi_output_header);
  ...
  do
  {
    ...
    for (uint64 i = 0; i < repeat_count; i++)
	{
      ...
      boot_log_message(ddi_log_string);
      #endif
      ret |= ddi_mem_set(invert_row, random, random_stride, random_seed, pattern, pattern_size);
    }
  ...
  *output_iter += sizeof(ddi_output_header)+ sizeof(ddi_rdwr_output);
  ...
  do
  {
    ...
    if(cache==1)
    {
      ...
      {
        ArmSetMemoryAttributes(ddr_driver_struct->ddr_regions_info.ddr_region[i].start_address, ddr_driver_struct->ddr_regions_info.ddr_region[i].size,EFI_MEMORY_WB);
      }
    }
    ...
    for (uint64 i = 0; i < repeat_count; i++)
    {
      ...
      if(cache==1)
      {
        for (uint64 i=0; i<num_ddr_test_region; i++)
        {
          for (uint64 j=ddr_test_region[i].start; j<ddr_test_region[i].end; j+=0x40)
          {
            ArmCleanInvalidateDataCacheEntryByMVA(j);
          }
        }
      ...
      if (verify)
      {
        ...
        swsys_srand_uefi(random_seed);
        ret |= ddi_mem_cmp(invert_row, random, random_stride, random_seed, pattern, pattern_size, &cmp_params);
      }
      else
      {
        ret |= ddi_mem_read(random, random_stride, pattern, pattern_size);
      }
...
#define ECDT_VOLTAGE_RAIL_LIMIT 7
