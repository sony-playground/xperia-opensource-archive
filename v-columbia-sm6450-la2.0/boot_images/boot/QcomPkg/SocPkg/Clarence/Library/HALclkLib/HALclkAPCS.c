...
)
{
  ...
  switch (eCore)
  {
    ...
    case HAL_CLK_APCS_CPU_CLUSTER0_C4:
	  HWIO_OUTFI(EPSSSLOW_CLKDOM1_COREd_L3_VOTING, 4, VC, nVcIndex);
      break;
      
    case HAL_CLK_APCS_CPU_CLUSTER0_C5:
	  HWIO_OUTFI(EPSSSLOW_CLKDOM1_COREd_L3_VOTING, 5, VC, nVcIndex);
      break;
...
)
{
  ...
  switch(eCore)
  {
    case HAL_CLK_APCS_CPU_CLUSTER0:
      ...
      nAddress = HWIO_ADDRI(EPSSSLOW_CLKDOM1_COREd_PERF_STATE_DESIRED, 4);
      nValue   = HWIO_FVAL(EPSSSLOW_CLKDOM1_COREd_PERF_STATE_DESIRED, SW_INDEX, nIndex);
      /* Program the hardware. */
      outpdw(nAddress, nValue);

      nAddress = HWIO_ADDRI(EPSSSLOW_CLKDOM1_COREd_PERF_STATE_DESIRED, 5);
      nValue   = HWIO_FVAL(EPSSSLOW_CLKDOM1_COREd_PERF_STATE_DESIRED, SW_INDEX, nIndex);
      /* Program the hardware. */
      outpdw(nAddress, nValue);
...
)
{
  ...
  HAL_clk_EPSS_SET_L3_PERF_LEVEL( HAL_CLK_APCS_CPU_CLUSTER0_C4, 0);
  HAL_clk_EPSS_SET_L3_PERF_LEVEL( HAL_CLK_APCS_CPU_CLUSTER0_C5, 0);
