...
----------   ---     -----------------------------------------------------------
2023-01-25   ss      Added pstm clients for SD card voting in multicard tray removal
2022-09-11   ssb     SD card rails ON/OFF support.
2022-07-27   ssb     Reverted back speed mode to default.
2022-07-11   ssb     Reorganised Sd card speed mode to SDR104
...
/* New PSTM nodes added to be used for Voting for LDO as part of Multicard Tray Removal FR */
static npa_client_handle sd_mmc_vdd_pstm_handle = NULL;
static npa_client_handle sd_mmc_vdd_px2_pstm_handle = NULL;
...
sdcc_bsp_err_type sdcc_bsp_get_vreg_switch_status (UINT32 driveno)
{
   if (driveno == SDCC_BSP_SLOT_1) {
	  ...
	  /* New PSTM handle introduced by PMIC as part of Multicard Tray Removal FR */
	  if (sd_mmc_vdd_px2_pstm_handle == NULL)
	  {
		 sd_mmc_vdd_px2_pstm_handle = npa_create_sync_client(PMIC_NPA_GROUP_ID_SD_MMC_VDD_PX2_PSTM, "sd_mmc_vdd_px2_pstm", NPA_CLIENT_REQUIRED);
      }
   ...
   if (sd_mmc_vdd_px2_handle == NULL || sd_mmc_vdd_px2_pstm_handle == NULL) {
      return SDCC_BSP_ERR_PMIC;
   }
   /* Issue the Requests to the corresponging PSTM handles */
   npa_issue_required_request(sd_mmc_vdd_px2_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
   npa_issue_required_request(sd_mmc_vdd_px2_pstm_handle, 0x1);
...
{ 
   ...
   if (sd_mmc_vdd_px2_pstm_handle == NULL || sd_mmc_vdd_px2_handle == NULL) {
	   return SDCC_BSP_ERR_PMIC;
    }
   /* Issue the Request to set the LDO at 1.9V */
...
sdcc_bsp_err_type sdcc_bsp_volt_set (UINT32 driveno, sdcc_bsp_regulator_mode mode)
{
   ...
   if (driveno == SDCC_BSP_SLOT_1)
   {
      if (sd_mmc_vdd_handle == NULL) 
      {
		 ...
		 if (sd_mmc_vdd_pstm_handle == NULL)
         {
            sd_mmc_vdd_pstm_handle = npa_create_sync_client(PMIC_NPA_GROUP_ID_SD_MMC_VDD_PSTM, "sd_mmc_vdd_pstm", NPA_CLIENT_REQUIRED);
            if (sd_mmc_vdd_pstm_handle == NULL)
            {
               return SDCC_BSP_ERR_PMIC;
            }
      ...
      {
		 /* Issue the request LDO with the Voltage required if not done already */
         npa_issue_required_request(sd_mmc_vdd_handle, PMIC_NPA_MODE_ID_GENERIC_ACTIVE);
		 npa_issue_required_request(sd_mmc_vdd_pstm_handle, 0x1);
         npa_issue_required_request(sd_mmc_vdd_px2_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
		 npa_issue_required_request(sd_mmc_vdd_px2_pstm_handle, 0x1);
		 volt_set_done = TRUE;    
      }
      else if (mode == SDCC_BSP_REGULATORS_OFF) 
      {
		 ...
		 npa_issue_required_request(sd_mmc_vdd_pstm_handle, PMIC_NPA_MODE_ID_GENERIC_OFF);
         npa_issue_required_request(sd_mmc_vdd_px2_handle, PMIC_NPA_MODE_ID_GENERIC_OFF);
		 npa_issue_required_request(sd_mmc_vdd_px2_pstm_handle, PMIC_NPA_MODE_ID_GENERIC_OFF);
		 volt_set_done = FALSE;   
      }
      else
      {
