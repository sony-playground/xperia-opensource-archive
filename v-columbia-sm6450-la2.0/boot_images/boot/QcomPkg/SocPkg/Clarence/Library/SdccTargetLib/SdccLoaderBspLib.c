...
----------   ---     -----------------------------------------------------------
2023-02-28   ss      Added Support for turning the LDO off in Loader Path
2022-11-14   ssb     Updated Slave ID on sdcc_bsp_volt_set()
2022-09-22   vm      Vote for correct LDO VDD_PX2
2022-09-11   ssb     SD card rails ON/OFF support.
2022-07-27   ssb     Reverted back speed mode to default.
2022-06-29   ssb     Reorganised Sd card speed mode to SDR104 and updated LDO as per ipcat.
2022-04-22   ssb     Updated Speed mode values as per DT
2022-04-15   ssb     Updated DT storage node under soc
2022-04-15   ssb     Add support for sdcc bootup using Device Tree Config
...
2020-04-28   jt      Clarence updates
	   ...
	   if (NULL != fdt_handle) 
        {
            ...
            {
               snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings/HS400", slot_num);
            }
                else if (SDCC_HS200_MODE == speed_mode)
            {
               snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/soc/storage/sdc%d/HsrSettings/HS200", slot_num);
            }
            else 
            {
   ...
   if (0 == result) 
   {
       ...
       if (0 == result) 
       {
           ...
           if (0 == result) 
           {
               if (slot_num == SDCC_BSP_SLOT_0) 
               {
                   base_addr = HWIO_SDC1_SDCC5_HC_ADDR;
               }
			   else if (slot_num == SDCC_BSP_SLOT_1)
			   {
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_0:
	    ...
	    DriveStrength[1] = SDC1_BSP_DEFAULT_DRIVE_STRENGTH_CTL1;
         ...
         if (NULL != fdt_handle) 
         {
             ...
             if (0 == fdt_get_uint32_prop_list (&node, "DriveStrength", DriveStrength, sizeof(DriveStrength)))
             {
                 #ifndef FEATURE_DISABLE_DT_LOGGING
                 if (enable_dt_logging) 
                 {
                     char log_str[SDCC_BSP_DT_LOG_STR_LEN] = {0};
                     snprintf(log_str, SDCC_BSP_DT_LOG_STR_LEN, "SDCC Drive Strength CTL-0 Setting from Device Tree: 0x%x\n", 
                              DriveStrength[0]);
                     snprintf(log_str, SDCC_BSP_DT_LOG_STR_LEN, "SDCC Drive Strength CTL-1 Setting from Device Tree: 0x%x\n", 
                              DriveStrength[1]); 
                     boot_log_message(log_str);
                 }
                 #endif
      ...
      case SDCC_BSP_SLOT_1:
        DriveStrength[0] = SDC2_BSP_DEFAULT_DRIVE_STRENGTH_CTL;
         ...
         {
             fdt_get_uint8_prop (&node, "EnableLogging", &enable_dt_logging);
             if (0 == fdt_get_uint32_prop (&node, "DriveStrength", &DriveStrength[0]))
             {
                 #ifndef FEATURE_DISABLE_DT_LOGGING
                 if (enable_dt_logging) 
                 {
...
#if defined(SDCC_TARGET_RAMDUMP_IMAGE) || defined(FEATURE_DEVICEPROGRAMMER_IMAGE)

      if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_28, PM_ON, TRUE))
      {
...
sdcc_bsp_err_type sdcc_bsp_lower_volt_level (UINT32 driveno)
{
   if (SDCC_BSP_SLOT_1 == driveno)
   {
      ...
      if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_B, PM_LDO_28, 1800 * 1000, TRUE))
      {
         return SDCC_BSP_ERR_PMIC; 
      }
#endif
...
{ 
   ...
   if (driveno == SDCC_BSP_SLOT_1 && mode == SDCC_BSP_REGULATORS_ON && volt_set_done != TRUE)
   {
   
#ifdef FEATURE_DEVICEPROGRAMMER_IMAGE

// Refer ipcat to update it for different Target,
// Bompoka   [PM6450]   B 1
#define LDO24_VSET_ADDR 0xd840 //SD Card Vdd Regulator register address
#define LDO24_EN_CTL_ADDR 0xd846 //LDO24 enable control register address
#define LDO24_VSET_LB 0xd840   //SD Card Vdd Regulator register address lower byte
#define LDO24_VSET_UB 0xd841   //SD Card Vdd Regulator register address upper byte
#define LDO24_VSET_SIZE 2       //SD Card Vdd Regulator register size
#define LDO24_EN_CTL_SIZE 1       //SD Card Vdd Regulator register size

#define LDO28_VSET_ADDR 0xdc40 //SD Card Vdd_px2 Regulator register address
#define LDO28_EN_CTL_ADDR 0xdc46 //LDO28 enable control register address
#define LDO28_VSET_LB 0xdc40   //SD Card Vdd_px2 Regulator register address lower byte
#define LDO28_VSET_UB 0xdc41   //SD Card Vdd_px2 Regulator register address upper byte
#define LDO28_VSET_SIZE 2       //SD Card Vdd_px2 Regulator register size
#define LDO28_EN_CTL_SIZE 1       //SD Card Vdd_px2 Regulator register size

       uint8_t data1[LDO24_EN_CTL_SIZE] = {0x0}; // disable LDO24
       if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x1 /* Bompoka   [PM6450]   B 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO24_EN_CTL_ADDR, data1, LDO24_EN_CTL_SIZE))
       {
         return SDCC_BSP_ERR_PMIC;
       }
       else
       {
         uint8_t data[LDO24_VSET_SIZE] = {0x90,0x0b}; //2960 mili volt in hex
         if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x1 /*Bompoka   [PM6450]   B 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO24_VSET_ADDR, data, LDO24_VSET_SIZE))
         {
           return SDCC_BSP_ERR_PMIC;
         }

         uint8_t data2[LDO24_EN_CTL_SIZE] = {0x80}; // Enable LDO24
         if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x1 /*Bompoka   [PM6450]   B 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO24_EN_CTL_ADDR, data2, LDO24_EN_CTL_SIZE))
         {
            return SDCC_BSP_ERR_PMIC;
         }
       }
   
       uint8_t data3[LDO28_EN_CTL_SIZE] = {0x0}; // disable LDO28
       if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x1 /*Bompoka   [PM6450]   B 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO28_EN_CTL_ADDR, data3, LDO28_EN_CTL_SIZE))
       {
         return SDCC_BSP_ERR_PMIC;
       }
       else
       {
         uint8_t data4[LDO28_VSET_SIZE] = {0x86,0x0b}; //2950 mili volt in hex
         if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x1 /*Bompoka   [PM6450]   B 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO28_VSET_ADDR, data4, LDO28_VSET_SIZE))
         {
           return SDCC_BSP_ERR_PMIC;
         }

         uint8_t data5[LDO28_EN_CTL_SIZE] = {0x80}; // Enable LDO28
         if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x1 /*Bompoka   [PM6450]   B 1*/, SPMI_ACCESS_PRIORITY_LOW, LDO28_EN_CTL_ADDR, data5, LDO28_EN_CTL_SIZE))
         {
            return SDCC_BSP_ERR_PMIC;
         }
       }

#elif SDCC_TARGET_RAMDUMP_IMAGE

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_24, PM_OFF, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_28, PM_OFF, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_B, PM_LDO_24, 2960 * 1000, TRUE))
       {
              return SDCC_BSP_ERR_PMIC;
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_B, PM_LDO_28, 2950 * 1000, TRUE))
       {
              return SDCC_BSP_ERR_PMIC;
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_B, PM_LDO_24, PM_ON, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }
