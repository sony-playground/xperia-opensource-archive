...
#define LEICA2_I2C_INSTANCE  I2C_INSTANCE_009
#define LEICA2_1_RESET_GPIO  29
#define LEICA2_2_RESET_GPIO  30
/*===========================================================================
...
pm_err_flag_type pm_post_pmic_initialization(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  {
    DEBUG(( EFI_D_ERROR, "Leica2 PMIC Init: Failed to locate PlatformInfo Protocol, Status =  (0x%x)\r\n", Status ));
    CORE_VERIFY(0);
  }
  ...
  if( !pPlatformInfoProtocol )
  {
    ...
    DEBUG(( EFI_D_ERROR, "Leica2 PMIC Init: Failed pPlatformInfoProtocol = NULL "));
    CORE_VERIFY(0);
  }
  ...
  {
    /*Two Leica2 PMICs with default SID as 0x8 and 0x9, So to communicate with them we are re-configuring one of Leica2 PMIC I2C SID to 0xC and 0xD
      To update only One Leica2 I2C SID, we are keeping One Leica2 Enabled and other in RESET state.
      So after this API, The Leica2 in RESET will be PM8008i with 0x8 & 0x9 as SID. and Enabled will be PM8008j with I2C SID 0xC & 0xD */
  ...
  *applicable = FALSE;
  ...
  if( ((platform_type == DALPLATFORMINFO_TYPE_MTP)
    ...
    || (platform_type == DALPLATFORMINFO_TYPE_HDK)) &&
       ((pmic_model == PMIC_IS_PM7250B) || (pmic_model == PMIC_IS_PMI632)) ) //tioman or nebula
  {
    *applicable = TRUE;
  }
...
static pm_err_flag_type pm_i2c_sid_config(void)
{
  ...
  uint8  packet_data[] = {0x06, 0x56, 0x03};      /* peripheral id, register address(SPMI_I2C_SID_LEGACY), register value */
  ...
  if (Status == EFI_SUCCESS)
  {
    config_LEA = EFI_GPIO_CFG( LEICA2_1_RESET_GPIO, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA );
    Status = TLMMProtocol->ConfigGpio((UINT32)config_LEA, TLMM_GPIO_ENABLE);
    if (Status != EFI_SUCCESS)
    {
      pm_log_message("*** Unable to Configure MSM GPIO 29! ***\n");
	  return PM_ERR_FLAG_FAILURE;
    }
  }
  ...
  if (Status == EFI_SUCCESS)
  {
    config_LEB = EFI_GPIO_CFG( LEICA2_2_RESET_GPIO, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA );
    Status = TLMMProtocol->ConfigGpio((UINT32)config_LEB, TLMM_GPIO_ENABLE);
    if (Status != EFI_SUCCESS)
    {
      pm_log_message("*** Unable to Configure MSM GPIO 30! ***\n");
	  return PM_ERR_FLAG_FAILURE;
    }
  }

  /* Drive LEICA2_A EN pin low while keeping LEICA2_B EN pin high */
  if (Status == EFI_SUCCESS)
  {
    Status = TLMMProtocol->GpioOut(config_LEA, GPIO_LOW_VALUE);
    if (Status != EFI_SUCCESS)
    {
      pm_log_message("*** Unable to drive Leica2_A EN pin low ***\n");
	  return PM_ERR_FLAG_FAILURE;
    }
  }
  if (Status == EFI_SUCCESS)
  {
    ...
    if (Status != EFI_SUCCESS)
    {
      pm_log_message("*** Unable to drive Leica2_B EN pin high ***\n");
	  return PM_ERR_FLAG_FAILURE;
    }
  }

  gBS->Stall(1000); /* Wait for LEICA2 to power up, 1mS */
  ...
  istatus = i2c_open(LEICA2_I2C_INSTANCE, &i2c_handle);
