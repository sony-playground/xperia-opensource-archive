...
/*
  ...
  ====================================================================
  $Header: //components/dev/core.boot/6.0/aravapal.core.boot.6.0.tip_dev/QcomPkg/SocPkg/Clarence/Library/ClockLib/ClockImageBIST.c#1 $
  ...
  ClockIdType nClockId;
   ...
   ULOG_RT_PRINTF_1(pDrvCtxt->hClockBISTLog, "\tTesting CPU %d", eCPU);
    for (nFreqLevel=MIN_PERF_LEVEL; nFreqLevel< nPlanLen; nFreqLevel++){
        if (DAL_SUCCESS == ClockApps_DetectCPUFrequency (eCPU, &nLastActiveFreq)){
            nRequestedFreq = aFreqPlan[nFreqLevel].nFreqHz;
            if (DAL_SUCCESS == ClockApps_SetCPUFrequency (eCPU, nRequestedFreq, &nAchievedFreq)){
                busywait(100);
                if (DAL_SUCCESS == ClockApps_DetectCPUFrequency (eCPU, &nCurrentActiveFreq)){
...
)
{
  ...
  ULOG_RT_PRINTF_0(pDrvCtxt->hClockBISTLog, "Running Image BIST:");
    for (eCPU = CLOCK_APPS_CPU_C0; eCPU < CLOCK_APPS_CPU_TOTAL; eCPU++){
        if (DAL_SUCCESS == ClockApps_GetCPUFrequencyLevels (eCPU, &aFreqLevels[eCPU])){
            //memset(aFreqPlan, 0x0,sizeof (aFreqPlan))
            if (DAL_SUCCESS == ClockApps_GetCPUFrequencyPlan(eCPU, aFreqPlan[eCPU], aFreqLevels[eCPU])){
