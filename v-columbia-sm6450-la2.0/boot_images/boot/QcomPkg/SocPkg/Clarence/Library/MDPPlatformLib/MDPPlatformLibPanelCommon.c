...
MDP_Status Panel_Default_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    {
      /* AMLED DVDD Pin */     
      if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_DIG_OUT))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: config PMIC_I gpio12 failed!\n"));
      }
      if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_VIN0))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 voltage failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_LEVEL_HIGH))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 out put level failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_SRC_GND))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 out src cfg failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_OUT_DRV_STR_HIGH))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 outdrv str failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 out buf cfg failed!\n"));
      }

      /* AMOLED VDDIO Pin*/
      if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_DIG_OUT))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: config PMIC_I gpio11 failed!\n"));
      }
      if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_VIN0))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio11 voltage failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_LEVEL_HIGH))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio11 out put level failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_SRC_GND))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio11 out src cfg failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_OUT_DRV_STR_HIGH))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio11 outdrv str failed!\n"));
      }
      else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
      {
        DEBUG((EFI_D_WARN, "DisplayDxe: set gpio11 out buf cfg failed!\n"));
      }
    }
...
MDP_Status Panel_Default_PowerDown(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    /* AMOLED DVDD Pin */ 
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_I, EFI_PM_GPIO_12, EFI_PM_GPIO_LEVEL_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 out put level failed!\n"));
    }
    
    /* AMOLED VDDIO Pin*/
    if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_LEVEL_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio12 out put level failed!\n"));
    }
...
/**********************************************************************************************
*
* FUNCTION: Panel_Default_Peripheral_PowerUp()
*
* DESCRIPTION:
*   Secondary power on sequence for other PMIC modules such as IBB/LAB.
*
***********************************************************************************************/
MDP_Status Panel_Default_Peripheral_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  MDP_Status                     Status                    = MDP_STATUS_OK;
  EFI_TLMM_PROTOCOL             *TLMMProtocol              = NULL;
  MDPPlatformPanelFunctionTable *pMDPSupportPanelFunctions = MDP_GET_PANELFUNC(eDisplayId);
  ...
  {
    DEBUG((EFI_D_ERROR, "DisplayDxe: Locate get pmic LCDB protocol failed!\n"));
    Status = MDP_STATUS_NO_RESOURCES;
  }
  else
  {
    ...
    if (EFI_SUCCESS != PmicLCDBProtocol->SetOutput(PMIC_C, Default_DISP_LCD_IBBLAB_MV, -Default_DISP_LCD_IBBLAB_MV, TRUE))
    {
    ...
    {
      DEBUG((EFI_D_ERROR, "DisplayDxe: Error to enable LCDBc\n"));
    }

    /* LCD VDDIO Pin*/
    if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_DIG_OUT))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: config PMIC_C gpio5 failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_VIN0))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 voltage failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_LEVEL_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 out src cfg failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_OUT_DRV_STR_MEDIUM))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 out buf cfg failed!\n"));
    }

    /* LCD DDIC Pin*/
    if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_DIG_OUT))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: config PMIC_C gpio6 failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_VIN0))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 voltage failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_LEVEL_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out src cfg failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_OUT_DRV_STR_MEDIUM))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out buf cfg failed!\n"));
    }
...
/**********************************************************************************************
*
* FUNCTION: Panel_Default_Peripheral_PowerDown()
*
* DESCRIPTION:
*   Secondary power off sequence for other PMIC modules such as IBB/LAB.
*
***********************************************************************************************/
MDP_Status Panel_Default_Peripheral_PowerDown(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  MDP_Status                     Status                    = MDP_STATUS_OK;
  EFI_TLMM_PROTOCOL             *TLMMProtocol              = NULL;
  MDPPlatformPanelFunctionTable *pMDPSupportPanelFunctions = MDP_GET_PANELFUNC(eDisplayId);
  ...
  else
  {
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_LEVEL_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_SRC_GND))
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_OUT_DRV_STR_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio5 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_C, EFI_PM_GPIO_5, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_LEVEL_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_SRC_GND))
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_OUT_DRV_STR_LOW))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_C, EFI_PM_GPIO_6, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
...
MDP_Status Panel_LCD_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set Default_DISP_LCD_BIAS_EN line high failed!\n"));
    }
...
MDP_Status Panel_LCD_PowerDown(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_LEVEL_LOW))
    {
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_I, EFI_PM_GPIO_11, EFI_PM_GPIO_OUT_DRV_STR_LOW))
    {
    ...
    if (EFI_SUCCESS != TLMMProtocol->GpioOut((UINT32)EFI_GPIO_CFG(Default_DISP_LCD_BIAS_EN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA), GPIO_LOW_VALUE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set Default_DISP_LCD_BIAS_EN line high failed!\n"));
    }
    
    if (EFI_SUCCESS != TLMMProtocol->ConfigGpio((UINT32)EFI_GPIO_CFG(Default_DISP_LCD_BIAS_EN, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_16MA), TLMM_GPIO_DISABLE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Configure GPIO %d for Default_DISP_LCD_BIAS_EN failed!\n", Default_DISP_LCD_BIAS_EN));
    }
...
MDP_Status Panel_Default_Brightness_Enable(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  {
    /* LCD WLED Enable Pin */
    if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_B, EFI_PM_GPIO_6, EFI_PM_GPIO_DIG_OUT))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: config PMIC_B gpio6 failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_B, EFI_PM_GPIO_6, EFI_PM_GPIO_VIN0))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 voltage failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_B, EFI_PM_GPIO_6, EFI_PM_GPIO_LEVEL_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_B, EFI_PM_GPIO_6, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out src cfg failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_B, EFI_PM_GPIO_6, EFI_PM_GPIO_OUT_DRV_STR_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_B, EFI_PM_GPIO_6, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio6 out buf cfg failed!\n"));
    }
    ...
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: config pmic_b gpio7 failed!\n"));
    }
