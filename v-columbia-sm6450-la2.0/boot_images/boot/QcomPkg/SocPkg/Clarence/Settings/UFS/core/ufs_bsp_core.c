...
$Header: //components/dev/core.boot/6.0/aravapal.core.boot.6.0.tip_dev/QcomPkg/SocPkg/Clarence/Settings/UFS/core/ufs_bsp_core.c#1 $
$DateTime: 2022/04/30 21:50:34 $

YYYY-MM-DD   who     what, where, why
---------------------------------------------------------------------- 
2022-05-27   rn      Updates for Clarence 
2022-04-26   ssb     Updated DT storage node under soc
2022-03-22   ss      Add ability to read Vcc voltage
...
{
   {
      ...
      {"gcc_ufs_phy_ice_core_clk", 0, 0, 0, 100000000, 200000000, 403000000, 403000000}, 
...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_regulator_mode mode)
{
   ...
   if (mode == UFS_BSP_REGULATORS_ON) {
      ...
      // PMIC_E, PM_LDO_5 is Clarence target specific UFS Device Vcc regulator
     // Refer ipcat to update it for different Target.	 	 
     // Leica2 [PM8010]    E 4	 	 
	
      Status=PmicVRegProtocol->VregGetLevel(0x4 /*Leica2 [PM8010] E 4*/, EFI_PM_LDO_5, &voltage_req_rail);
      if (Status != EFI_SUCCESS) {
         return UFS_BSP_ERR_PMIC;  
      }
