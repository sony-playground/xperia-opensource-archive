    ...
    do{
      switch(eCore)
      {
        ...
        case HAL_CLK_APCS_CPU_CLUSTER2: 
          bDCVSInProgress = HWIO_INF(EPSSSLOW_CLKDOM3_DCVS_IN_PROGRESS, STATUS);
          break;
...
)
{
  ...
  switch (eCore)
  {
    ...
    case HAL_CLK_APCS_CPU_CLUSTER2_C0:
      HWIO_OUTFI(EPSSSLOW_CLKDOM3_COREd_L3_VOTING, 0, VC, nVcIndex);
      break;
...
)
{
  ...
  HAL_clk_EPSS_SET_L3_PERF_LEVEL( HAL_CLK_APCS_CPU_CLUSTER2_C0, 0);
...
)
{
  ...
  HWIO_OUTF(VCODEC_VIDEO_CC_MVS0C_DIV_DCDR, DCD_ENABLE, 0x1);
