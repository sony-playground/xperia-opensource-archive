...
bool HAL_clk_xbl_ZondaEvoV2PLLConfigPLL      ( HAL_clk_xbl_SourceDescType *pSource, const HAL_clk_xbl_PLLConfigType *pmConfig, HAL_clk_xbl_SourceConfigMode eMode );
...
void HAL_clk_xbl_PongoEvoPLLWaipioSetRegSettings ( HAL_clk_xbl_SourceDescType *pSource, const HAL_clk_xbl_SourceRegSettingsType *pmSettings);
...
HAL_clk_xbl_SourceControlType HAL_clk_xbl_PongoevoWaipioPLLControl =
{
  ...
  .SetRegSettings   = HAL_clk_xbl_PongoEvoPLLWaipioSetRegSettings,
};
...
)
{
   ...
   * For Waipio V2: The internal PLL L value register is treated as don't care.
   * We need to write to the external register.
   */
  HAL_CLK_PLL_HWIO_OUT(PLL_L_VAL, pSource, pConfig->nL);
  
  /*
   * Waipio V2 CPU Zonda PLLs get their L values from an external register
   * outside the PLL.
   * HW Cr: CR#QCTDD07083928
   */
  if (Clock_GetChipFamily()  == CHIPINFO_FAMILY_WAIPIO &&
      Clock_GetChipVersion() >= CHIPINFO_VERSION(2, 0))
  {
...
void HAL_clk_xbl_PongoEvoPLLWaipioSetRegSettings
(
  HAL_clk_xbl_SourceDescType              *pSource,
