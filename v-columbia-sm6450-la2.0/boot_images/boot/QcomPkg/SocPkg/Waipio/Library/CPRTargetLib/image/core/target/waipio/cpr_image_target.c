 ...
 * $Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Waipio/Library/CPRTargetLib/image/core/target/waipio/cpr_image_target.c#10 $
 * $Change: 34685879 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    cpr_settings *cpr_settings_8350 =
        (cpr_settings*)cpr_image_malloc(sizeof(cpr_settings));
    CPR_ASSERT(cpr_settings_8350);

    cpr_settings_8350->railsCount = 2;
    cpr_settings_8350->enablementsCount = 2;
    cpr_settings_8350->controllersCount = 2;

    cpr_settings_8350->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_8350->railsCount);
    for (i = 0; i < cpr_settings_8350->railsCount; i++) {
        cpr_settings_8350->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_8350->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_8350->controllersCount);
    for (i = 0; i < cpr_settings_8350->controllersCount; i++) {
        cpr_settings_8350->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_8350->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_8350->railsCount);
    for (i = 0; i < cpr_settings_8350->railsCount; i++) {
        cpr_settings_8350->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_8350->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_8350->enablementsCount);
    for (i = 0; i < cpr_settings_8350->enablementsCount; i++) {
        cpr_settings_8350->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_8350->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_8350, /*idx*/ 0);

    cpr_rail mm_rail;
    mm_rail.id = CPR_RAIL_MM;
    cpr_smem_deserialize_settings(&mm_rail, cpr_settings_8350, /*idx*/ 1);

    return cpr_settings_8350;
}
