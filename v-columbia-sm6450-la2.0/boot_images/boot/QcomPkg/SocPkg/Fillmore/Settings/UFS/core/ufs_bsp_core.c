...
$Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Fillmore/Settings/UFS/core/ufs_bsp_core.c#13 $
$DateTime: 2022/03/15 13:02:23 $

YYYY-MM-DD   who     what, where, why
----------------------------------------------------------------------
2022-02-10   cm      Add max freq support
2021-11-28   vm      Add ability to read Vcc voltage
2021-01-11   vm      Skip device tree config read for RUMI
2021-09-08   jt      Add ability to check battery status 
2021-08-13   jt      Add device tree support
2021-02-03   gml     Add reference clock
...
{ 
	...
	{ VOLT_CORNER_TURBO,    UFS_GEAR_4,       { 300, 12,  403}}
};
...
{
   {
      {"gcc_ufs_phy_axi_clk", 0, 0, 0, 75000000, 150000000, 300000000, 403000000}, 
      {"gcc_ufs_phy_ice_core_clk", 0, 0, 0, 75000000, 150000000, 300000000, 600000000}, 
      {"gcc_ufs_phy_unipro_core_clk", 0, 0, 0, 75000000, 150000000, 300000000, 600000000},
      ...
      {"gcc_ufs_0_clkref_en", 0, 0, 0, 0, 0, 0, 0}
   } 
};
...
void ufs_bsp_autoH8 (uint8_t hostid, uint16_t idle_time_ms)
{
   if (hostid == 0) {
      ...
      HWIO_REG_OUT(ufs_bsp_get_reg_baseaddress(hostid), UFS_MEM_AHIT_OFFS, (0xc00 | (idle_time_ms & 0x3ff)));
   }
}
...
ufs_bsp_err_type ufs_bsp_update_maxclk_seq(uint8_t hostid, uint8_t clk_scale)
{
	  return UFS_BSP_NO_ERROR; 
}
...
void ufs_bsp_get_default_config (uint8_t hostid, ufs_bsp_config *cfg)
{
   ...
   memcpy (&cfg->perf_speed_params, &cfg->init_speed_params, sizeof(ufs_bsp_speed_config)); 
...
ufs_bsp_err_type ufs_bsp_get_dt_config (uint8_t hostid, ufs_bsp_config *cfg)
{
   ...
   if ((EFI_SUCCESS != status) || (NULL == DtbProtocol)) {
      ...
      snprintf (fdt_node_str, DT_PATH_LEN, "/storage/ufs%d", hostid);
      ...
      snprintf (fdt_node_str, DT_PATH_LEN, "/storage/ufs%d/timeout_values", hostid); 
      result = DtbProtocol->FdtGetNodeHandle (&node, fdt_node_str);   
      if (result == 0) {
      ...
      snprintf (fdt_node_str, DT_PATH_LEN, "/storage/ufs%d/init_speed_params", hostid); 
      result = DtbProtocol->FdtGetNodeHandle (&node, fdt_node_str); 
      if (result == 0) {
      ...
      snprintf (fdt_node_str, DT_PATH_LEN, "/storage/ufs%d/perf_speed_params", hostid); 
      result = DtbProtocol->FdtGetNodeHandle (&node, fdt_node_str); 
      if (result == 0) {
...
ufs_bsp_err_type ufs_bsp_volt_set(uint8_t hostid, ufs_bsp_regulator_mode mode)
{
   ...
   static npa_client_handle vccq2_client_handle = NULL;
   ...
   if (hostid == 0) {
	  ...
	  if (vccq2_client_handle == NULL) {
         vccq2_client_handle = npa_create_sync_client(PMIC_NPA_GROUP_ID_UFS0_VCCQ2, "ufs0_vccq2", NPA_CLIENT_REQUIRED); 
         if (vccq2_client_handle == NULL) {
            return UFS_BSP_ERR_PMIC;
         }
      }
   ...
   if (mode == UFS_BSP_REGULATORS_ON) {
      ...
      // PMIC_B, PM_LDO_7 is Fillmore target specific UFS Device Vcc regulator
      ...
      Status=PmicVRegProtocol->VregGetLevel(0x1 /*PAPIO [PM8350] B 1*/, EFI_PM_LDO_7, &voltage_req_rail);
      if (Status != EFI_SUCCESS) {
         return UFS_BSP_ERR_PMIC;  
      }
      ...
      if(voltage_req_rail >= 2400000 && voltage_req_rail <= 2700000) { 
        ...
        npa_issue_required_request(vcc_client_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
        npa_issue_required_request(vccq2_client_handle, PMIC_NPA_MODE_ID_GENERIC_ACTIVE);
	  }
   ...
   /* Lower Vcc to 0V for UFS device sleep */
      ...
      npa_issue_required_request(vccq2_client_handle, PMIC_NPA_MODE_ID_GENERIC_STANDBY);
...
ufs_bsp_err_type ufs_bsp_configure_smmu()
{
   ...
   if (HAL_IOMMU_ERR_OK != fp_table->domain_attach_func(ufs_mem_domain_hdl, (void *)"UFS_MEM", 
