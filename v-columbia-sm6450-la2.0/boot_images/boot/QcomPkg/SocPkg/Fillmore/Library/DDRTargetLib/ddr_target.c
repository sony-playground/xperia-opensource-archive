...
#define PMIC_VDDQ_LDO PM_LDO_3
#define PMIC_VDD2H         PMIC_K
#define PMIC_VDD2H_SMPS    PM_SMPS_1
...
#define REG_OFFSET_CABO_CH(ch)            (SEQ_DDR_SS_CABO0_CABO_OFFSET + (ch * 0x100000))
#define SEQ_DDR_SS_MC0_MC5_OFFSET                                    0x19270000 
#define SEQ_DDR_SS_CABO0_CABO_OFFSET                                 0x192d0000
/*==============================================================================
                                  DATA
==============================================================================*/
...
void ddr_target_init(boot_mode_type boot_mode)
{
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
    ...
    if((g_boot_mode == 0 )||(g_boot_mode == BOOT_MODE_RAMDUMP)) //if (PcdGet32 (PcdBuildType) == 0)
    {
      ...
      err = pm_smps_sw_mode(2, PM_SMPS_8, PM_SW_MODE_NPM); // CX SMPS8C to PWM
      CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);
      err = pm_smps_sw_mode(1, PM_SMPS_2, PM_SW_MODE_NPM); // VDDA_EBI SMPS2B to PWM
      CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);     
    }
  }
  ...
  if (platform == DALPLATFORMINFO_TYPE_RUMI)
  {
	  ddr_shared_data->ddr_init_clk_idx = 0xC; //3.2 GHz for LP5 in D4N mode
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
	...
	{	
	    ...
	    if(HWIO_DDR_REG_FUSE_INPUTS_STATUS_INM(HWIO_DDR_REG_FUSE_INPUTS_STATUS_MC4_FUSE_STATUS_BMSK) == 0)
	    {
		  ...
		  err = pm_smps_volt_level(PMIC_VDD2H,PMIC_VDD2H_SMPS, 1080 * 1000, TRUE);
		  CORE_VERIFY(err == PM_ERR_FLAG_SUCCESS);
...
void print_ddr_frequency()
{
  ...
  if((ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5))
	...
	clk_period = HWIO_CABO_BROADCAST_ORCABO_GLOBAL_DDR_CLK_PERIOD_CFG_INM(HWIO_CABO_BROADCAST_ORCABO_GLOBAL_DDR_CLK_PERIOD_CFG_DDR_CLK_PERIOD_BMSK);	
...
void ddr_post_setup()
{
    ...
    if(g_boot_mode !=  BOOT_MODE_RAMDUMP ) //if (!boot_dload_is_dload_mode_set()) 
    {
		...
		else 
		{
			uint8 index = 0;
            uint8 max_index = 16;
            uint8 set_lp4_index = 1;
            uint8 set_lp5_index = 12;
			if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
			{
                    ...
                    {
                     ddr_driver_struct->aop_params->mc_states[index].freq_enable = FALSE;
                    }
                    ddr_driver_struct->aop_params->mc_states[set_lp5_index].freq_enable = TRUE;
			}
			else
			{
                    ...
                    ddr_driver_struct->aop_params->mc_states[set_lp4_index].freq_enable = TRUE;
			}
		}
