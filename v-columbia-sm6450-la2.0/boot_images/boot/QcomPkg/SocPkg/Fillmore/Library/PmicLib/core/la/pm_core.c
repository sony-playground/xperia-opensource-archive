  ...
  *applicable = FALSE;
  ...
  if( ((platform_type == DALPLATFORMINFO_TYPE_MTP)
    ...
    || (platform_type == DALPLATFORMINFO_TYPE_HDK)) &&
       (pmic_model == PMIC_IS_PM7250B || pm_get_pmic_model(PMIC_D) == PMIC_IS_PM7325B
       || pm_get_pmic_model(PMIC_D) == PMIC_IS_PM8350B) )
  {
    *applicable = TRUE;
  }
...
static pm_err_flag_type pm_i2c_sid_config(void)
{
  ...
  uint8  packet_data[] = {0x06, 0x56, 0x03};      /* peripheral id, register address, register value */
