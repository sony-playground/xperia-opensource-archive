...
2022-04-14   ssb     Add support for sdcc bootup using Device Tree Config
2022-02-07	 cm		 Add sdcc_bsp_get_default_hsr_settings()
2021-12-22   gml     Skip voltage re-init for SD Card
...
2021-03-16   vm      Removed sdcc4 instance 
2020-06-04   jt      Add power management support  
2020-04-28   jt      Fillmore updates
                                             ...
                                             sdcc_bsp_hsr_settings_type *hsr_settings)
{
	...
	{
	hsr_settings->dll_config = SDCC_BSP_DLL_CONFIG_DEFAULT_VAL;
	hsr_settings->dll_config_2  = SDCC_BSP_DLL_CONFIG_2_DEFAULT_VAL;	
	hsr_settings->dll_config_3  = SDCC_BSP_DLL_CONFIG_3_DEFAULT_VAL;
	hsr_settings->dll_test_ctrl = SDCC_BSP_DLL_TEST_CTRL_DEFAULT_VAL;
	hsr_settings->ddr_config = 	SDCC_BSP_DDR_CONFIG_DEFAULT_VAL;
	hsr_settings->dll_user_ctrl = SDCC_BSP_DLL_USER_CTRL_DEFAULT_VAL;	 
	}
}											 
/*=============================================================================
                                             ...
                                             sdcc_bsp_hsr_settings_type *hsr_settings)
{
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
        ...
        if (NULL != fdt_handle)
        {
            snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d/HsrSettings", slot_num);
...
sdcc_bsp_gpio_control (UINT32 slot_num, UINT32 state)
{
   ...
   if (0 == result) 
   {
       snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d", slot_num);
   ...
   switch(driveno)
   {
      case SDCC_BSP_SLOT_1:
         ...
         else
         {
            if (NULL != fdt_handle)
            {
               snprintf (fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d", driveno); 
...
// 1 TOKEI  [PMK8003]   A 0
// 2 PAPIO  [PM8350]    B 1
// 3 PALANI [PM8350C]   C 2
// 4 PUHI   [PM8350BH]  D 3
// 5 NALO   [PMR735B]   F 5

#define LDO09_VSET_ADDR 0xC940 //SD Card Vdd Regulator register address
#define LDO09_EN_CTL_ADDR 0xC946 //LDO9 enable control register address
#define LDO09_VSET_LB 0xC940   //SD Card Vdd Regulator register address lower byte
#define LDO09_VSET_UB 0xC941   //SD Card Vdd Regulator register address upper byte
...
#define LDO06_VSET_ADDR 0xC640 //SD Card Vdd_px2 Regulator register address
#define LDO06_EN_CTL_ADDR 0xC646 //LDO6 enable control register address
#define LDO06_VSET_LB 0xC640   //SD Card Vdd_px2 Regulator register address lower byte
#define LDO06_VSET_UB 0xC641   //SD Card Vdd_px2 Regulator register address upper byte
...
#define LDO06_EN_CTL_SIZE 1       //SD Card Vdd_px2 Regulator register size
       ...
       uint8_t data1[LDO09_EN_CTL_SIZE] = {0x0}; // disable LDO9
       if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, LDO09_EN_CTL_ADDR, data1, LDO09_EN_CTL_SIZE))
       {
         return SDCC_BSP_ERR_PMIC;
       }
       else
       {
         ...
         if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, LDO09_VSET_ADDR, data, LDO09_VSET_SIZE))
         {
           return SDCC_BSP_ERR_PMIC;
         }

         uint8_t data2[LDO09_EN_CTL_SIZE] = {0x80}; // Enable LDO9
         if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, LDO09_EN_CTL_ADDR, data2, LDO09_EN_CTL_SIZE))
         {
            return SDCC_BSP_ERR_PMIC;
         }
       }
   
       uint8_t data3[LDO06_EN_CTL_SIZE] = {0x0}; // disable LDO6
       if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, LDO06_EN_CTL_ADDR, data3, LDO06_EN_CTL_SIZE))
       {
         return SDCC_BSP_ERR_PMIC;
       }
       else
       {
         ...
         if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, LDO06_VSET_ADDR, data4, LDO06_VSET_SIZE))
         {
           return SDCC_BSP_ERR_PMIC;
         }

         uint8_t data5[LDO06_EN_CTL_SIZE] = {0x80}; // Enable LDO6
         if ( SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, LDO06_EN_CTL_ADDR, data5, LDO06_EN_CTL_SIZE))
         {
            return SDCC_BSP_ERR_PMIC;
         }
       }
...
#else
       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_C, PM_LDO_9, PM_OFF, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_C, PM_LDO_6, PM_OFF, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_C, PM_LDO_9, 2960 * 1000, TRUE))
       {
              return SDCC_BSP_ERR_PMIC;
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_volt_level(PMIC_C, PM_LDO_6, 2950 * 1000, TRUE))
       {
              return SDCC_BSP_ERR_PMIC;
       }

       if (PM_ERR_FLAG_SUCCESS != pm_ldo_sw_enable(PMIC_C, PM_LDO_9, PM_ON, TRUE))
       {
              return SDCC_BSP_ERR_PMIC; 
       }
...
sdcc_bsp_err_type sdcc_bsp_get_config (UINT32 driveno, sdcc_bsp_config *sdcc_config_data)
{
	...
	sdcc_config_data->SpeedMode       =  SDCC_HS400_MODE;
