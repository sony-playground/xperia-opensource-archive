...
----------   ---     ----------------------------------------------------------- 
2022-04-12   ssb     Updated Speed mode values as per DT
...
static npa_client_handle sdc_sd_mmc_vdd_px2_handle = NULL;
                                             ...
                                             sdcc_bsp_hsr_settings_type *hsr_settings)
{
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
        ...
        if (NULL != DtbProtocol) 
        {
            snprintf (FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d/HsrSettings", slot_num); 
...
sdcc_bsp_gpio_control (UINT32 slot_num, UINT32 state)
{
   ...
   if (NULL == DtbProtocol) 
   {
       ...
       {
           snprintf (FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d", slot_num);
           if (0 == DtbProtocol->FdtGetNodeHandle(&Node, FdtNodeStr)) 
           {
...
sdcc_bsp_err_type sdcc_bsp_get_vreg_switch_status (UINT32 driveno)
{
   if (driveno == SDCC_BSP_SLOT_1) {
      ...
      if (sdc_sd_mmc_vdd_px2_handle == NULL) {
         sdc_sd_mmc_vdd_px2_handle = npa_create_sync_client(PMIC_NPA_GROUP_ID_SD_MMC_VDD_PX2, "sd_mmc_vdd_px2", NPA_CLIENT_REQUIRED);
      }
   }
   else {
      return SDCC_BSP_ERR_INVALID_PARAM;
   ...
   npa_issue_required_request(sdc_sd_mmc_vdd_px2_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
...
{ 
   ...
   npa_issue_required_request(sdc_sd_mmc_vdd_px2_handle, PMIC_NPA_MODE_ID_GENERIC_ACTIVE);
   ...
   switch(driveno)
   {
      case SDCC_BSP_SLOT_1:
         ...
         else
         {
            if (NULL != DtbProtocol)
            {
               snprintf (FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d", driveno); 
...
sdcc_bsp_err_type sdcc_bsp_volt_set (UINT32 driveno, sdcc_bsp_regulator_mode mode)
{
   ...
   static npa_client_handle sdc_sd_mmc_vdd_handle = NULL;
   ...
   if (driveno == SDCC_BSP_SLOT_1 && mode == SDCC_BSP_REGULATORS_ON && volt_set_done != TRUE) {
	  ...
	  if (sdc_sd_mmc_vdd_handle == NULL) {
          sdc_sd_mmc_vdd_handle = npa_create_sync_client(PMIC_NPA_GROUP_ID_SD_MMC_VDD, "sd_mmc_vdd", NPA_CLIENT_REQUIRED);
		  if (sdc_sd_mmc_vdd_handle == NULL) {
              return SDCC_BSP_ERR_PMIC;
	      }
      }	  
      ...
      npa_issue_required_request(sdc_sd_mmc_vdd_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
	  npa_issue_required_request(sdc_sd_mmc_vdd_px2_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
