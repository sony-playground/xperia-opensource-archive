...
#define QUSB_DCI_PMI_INDEX                        (8) //charging pmic index for fillmore
...
static pm_chg_charger_port_type qusb_ldr_utils_get_chgr_type(void)
{
  ...
  if((pmic_model == PMIC_IS_UNKNOWN) || (pmic_model == PMIC_IS_INVALID))
  {
    ...
    return usb_charger_type; //return SDP to allow FEDL on no-PMI devices
  }
...
void qusb_ldr_utils_handle_charger(void)
{
  ...
  if (FALSE == qusb_hs_phy_init())
  {
    ...
    qusb_uart_log("usb_dp_toggled_cdp", QUSB_LDR_UTILS_CDP_DP_TOGGLE_MS);
