 ...
 *
 * $DateTime: 2022/05/08 23:26:00 $
 * $Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Fillmore/Library/CPRTargetLib/image/core/target/fillmore/cpr_image_target.c#8 $
 * $Change: 37039811 $
 */
...
static EFI_CLOCK_PROTOCOL *camClockProtocol = NULL;
static UINTN  camClkPwrDomainId;
static UINTN  cameraXOClkId;
static UINTN  cameraAHBClkId;
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    cpr_settings *cpr_settings_7450 =
        (cpr_settings*)cpr_image_malloc(sizeof(cpr_settings));
    CPR_ASSERT(cpr_settings_7450);

    cpr_settings_7450->railsCount = 2;
    cpr_settings_7450->enablementsCount = 2;
    cpr_settings_7450->controllersCount = 2;

    cpr_settings_7450->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_7450->railsCount);
    for (i = 0; i < cpr_settings_7450->railsCount; i++) {
        cpr_settings_7450->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_7450->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_7450->controllersCount);
    for (i = 0; i < cpr_settings_7450->controllersCount; i++) {
        cpr_settings_7450->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_7450->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_7450->railsCount);
    for (i = 0; i < cpr_settings_7450->railsCount; i++) {
        cpr_settings_7450->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_7450->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_7450->enablementsCount);
    for (i = 0; i < cpr_settings_7450->enablementsCount; i++) {
        cpr_settings_7450->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_7450->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_7450, /*idx*/ 0);

    cpr_rail mm_rail;
    mm_rail.id = CPR_RAIL_MM;
    cpr_smem_deserialize_settings(&mm_rail, cpr_settings_7450, /*idx*/ 1);

    return cpr_settings_7450;
}
...
static void cpr_image_target_get_camera_power_domain_info(void)
{
    if (camClockProtocol==NULL)
    {
        //get global clock protocol info
        gBS->LocateProtocol(&gEfiClockProtocolGuid, NULL,
                (VOID **)&camClockProtocol);
    }
    CORE_VERIFY_PTR(camClockProtocol);

    //Get power domain info for titan gdsc
    if (camClkPwrDomainId)
        return;

    if (camClockProtocol->GetClockPowerDomainID(camClockProtocol,
                "cam_cc_titan_top_gdsc", &camClkPwrDomainId) != EFI_SUCCESS)
	{
		CPR_LOG_ERROR("Failed to get titan ClockPowerDomainID");
		CPR_ASSERT(0);
	}
	
	if (camClockProtocol->GetClockID(camClockProtocol,
                "gcc_camera_xo_clk", &cameraXOClkId) != EFI_SUCCESS)
	{
		CPR_LOG_ERROR("Failed to get camera XO ClockID");
		CPR_ASSERT(0);
	}
	
	if (camClockProtocol->GetClockID(camClockProtocol,
                "gcc_camera_ahb_clk", &cameraAHBClkId) != EFI_SUCCESS)
	{
		CPR_LOG_ERROR("Failed to get camera AHB ClockID");
		CPR_ASSERT(0);
	}
}
        ...
        cpr_domain_info* info)
{
	...
	cpr_image_target_get_camera_power_domain_info();
	...
	if (info->u.mode == CPR_VOLTAGE_MODE_OFF)
	{
		...
		//disable titan power domain first and turn off rail
		if (camClockProtocol->DisableClockPowerDomain(camClockProtocol,
                    camClkPwrDomainId) != EFI_SUCCESS)
		{
	...
	{
		//turn on rail first and enable titan power domain 
		req_id = rpmh_issue_command(aging_client_handle, RPMH_SET_ACTIVE, 1,
				arc_addr, hlvl);
		rpmh_barrier_single (aging_client_handle, req_id);

		
		if (camClockProtocol->EnableClock(camClockProtocol,
                    cameraXOClkId) != EFI_SUCCESS)
		{
			CPR_LOG_ERROR("Failed to Enable Camera XO ClockID");
			CPR_ASSERT(0);
		}
		
		if (camClockProtocol->EnableClock(camClockProtocol,
                    cameraAHBClkId) != EFI_SUCCESS)
		{
			CPR_LOG_ERROR("Failed to Enable Camera AHB ClockID");
			CPR_ASSERT(0);
		}
		
		if (camClockProtocol->EnableClockPowerDomain(camClockProtocol,
                    camClkPwrDomainId) != EFI_SUCCESS)
		{
			CPR_LOG_ERROR("Failed to Enable Titan ClockPowerDomain");
			CPR_ASSERT(0);
		}
	}
