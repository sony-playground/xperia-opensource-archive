...
MDP_Status Panel_Default_PowerUp(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    if (EFI_SUCCESS != TLMMProtocol->ConfigGpio((UINT32)EFI_GPIO_CFG(AMOLED_EXT_DVDD_VREG_EN, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_16MA), TLMM_GPIO_ENABLE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Configure GPIO %d for AMOLED_EXT_DVDD_VREG_EN failed!\n", AMOLED_EXT_DVDD_VREG_EN));
    }

    if (EFI_SUCCESS != TLMMProtocol->GpioOut((UINT32)EFI_GPIO_CFG(AMOLED_EXT_DVDD_VREG_EN, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_16MA), GPIO_HIGH_VALUE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Set AMOLED_EXT_DVDD_VREG_EN line high failed!\n"));
    }
...
MDP_Status Panel_Default_PowerDown(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  else
  {
    ...
    if (EFI_SUCCESS != TLMMProtocol->ConfigGpio((UINT32)EFI_GPIO_CFG(AMOLED_EXT_DVDD_VREG_EN, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_16MA), TLMM_GPIO_DISABLE))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: Configure GPIO %d for AMOLED_EXT_DVDD_VREG_EN failed!\n", AMOLED_EXT_DVDD_VREG_EN));
    }
...
MDP_Status Panel_Default_Brightness_Enable(MDP_Display_IDType eDisplayId, Panel_PowerCtrlParams *pPowerParams)
{
  ...
  {
    /* LCD WLED Enable Pin */
    if (EFI_SUCCESS != PmicGpioProtocol->CfgMode(PMIC_C, EFI_PM_GPIO_7, EFI_PM_GPIO_DIG_OUT))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: config PMIC_C gpio7 failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetVoltageSource(PMIC_C, EFI_PM_GPIO_7, EFI_PM_GPIO_VIN0))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio7 voltage failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutputLevel(PMIC_C, EFI_PM_GPIO_7, EFI_PM_GPIO_LEVEL_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio7 out put level failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutSrcCfg(PMIC_C, EFI_PM_GPIO_7, EFI_PM_GPIO_SRC_GND))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio7 out src cfg failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_C, EFI_PM_GPIO_7, EFI_PM_GPIO_OUT_DRV_STR_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio7 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_C, EFI_PM_GPIO_7, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
    ...
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutDrvStr(PMIC_C, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_DRV_STR_HIGH))
    {
      DEBUG((EFI_D_WARN, "DisplayDxe: set gpio8 outdrv str failed!\n"));
    }
    else if (EFI_SUCCESS != PmicGpioProtocol->SetOutBufCfg(PMIC_C, EFI_PM_GPIO_8, EFI_PM_GPIO_OUT_BUF_CFG_CMOS))
    {
...
MDP_Status Panel_Default_Brightness_Level(MDP_Display_IDType eDisplayId, BacklightConfigType *pBacklightConfig)
{
  ...
  if (MDP_PANEL_BACKLIGHTTYPE_PMIC == pBacklightConfig->eBacklightType)
  {
    ...
    case MDP_PMIC_BACKLIGHT_CONTROLTYPE_WLED:
      {
        ...
        else
        {
          ...
          if (EFI_SUCCESS != PmicWledProtocol->SetLedDutyCycle(PMIC_C, EFI_PM_MODULATOR_TYPE_A, uWLEDValue))
          {
            DEBUG((EFI_D_ERROR, "Wled Protocol SetLedDutyCycle for ModulatorA failed\n"));
          }

          if (EFI_SUCCESS != PmicWledProtocol->SetLedDutyCycle(PMIC_C, EFI_PM_MODULATOR_TYPE_B, uWLEDValue))
          {
    ...
    case MDP_PMIC_BACKLIGHT_CONTROLTYPE_PWM:
    {
      ...
      else if (EFI_SUCCESS != PmicPwmProtocol->PwmConfig(PMIC_C, EFI_PM_PWM_RSRC_PWM_4, uBrightnessValue, 1, 2, EFI_PM_PWM_CLK_19_2_MHZ, EFI_PM_PWM_SIZE_9BIT))
      {
        DEBUG((EFI_D_ERROR, "DisplayDxe: PwmConfig failed!\n"));
      }
      else if (EFI_SUCCESS != PmicPwmProtocol->SetPwmValue(PMIC_C, EFI_PM_PWM_RSRC_PWM_4, uBrightnessValue))
      {
