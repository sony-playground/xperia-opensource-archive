  ...
  )
{
...
#endif		
  {
    ...
    if (PlatformInfo.platform != EFI_PLATFORMINFO_TYPE_ADP)
    {
        ...
        else if (Index == USB_CORE_0_SOC && PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_RUMI)
        {
           ...
           DBG(EFI_D_ERROR, "UsbConfigInit, start device controller on port: %d\n", Index);
           Status = UsbStartController((QCOM_USB_CORE_NUM)Index, USB_DEVICE_MODE_SS);
  ...
  )
{
  ...
  DBG(EFI_D_WARN, "Stopping Core %d in mode: %d ", CoreNum, ModeType);
