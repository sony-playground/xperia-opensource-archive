...
if /I %TEST_MODE%=="FALSE" (
  ...
  if /I "%VARIANT%"=="LAA" (
    ...
    \\stone\aswcrm\smart\nt\bin\findbuild.exe -in=location -la -ap BOOT.MXF.1.0-?????-LAHAINA-? > %TEMP%\boot_build.log
...
if /I %SEND_LITE%=="TRUE" (
  ...
  %boot_build_path%\boot_images\boot\QcomPkg\Tools\Storage\fh_loader\QSaharaServer.exe -k -b %boot_build_path%\boot_images\boot\QcomPkg\SocPkg\Lahaina\Bin\%VARIANT%\DEBUG\ -s 13:prog_firehose_lite.elf -p \\.\%port_number%
) 

if /I %SEND_LITE%=="FALSE" (
  echo Send device programmer DDR ...
  %boot_build_path%\boot_images\boot\QcomPkg\Tools\Storage\fh_loader\QSaharaServer.exe -k -b %boot_build_path%\boot_images\boot\QcomPkg\SocPkg\Lahaina\Bin\%VARIANT%\DEBUG\ -s 13:prog_firehose_ddr.elf -p \\.\%port_number%
)
...
if /I %FLASH_MODE%=="FALSE" (
  if /I "%VARIANT%"=="LAA" (
    ...
    \\stone\aswcrm\smart\nt\bin\findbuild.exe -in=location -la -ap Lahaina.LA.1.0-?????-STD.INT-? > %TEMP%\meta_build.log
  ) else if /I "%VARIANT%"=="LAB" (
    echo Findbuild meta for variant [%VARIANT%]
    \\stone\aswcrm\smart\nt\bin\findbuild.exe -in=location -la -ap Lahaina.LA.0.1-?????-STD.INT-? > %TEMP%\meta_build.log
  )
...
%boot_build_path%\boot_images\boot\QcomPkg\\Tools\Storage\fh_loader\fh_loader.exe --porttracename=%TEMP%\port_trace.txt --json_in=%TEMP%\meta_cli.json --search_path=%CD%\Bin\%VARIANT%\DEBUG --port=\\.\%port_number% --memoryname=ufs --files=shrm.elf,xbl.elf,xbl_config.elf > %TEMP%\fh_loader.log
if errorlevel 1 (
