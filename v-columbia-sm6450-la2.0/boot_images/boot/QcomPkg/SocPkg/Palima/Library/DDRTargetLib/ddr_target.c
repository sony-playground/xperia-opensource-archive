...
#define PMIC_VDD2L         PMIC_B
#define PMIC_VDD2L_LDO     PM_LDO_3
...
void ddr_target_init(boot_mode_type boot_mode)
{
  ...
  chip_id = ChipInfo_GetChipId();
  if (CHIPINFO_ID_SM_TOFINO == chip_id)
  {
	  ddr_shared_data->chip_type = 1;  // SM_TOFINO
  }
  else 
  {
	  ddr_shared_data->chip_type = 0;  // Palima and other variants
  }
...
boolean ddr_post_init()
{
  ...
  uint32 pmic_mode_cfg = 0;
  
  DalPlatformInfoPlatformType platform = DalPlatformInfo_Platform();
  
  if(ddr_driver_struct != NULL){
  ...
  if (platform != DALPLATFORMINFO_TYPE_RUMI)
  {
	...
	if (ddr_shared_data->device_params.device_type == DDR_TYPE_LPDDR5)
	{
      ...
      if(ddr_driver_struct->aop_params!=NULL && ddr_driver_struct->aop_params->mc_states != NULL){
        
        pmic_mode_cfg = ddr_driver_struct->aop_params->mc_states[1].pmic_mode_cfg;
        pm_status = pm_ldo_volt_level(PMIC_VDD2L, PMIC_VDD2L_LDO, ddr_driver_struct->aop_params->aop_pmic_settings[4].values[pmic_mode_cfg] * 1000, TRUE);
        if (pm_status != PM_ERR_FLAG_SUCCESS)
        {
		      snprintf (ddr_log_string, DDR_LOG_LEN, "pm_status: %d, pmic_mode_cfg: %d", pm_status, pmic_mode_cfg);
		      boot_log_message(ddr_log_string);  
          return FALSE;
        }
      }
