 ...
 *
 * $DateTime: 2022/08/19 10:28:24 $
 * $Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Palima/Library/CPRTargetLib/image/core/target/palima/cpr_image_target.c#5 $
 * $Change: 39171631 $
 */
...
const cpr_settings* cpr_image_target_get_settings(void)
{
    ...
    cpr_settings *cpr_settings_8475 =
        (cpr_settings*)cpr_image_malloc(sizeof(cpr_settings));
    CPR_ASSERT(cpr_settings_8475);

    cpr_settings_8475->railsCount = 2;
    cpr_settings_8475->enablementsCount = 2;
    cpr_settings_8475->controllersCount = 2;

    cpr_settings_8475->railVVPs = (cpr_versioned_voltage_plan **) \
        cpr_image_malloc(sizeof(cpr_versioned_voltage_plan *) * \
                cpr_settings_8475->railsCount);
    for (i = 0; i < cpr_settings_8475->railsCount; i++) {
        cpr_settings_8475->railVVPs[i] = (cpr_versioned_voltage_plan *) \
            cpr_image_malloc(sizeof(cpr_versioned_voltage_plan));
    }

    cpr_settings_8475->controllers = (cpr_hal_controller **) \
        cpr_image_malloc(sizeof(cpr_hal_controller *) * \
                cpr_settings_8475->controllersCount);
    for (i = 0; i < cpr_settings_8475->controllersCount; i++) {
        cpr_settings_8475->controllers[i] = (cpr_hal_controller *) \
            cpr_image_malloc(sizeof(cpr_hal_controller));
    }

    cpr_settings_8475->rails = (cpr_rail **) cpr_image_malloc(
            sizeof(cpr_rail *) * cpr_settings_8475->railsCount);
    for (i = 0; i < cpr_settings_8475->railsCount; i++) {
        cpr_settings_8475->rails[i] =
            (cpr_rail *) cpr_image_malloc(sizeof(cpr_rail));
    }

    cpr_settings_8475->enablements = (cpr_enablement **) \
        cpr_image_malloc(sizeof(cpr_enablement *) * \
                cpr_settings_8475->enablementsCount);
    for (i = 0; i < cpr_settings_8475->enablementsCount; i++) {
        cpr_settings_8475->enablements[i] =
            (cpr_enablement *)cpr_image_malloc(sizeof(cpr_enablement));
    }

    cpr_settings_8475->miscCfg = (cpr_misc_cfg *)
        cpr_image_malloc(sizeof(cpr_misc_cfg));

    cpr_rail gfx_rail;
    gfx_rail.id = CPR_RAIL_GFX;
    cpr_smem_deserialize_settings(&gfx_rail, cpr_settings_8475, /*idx*/ 0);

    cpr_rail mm_rail;
    mm_rail.id = CPR_RAIL_MM;
    cpr_smem_deserialize_settings(&mm_rail, cpr_settings_8475, /*idx*/ 1);

    return cpr_settings_8475;
}
...
bool cpr_image_apply_margin_offset(void)
{
    return false;
}

int32 cpr_get_vd_margin(uint32 railId,uint32 modeIdx)
{
  return 0;
}
