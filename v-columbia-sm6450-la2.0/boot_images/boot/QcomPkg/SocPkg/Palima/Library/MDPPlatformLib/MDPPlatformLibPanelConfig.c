  ...
  else
  {
    ...
    else
    {
      ...
      case EFI_PLATFORMINFO_TYPE_CDP:
        ...
        {
          MDPPlatformSetPanelName(pPlatformPanel->szName, DISPLAY_PANEL_CDP_RCM);
          break;
        }   
      ...
      case EFI_PLATFORMINFO_TYPE_MTP:
      {
        if ((0x10000 == pPlatformInfo->sEFIPlatformType.version) &&
            (0x2     == pPlatformInfo->sEFIPlatformType.subtype))
        {
          // Clear Panel name for Platforms with subtype 0x2
          MDPPlatformClearPanelName(pPlatformPanel->szName);
        } 
        else if ((0x10000 == pPlatformInfo->sEFIPlatformType.version) &&
                 (0x3     == pPlatformInfo->sEFIPlatformType.subtype))
        {
          MDPPlatformSetPanelName(pPlatformPanel->szName, DISPLAY_PANEL_MTP_SUBTYPE_3);
        }   
        else
        {
          ...
          if ((pPlatformInfo->uPanelIdOverride & PANEL_OVERRIDE_ENABLE_MASK) &&  // If the panel ID override is enabled and within range, force that panel configuration.
                   (uPanelIdOverride                < uPanelCnt))
          {
            MDPPlatformGetPanelList(eAutoDetect, &pPanelList, &uPanelCnt, &uPanelIdOverride);
            DEBUG((EFI_D_WARN, "DisplayDxe: Panel override enabled (Index: %d ID:0x%x)\n", PANEL_OVERRIDE_PANELID(pPlatformInfo->uPanelIdOverride), uPanelID));
      ...
      case EFI_PLATFORMINFO_TYPE_QRD:
        MDPPlatformSetPanelName(pPlatformPanel->szName, "r66451_144hz_fhd_dsc_cmd_cphy");
        break;
