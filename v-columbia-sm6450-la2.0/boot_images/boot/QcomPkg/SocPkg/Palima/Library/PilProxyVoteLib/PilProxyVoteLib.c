...
#define MODEM_CLIENT_NAME	"PIL_MODEM"
...
ICBArb_MasterSlaveType aMasterSlave_cdsp[] =
 { 
	{ ICBID_MASTER_CDSP_PROC , ICBID_SLAVE_CLK_CTL } ,
	// todo voting on MM Noc hang due to reduce RTL on rumi
	//{ ICBID_MASTER_CDSP_HCP,   ICBID_SLAVE_EBI1    }
 };

ICBArb_RequestType aRequest_cdsp[] =
{
...
/**
  ...
  @retval otherwise             fail
**/
STATIC EFI_STATUS IcbVote_cdsp_cdsp(CONST CHAR8* ClientName, CONST CHAR8* ResourceName)
{
	PIL_NPA_ENTRY*	NpaEntry;
	...
	npa_issue_vector_request(Hdl, sizeof(aRequest_cdsp)/(sizeof(npa_resource_state)),
						  (npa_resource_state *)aRequest_cdsp );
	return EFI_SUCCESS;
}
...
EFI_STATUS PILProxyVote(UINT32 SubsysId)
{
	switch (SubsysId)
	{
		case TZ_PIL_AUTH_QDSP6_PROC:
			...
			if (EFI_SUCCESS != RegVote(ADSP_CLIENT_NAME, "/vcs/vdd_lpi_mx", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
			
			if (EFI_SUCCESS != RegVote(ADSP_CLIENT_NAME, "/vcs/vdd_lpi_cx", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
		
			break;
		
		case TZ_PIL_AUTH_SSC_PROC:
			if (EFI_SUCCESS != RegVote(SLPI_CLIENT_NAME, "/vcs/vdd_lpi_mx", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
				
			if (EFI_SUCCESS != RegVote(SLPI_CLIENT_NAME, "/vcs/vdd_lpi_cx", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
		...
		case TZ_PIL_AUTH_CDSP_PROC:
			...
			if (EFI_SUCCESS != RegVote(CDSP_CLIENT_NAME, "/vcs/vdd_mxc", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
			
			if (EFI_SUCCESS != IcbVote_cdsp_cdsp(CDSP_CLIENT_NAME, "/icb/arbiter"))
				return EFI_UNSUPPORTED;
		...
		case TZ_PIL_AUTH_MODEM_SW_PROC:
			if (EFI_SUCCESS != RegVote(MODEM_CLIENT_NAME, "/vcs/vdd_mx", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
			
			if (EFI_SUCCESS != RegVote(MODEM_CLIENT_NAME, "/vcs/vdd_cx", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
			
			if (EFI_SUCCESS != RegVote(MODEM_CLIENT_NAME, "/vcs/vdd_mxc", RAIL_VOLTAGE_LEVEL_NOM))
				return EFI_UNSUPPORTED;
...
EFI_STATUS PILProxyUnvote(UINT32 SubsysId)
{
	switch (SubsysId)
	{
		...
		case TZ_PIL_AUTH_CDSP_PROC:
			...
			if (EFI_SUCCESS != RegUnvote(CDSP_CLIENT_NAME, "/vcs/vdd_mxc"))
				return EFI_UNSUPPORTED;
		...
		case TZ_PIL_AUTH_MODEM_SW_PROC:
			if (EFI_SUCCESS != RegUnvote(MODEM_CLIENT_NAME, "/vcs/vdd_mx"))
				return EFI_UNSUPPORTED;
			
			if (EFI_SUCCESS != RegUnvote(MODEM_CLIENT_NAME, "/vcs/vdd_cx"))
				return EFI_UNSUPPORTED;
			
			if (EFI_SUCCESS != RegUnvote(MODEM_CLIENT_NAME, "/vcs/vdd_mxc"))
				return EFI_UNSUPPORTED;
