...
#define QUSB_DCI_PMI_INDEX                    (3)   /*For PMIC_D*/
...
**/
volatile uint8 usb_hs_phy_reset_cnt =0;
VOID
SynopsysGccReset(
...
)
{
  ...
  if (UsbCore->CoreType == USB_CONFIG_SSUSB1)
  {
    ...
    if ((FALSE == is_eud_active) && (usb_hs_phy_reset_cnt ==0)) 
#else
  ...
  )
{
  ...
  if (CoreNum == USB_CORE_0_SOC) {
    ...
    *UsbCoreInterface = &UsbCore->CoreInterface;
    ...
    usb_shared_repeater_init(USB_CORE_0_SOC);
  }
#ifdef USB_SEC_PORT_FLAG	
