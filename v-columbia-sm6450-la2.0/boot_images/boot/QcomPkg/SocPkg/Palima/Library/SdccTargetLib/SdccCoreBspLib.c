...
2022-02-18   cm      Update mode as per Level shifter support
2022-01-05   cm      Updates for Palima 
...
static npa_client_handle sdc_client_handle = NULL;
                                             ...
                                             sdcc_bsp_hsr_settings_type *hsr_settings)
{
	...
	if(speed_mode==SDCC_SDR50_MODE)
	{
		hsr_settings->dll_config_2  = SDCC_BSP_DLL_CONFIG_2_DEFAULT_SDR50_VAL;
	}
	else if(speed_mode==SDCC_SDR104_MODE)
	{
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
        ...
        if (NULL != DtbProtocol) 
        {
            ...
            {
               snprintf(FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d/HsrSettings/SDR104", slot_num);
            }
            else if (SDCC_SDR50_MODE == speed_mode) 
            {
               snprintf(FdtNodeStr, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d/HsrSettings/SDR50", slot_num);
            }
            else 
            {
...
sdcc_bsp_err_type sdcc_bsp_get_vreg_switch_status (UINT32 driveno)
{
   if (driveno == SDCC_BSP_SLOT_1) {
      ...
      if (sdc_client_handle == NULL) {
         sdc_client_handle = npa_create_sync_client(PMIC_NPA_GROUP_ID_SD_MMC_VDD_PX2, "sd", NPA_CLIENT_REQUIRED);
      }
   }
   else {
      return SDCC_BSP_ERR_INVALID_PARAM;
   ...
   npa_issue_required_request(sdc_client_handle, PMIC_NPA_MODE_ID_GENERIC_HV);
...
{ 
   ...
   npa_issue_required_request(sdc_client_handle, PMIC_NPA_MODE_ID_GENERIC_ACTIVE);
