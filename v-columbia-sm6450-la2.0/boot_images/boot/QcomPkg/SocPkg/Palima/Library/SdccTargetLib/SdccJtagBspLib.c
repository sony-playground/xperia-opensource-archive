...
2021-11-09   cm      Run sdcard in HS mode
   ...
   switch(slot_num)
   {
      case SDCC_BSP_SLOT_1:
        ...
        if (NULL != fdt_handle)
        {
            ...
            {
               snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d/HsrSettings/SDR104", slot_num);
            }
            else if (SDCC_SDR50_MODE == speed_mode) 
            {
               snprintf(fdt_node_str, SDCC_BSP_DT_PATH_LEN, "/storage/sdc%d/HsrSettings/SDR50", slot_num);
            }
            else 
            {
