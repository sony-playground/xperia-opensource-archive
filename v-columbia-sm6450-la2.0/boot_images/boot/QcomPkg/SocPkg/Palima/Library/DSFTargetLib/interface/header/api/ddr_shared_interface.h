...
--------   ---       --------------------------------------------------------
01/28/21   mkma      dcb size fixed to be multiple of 512
...
#define DSF_INTERFACE_VERSION   21
...
#define DCB_MAX_SIZE ((1024*34)+4) // must be multiple of 4, must not be multiple of 512

#define MAX_NUM_LCP_ADDR_REGIONS 10
typedef struct
{
    ...
    uint8  attr;        // Region attributes (bit 0 - Encrypt, bit 1 - ECC, bit 2 - MTE, bit 3 - Meta Deta)
    uint8  reserved0;   // Reserved for future use
...
typedef struct
{
    ...
    uint16                                      vddq_in_mv_dvfsq_disabled;      // VDDQ  voltage required after DDR_FW_Init for DVFSQ disabled
    uint16                                      vdd2h_in_mv;                    // VDD2H voltage required after DDR_FW_Init
    ...
    uint8										chip_type;						// 1: SM_TOFINO, 0: Palima and other variants
}
DDR_SHARED_STRUCT;

#define MAX_NUM_TRAININGS   36
