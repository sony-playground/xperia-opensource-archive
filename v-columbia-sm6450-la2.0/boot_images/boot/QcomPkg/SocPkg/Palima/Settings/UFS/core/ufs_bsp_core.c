...
$Header: //components/rel/core.boot/6.0/QcomPkg/SocPkg/Palima/Settings/UFS/core/ufs_bsp_core.c#7 $
$DateTime: 2022/03/23 01:34:08 $

YYYY-MM-DD   who     what, where, why
----------------------------------------------------------------------
2022-03-21   cm      Enable turbo freq corner for quest
...
{ 
	...
	{ VOLT_CORNER_TURBO,    UFS_GEAR_4,       { 213, 9,  425}}
};

// Below values are in the following order:   
// {clock name, clock_id, OFF, MINSVS, LOWSVS, SVS, NOM, TURBO_L1}
static UFS_CLOCK_LIST ufs_clocks_array[] = 
{
   {
      {"gcc_ufs_phy_axi_clk", 0, 0, 0, 75000000, 150000000, 300000000, 850000000}, 
      {"gcc_ufs_phy_ice_core_clk", 0, 0, 0, 75000000, 150000000, 300000000, 850000000}, 
      {"gcc_ufs_phy_unipro_core_clk", 0, 0, 0, 75000000, 150000000, 300000000, 850000000},
...
ufs_bsp_err_type ufs_bsp_update_maxclk_seq(uint8_t hostid, uint8_t clk_scale)
{
	...
	val = HWIO_REG_IN (base_addr, VENSPEC_TEST_BUS_CTRL_2);
	if(clk_scale){
	//Write UFS_TEST_BUS_CTRL_2.HCI_SEL[4]=1
	   val |= (1<<4);
	}
	else{
	//Write UFS_TEST_BUS_CTRL_2.HCI_SEL[4]=1
	 ...
	 HWIO_REG_OUT (base_addr, VENSPEC_TEST_BUS_CTRL_2, val);  
       //Write UFS_MEM_ICE_CONTROL=0
/*	  val = HWIO_REG_IN (base_addr, ICE_CONTROL);
	  val &= ~(1<<1);
	  HWIO_REG_OUT (base_addr, ICE_CONTROL, val); */
	  return UFS_BSP_NO_ERROR; 
}
