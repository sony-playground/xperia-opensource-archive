...
----------------------------------------------------------------------
2022-06-06   vm      Set CX rail to 776mV in Device Programmer Path          
...
#define SMPS06_VSET_ADDR 0xAA40 
#define SMPS06_EN_CTL_ADDR 0xAA46 
#define SMPS06_VSET_SIZE 2       
#define SMPS06_EN_CTL_SIZE 1       

   if(mode == UFS_BSP_REGULATORS_ON) {
	   /* In Palima We are seeing UFS errors if CX rail is between 680mV to 700mV. 
	      As a SW Workaround we are setting CX rail to default 776mV in Device 
		  Programmer.In other Paths UFS is already up & running before CX rail goes 
		  to 700mV. */
	   
       uint8_t data2[SMPS06_VSET_SIZE] = {0x08,0x03}; //776 mili volt in hex
       if (SPMI_SUCCESS !=  SpmiBus_WriteLong(0x0, 0x2 /*PALANI [PM8350C]   C 2*/, SPMI_ACCESS_PRIORITY_LOW, SMPS06_VSET_ADDR, data2, SMPS06_VSET_SIZE))
       {
