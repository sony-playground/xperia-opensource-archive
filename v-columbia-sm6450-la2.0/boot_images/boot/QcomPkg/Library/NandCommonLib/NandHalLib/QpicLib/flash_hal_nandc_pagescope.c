 ...
 *   order.
 *
 *  $Header: //components/rel/core.boot/6.0/QcomPkg/Library/NandCommonLib/NandHalLib/QpicLib/flash_hal_nandc_pagescope.c#1 $ $DateTime: 2020/06/18 10:10:50 $ $Author: pwbldsvc $
 *
 *
  ...
  struct flash_hal_predef_op_list *op_list, enum flash_hal_status *status)
{
 ...
 /*For QPIC v1 instead of buffer being written for the whole register 
  "QPIC_NAND_ERASED_CW_DETECT_STATUS" qpic only gets the status 
  fields of that register due to which the status for CW/Page gets reflected in bytes '0-3'
  instead of being reflected in bytes '4-7'. So shifting it by 4. This was changed on v2 to 
  reflect the whole register and so no shifting is required. */
   
  if(priv_data->qpic_step_version == FLASH_HAL_QPIC_CTRL_STEP_VERSION)
  {
    erase_cw_mask = FLASH_NANDC_ERASED_CW_DET_STATUS_MASK >> 4;
    erase_cw_status = FLASH_ERASED_CW_DETECT_STATUS >> 4;
  }
