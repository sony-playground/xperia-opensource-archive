 ...
 *
 * Copyright (c) 2008-2013,2015, 2019-2020 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
 ...
 *   order.
 *
 *  $Header: //components/rel/core.boot/6.0/QcomPkg/Library/NandCommonLib/flash_nand_entry.c#1 $ $DateTime: 2020/06/18 10:10:50 $ $Author: pwbldsvc $
 *
 *
...
int nand_entry_mem_alloc(uint32 hal_mem_size, uint32 dal_mem_size)
{
  ...
  if ((NAND_DEVICE_DONE == result) && hal_mem_size)
  {
    ...
    {
      flash_memset((void *)meminfo->hal_mem_info.virtual_addr, 0xFF, 10); 
    }
#if 0
  ...
  uint32 buffer_size)
{
  ...
  sbuffer = (buffer_size > (page_size * page_count))? 
              (void *)((uint32)buffer + (page_size * page_count)): NULL;
