...
void kern_init (void (*tfn)(void*),    void* arg, 
                ...
                unsigned early_init_core_count, unsigned core_mask);
...
{
  EFI_STATUS Status;
  UINT32 MaxCoreCnt = 0, EarlyInitCoreCnt = 0, CoreMask = 0;
  UINT32 LibVer;
  ...
  else
  {
    ...
    CoreMask = BootContinueParameterBlkPtr->AvailMaxCoreMask;
...
//  InfoBlkPtr->ThreadList = thread_list;
  ...
  /* Launch Scheduler */
    kern_init (BootContinueFn, 0, (void*)KernHeapMemInfo.MemBase, KernHeapMemInfo.MemSize, &_ModuleEntryPoint, MaxCoreCnt, EarlyInitCoreCnt, CoreMask);
  }
