...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/UfsCommonLib/ufs_api.c#8 $
$DateTime: 2022/04/19 01:34:09 $ $Author: pwbldsvc $
                   ...
                   struct ufs_crypto_context *crypt_context)
{
   ...
   // Setting up a pending transfer, if necessary
   if (xfr_context != NULL) {
      rc = ufs_setup_pending_xfer(handle);
   if (rc != EOK) {
      return rc;
...
// Set UFS device mode to the one requested
