...
/** Default log level */
#define PCIE_OSAL_MSG_LEVEL           PCIE_OSAL_LOG_LEVEL_HIGH
...
{
   npa_client_handle nhandle = NULL;

   if (pmic_config == NULL)
      return PCIE_SUCCESS;  // Return success if config is not avilable (means not required)

   if (rsrc_ctxt == NULL)
      return PCIE_ERROR;

   nhandle = rsrc_ctxt->pmic_handle;
   if (nhandle == NULL)
   {
      nhandle = npa_create_sync_client (pmic_config->node_id, "pcie",
                                              NPA_CLIENT_REQUIRED);
      if (nhandle == NULL)
      {
      ...
      rsrc_ctxt->pmic_handle = nhandle;
   }

   if (enable)
      npa_issue_required_request (nhandle, PMIC_NPA_MODE_ID_GENERIC_ACTIVE);
   else
      npa_issue_required_request (nhandle, PMIC_NPA_MODE_ID_GENERIC_OFF);
                                     ...
                                     const pcie_port_config_t  *port_cfg)
{
   ...
   if (pcie_ldo_op (resources_ctxt, port_cfg->pmic_cfg, ENABLE) != PCIE_SUCCESS)
   {
      PCIE_HOST_LOG_ERROR ("Unable to turn ON ldos\n");
      return PCIE_ERROR;
   }

   res = pcie_enable_clocks (resources_ctxt, port_cfg->port_clk_cfg);
   if (res != PCIE_SUCCESS)
   {
   ...
   if (pcie_configure_smmu (resources_ctxt, port_cfg->iommu_cfg) != PCIE_SUCCESS)
   {
                                     ...
                                     const pcie_port_config_t  *port_cfg)
{
   ...
   res = pcie_disable_clocks (resources_ctxt, port_cfg->port_clk_cfg);
   if (res != PCIE_SUCCESS)
   {
   ...
   if (pcie_ldo_op (resources_ctxt, port_cfg->pmic_cfg, DISABLE) != PCIE_SUCCESS)
   {
...
pcie_status_t enable_rsc_module(void)
{
  ...
  if (EFI_ERROR (Status))
  {
     ...
     ++rsc_init_complete;
     return PCIE_SUCCESS; // On this platform RSC init is not required for PLIM not present
  }
