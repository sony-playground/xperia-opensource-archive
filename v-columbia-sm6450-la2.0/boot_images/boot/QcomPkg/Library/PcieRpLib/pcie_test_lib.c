...
/*
 ==============================================================================
  ...
  --------   ---     ---------------------------------------------------------
  04/04/22   Ps      Corrected the sequence for LA 
...
pcie_status_t map_mem_to_pcie_space (uint32 port_index, uint64 mem_space, uint64 size, uint64** pcie_space_ptr)
{
   ...
   pstatus = pcie_rp_get_mem_resources (port_index, PCIE_RESOURCE_MEM_MAP_SPACE, &mem_res);
   if (pstatus != PCIE_SUCCESS)
   {
...
pcie_status_t pcie_get_link_status (uint32 port_index, pcie_link_status_t* caps)
{
   ...
   caps->L0s_counter = pcie_rp_read_parf_reg (port_index, PCIE_PARF_DEBUG_CNT_PM_LINKST_IN_L0S_OFF);
   caps->L1_1_counter = pcie_rp_read_parf_reg (port_index, PCIE_PARF_DEBUG_CNT_AUX_CLK_IN_L1SUB_L1_OFF);
   caps->L1_2_counter = pcie_rp_read_parf_reg (port_index, PCIE_PARF_DEBUG_CNT_AUX_CLK_IN_L1SUB_L2_OFF);
