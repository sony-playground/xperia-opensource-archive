...
void print_hex_data (uint64 reg_addr, uint32 bytes)
{
   ...
   DEBUG ((EFI_D_ERROR, "Data @ : 0x%X", reg_addr));
   for (i=0; i < bytes; i += 4)
   {
      if ((i % 16) == 0)
         DEBUG ((EFI_D_ERROR, "\r\n 0x%03X ", i));
      ...
      DEBUG ((EFI_D_ERROR, " 0x%08X", reg_val));
   }
   DEBUG ((EFI_D_ERROR, "\r\n"));
}
...
void print_error_bit_messages (uint32 err_status, err_msg_type* err_msg_pool, uint32 count)
{
   ...
   for (i=0; i < count; ++i)
   {
      ...
      {
         DEBUG ((EFI_D_ERROR, " %a,", err_msg_pool[i].msg_str));
         ++found;
      }
   }
   if (found)
...
static pcie_status_t pcie_print_aer_logs (uint32 port_index, uint32 bdf_val)
{
   ...
   if (bdf_val == 0)
      DEBUG ((EFI_D_ERROR, "Host AERs:\r\n----------\r\n"));
   else
      DEBUG ((EFI_D_ERROR, "EP AERs: 0x%X\r\n----------\r\n", bdf_val));

   reg_addr = pcie_cap_addr + PCIE_DEVICE_CONTROL_STATUS_OFFSET;
   reg_val = MmioRead32 (reg_addr);
   reg_val >>= 16;
   reg_val &= 0x7F;
   DEBUG ((EFI_D_ERROR, "   Device status : 0x%X\r\n", reg_val));
   ...
   DEBUG ((EFI_D_ERROR, "   Uncorrectable error status : 0x%X (First err bit : %d)\r\n", reg_val, (ctrl & 0x1F)));
   ...
   DEBUG ((EFI_D_ERROR, "   Root error status          : 0x%X  (Adv Err Intr Msg %d)\r\n", reg_val, ((reg_val >> 27) & 0x1F)));
   ...
   if (reg_val)
      MmioWrite32 (reg_addr, reg_val);
   DEBUG ((EFI_D_ERROR, "   ERR/Corr Err src ID Reg    : 0x%X 0x%X\r\n", ((reg_val >> 16) & 0xFFFF), (reg_val & 0xFFFF)));
...
//   DEBUG ((EFI_D_ERROR, "Multi Hdr Recording, ECRC Check, Generation Capble : 0x%X\r\n", cmd_val));
   if (cmd_val)
   {
      cmd_val = cmd_val << 1;
...
//#define PRINT_AER_STATE_ON_INIT
   ...
   DEBUG ((EFI_D_ERROR, "AER state on setup: BDF 0x%X\r\n", HOST_BDF_VAL));
...
pcie_status_t pcie_rp_setup_aspm (uint32 port_index)
{
   ...
   if (status != PCIE_SUCCESS)
   {
      DEBUG ((EFI_D_WARN, "No PCIe Cap for Host on PCI%d\r\n", port_index));
      return PCIE_SUCCESS;
   }
   ...
   if (status != PCIE_SUCCESS)
   {
      DEBUG ((EFI_D_ERROR, "No PCIe Cap on EP PCI%d\r\n", port_index));
   ...
   TempReg = TempReg | (port_cfg->aspm << PCIE_CAP_ACTIVE_STATE_LINK_PM_SUPPORT_SHFT);
