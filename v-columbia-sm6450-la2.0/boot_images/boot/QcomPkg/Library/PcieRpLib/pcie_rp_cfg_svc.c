...
/*==============================================================================
  ...
  ---------  ----     ---------------------------------------------------------
   03/07/22   Ps      Changed pointers to make it compatible with settings
...
extern pcie_target_config_t *pcie_tgt_config_data[];
extern uint32 pcie_tgt_config_data_cnt;
...
static pcie_status_t pcie_get_target_platform_config (pcie_target_config_t ** target_cfg_ptr)
{
   ...
   for (i = 0; i < pcie_tgt_config_data_cnt; ++i)
   {
      cfg_chip_id = pcie_tgt_config_data[i]->target_chip_id;
      cfg_family = pcie_tgt_config_data[i]->target_chip_family;
      cfg_plat_type = pcie_tgt_config_data[i]->target_platform_type;
      ...
      if ((cfg_chip_id == 0) || (cfg_chip_id == PCIE_TARGET_CHIP_ID_IGNORE))
      {
         if (cfg_family == chip_family)
            family_match_cfg = pcie_tgt_config_data[i];
      }
      else if (cfg_plat_type == 0)
      {
         if (cfg_family != 0)
         {
            if ((cfg_chip_id == chip_id) && (cfg_family == chip_family))
            {
               ...
               best_match_cfg = pcie_tgt_config_data[i];
               break;
            }
         }
         else if (cfg_chip_id == chip_id)
            id_match_cfg = pcie_tgt_config_data[i];
      }
   }
