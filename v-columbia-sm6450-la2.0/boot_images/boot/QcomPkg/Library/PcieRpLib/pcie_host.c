...
/*
 ==============================================================================
  ...
  --------   ---     ---------------------------------------------------------
  03/07/22   Ps      Register settings for BAR config & memio
...
#define PCIE_MAX_ACTIVE_ROOT_PORTS                    8
...
/* Maximum number for drivers the host driver can support*/
...
pcie_status_t pcie_assign_resources (pcie_dev_t *root_port)
{
   ...
   status = pcie_rp_get_mem_resources (port_index, PCIE_RESOURCE_MEM_MAP_SPACE, &mem_res);
...
pcie_status_t pcie_rc_init (void)
{
   ...
   /* TODO: Still too many logs in this file, cut down and reduce them */
   pcie_osal_log_init(PCIE_OSAL_LOG_HOST_RC, "PCIeHostRc", 4096, PCIE_HOST_LOG_LEVEL);
...
//   root_h->port_state = PORT_ENUMERATED; TODO needed?
   ...
   // Set AER by default when enumeration happens
   pcie_rp_setup_aer (rc_index, ALL_BDF_VAL, 1);

   /* Issue probe callback to all registered clients */
   pcie_probe (NULL);
   
   /* This will make sure we are picking the ADDR space size as oer target configuration.
      If this is not done memio may fail on some targets */
   pstatus = pcie_rp_get_mem_resources (rc_index, PCIE_RESOURCE_DBI_BASE, &mem_res);
   parf_reg_write (root_h, PCIE_PARF_SLV_ADDR_SPACE_SIZE_OFFS, mem_res.sz);  
