...
#define MAX_CAPABILITY_CACHE    12
...
#define PCIE_MAX_ROOT_PORTS  8
pcie_rp_handle pcie_rp_ctxt[PCIE_MAX_ROOT_PORTS];
...
static pcie_status_t setup_ecam_blocker (pcie_rp_handle* rp_handle, uint32 port_link_is_up)
{
   ...
   /* ECAM base */
   MmioWrite32((HWIO_OFFS(PCIE_PARF_ECAM_BASE) + port_mem[ahb_reg].pa),
              (UINT32)port_mem[axi_reg].pa);

   /* ECAM blocker wr & rd base for RC */
   // blocking_start, , allow_start, allow_end;

   blocking_start = port_mem[axi_reg].pa + PCIE_CONFIG_SPACE_LENGTH;
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_BASE) + port_mem[ahb_reg].pa),
              (UINT32)blocking_start);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_BASE_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_start >> 32));

   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_BASE) + port_mem[ahb_reg].pa),
              (UINT32)blocking_start);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_BASE_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_start >> 32));
   ...
   {
      blocking_end = port_mem[axi_reg].pa + (PCIE_SINGLE_BUS_ADDR_SPACE_SIZE - 1);
   ...
   else
   {
      ...
      blocking_end = port_mem[axi_reg].pa + (port_mem[axi_reg].sz - 1);
   }

   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_LIMIT) + port_mem[ahb_reg].pa),
              (UINT32)blocking_end);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_LIMIT_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_end >> 32));

   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_LIMIT) + port_mem[ahb_reg].pa),
              (UINT32)blocking_end);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_LIMIT_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_end >> 32));

   /* ECAM blocker wr & rd base for EP */
   blocking_start = port_mem[axi_reg].pa + PCIE_SINGLE_BUS_ADDR_SPACE_SIZE + PCIE_CONFIG_SPACE_LENGTH;
   blocking_end = port_mem[axi_reg].pa + PCIE_SINGLE_BUS_ADDR_SPACE_SIZE + (PCIE_SINGLE_BUS_ADDR_SPACE_SIZE - 1);

   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_BASE_2) + port_mem[ahb_reg].pa),
              (UINT32)blocking_start);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_BASE_2_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_start >> 32));

   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_BASE_2) + port_mem[ahb_reg].pa),
              (UINT32)blocking_start);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_BASE_2_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_start >> 32));

   /* ECAM blocker wr & rd limit for EP */
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_LIMIT_2) + port_mem[ahb_reg].pa),
              (UINT32)blocking_end);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_WR_LIMIT_2_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_end >> 32));

   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_LIMIT_2) + port_mem[ahb_reg].pa),
              (UINT32)blocking_end);
   MmioWrite32((HWIO_OFFS(PCIE_PARF_BLOCK_SLV_AXI_RD_LIMIT_2_HI) + port_mem[ahb_reg].pa),
              (UINT32)(blocking_end >> 32));

   /* Enable ECAM blocker regions in the end after the configurations */
   MmioOr32((HWIO_OFFS(PCIE_PARF_SYS_CTRL) + port_mem[ahb_reg].pa), block_cmd);

   return PCIE_SUCCESS;
}

pcie_status_t configure_iatu (const mem_region_t*        port_mem,
                              ...
                              uint64                     tlp_addr)
{
   if ((port_mem == NULL) || (atu_idx >= MaxAtuAssigned))
     return PCIE_INVALID_PARAM;

   /* Step 1. Setup the Region Base and Limit Address Registers. */
   MmioWrite32((HWIO_PCIE_IATU_LWR_BASE_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              (UINT32)rc_addr_base);
   MmioWrite32((HWIO_PCIE_IATU_UPPER_BASE_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              (UINT32)(rc_addr_base >> 32));
   MmioWrite32((HWIO_PCIE_IATU_LIMIT_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              (UINT32)rc_addr_limit);

   /* Step 2. Setup the Target Address Registers. */
   MmioWrite32((HWIO_PCIE_IATU_LWR_TARGET_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              (UINT32)tlp_addr);
   MmioWrite32((HWIO_PCIE_IATU_UPPER_TARGET_ADDR_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              (UINT32)(tlp_addr >> 32));

   /* Step 3. Configure the region through the Region Control 1 Register. */
   MmioWrite32((HWIO_PCIE_IATU_REGION_CTRL_1_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              tlp_type);

   /* Step 4. Enable the region. */
   MmioWrite32((HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_OFFS(atu_idx) + port_mem[axi_reg].pa),
              HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_REGION_EN_BMSK);

   return PCIE_SUCCESS;
}

pcie_status_t pcie_disable_ob_iatu (const mem_region_t* port_mem, pcierc_atu_slot_t atu_idx)
{
   uint32 val;
   ...
   /* Disable the region. */
   val = MmioRead32 ((HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa));

   val &= (~HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_REGION_EN_BMSK);
   MmioWrite32((HWIO_PCIE_IATU_REGION_CTRL_2_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa), val);

   /* Clear all Region Base, Limit and Target Address Registers. */
   MmioWrite32((HWIO_PCIE_IATU_LWR_BASE_ADDR_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa), 0);
   MmioWrite32((HWIO_PCIE_IATU_UPPER_BASE_ADDR_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa), 0);
   MmioWrite32((HWIO_PCIE_IATU_LIMIT_ADDR_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa), 0);
   MmioWrite32((HWIO_PCIE_IATU_LWR_TARGET_ADDR_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa), 0);
   MmioWrite32((HWIO_PCIE_IATU_UPPER_TARGET_ADDR_OFF_OUTBOUND_a_OFFS (atu_idx) + port_mem[axi_reg].pa), 0);
...
pcie_status_t pcie_rp_setup_config_space_ATU  (pcie_rp_handle* rp_handle)
{
   ...
   if ((rp_handle == NULL) || (rp_handle->port_config == NULL) ||
      (rp_handle->port_config->port_mem_bases == NULL))
     return PCIE_INVALID_PARAM;

   port_cfg = rp_handle->port_config;
   port_mem = port_cfg->port_mem_bases;

   // TODO: determine address based off of the bus number as well
   ep_cfg_addr_base = port_mem[axi_reg].pa + PCIE_SINGLE_BUS_ADDR_SPACE_SIZE;
   ep_cfg_addr_limit = ep_cfg_addr_base + PCIE_SINGLE_BUS_ADDR_SPACE_SIZE - 1;

   // TODO: Store VA for these instead
   rp_handle->host_config_addr = port_mem[axi_reg].pa;
   rp_handle->ep_config_addr = ep_cfg_addr_base;

   pstatus = configure_iatu (port_mem, AtuSlotCfgOut, PCIE_TLP_TYPE0_CFG_RW,
...
pcie_status_t start_link_training (pcie_rp_handle* rp_handle)
{
   ...
   port_mem_regions = rp_handle->port_config->port_mem_bases;

   /* TODO: Optimize this by storing VA mem region in the context */
   parf_va = OSAL_CONVERT_PA_TO_VA(port_mem_regions[ahb_reg].pa);
...
uint32 check_if_linkup (pcie_rp_handle* rp_handle)
{
   ...
   /* TODO: Optimize this by storing VA mem region in the context */
   parf_va = OSAL_CONVERT_PA_TO_VA(port_mem_regions[ahb_reg].pa);
   dbi_va = OSAL_CONVERT_PA_TO_VA(port_mem_regions[axi_reg].pa);
...
pcie_status_t pcie_rp_init_port_resources (pcie_rp_handle* rp_handle)
{
   ...
   if (pcie_initialize_gpios (&rp_handle->rsrc_ctxt, rp_handle->port_config->port_gpio_cfg) != PCIE_SUCCESS)
   {
...
uint32 pcie_rp_get_ltr_th_scale (uint32 port_index)
{
   ...
   return rp_handle->port_config->ltr_threshold_scale;
}
...
ltr_threshold_scale_t pcie_rp_get_ltr_th_val (uint32 port_index)
{
   ...
   return rp_handle->port_config->ltr_threshold_val;
}
...
pcie_rp_handle* pcie_rp_get_port_handle (uint32 port_index)
{
   ...
   if (port_index >= ARRAY_SIZE(pcie_rp_ctxt))
      return NULL;

   rp_handle = &pcie_rp_ctxt[port_index];
...
pcie_status_t pcie_rp_lib_init (void)
{
   ...
   {
      pcie_rp_ctxt[i].port_state = PORT_NOT_AVAILABLE; 
...
pcie_status_t pcie_rp_get_cfg_space_addr (uint32 port_index, uint16 bdf, uint32** ptr)
{
   ...
   *ptr = (uint32*)(rp_handle->port_config->port_mem_bases[axi_reg].pa + (bdf * PCIE_CONFIG_SPACE_SIZE));
...
pcie_status_t pcie_rp_enumerate_ports (uint32 ports_mask)
{
   ...
   for (i = 0; i < port_cnt; ++i)
   {
      ...
      if (rp_handle->port_config->is_usb4_tunneled)
...
pcie_status_t pcie_rp_setup_io_space_ATU (uint32 port_index, uint64 BaseAddr, uint64 BaseLimit, uint64 TranslationAddr)
{
   ...
   if ((port_cfg == NULL) || (port_cfg->port_mem_bases == NULL))
      return PCIE_ERROR;

   pstatus = configure_iatu (port_cfg->port_mem_bases, AtuSlotIoOut, PCIE_TLP_IORW,
...
pcie_status_t  pcie_rp_setup_mem_rw_ATU (uint32 port_index, uint64 BaseAddr, uint64 BaseLimit, uint64 TranslationAddr)
{
   ...
   pstatus = configure_iatu (port_cfg->port_mem_bases, AtuSlotMemOut, PCIE_TLP_MEM_REQ,
                            BaseAddr, BaseLimit, TranslationAddr);
   if (pstatus != PCIE_SUCCESS)
      PCIE_HOST_LOG_ERROR ("ATU IO Setup Failed\r\n");
...
pcie_status_t  pcie_rp_unmap_mem_rw_ATU (uint32 port_index)
{
   ...
   pstatus = pcie_disable_ob_iatu (port_cfg->port_mem_bases, AtuSlotMemOut);
   if (pstatus != PCIE_SUCCESS)
      PCIE_HOST_LOG_ERROR ("ATU IO Setup Failed\r\n");
...
pcie_status_t pcie_rp_enable_loopback (uint32 port_index, pcie_loopback_type lpb_type)
{
   pcie_status_t pstatus;
   pcie_rp_handle* rp_handle;
   ...
   switch (lpb_type)
   {
      case PCIE_LOCAL_DIGITAL_LOOPBACK:
         /* Just enable link, for local digital loopback, controller init process should have
      ...
      case PCIE_REMOTE_DIGITAL_LOOPBACK:
         //   enumeration should have already happened
         pstatus = PCIE_SUCCESS;
         break;

      default:
         pstatus =  PCIE_INVALID_PARAM;
         break;
   }
...
pcie_status_t pcie_rp_get_mem_resources (uint32 port_index, pcie_resource_type rsrc, pcie_mem_resource_t *res)
{
   ...
   if (rsrc == PCIE_RESOURCE_MEM_MAP_SPACE)
   {
