...
2020/06/04   sds     Use safe floor division for IB->bcm_unit to avoid 
                     accidental round up by clients
2020/04/17   sds     Fix for post_init race condition
2019/02/25   sds     Workaround for QCTDD05821154
...
#define BCM_HW_VERSION_2_0                                      0x00020000

#define DEFER_POOL_INITIAL_SIZE                                          5

#define AXICFGINT_MAX_WAIT_US                                         5000
#define AXICFGINT_WAIT_SIZE                                             10
...
static icb_rpmh_request_type *local_clk_queue = NULL; /**< local clock queue */
...
)
{
  ...
  switch( master->bus->type )
  {
    ...
    case ICB_BUS_NOC:
    {
      ...
      {
        HAL_noc_RegulatorModeType regulator_mode = {false, false, false, false};
         ...
         * There's no guarantee that this can be atomically configured.
         * Due to our handling of REGULn, n=1...3, this is done in two steps. */
        HAL_noc_SetRegulatorMode( master->bus->info,
                                  master->qos_port_ids[port],
                                  0,
                                  &regulator_mode );

        for( regulator = 0; regulator < qos->num_extra_regulators; regulator++ )
        {
          HAL_noc_SetRegulatorMode( master->bus->info,
                                    master->qos_port_ids[port],
                                    regulator + 1,
        ...
        for( regulator = 0; regulator < qos->num_extra_regulators; regulator++ )
        {
          HAL_noc_SetRegulator( master->bus->info,
                                master->qos_port_ids[port],
                                regulator + 1,
                                &qos->extra_regulators[regulator].regulator);
        }
        ...
        for( regulator = 0; regulator < qos->num_extra_regulators; regulator++ )
        {
          HAL_noc_SetRegulatorMode( master->bus->info,
                                    master->qos_port_ids[port],
                                    regulator + 1,
                                    &qos->extra_regulators[regulator].regulator_mode);
        }
        ...
        HAL_noc_SetQosDisable( master->bus->info,
                               master->qos_port_ids[port],
                               qos->qos_disable );
      }
...
)
{
  ...
  switch( master->bus->type )
  {
    case ICB_BUS_NOC:
    {
      ...
      override->regulator_mode.noncritical = qos_policy->noc.regulatorMode.noncritical;
      override->regulator_mode.critical    = qos_policy->noc.regulatorMode.critical;
      override->urgency_forwarding_enabled = qos_policy->noc.urgencyForwardingEnabled;
      override->qos_disable                = qos_policy->noc.qosDisable;
      override->num_extra_regulators       = qos_policy->noc.numExtraRegulators;
      override->is_applied                 = false;


      /* Copy extra regulators, if present. */
      if( NULL != override->extra_regulators )
      {
        for( regulator = 0; regulator < qos_policy->noc.numExtraRegulators; regulator++ )
        {
          override->extra_regulators[regulator].regulator.low_priority     = qos_policy->noc.extraRegulators[regulator].low_priority;
          override->extra_regulators[regulator].regulator.high_priority    = qos_policy->noc.extraRegulators[regulator].high_priority;
          override->extra_regulators[regulator].regulator.bandwidth        = qos_policy->noc.extraRegulators[regulator].bandwidth;
          override->extra_regulators[regulator].regulator.saturation       = qos_policy->noc.extraRegulators[regulator].saturation;
          override->extra_regulators[regulator].regulator_mode.read        = qos_policy->noc.extraRegulatorModes[regulator].read;
          override->extra_regulators[regulator].regulator_mode.write       = qos_policy->noc.extraRegulatorModes[regulator].write;
          override->extra_regulators[regulator].regulator_mode.noncritical = qos_policy->noc.extraRegulatorModes[regulator].noncritical;
          override->extra_regulators[regulator].regulator_mode.critical    = qos_policy->noc.extraRegulatorModes[regulator].critical;
        }
      }
      break;
    }
 ...
 * safe_floor_div
 *
 * @brief Floor division, but only return zero if the dividend was zero.
 *
 * @param dividend
 * @param divisor
*/
static uint64_t safe_floor_div
(
  uint64_t dividend,
  uint32_t divisor
)
...
)
{
  ...
  {
    /* There are two things we're checking for:
     * 1.) Return if secondary insertion of a shared BCM.
     *     BCM families are updated together, so there's no need to queue this again.
     *     Just check against hardware id and return if they match.
     *     This is a far less error prone method than manipulating the is_queued flags. */
    if( req->node->hw_id == iter->node->hw_id )
    {
      return;
    }
    /* 2.) Break out and insert the node if the clock id of the node
     *     to be inserted is less than the current node to be compared. */
...
/**
 * process_local_clk_requests
 *
 * @brief Process a queue and commit to hardware
 *
 * @param A pointer to the head of the queue to process
 * @param Is Next Awake Set
 *
 * @returns The barrier id
*/
static uint32_t process_local_clk_requests
(
  icb_rpmh_request_type **queue_head,
  bool                    is_nas,
...
)
{
  ...
  while( NULL != *queue_head )
  {
    hw_node = (icb_hw_node_type *)(*queue_head)->node;
    /* Clock requests for required and suppressible should be done with the larger one first. */
    if(hw_node != NULL)
    {
      /* Set single function call attributes to mimic the client of ICB */
      reqAttrs = (npa_request_attribute)client->request_attr;
      reqAttrs &= ~NPA_REQUEST_CHANGED_TYPE;  /* Mask client type change attribute */
      npa_set_request_attribute( hw_node->local_clk_supp_id, reqAttrs );
      npa_set_request_attribute( hw_node->local_clk_req_id, reqAttrs );

      if(hw_node->active.val > hw_node->sleep.val)
      {
        npa_issue_suppressible_request( hw_node->local_clk_supp_id, hw_node->active.val );
        npa_issue_required_request( hw_node->local_clk_req_id, hw_node->sleep.val );
      } else {
        npa_issue_required_request( hw_node->local_clk_req_id, hw_node->sleep.val );
        npa_issue_suppressible_request( hw_node->local_clk_supp_id, hw_node->active.val );
      }
    }
    (*queue_head)->is_queued = false;
...
/**
 * encode_local_clk_value
 *
 * @brief Encode scaled Ib/Ab values into a local_clk request
 *
 * @param
 * @param ib the scaled ib to use
 * @param ab the scaled ab to use
*/
uint32_t encode_local_clk_value
(
  icb_hw_node_type *hw_node,
  uint64_t          ib,
  uint64_t          ab
...
{
  /* Perform the BCM math for AB and IB aggregation using the constants from the hw_node for a local_clk request */
  uint64_t value;
  /* C1 and C2 are both fixed point values where 100 represents 1.00 */
  value = MAX(hw_node->C1 * ab, hw_node->C2 * ib) / 100;
  return value;
}
    ...
    switch( hw_node->type )
    {
      case ICB_HW_NODE_KIND_BANDWIDTH:
        if( state->supp_ib != 0 || state->supp_ab != 0 )
        {
          ...
          ib = safe_floor_div(state->ib, hw_node->bw_unit);
      ...
      case ICB_HW_NODE_KIND_VECTOR:
        ...
        {
          AxiCfgInt_SetVector(hw_node, state);
...
bool AxiCfgInt_Init( void )
{
  ...
  ULOG_RT_PRINTF_0(icb_log, "  AxiCfgInt_Init:");
  ...
  if( AXICFGINT_STATE_UNINITIALIZED == state && AxiCfgOs_Init() )
  {
    ...
    {
      ULOG_RT_PRINTF_0(icb_log, "  ... FAILED: AxiCfgInt_UpdateInfo");
      return false;
    }
    ...
    {
      /* Unable to map, error. */
      ULOG_RT_PRINTF_0(icb_log, "  ... FAILED: Unable map BCM HW");
      return false;
    }
    ...
    for( index = 0; index < icb_info->num_topologies; index++ )
    {
      ...
      for( i = 0; i < topology->num_buses; i++ )
      {
            ...
            (NULL == bus->info && ICB_BUS_VIRT != bus->type) )
        {
          ...
          ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: Invalid bus, index %d", i);
          return false;
        }
        ...
        for( pwr_idx = 0;
            ...
            pwr_idx++ )
        {
          if( !AxiCfgOs_GetPowerDomainId( bus->pwr_domain_names[pwr_idx],
                                          &bus->pwr_domain_ids[pwr_idx] ) )
          {
            ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: Invalid power domain: %s",
                             bus->pwr_domain_names[pwr_idx]);
            return false;
          }
        }
        ...
        for( clk_idx = 0;
            ...
            clk_idx++ )
        {
          if( !AxiCfgOs_GetClockId( bus->en_cfg_clk_names[clk_idx],
                                    &bus->en_cfg_clk_ids[clk_idx] ) )
          {
            ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: Invalid clock: %s",
                             bus->en_cfg_clk_names[clk_idx]);
            return false;
          }
        }
        ...
        switch( bus->type )
        {
          case ICB_BUS_NOC:
            ...
            * Indicates no need to for hwio mapping. */
            ...
            {
              /* Unable to map, error. */
              ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: Unable to map %s", bus->base_name);
              return false;
            }
            break;
          case ICB_BUS_VIRT:
          ...
          default:
            /* Invalid bus type. */
            ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: Invalid bus type %s", bus->base_name);
            return false;
        }
      }
      ...
      {
        ULOG_RT_PRINTF_0(icb_log, "  ... FAILED: Unable create RPMh handle");
        return false;
      }
      ...
      for( i = 0; i < topology->num_hw_nodes; i++ )
      {
        ...
        * Use aux fetch to check if the BCM exists. */
        if( hw_node->type == ICB_HW_NODE_KIND_LOCAL_CLK )
        {

          if( hw_node->width == 0 || hw_node->bw_unit == 0 ||
              hw_node->C1 == 0 || hw_node->C2 == 0 )
          {
            ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: LOCAL_CLK BCM %s needs details", hw_node->id);
            return false;
          }
          if( hw_node->local_clk_name != NULL )
          {
            hw_node->local_clk_req_id = npa_create_sync_client( hw_node->local_clk_name,
                                                                hw_node->id, NPA_CLIENT_REQUIRED );
            hw_node->local_clk_supp_id = npa_create_sync_client( hw_node->local_clk_name,
                                                                 hw_node->id, NPA_CLIENT_SUPPRESSIBLE );
            if( hw_node->local_clk_req_id == NULL || hw_node->local_clk_supp_id == NULL )
            {
              ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: LOCAL_CLK BCM %s npa client failed", hw_node->id);
              return false;
            }
          }
        }
        else if( CMD_DB_SUCCESS == cmd_db_query_aux_data(hw_node->id, &len, (uint8_t *)&bcm_aux) )
        {
        ...
        {
          ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: cmddb look up of BCM %s", hw_node->id);
          return false;
        }
      }
    }
    ...
    for( i = 0; i < icb_info->num_standalone_nodes; i++ )
    {
      ...
      {
        ULOG_RT_PRINTF_1(icb_log, "  ... FAILED: Unable create RPMh handle %s", data->resource_name);
        return false;
      }

      /* If the standalone node is associated with a BCM, initialize it. */
      if( NULL != hw_node )
      {
        struct bcm_db bcm_aux;
        uint8_t len = sizeof(bcm_aux);
...
{
  uint32_t topo_idx, bus_idx, clk_idx, pwr_idx, hw_idx, idx;
  icb_bus_def_type *bus;
  icb_topology_type *topology;
       ...
       topo_idx++ )
  {
    ...
    /* Clean hw_node NPA handles */
    for( hw_idx = 0;
         hw_idx < topology->num_hw_nodes;
         hw_idx ++ )
    {
      topology->hw_nodes[hw_idx]->local_clk_req_id = NULL;
      topology->hw_nodes[hw_idx]->local_clk_supp_id = NULL;
    }
...
{
  uint64_t ib = safe_floor_div(MAX(state->ib,state->supp_ib), hw_node->bw_unit);
  uint64_t ab = CEILING_DIV((state->ab + state->supp_ab), hw_node->bw_unit);
  
  switch( hw_node->type )
  {
    case ICB_HW_NODE_KIND_LOCAL_CLK:
    {
      /* encode_local_clk_value will perform the BCM front end math. */
      /*  The div by bw_unit should give a value in kHz for the clock request */
      /* Active is larger of required and suppressible */
      hw_node->active.val = encode_local_clk_value( hw_node, ib, ab);
      /* Sleep is just the required */
      hw_node->sleep.val = encode_local_clk_value( hw_node, safe_floor_div(state->ib, hw_node->bw_unit),
                                                   CEILING_DIV(state->ab, hw_node->bw_unit) );
      request_enqueue( &local_clk_queue, &hw_node->active );
      break;
    }
    default:
    {
...
)
{
  ...
  /* Aggregate our related BCMs. */
  for( idx = 0; idx < hw_node->num_relatives; idx++ )
  {
    /* If this is a regular request (i.e. not from icbarb_query)
     * use the normal state variable for the relative resource */
    if( is_request )
    {
      if( 0 != hw_node->relatives[idx]->state.total_bw )
      {
        hw_node->active.val |= hw_node->relatives[idx]->output;
      }
    }
    /* Otherwise, this must be a query, so use that state variable instead. */
    else
    {
      if( 0 != hw_node->relatives[idx]->query_state.total_bw )
      {
        hw_node->active.val |= hw_node->relatives[idx]->output;
      }
    }
  ...
  /* Add VALID bit if necessary and keep all of the family in sync. */
  if( 0 != hw_node->active.val )
  {
    hw_node->active.val |= BCM_VOTETABLE_VOTE_VALID_BMSK;
  }
  for( idx = 0; idx < hw_node->num_relatives; idx++ )
  {
    hw_node->relatives[idx]->active.val = hw_node->active.val;
  }
...
{
  uint32_t req_id            = 0;
  bool wait_for_completion   = !(client->request_attr & NPA_REQUEST_FIRE_AND_FORGET);
  bool is_nas                = client->request_attr & NPA_REQUEST_NEXT_AWAKE;
  icb_rpmh_handle_type *rpmh = &topology->rpmh_handle;

  ULOG_RT_PRINTF_1(icb_log, "[%s]   Commit", client->name);
  ...
  (void)process_local_clk_requests( &local_clk_queue, is_nas, client );
  ...
  {
    ULOG_RT_PRINTF_2(icb_log, "[%s]   Wait: %d", client->name, req_id);
    rpmh_barrier_all( rpmh->handle, req_id );
  }

  /* Process defer queue. */
  process_defer_queue( is_nas );
  ULOG_RT_PRINTF_1(icb_log, "[%s] Complete", client->name);
}
...
)
{
  ...
  switch( hw_node->type )
  {
    case ICB_HW_NODE_KIND_BANDWIDTH:
    {
      uint64_t ib = safe_floor_div(state->ib, hw_node->bw_unit);
  ...
  if( wait_for_completion )
  {
    ...
    /* Workaround for QCTDD05821154: Verify OFF -> ON transitions
    * Fixed after BCM versions 2.0 */
    if( BCM_HW_VERSION_2_0 >= icb_info->hw.version &&
        0 == state->prev_bw && 0 != state->total_bw )
    {
      uint32_t waited = 0;

      while( 0 == HWIO_INXFI( icb_info->hw.base_addr,
                              RPMH_BCM_STATUS_BE_CDm,
                              hw_node->clk_id,
                              CURR_CP ) &&
              waited < (AXICFGINT_MAX_WAIT_US / AXICFGINT_WAIT_SIZE) )
      {
        ...
        ( NULL != master->qos_port_ids ) )
    {
      ...
      if( NULL == qos_policy )
      {
        ...
        if( NULL != master->qos_policy_override )
        {
          ...
          {
            case ICB_BUS_NOC:
              DALSYS_Free(master->qos_policy_override->noc->extra_regulators);
        ...
        * Otherwise, just overwrite it. */
        if( NULL == master->qos_policy_override )
        {
          ...
          switch( master->bus->type )
          {
            case ICB_BUS_NOC:
            {
              /* Sanity regulator length */
              if( AXICFG_EXTRA_REGULATOR_CNT < qos_policy->noc.numExtraRegulators )
              {
                DALSYS_Free(master->qos_policy_override);
                break;
              }

              /* Allocate the NoC container, but try to be nice to the heap.
               * We always allocate at max supported size to avoid potentially allocating many small and different sized buffers.
               * The hope with using a fixed size is that future allocations will reuse previously freed buffers. */
              if( DAL_SUCCESS == DALSYS_Malloc( sizeof(icb_noc_master_policy_type),
                                                (void **)&master->qos_policy_override->noc ) )
              {
                /* Allocate the extra regulators container, but try to be nice to the heap.
                * We always allocate at max supported size to avoid potentially allocating many small and different sized buffers.
                * The hope with using a fixed size is that future allocations will reuse previously freed buffers. */
                if( DAL_SUCCESS != DALSYS_Malloc( sizeof(icb_noc_extra_regulator_type[AXICFG_EXTRA_REGULATOR_CNT]),
                                                  (void **)&master->qos_policy_override->noc->extra_regulators ) ) 
                {
                  /* Allocation failed. */
                  DALSYS_Free(master->qos_policy_override->noc);
                  DALSYS_Free(master->qos_policy_override);
        ...
        /* Mark the old policy as no longer applied. */
        switch( master->bus->type )
        {
          case ICB_BUS_NOC:
          {
            master->qos_policy->noc->is_applied = false;
            break;
          }
          default:
          {
...
void AxiCfgInt_Flush( void )
{
  ...
  while( NULL != local_clk_queue )
  {
    /* Pop the head of the queue. */
    iter = local_clk_queue;
    iter->is_queued = false;
    local_clk_queue = iter->next;
    iter->next = NULL;
  }
}
