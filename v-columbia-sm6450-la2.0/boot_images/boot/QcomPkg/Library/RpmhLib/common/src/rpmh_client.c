...
static rpmh_resource_command_t *rpmh_add_resource_command (uint32 address)
{
  ...
  for (i = 0; i < old_num_resource_commands; ++i)
  {
    if (rpmh.resource_commands[i].address > address)
    {
      ...
      memsmove (&rpmh.resource_commands[i+1], (old_num_resource_commands - i) * sizeof(rpmh_resource_command_t),
                &rpmh.resource_commands[i], (old_num_resource_commands - i) * sizeof(rpmh_resource_command_t));
      break;
    }
  }
...
uint32 rpmh_issue_command_set (rpmh_client_handle client_handle, rpmh_command_set_t *cmd_set)
{
  ...
  else if(is_request_explicit)
  {
    ...
    memscpy(&(explicit_cmds->cmd_sets[explicit_cmds->cmd_sets_in_use - 1]), sizeof(rpmh_command_set_internal_t),
            command_set, sizeof(rpmh_command_set_internal_t));
  }
