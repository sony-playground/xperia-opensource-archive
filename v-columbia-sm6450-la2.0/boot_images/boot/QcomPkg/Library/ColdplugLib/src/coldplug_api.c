...
/*===========================================================================
  ...
  $Header: //components/rel/core.boot/6.0/QcomPkg/Library/ColdplugLib/src/coldplug_api.c#7 $ $DateTime: 2023/02/28 07:46:17 $ $Author: pwbldsvc $

  when         who   what, where, why
  ----------   ---   ---------------------------------------------------------
  2023-02-10   ss    Added Optimization for allocating Coldplug Slots
  2022-09-15   rn    Moved boot device info to flash specific functions
	...
	int is_embedded_device)
{
	...
	for (slot_no = 0; slot_no < cdev->max_slots; ++slot_no) {
		...
		for (lun_no = 0; lun_no < cdev->max_luns; ++lun_no) {
			...
			cslot->is_embedded_device = dev_info->is_embedded_device;
        ...
        /* Check if we have any good Luns in this slot having any partition data otherwise allow that slot */
        /* to be filled by other Slots , Skip check for Non-Embedded device (Like SD card) as they may not */
        /* contain partition data */
		if((cslot->user_luns_count == 0) && (cslot->is_embedded_device != 0))
			cslot->is_in_use = 0;
	}
	...
	int is_embedded_device)
{
	...
	for (slot_no = 0; slot_no < cdev->max_slots; ++slot_no) {
		...
		if (result != 0) {
            /* Free the slot entry if we are not able to intialize the slot */
			cslot->is_in_use = 0;
	    ...
	    result = cdev->bdev_ops->get_boot_info(bdev_hdl,dev_info);
		
		if (result != COLDPLUG_RESULT_SUCCESS) {
            /* Free the Slot entry if we are not able to get boot_info from the boot lun */
			cslot->is_in_use=0;
				cdev->bdev_ops->close(bdev_hdl);
				continue;
		}
        clun->lba_size    =  dev_info->bytes_per_lba;
		clun->total_lba   =  dev_info->total_lba_count;
		clun->is_boot_lun =  dev_info->is_boot_lun_present;
		cslot->boot_lun = clun;
		cslot->is_embedded_device = dev_info->is_embedded_device;
