/** @file npawrapper.c
  ...
  Copyright (c) 2013-2015,2021 Qualcomm Technologies Inc. All rights reserved. 
...
--------   ---     -----------------------------------------------------------
05/27/21   amr     Adding new Batch Requests api's.
 ...
 * @param state : Requested resource state
 */
void npa_issue_batch_request( npa_batch_request_handle   client )
{
    EFI_NPA_PROTOCOL * npa_protocol;
    EFI_STATUS status;
        status = gBS->LocateProtocol(&gEfiNpaProtocolGuid, NULL, (void**)&npa_protocol);
        if(status == EFI_SUCCESS)
        {
            npa_protocol->IssueBatchRequest( client );
        }
        else
        {
            DEBUG ((EFI_D_ERROR, "Locate NpaProtocol failed:%r\n", status));
                               ...
                               npa_resource_state       *vector )
{
    ...
    EFI_STATUS status;
        ...
        if(status == EFI_SUCCESS)
        {
            npa_protocol->BatchVectorRequest( handle, client, num_elems, vector );
        }
        else
        {
            DEBUG ((EFI_D_ERROR, "Locate NpaProtocol failed:%r\n", status));
                               ...
                               npa_resource_state        state )
{
    ...
    EFI_STATUS status;
        ...
        if(status == EFI_SUCCESS)
        {
            npa_protocol->BatchScalarRequest( handle, client, state );
        }
        else
        {
            DEBUG ((EFI_D_ERROR, "Locate NpaProtocol failed:%r\n", status));
