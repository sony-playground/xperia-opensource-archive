...
boolean qusb_al_check_for_fedl(uint32 core_id, uint32 check_dp_high, uint32 check_dp_timeout_ms)
{
  return  qusb_ldr_utils_fedl_check(check_dp_high, check_dp_timeout_ms);
}
...
static void qusb_al_bulk_rx_complete(struct qusb_urb* urb)
{
  ...
  if (urb->transfer_length == urb->actual_length || short_packet_received)
  {
    ...
    {
      if (qusb_al_ctx->zlp_mode == QUSB_LACKS_ZLP) // No ZLP is expected
      {
