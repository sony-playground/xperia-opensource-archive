...
#define VSYSMIN_RECOVER_STEP_MV                             (100)                   // 100mV
...
#define SCHG_P_USB_TYPE_C_CFG                               (0x1358)
#define SCHG_P_USB_CMD_APSD                                 (0x1341)
#define SCHG_P_TYPEC_TYPE_C_STATE_MACHINE_STATUS            (0x1509)
...
#define PM_7250B_INTERNAL_PULL_UP_BAT_THM (0x0)
...
static uint32 pm_app_chg_get_cell_vbat_min(void);
...
static pm_err_flag_type pm_app_chg_enable_charge_on_dam_snk(uint32 charger_index, boolean enable);
...
pm_app_chg_trim_otg_offset_and_gain(void)
{
  ...
  pm_model_type pm_model = pm_get_pmic_model(pmic_index_charger);

  if(pm_model == PMIC_IS_PM8350B)
  {
...
pm_app_chg_init(void)
{
  ...
  err_flag |= pm_app_chg_enable_charge_on_dam_snk(charger_index, TRUE);
...
pm_app_chg_output_charging_status(void)
{
  ...
  adc_status_type adc_status = ADC_STATUS_SUCCESS;
  adc_result_type vusb_adc_result = {0, 0, 0};
  adc_result_type vwls_adc_result = {0, 0, 0};
  adc_result_type iusb_adc_result = {0, 0, 0};
  ...
  //Reading iusb before vusb due to OV issue on Nebula- PMICPR3361
  adc_status = adc_read(ADC_INPUT_USB_IN_I, &iusb_adc_result);
  if(adc_status != ADC_STATUS_SUCCESS)
  {
    pm_log_message("ADC %s sts:%d", ADC_INPUT_USB_IN_I, adc_status);
    return PM_ERR_FLAG_FAILURE;
  }

  adc_status = adc_read(ADC_INPUT_USB_IN, &vusb_adc_result);
  if(adc_status != ADC_STATUS_SUCCESS)
  {
    pm_log_message("ADC %s sts:%d", ADC_INPUT_USB_IN, adc_status);
    return PM_ERR_FLAG_FAILURE;
  }

  if(NULL != pm_scpq_get_data(charger_index))
  {
    adc_status = adc_read("VWLS", &vwls_adc_result);
    if((ADC_STATUS_SUCCESS != adc_status) && (ADC_STATUS_CHANNEL_NOT_SUPPORTED != adc_status))
    {
      // Some charger chipset doesn't support wireless charging. WLS channel would be unavailable
      pm_log_message("adc_read error: %d. %s %d", adc_status, __func__, __LINE__);
      return PM_ERR_FLAG_FAILURE;
    }
  }
  ...
  pm_log_message("Charger:%s V%s:%dmV I%s:%dmA CHG_STS:%s",
                  ...
                  chg_path,
                  power_by_usbin ? vusb_adc_result.physical : vwls_adc_result.physical,
                  chg_path,
                  iusb_adc_result.physical,
...
pm_app_chg_apply_charger_config(void)
{
  ...
  if(charger_dt_config->batt_detect_src < PM_BMD_SRC_INVALID)
  {
    ...
    {
      pm_app_chg_enable_bpd_cfg(FALSE);
      ...
      pm_app_chg_enable_bpd_cfg(TRUE);
    }
  }

  if((pm_model != PMIC_IS_SMB2351) && (pm_model != PMIC_IS_PM7250B) && (pm_model != PMIC_IS_PMI632)) //SMB2351 doesn't have QBG and SCPQ module
  {
    ...
    {
      err_flag |= pm_qbg_set_qbg_mode_ctl2_ichg_fs_opt(pmic_index, (charger_dt_config->ichg_fs == PM_CHG_FS_20A) ? TRUE : FALSE);
    }

    //Puhi PBS OTP code will configure QBG_MAIN_QBG_MODE_CTL1, so read value and configure QBG_MAIN_CMN_ADC_CTL7 accordingly.
    err_flag |= pm_qbg_get_qbg_mode_ctl1_batt_opt(pmic_index, &is_1s_battery, &battery_2s_option);
    if(is_1s_battery || (PM_CHG_DIS_VBAT_2S_MID == battery_2s_option) || (PM_CHG_EN_VBAT_2S_MID1 == battery_2s_option))
    {
  ...
  if(CHIPINFO_FAMILY_NETRANI == DalChipInfo_ChipFamily() || CHIPINFO_FAMILY_CLARENCE == DalChipInfo_ChipFamily() || CHIPINFO_FAMILY_FILLMORE == DalChipInfo_ChipFamily())
  {
    if(pm_model == PMIC_IS_PM7250B)
    {
		uint8 channel_id = PM_THERM_CHANNEL_100K; // default 100K
		if(charger_dt_config->batt_therm_pull_up == PM_THERM_PULL_UP_30K)
		{
			channel_id = PM_SCHG_BATIF_PULL_UP_RES_30K;
			err_flag |=  pm_schg_batif_set_adc_internal_pull_up(pmic_index, PM_7250B_INTERNAL_PULL_UP_BAT_THM, channel_id);
		}
		else if(charger_dt_config->batt_therm_pull_up == PM_THERM_PULL_UP_400K)
		{
			channel_id = PM_SCHG_BATIF_PULL_UP_RES_400K;
			err_flag |=  pm_schg_batif_set_adc_internal_pull_up(pmic_index, PM_7250B_INTERNAL_PULL_UP_BAT_THM, channel_id);
		}
		...
		if(charger_dt_config->ignore_temp_in_dbc)
		{
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_AFP_COLD, FALSE);
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_AFP_HOT, FALSE);
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_HARD_LIMIT, FALSE);
#ifdef FEATURE_XBOOT
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_COLD_SL_CCC, FALSE);
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_HOT_SL_CCC, FALSE);
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_COLD_SL_FVC, FALSE);
			err_flag |=  pm_schg_chgr_jeita_en_cfg(pmic_index, PM_SCHG_CHGR_JEITA_HOT_SL_FVC, FALSE);
#endif /* FEATURE_XBOOT */
		}
		else
		{
  ...
  {
    if((pm_model == PMIC_IS_PM7250B) || (pm_model == PMIC_IS_PMI632))
    {
      err_flag |= pm_config_jeita();
    }
  }
  ...
  {
    if((pm_model == PMIC_IS_PM7250B) || (pm_model == PMIC_IS_PMI632) || (pm_model == PMIC_IS_SMB2351))
    {
  ...
  if(pm_model == PMIC_IS_PM8350B)
  {
    if(charger_dt_config->wls_usb_prior_swap == 1)
    {
      err_flag |= pm_comm_write_byte_mask(3, 0x2980, 0x80, 0x80);
    }
    else
    {
      err_flag |= pm_comm_write_byte_mask(3, 0x2980, 0x80, 0x00);
    }
  }
...
pm_app_chg_check_weak_battery_status(void)
{
...
#ifndef FEATURE_XBOOT
  uint32 cell_threshold = (PM_IMG_LOADER == pm_get_img_type()) ? 
  ...
  while(TRUE)
  {
    ...
    if(TRUE == first_loop)
    {
      ...
      if((pm_model == PMIC_IS_PM8350B) || (pm_model == PMIC_IS_PM7325B))
      {
        ...
        //May vary during targets and may not need
        err_flag |= pm_scpq_chgr_vph_track_threshold_sel(pmic_index_charger, PM_SCPQ_VPH_TRACK_SEL_7_PCT);

        //Disable battery differential sensing
        err_flag |= pm_scpq_misc_set_eng_sdcdc_spare_cfg(pmic_index_charger, PM_SCPQ_MISC_ENG_SDCDC_SPARE_CFG_EN_BATT_DIFFSENSE, FALSE);
      }
...
static pm_err_flag_type
pm_app_chg_enable_charge_on_dam_snk(uint32 charger_index, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;

  if(NULL != pm_scpq_get_data(charger_index))
  {
    // Do nothing. PBS will enable it for Puhi based target.
  }
  else if(NULL != pm_schg_get_data(charger_index))
  {
    err_flag = pm_schg_typec_en_chg_on_dam_snk(charger_index, enable);
  }
  else
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
...
pm_app_chg_configure_usb_icl(uint32 icl_ma)
{
  ...
  do
  {
    ...
    if((PM_CHARGER_PORT_SDP_CHARGER == charger_type) && ((TRUE != is_dam_cable) && (FALSE != battery_present)))
    {
    ...
    if(NULL != pm_schg_get_data(charger_index))
    {
      ...
      err_flag |= pm_schg_usb_configure_usbin_icl_options(charger_index, PM_SCHG_USB_ICL_OPTIONS_USB51_MODE, TRUE);
#endif /* FEATURE_XBOOT */
...
/* Get the minimum cell vbat for 2S battery, for 1S return vbat directly */
static uint32 
pm_app_chg_get_cell_vbat_min()
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 vbat_min = 0;
  uint32 vbat_1 = 0, vbat_2 = 0;
  boolean battery_is_2s = FALSE;
  ...
  // For 1s battery return vbat directly
  if(FALSE == battery_is_2s)
  {
    err_flag |= pm_chg_get_vbatt(&vbat_min);
    pm_log_message("Vbat:%dmV", vbat_min);
  }
  else
  {
    err_flag |= pm_app_chg_get_2cell_vbat(&vbat_1, &vbat_2);
    vbat_min = vbat_1 < vbat_2 ? vbat_1 : vbat_2;
    pm_log_message("2S Vbat cell1:%dmV, cell2:%dmV", vbat_1, vbat_2);
  }

  CORE_VERIFY(PM_ERR_FLAG_SUCCESS == err_flag);
  return vbat_min;
}
...
pm_app_chg_battery_uv_oc_detection()
{
  ...
  pm_model_type pmic_model = pm_get_pmic_model(charger_index);

  vcell = pm_app_chg_get_cell_vbat_min();
  if (((PMIC_IS_PM8350B == pmic_model) || (PMIC_IS_PM7325B == pmic_model)) && (vcell < vcell_thld))
  {
...
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_scpq_misc_wdog_cfg_type  wdog_cfg;
  uint32 charger_index = PMIC_INDEX_MAX;
  ...
  if(NULL != pm_scpq_get_data(charger_index))
  {
    err_flag |= pm_scpq_misc_get_config_wdog(charger_index, &wdog_cfg);
    ...
    err_flag |= pm_scpq_misc_config_wdog(charger_index, wdog_cfg);
  }
...
pm_app_chg_pet_wdog(void)
{
  ...
  if(NULL != pm_scpq_get_data(charger_index))
  {
    err_flag |= pm_scpq_misc_bark_bite_wdog_pet(charger_index);
    if(PM_ERR_FLAG_SUCCESS != err_flag)
    {
...
static boolean
pm_app_chg_is_charger_pmic_available(void)
{
  pm_dt_charger_config *charger_dt_config = pm_get_charger_dt_config();
  uint32 charger_pmic_index = charger_dt_config->charger_pmic_index;
  boolean available = FALSE;

  if(NULL != pm_scpq_get_data(charger_pmic_index) ||
     NULL != pm_schg_get_data(charger_pmic_index))
  {
    available = TRUE;
  }
...
pm_err_flag_type pm_app_chg_aicl_from_max_for_low_vbat(void)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  boolean battery_is_2s = FALSE;

  if(pm_app_chg_is_charger_pmic_available())
  {
    err_flag |= pm_chg_is_2s_batt(&battery_is_2s);
    if(err_flag != PM_ERR_FLAG_SUCCESS)
    {
    ...
    uint32 voltage_in_mv = 0;
    err_flag |= pm_chg_get_vbatt(&voltage_in_mv);


    if(!battery_present || (voltage_in_mv <vcell_thld))
    {
      err_flag |= pm_app_chg_wait_apsd_and_aicl_from_max();
    }
  }
...
pm_app_chg_wait_apsd_and_aicl_from_max(void)
{
  ...
  uint8 typec_status = 0;
  static uint8 run_apsd_once = 0;
  pm_dt_charger_config *charger_dt_config = pm_get_charger_dt_config();
  uint32 pmic_index = charger_dt_config->charger_pmic_index;
  ...
  err_flag |= pm_comm_read_byte(pmic_index,SCHG_P_TYPEC_TYPE_C_STATE_MACHINE_STATUS , &typec_status);

  if ((typec_status == 0x2C) && (run_apsd_once == 0) && ((pmic_index == PMIC_I) && (pm_get_pmic_model(PMIC_I) == PMIC_IS_PM7250B)))
  {
     err_flag = pm_chg_is_battery_present(&battery_present);
     uint32 voltage_in_mv = 0;
     boolean battery_is_2s = FALSE;
     ...
     if(!battery_present || (voltage_in_mv <vcell_thld))
     {
		   err_flag |= pm_comm_write_byte(pmic_index,SCHG_P_USB_TYPE_C_CFG , 0x40);
 		   err_flag |= pm_comm_write_byte(pmic_index,SCHG_P_USB_CMD_APSD , 0x01);
		   run_apsd_once = 1;
		   pm_log_message("setting apsd re-run");
	   }
   }
