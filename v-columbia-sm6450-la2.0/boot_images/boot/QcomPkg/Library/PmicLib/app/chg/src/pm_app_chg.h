...
/* 
 * If the target is chargable, and battery is not present or VBAT low,
 * wait until APSD complete and then re-run AICL to start from max..
 * @return: PM_ERR_FLAG_SUCCESS if success, otherwise error code.
 *
 */
pm_err_flag_type
pm_app_chg_aicl_from_max_for_low_vbat(void);
