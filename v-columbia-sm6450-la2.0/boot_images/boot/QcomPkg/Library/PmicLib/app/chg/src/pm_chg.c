...
pm_chg_get_ibatt(int32* current_in_ma)
{
  ...
  if(NULL != pm_qbg_get_data(pmic_index))
  {
  ...
  else if(NULL != pm_qgauge_get_data(pmic_index))
  {
    err_flag =  pm_qgauge_get_ibat(pmic_index, current_in_ma);
    *current_in_ma = (*current_in_ma)* scale /10;
  }
  else
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
...
#define MIN_CELL_OCV   3000
  ...
  if(NULL != pm_qbg_get_data(pmic_index))
  {
    err_flag = pm_qbg_get_qbg_mode_ctl1_batt_opt(pmic_index, &is_1s_battery, &batt_2s_option);
    if(PM_ERR_FLAG_SUCCESS != err_flag)
    {
  ...
  {
    err_flag = pm_qgauge_get_pon_ocv_acc_data(pmic_index, PM_QG_ACC0, &data0);
  }
  else
  {
    return PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  ...
  *voltage_in_mv = cell1_vbatt = (uint64)data0 * QBG_VBAT_NUMR / QBG_VBATT_DENR / 1000;
  ...
  if(is_1s_battery || batt_2s_option == PM_CHG_DIS_VBAT_2S_MID)
  {
...
pm_err_flag_type 
pm_chg_get_typec_connector_type(pm_typec_connector_type* typec_connector_type)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 pmic_index = PMIC_INDEX_MAX;
  
  if(NULL == typec_connector_type)
  {
    return PM_ERR_FLAG_INVALID_PARAMETER;
  }
  ...
  {
    err_flag = pm_schg_typec_get_usb_connector_type(pmic_index, typec_connector_type);
  }
  else
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
...
pm_chg_is_2s_batt(boolean* battery_type_is_2s)
{
  ...
  {
    err_flag |= pm_scpq_chgr_get_bat_2s_chg_cfg(charger_index, &is_2s_batt);
  }
