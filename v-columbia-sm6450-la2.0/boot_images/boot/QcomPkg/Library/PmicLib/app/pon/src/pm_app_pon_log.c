...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/app/pon/src/pm_app_pon_log.c#11 $
$Author: pwbldsvc $
$DateTime: 2023/02/24 04:44:52 $
when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/26/23   UD      add pm_app_pon_get_all_pon_events
...
#define INFO_LVL_LOG 2

#define PON_EVENT_LOG_SIGNATURE 0x474F4C50 //PLOG
...
static void pm_output_pon_log_buffer();
...
static pm_err_flag_type pm_pon_parse_pon_pbs_log()
{
  ...
  if(sw_config_dt != NULL)
  {
    verbose_pon_logging = sw_config_dt->verbose;
  }
  ...
  if(INFO_LVL_LOG == verbose_pon_logging)
  {
    pm_output_pon_log_buffer();
  }
  ...
  do
  {
    ...
    while(end_index >= start_index)
    {
      ...
      case PM_PON_EVENT_FAULT_PMIC_O:
...
static pm_err_flag_type pm_get_fault_reason_string(pm_pon_event_type event, uint8 fault1, uint8 fault2, uint8 fault3, char* fault_string, uint8 fault_string_size)
{
  ...
  char* fault_reason2[8] = {"", "", "", "FAULT_N", "FAULT_WATCHDOG_TO", "PBS_NACK", "RESTART_PON", "OTST3"};
  ...
  char* gp_fault_follower_pmic[] = {"MISC_VREG_OCP", "PBUS FAULT", "SPMI1_SLAVE_WDOG", "INTR_WDOG", " ", " ", " ", " ", " ", " " , " ", " "};
  ...
  if(fault1 != 0)
  {
    for(index = 0; index < 8; ++index)
    {
      if((fault1 & (1 << index)) != 0)
      {
        ...
        {
          if(event == PM_PON_EVENT_FAULT_REASON_PART1)// for Tokei
          {
  ...
  if(fault3 != 0)
  {
    for(index = 0; index < 8; ++index)
    {
      if((fault3 & (1 << index)) != 0)
      {
        ...
        strlcat(fault_string, fault_reason3[index], fault_string_size);

        if(event == PM_PON_EVENT_FAULT_REASON_PART1)// For Tokei
        {
          strlcat(fault_string, gp_fault_primary_pmic[index+4], fault_string_size); // adding offset 4 , as gp_fault 0-3 are present in fault reason1.
        }
        else
        {
          strlcat(fault_string, gp_fault_follower_pmic[index+4], fault_string_size);
        }
      }
    }
  }
...
static void pm_log_fundamental_reset(pm_pon_log_type pon_log)
{
  boolean xvdd_reset = FALSE, dvdd_reset = FALSE;
  char* s3_reason = NULL;
  uint8 state = pon_log.state;
  uint8 pon_pbl_status = pon_log.byte1;
  uint8 s3_reset_reason = pon_log.byte0;

  // BIT6: XVDD_RB_OCCURRED
  // BIT7: DVDD_RB_OCCURRED
  if(pon_pbl_status & 0x40)
  {
    xvdd_reset = TRUE;
  }
  else if(pon_pbl_status & 0x80)
  {
    // if xvdd_reset TRUE, don't care if dvdd_reset is TRUE or NOT
    dvdd_reset = TRUE;
  }
  ...
  if((state != 0)
    ...
    || ((s3_reset_reason != 0) && (0 != (s3_reset_reason & (s3_reset_reason - 1)))))
  {
    // state == 0 means fault
...
static void pm_log_pon_trigger(pm_pon_log_type log)
{
  ...
  case PON_PBS_SYS_OK_RSN_TIOMAN:
  case PON_PBS_SYS_OK_RSN_NEBULA:
...
static void pm_log_fault_reason(pm_pon_event_type event, uint8 fault1, uint8 fault2, uint8 fault3)
{
#define FAULT_REASONS_SIZE 32
  char fault_reasons[FAULT_REASONS_SIZE] = {0};
  char fault_pmic_name[FAULT_REASONS_SIZE] = {0};
  ...
  if(PM_ERR_FLAG_SUCCESS == pm_get_fault_reason_string(event, fault1, fault2, fault3, fault_reasons, FAULT_REASONS_SIZE)
    && (PM_ERR_FLAG_SUCCESS == pm_pon_get_fault_pmic_string(event, fault_pmic_name, FAULT_REASONS_SIZE)))
  {
    pm_log_message("%s on %s", fault_reasons, fault_pmic_name);
  }
  else
  {
    // Output raw data if the fault event is not pre-defined
    pm_log_message("Event 0x%02X, Fault data: 0x%02X 0x%02X 0x%02X", (int)event, fault1, fault2, fault3);
  }
}
