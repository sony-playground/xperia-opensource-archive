...
DALResult DAL_AdcDeviceAttach(DALDEVICEID DevId, DalDeviceHandle **phAdcDev)
{
   ...
   if (CHIPINFO_FAMILY_SM8250 == DalChipInfo_ChipFamily())
   {
      if (pm_get_pmic_model(1) == PMIC_IS_PM855B)
      {
         pszAdcBspPropName = "ADC_BSP_PM855_PM855B_PM855L";
      }
   }
   else if (CHIPINFO_FAMILY_LAHAINA == DalChipInfo_ChipFamily())
   {
      pszAdcBspPropName = "ADC_BSP_PMK8350_PM8350_PM8350B";
   }
   
   if((CHIPINFO_FAMILY_FILLMORE == DalChipInfo_ChipFamily())
      || (CHIPINFO_FAMILY_CLARENCE == DalChipInfo_ChipFamily()))
   {
    uint32 charger_pmic_index = PMIC_INDEX_MAX;
    err_flag |= pm_chg_get_primary_charger_pmic_index(&charger_pmic_index);
    if(err_flag == PM_ERR_FLAG_SUCCESS)
    {
      pm_model_type pmic_model = pm_get_pmic_model(charger_pmic_index);

      switch(pmic_model)
      {
        case PMIC_IS_PM7250B:
          pszAdcBspPropName = "ADC_BSP_PMK8350_PM8350_PM7250B";
          break;
        case PMIC_IS_PMI632:
          pszAdcBspPropName = "ADC_BSP_PMI632";
          break;
        case PMIC_IS_PM7325B:
        case PMIC_IS_PM8350B:
          pszAdcBspPropName = "ADC_BSP_PMK8350_PM8350_PM8350B";
          break;
        default:
          break;
   ...
   /* Get the properties */
   result = DALSYS_GetDALPropertyHandleStr("/core/pmic/adc", hAdcProperties);
   if (result != DAL_SUCCESS)
   {
      return result;
   }
