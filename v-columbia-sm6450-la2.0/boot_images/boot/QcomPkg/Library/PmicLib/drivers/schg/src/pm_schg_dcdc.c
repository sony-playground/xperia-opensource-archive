...
pm_err_flag_type pm_schg_dcdc_configure_vsysmin(uint32 pmic_index, uint32 vsysmin_mv)
{
  ...
  uint32 vsysmin_cfg_max = 3400;
  uint32 vsysmin_cfg_min = 2700;
  const uint32 vsysmin_cfg_step = 100;
...
pm_err_flag_type pm_schg_dcdc_set_ref_max_psns(uint32 pmic_index, uint32 current_mA)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_schg_data_type *schg_ptr = pm_schg_get_data(pmic_index);
  ...
  else if(current_mA > CFG_REF_MAX_PSNS_MAX)
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  }
  else
