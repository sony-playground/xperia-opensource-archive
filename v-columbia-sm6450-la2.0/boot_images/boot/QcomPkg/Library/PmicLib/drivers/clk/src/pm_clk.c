...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/clk/src/pm_clk.c#2 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_clk_drv_strength(uint32 pmic_chip, pm_clk_type periph, pm_clk_drv_strength_type drive_strength)
{
    ...
    /* Set strength */
    err_flag = pm_comm_write_byte_mask(clk_ptr->comm_ptr->slave_id, reg, DRV_STR_MASK, (pm_register_data_type)(drive_strength));  
...
pm_err_flag_type pm_clk_pin_ctrled(uint32 pmic_chip, pm_clk_type periph, pm_on_off_type on_off)
{
    ...
    reg = REG_CLK(revision2);
	err_flag = pm_comm_read_byte(SLAVE_ID, reg, &data);
	
	reg = (data >= DIG_MAJ_VER_4) ? REG_CLK(pc_en_ctl) : REG_CLK(en_ctl) ;

	data = (PM_ON == on_off) ? 0xFF : 0x00 ;
...
pm_err_flag_type pm_clk_get_status(uint32 pmic_chip, pm_clk_type clk, pm_clk_status_type *clk_status)
{
    ...
    if (clk_ptr->periph_subtype[periph_index] != PM_HW_MODULE_CLOCK_XO_CORE)
    {
        reg = clk_ptr->base_address + (pm_register_address_type)(clk_ptr->clk_reg_table->peripheral_offset * periph_index) +
           clk_ptr->clk_reg_table->drv_ctl1;
        /* Set strength */
        err_flag = pm_comm_read_byte_mask(clk_ptr->comm_ptr->slave_id, reg, 0x03, &data);
