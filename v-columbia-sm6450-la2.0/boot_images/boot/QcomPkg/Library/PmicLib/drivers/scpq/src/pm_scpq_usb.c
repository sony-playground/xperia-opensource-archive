                                              ...
                                              pm_scpq_usb_aicl_step_timming_sel_type step_timming_sel)
{
  ...
  else if(aicl_cfg_type >= PM_SCPQ_USB_AICL_INVALID)
  {
    err_flag = PM_ERR_FLAG_INVALID_PARAMETER;
  }
  else
  {
    switch(aicl_cfg_type)
    {
    case PM_SCPQ_USB_AICL_COLLAPSE_GF_SEL:
      {
        pm_register_address_type usb_aicl_cfg = ADDRESS(usb_aicl_cfg);
        err_flag = pm_comm_write_byte_mask(SLAVE_ID, usb_aicl_cfg, (3 << PM_SCPQ_USB_AICL_COLLAPSE_GF_SEL), collapse_gf_sel);
        break;
      }

    case PM_SCPQ_USB_AICL_STEP_TIMING_SEL:
      {
        pm_register_address_type usb_aicl_cfg = ADDRESS(usb_aicl_cfg);
        err_flag = pm_comm_write_byte_mask(SLAVE_ID, usb_aicl_cfg, (3 << PM_SCPQ_USB_AICL_STEP_TIMING_SEL), step_timming_sel);
        break;
      }

    case PM_SCPQ_USB_AICL_START_AT_MAX:
    case PM_SCPQ_USB_AICL_HDC_EN:
    case PM_SCPQ_USB_AICL_EXT_BST_EN:
    case PM_SCPQ_USB_AICL_EN:
      {
        pm_register_address_type usb_aicl_cfg = ADDRESS(usb_aicl_cfg);
