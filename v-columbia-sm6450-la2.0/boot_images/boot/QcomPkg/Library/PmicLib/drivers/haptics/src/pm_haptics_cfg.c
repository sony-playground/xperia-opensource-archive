...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/haptics/src/pm_haptics_cfg.c#8 $
$Author: pwbldsvc $
$DateTime: 2021/07/27 15:30:56 $
...
pm_err_flag_type pm_haptics_cfg_play(uint32 pmic_chip,  pm_on_off_type enable) 
{
  ...
  if (enable == PM_ON) {
    ...
    err_flag |= pm_comm_write_byte(slave_id, HAPTICS_REG_ADDR(autores_cfg), 0xB2);   
    err_flag |= pm_comm_write_byte(slave_id, HAPTICS_REG_ADDR(drv_duty), 0x66);   
    err_flag |= pm_comm_write_byte(slave_id, HAPTICS_REG_ADDR(fifo_play_rate), 0x00);
    ...
    err_flag |= pm_comm_write_byte(slave_id, HAPTICS_REG_ADDR(autores_cfg), 0x8A);   
