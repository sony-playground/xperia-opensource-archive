...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs.c#2 $
$Author: pwbldsvc $
...
pm_pbs_ram_sequences_verify(uint32 slave_id, uint16 ram_addr, uint32* seq_ptr, uint8 num_seqs, uint16 periph_id) 
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 seq_read = 0;
  uint8 seq_cnt = 0;
  uint16 pbs_core_base_addr = periph_id * 0x100;
  /* Configuring read access in burst mode to PBS RAM */
  err_flag = pm_pbs_config_access(slave_id, ram_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, periph_id);
  ...
  for (seq_cnt = 0;seq_cnt < num_seqs;seq_cnt++)
  {
    ...
    err_flag = pm_comm_read_byte_array(slave_id, pbs_core_base_addr + MEM_INTF_RD_DATA0_ADDR, 4, (uint8*)&seq_read);
 ...
 */
static pm_err_flag_type 
pm_pbs_ram_sequences_load(uint32 slave_id, uint16 ram_addr, uint32* seq_ptr, uint8 num_seqs,uint16 periph_id) 
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 seq_cnt = 0;
  uint16 pbs_core_base_addr = periph_id * 0x100;
  ...
  err_flag = pm_pbs_config_access(slave_id, ram_addr, PM_PBS_ACCESS_WRITE_BURST, PM_PBS_TYPE_RAM, 1);
  ...
  for (seq_cnt = 0;seq_cnt < num_seqs;seq_cnt++)
  {
    ...
    err_flag = pm_comm_write_byte_array(slave_id, pbs_core_base_addr + MEM_INTF_WR_DATA0_ADDR, 4, (uint8*)&seq_ptr[seq_cnt]);
  ...
  err_flag = pm_pbs_ram_sequences_verify(slave_id, ram_addr, seq_ptr, num_seqs,periph_id);
