...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_driver.c#4 $
$Author: pwbldsvc $
...
#define PBS_CORE_NVM_CFG1_ADDR         0xA2
#define PBS_CORE_NVM_CFG2_ADDR         0xA3
#define FOUR_BYTES_MASK                0xFFFFFFFF

#define PMIC_PBS_CORE_MAX_HW_VER       0x52
                         ...
                         uint16 *otp_size, uint16 *pbs_size)
{
  ...
  err_flag |= pm_comm_read_byte(slave_id, pm_pbs_data->perph_base_addr + PBS_CORE_REVISION2_ADDR,
                                &maj_ver);
  err_flag |= pm_comm_read_byte(slave_id, pm_pbs_data->perph_base_addr + PBS_CORE_NVM_CFG1_ADDR,
                                &otp_size_base);

  err_flag |= pm_comm_read_byte(slave_id, pm_pbs_data->perph_base_addr + PBS_CORE_NVM_CFG2_ADDR,
                                &otp_size_mult);
  ...
  *otp_size = otp_size_base * (1 << (otp_size_mult & otp_mask)) * 8;
  ...
  else if (supported_version >= PM_PBS_VERSION_5P0)
  {
    ...
    err_flag |= pm_comm_read_byte(slave_id, pm_pbs_data->perph_base_addr + PBS_CORE_RAM_CFG3_ADDR, &pbs_size_base_msb);
...
pm_pbs_ram_image_load(uint32 slave_id, pm_pbs_ram_data_type *data, unsigned size, uint8 periph_id)
{
  int i;
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint8 pmic_index, read_perphtype, read_perphsubtype;
  pm_pbs_info_data_type **pm_pbs_temp_data = NULL;
  pm_pbs_info_data_type *pm_pbs_data = NULL;
  uint16 pbs_core_base_addr = periph_id * 0x100;
  uint32 support_version = 0;
  uint8 step_size = 0; 
  uint8 wr_byte_size = 0; 
  ...
  else
  {
    ...
    for (i = 0; i < size; i = i + step_size)
    {
    err_flag = pm_comm_write_byte_array(slave_id, pbs_ram_wr_addr, wr_byte_size, (uint8 *)&data[i]);
      if (err_flag != PM_ERR_FLAG_SUCCESS)
      {
        return err_flag;
      }
  ...
  err_flag = pm_pbs_disable_access(slave_id, FALSE, periph_id);
  if (err_flag != PM_ERR_FLAG_SUCCESS)
  {
    return err_flag;
  }
