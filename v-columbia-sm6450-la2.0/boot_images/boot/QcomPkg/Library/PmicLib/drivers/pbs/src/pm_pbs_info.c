...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/pbs/src/pm_pbs_info.c#3 $
$Author: pwbldsvc $
...
static pm_err_flag_type pm_pbs_info_otp_read (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_pbs_config_access(slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_ROM, 1);
  ...
  err_flag = pm_comm_read_byte_array(slave_id, PMIO_PBS1_CORE_MEM_INTF_RD_DATA0_ADDR, 4, (uint8*)&read_info_temp);
  ...
  err_flag = pm_pbs_disable_access(slave_id, PM_PBS_TYPE_ROM, 1);
...
static pm_err_flag_type pm_pbs_info_ram_read (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_pbs_config_access(slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, 1);
  ...
  /* Configuring to disable PBS core read access */
  err_flag = pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM, 1);
...
static pm_err_flag_type pm_pbs_info_read_from_periph (uint32 slave_id, pm_pbs_info_smem_type *pbs_info_ptr)
{
  ...
  err_flag = pm_pbs_config_access(slave_id, pbs_element_info_temp_addr, PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_ROM, 1);
     ...
     err_flag = pm_comm_read_byte_array(slave_id, PMIO_PBS1_CORE_MEM_INTF_RD_DATA0_ADDR, 4, (uint8*)&read_info_temp32);
     if (err_flag != PM_ERR_FLAG_SUCCESS)
     {
    	return err_flag;
     }
...
pm_err_flag_type pm_pbs_info_rom_init (void) 
{
  ...
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    ...
    err_flag = pm_comm_read_byte(slave_id, PMIO_PBS1_CORE_PERPH_TYPE_ADDR, &data);
    ...
    {
      /* Configuring to disable PBS core read access */
      pm_pbs_disable_access(slave_id, PM_PBS_TYPE_ROM, 1);
...
pm_err_flag_type pm_pbs_info_ram_init (void) 
{
  ...
  for (pmic_index = 0; pmic_index < PM_MAX_NUM_PMICS; pmic_index++)
  {
    ...
    {
      /* Configuring to disable PBS core read access */
      pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM, 1);
...
pm_err_flag_type pm_pbs_mem_read(uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
    ...
    err_flag |= pm_pbs_config_access(slave_id, start_address , PM_PBS_ACCESS_READ_BURST, PM_PBS_TYPE_RAM, 1);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
    	err_flag |= pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM, 1);
        return err_flag;
    ...
    {
       	err_flag |= pm_comm_read_byte_array(slave_id, PMIO_PBS1_CORE_MEM_INTF_RD_DATA0_ADDR, 4, (uint8 *)&read_data);
       	if (err_flag != PM_ERR_FLAG_SUCCESS)
       	{
          	err_flag |= pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM, 1);
    		return err_flag;
...
pm_err_flag_type pm_pbs_mem_write(uint8 slave_id, uint32 start_address, uint32* data_ptr, uint32 data_size)
{
    ...
    /* Configuring to enable PBS core access for ROM reads */   
    err_flag |= pm_pbs_config_access(slave_id, start_address , PM_PBS_ACCESS_WRITE_BURST, PM_PBS_TYPE_RAM, 1);
    if (err_flag != PM_ERR_FLAG_SUCCESS)
    {
       	err_flag |= pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM, 1);
        return err_flag;
    ...
    for (i =0 ; i < data_size ; i++)
    {
       err_flag |= pm_comm_write_byte_array(slave_id, PMIO_PBS1_CORE_MEM_INTF_WR_DATA0_ADDR, 4, (uint8*)&data_ptr[i]);
        if (err_flag != PM_ERR_FLAG_SUCCESS)
        {
          err_flag |= pm_pbs_disable_access(slave_id, PM_PBS_TYPE_RAM, 1);
          return err_flag;
