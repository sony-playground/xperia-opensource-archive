...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/pon/src/pm_pon.c#9 $
$Author: pwbldsvc $
...
#define SUPPORTED_DIG_VERSION (0x01<<8 | 0x02) // DIG_MAJOR 1 , DIG_MINOR 2
#define SUPPORTED_ANA_VERSION (0x00<<8 | 0x00) // ANA_MAJOR 0 , ANA_MINOR 0
...
static pm_err_flag_type pm_pon_event_store_and_sort(void)
{
  ...
  { 
    err_flag |= pm_comm_read_byte(0, PON_LOG_PUSH_PTR, &push_ptr); 
    err_flag |= pm_comm_read_byte_array(0, PON_PBS_PON_LOGGING_AREA_START, PON_EVENT_LOG_AREA_SIZE, pon_events_logged); 
    push_ptr = PON_LOG_PUSH_PTR_INDEX(push_ptr);
