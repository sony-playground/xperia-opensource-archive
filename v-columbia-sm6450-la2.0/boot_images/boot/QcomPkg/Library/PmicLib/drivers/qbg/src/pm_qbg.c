...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/qbg/src/pm_qbg.c#2 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_qbg_set_qbg_mode_ctl1_batt_opt(uint32 device_index, BOOLEAN is_1s_battery, pm_chg_2s_battery_option_type option_2s)
{
  ...
  pm_register_data_type mask   = 0xE0;
  ...
  else
  {
    ...
    data = (option_2s << 5) | (is_1s_battery << 7);
...
pm_err_flag_type pm_qbg_get_qbg_mode_ctl1_batt_opt(uint32 device_index, BOOLEAN* p_is_1s_battery, pm_chg_2s_battery_option_type* p_2s_option)
{
  ...
  if (NULL == qbg_ptr || NULL == p_is_1s_battery || NULL == p_2s_option)
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  }
  else
  {
    ...
    *p_is_1s_battery = ((data & 0x80) >> 7) ? TRUE : FALSE;
    *p_2s_option = (data & 0x60) >> 5;
  }
...
pm_err_flag_type pm_qbg_get_qbg_mode_ctl2_ichg_fs_opt(uint32 device_index, BOOLEAN* p_support_20A)
{
  ...
  if (NULL == qbg_ptr || NULL == p_support_20A)
  {
    err_flag = PM_ERR_FLAG_FEATURE_NOT_SUPPORTED;
  }
  else
  {
    reg = ADDRESS(mode_ctl2);
    err_flag = pm_comm_read_byte_mask(SLAVE_ID, reg, mask, &data);
    *p_support_20A = (data & 0x01)? TRUE : FALSE;
  }
