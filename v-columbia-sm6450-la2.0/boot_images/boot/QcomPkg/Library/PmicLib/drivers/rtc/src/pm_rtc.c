...
*
*  &copy; Copyright 2012 - 2020 QUALCOMM Technologies, Inc All Rights Reserved
*/
...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/drivers/rtc/src/pm_rtc.c#3 $
$Author: pwbldsvc $
$DateTime: 2021/12/13 10:45:39 $
...
pm_err_flag_type pm_rtc_alarm_enable(uint32 pmic_chip, boolean enable)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_rtc_data_type * rtc_ptr = pm_rtc_get_data(pmic_chip);
...
/* Set rtc alarm time*/
pm_err_flag_type pm_rtc_alarm_set_time(uint32 pmic_chip, pm_rtc_time_type * set_time)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_rtc_data_type * rtc_ptr = pm_rtc_get_data(pmic_chip);
  uint32 seconds = 0;
  pm_rtc_time_type current_time = {0};
