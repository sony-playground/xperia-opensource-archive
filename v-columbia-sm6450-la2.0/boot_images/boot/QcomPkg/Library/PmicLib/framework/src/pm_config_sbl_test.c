...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/framework/src/pm_config_sbl_test.c#4 $
$Author: pwbldsvc $
$DateTime: 2021/03/19 06:07:22 $
...
pm_settings_test_flag_reg_info_type settings_test_flag_reg[2] = {{0x88F},{SDAM_2_MEM_21_ADDR}};
       ...
       { 
        switch(rc->reg_operation)  
        {
           case PM_SBL_WRITE:  
           {
              ...
              { 
                err_flag |= pm_comm_read_byte_mask_ex(rc->busid, rc->sid, rc->address, rc->mask, &read_data); //address data at original location is being changed in sbl write function so not adding offset in base address
         ...
         if (last_pbs_ctl_cfg[0]->data != 0 || last_pbs_ctl_cfg[1]->data != 0)
           {
              ...
              err_flag |= pm_comm_write_byte_mask(last_pbs_ctl_cfg[0]->sid, last_pbs_ctl_cfg[0]->address, last_pbs_ctl_cfg[0]->mask, last_pbs_ctl_cfg[0]->data);
              err_flag |= pm_comm_write_byte_mask(last_pbs_ctl_cfg[1]->sid, last_pbs_ctl_cfg[1]->address, last_pbs_ctl_cfg[1]->mask, last_pbs_ctl_cfg[1]->data);
              err_flag |= pm_comm_write_byte_mask(last_pbs_ctl_cfg[2]->sid, last_pbs_ctl_cfg[2]->address, last_pbs_ctl_cfg[2]->mask, last_pbs_ctl_cfg[2]->data);
              err_flag |= pm_comm_write_byte_mask(last_pbs_ctl_cfg[3]->sid, last_pbs_ctl_cfg[3]->address, last_pbs_ctl_cfg[3]->mask, last_pbs_ctl_cfg[3]->data);
           }
        }
     }
    }
...
pm_err_flag_type pm_settings_validation_flag(uint8 *flag, boolean reset)
{
   ...
   *flag=0x00; 
   ...
   {
     flag_reg_ptr = &settings_test_flag_reg[0];
   }
   else
   {
     flag_reg_ptr = &settings_test_flag_reg[1];
   }
   ...
   if (flag_reg_ptr)
   { 
      err_flag |= pm_comm_read_byte_mask(0, flag_reg_ptr->flag_register, 0x01, flag); // read bit 0 of register retrived to decide  whether test needs to be triggered or not
   }
   if (reset) 
   {
      err_flag |= pm_comm_write_byte_mask(0, flag_reg_ptr->flag_register, 0x01, 0x00); // reset flag for validation test
   }    
   return err_flag;
}
