...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/framework/src/pm_version.c#3 $
$Author: pwbldsvc $
$DateTime: 2021/01/26 10:21:04 $
...
pm_err_flag_type pm_version_detect(void) 
{
  ...
  const             uint8 num_of_bytes_rev_id = 7;
  ...
  for (i = 0; i < PM_MAX_BUSES_SUPPORTED; i++)
  {
          ...
          slave_id += slave_count, pmic_index += arch_pre7 ? 1 : slave_count )  
    {
      ...
      err_flag = pm_comm_read_byte_array_ex(i, slave_id, base_addr, num_of_bytes_rev_id, (uint8*) pmic_rev_info);
