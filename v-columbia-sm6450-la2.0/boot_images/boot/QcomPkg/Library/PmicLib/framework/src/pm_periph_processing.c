...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/framework/src/pm_periph_processing.c#3 $
$Author: pwbldsvc $
$DateTime: 2021/08/05 01:36:28 $
...
pm_config_lock_by_bit(void)
{
  ...
  for(i = 0; i < *lock_by_bit_arr_size; i++)
  {
    ...
    rslt = SpmiBus_GetChannel((uint8)lock_by_bit_data_ptr[i].bus_id, (uint8)lock_by_bit_data_ptr[i].slave_id, (uint8)lock_by_bit_data_ptr[i].ppid, &channel);
    ...
    if(pm_is_slave_id_supported(slave_id) == TRUE) //write only if valid slave id 
    {
      ...
      if(lock_by_bit_data_ptr[i].ppid != 0x8) //don't write for PON_PBS for gen1
      {                                      
        err_flag |= pm_comm_write_byte_ex(lock_by_bit_data_ptr[i].bus_id, slave_id, addr, lock_by_bit_data_ptr[i].lock_value);
      }
    }
  }
