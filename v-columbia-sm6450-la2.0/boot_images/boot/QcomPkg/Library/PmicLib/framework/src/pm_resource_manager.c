...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/framework/src/pm_resource_manager.c#10 $
$Author: pwbldsvc $
$DateTime: 2022/12/08 01:14:34 $
...
void pm_resource_manager_create_pmic_resource(pm_comm_info_type *comm_ptr, peripheral_info_type *peripheral_info, uint8 pmic_index)
{
  switch((pm_hw_module_type)peripheral_info->peripheral_type)
  {
     ...
     case PM_HW_MODULE_LPG:
       switch(peripheral_info->peripheral_subtype)
       {
          case PM_HW_MODULE_LPG_LUT:
            pm_lpg_driver_init(comm_ptr, peripheral_info, pmic_index);
            break;
          case PM_HW_MODULE_LPG_PWM:
          case PM_HW_MODULE_LPG_PWM_BOMPOKA:
     ...
     case PM_HW_MODULE_HAPTICS:
       switch(peripheral_info->peripheral_subtype)
       {
          ...
          case PM_HW_MODULE_HAPTICS_VIB_LDO:
     ...
     case PM_HW_MODULE_CHARGER:
       switch(peripheral_info->peripheral_subtype)
       {
          case PM_HW_MODULE_CHARGER_COINCELL:
            ...
            break;
          default:
	       /*Subtype of Tioman(schg)->0x80, Nebula(schg) -> 0x80, Puhi(scpq)->0x80 and Cortes_L(scpq)->0x90, so its not helping to diff schg/scpq
            Detecting Tioman/Nebula specifically to install schg module*/
           if((PMIC_IS_PM7250B == pm_get_pmic_model(pmic_index)) || (PMIC_IS_PMI632 == pm_get_pmic_model(pmic_index)))
		   {
     ...
     case PM_HW_MODULE_FG:
       switch(peripheral_info->peripheral_subtype)
       {
         case PM_HW_MODULE_QBG_MAIN:
           pm_qbg_driver_init(comm_ptr, peripheral_info, pmic_index);
           break;
         case PM_HW_MODULE_BMS_QGAUGE:
         case PM_HW_MODULE_BMS_QGAUGE2:
           pm_qgauge_driver_init(comm_ptr, peripheral_info, pmic_index);
           break;
         default:
           break;
       }
