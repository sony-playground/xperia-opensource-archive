...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/framework/src/pm_target_information.c#1 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_target_information_init ( void )
{
  ...
  if ( pm_target_info_init == TRUE )
  {
    return err_flag;
  }

  if ( DAL_SUCCESS !=
       DALSYS_GetDALPropertyHandleStr("/pmic/target", hProp_target) )
  {
    DALSYS_LogEvent(
...
pm_target_information_get_specific_info ( uint32 prop_id )
{
  ...
  if ( DAL_SUCCESS ==
       DALSYS_GetPropertyValue(hProp_target, NULL, prop_id, &prop_target) )
  {
                                              ...
                                              uint8 pmic_index )
{
  ...
  if ( DAL_SUCCESS == DALSYS_GetPropertyValue(hProp_target, NULL,
                                              prop_id, &prop_target) )
  {
