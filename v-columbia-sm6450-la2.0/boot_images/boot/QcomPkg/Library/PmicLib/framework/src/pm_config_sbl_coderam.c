...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/framework/src/pm_config_sbl_coderam.c#4 $
$DateTime: 2021/11/16 06:57:39 $  $Author: pwbldsvc $
...
pm_psi_get_heap_buffer_info(void **base_addr, uint32 *buf_size)
{
   ...
   if (ptstart > ((uintptr_t)ptpdm->pm_sdam_seq)) ptstart = (uintptr_t)ptpdm->pm_sdam_seq;
...
pm_psi_free_config_buffer(void)
{
   ...
   err_flag = pm_psi_get_heap_buffer_info(&heap_base_addr, &heap_mem_size);
   if (err_flag != PM_ERR_FLAG_SUCCESS) {
      return err_flag;
   }

   status = DALSYS_HeapInit(heap_base_addr, heap_mem_size, DALHEAP_FREE_SLOTS);
