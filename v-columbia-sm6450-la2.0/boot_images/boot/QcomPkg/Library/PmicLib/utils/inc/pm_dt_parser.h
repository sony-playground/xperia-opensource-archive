...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/utils/inc/pm_dt_parser.h#28 $
$Author: pwbldsvc $
...
typedef struct
{
  boolean  verbose;             // for verbose logging
  boolean  apply_rsns_trim;     // Whether to apply rsns trim value
  uint32   rsns_trim_value;     // R-sense trim value
  boolean   misc_gpio_invert;	//misc gpio invert
...
typedef struct
{
  ...
  boolean bq25790_enable;
  uint32 bq25790_i2c_instance;
  uint8 sysok_cfg_option;
  uint8 bcl_enable_option;
...
{
  uint32 reset_gpio[2];
  uint32 leica2_1_gpio[2];
  uint32 leica2_2_gpio[2];
}pm_dt_camera_config;
