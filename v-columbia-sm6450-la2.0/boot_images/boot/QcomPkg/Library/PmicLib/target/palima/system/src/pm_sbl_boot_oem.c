...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/palima/system/src/pm_sbl_boot_oem.c#11 $
$Author: pwbldsvc $
...
#define NO_OF_TIMES_HARD_RESET 3
...
pm_err_flag_type
pm_check_leica_status()
{
   uint8 leica1_status , leica2_status;
   uint8 no_of_times_hr = 0;
   pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
   DalPlatformInfoPlatformType platform_type = DALPLATFORMINFO_TYPE_UNKNOWN;
   platform_type = DalPlatformInfo_Platform();
   if(DALPLATFORMINFO_TYPE_ATP == platform_type){
     //No WA for ATP platforms 
     pm_log_message("PMICSW: Platform type is ATP");
     return err_flag;
   }
   err_flag = pm_comm_read_byte_ex(0, PMIC_A, 0x72BC, &leica1_status);
   err_flag = pm_comm_read_byte_ex(0, PMIC_A, 0x72BD, &leica2_status);
   err_flag = pm_comm_read_byte_ex(0, PMIC_A, 0x72B6, &no_of_times_hr);
   if(0x1 == leica1_status || 0x1 == leica2_status){
      no_of_times_hr = no_of_times_hr + 1;
      pm_comm_write_byte_ex(0, PMIC_A, 0x72B6, no_of_times_hr);
      if(no_of_times_hr < NO_OF_TIMES_HARD_RESET){
        /*Go for a Hard reset*/
        boot_hw_reset(BOOT_HARD_RESET_TYPE);
      }
      else{
        pm_log_message("Leica2 Detection is failing even after %d Hard Resets",no_of_times_hr);
        //Reset this SDAM MEM so that the WA can be run when the device resets again after this
        no_of_times_hr = 0;
        err_flag = pm_comm_write_byte_ex(0, PMIC_A, 0x72B6, no_of_times_hr);
      }
   }
   else{
      /*Reset the HR counter */
      no_of_times_hr = 0;
      pm_comm_write_byte_ex(0, PMIC_A, 0x72B6, no_of_times_hr);
   }
   return err_flag;
...
pm_driver_post_init(void)
{
  ...
  err_flag |= pm_gpio_cfg_mode(PMIC_C, PM_GPIO_7, 0x2);
...
pm_err_flag_type pm_tgt_aop_pre_init(void)
{
  ...
  err_flag = pm_check_leica_status();
