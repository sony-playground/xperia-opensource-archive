...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/palima/system/src/pm_pon_target.c#2 $
$Author: pwbldsvc $
...
pm_err_flag_type pm_pon_get_fault_pmic_string(pm_pon_event_type event, char* fault_pmic, int size)
{
  ...
  case PM_PON_EVENT_FAULT_REASON_PART2:
    ...
    break;
  case PM_PON_EVENT_PAPIO_FAULT:
    strlcpy(fault_pmic, "Papio", size);
    break;
  ...
  case PM_PON_EVENT_SECO_FAULT:
    ...
    break;
  case PM_PON_EVENT_NALO_FAULT:
    strlcpy(fault_pmic, "Nalo", size);
    break;
  case PM_PON_EVENT_HAPUNA_FAULT:
    strlcpy(fault_pmic, "Hapuna", size);
    break;
...
pm_err_flag_type pm_get_target_specific_pon_trig_string(uint16 pon_dataset, char** pon_trig_str)
{
  ...
  switch(pon_dataset)
  {
  case PON_PBS_PAPIO_OCP_RSN:
    *pon_trig_str = "PAPIO OCP";
    break;
