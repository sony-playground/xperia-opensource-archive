...
$Header: //components/dev/core.boot/6.0/aravapal.core.boot.6.0.tip_dev/QcomPkg/Library/PmicLib/target/netrani/system/src/pm_sbl_boot_oem.c#1 $
$Author: aravapal $
...
static pm_err_flag_type pm_bclbig_comp_en_ctl(void);

static void adc_config_parallel_sns_k()
{
  uint16 tioman_data = 0xC000;
  if(pm_get_pmic_model(PMIC_I) == PMIC_IS_PM7250B)
  {
	pm_comm_write_byte(0x8, 0x306C, (uint8)tioman_data); //ADC_FG7_CMN2_PARALLEL_SNS_K_D0
    pm_comm_write_byte(0x8, 0x306D, (uint8)(tioman_data >> 8)); //ADC_FG7_CMN2_PARALLEL_SNS_K_D1
  }
}
...
static pm_err_flag_type pm_config_ibat_rsns(void)
{
  ...
  if(charger_dt_config->ibat_via_ext_sense)
  {
	...
	err_flag |= pm_comm_write_byte(charger_dt_config->charger_pmic_index,ADC_CMN2_EXT_SNS_RESISTOR_SEL,ext_rsns_sel);
...
pm_update_charger_pmic_index(void)
{
  ...
  //update haptics pmic index to Nebula if it is detected. this fix is specific to Clarence target.
  ...
  {
    pm_log_message("failed to fetch haptics config ptr\n");
	return PM_ERR_FLAG_INVALID_CONFIG_SETTING; 
  }
  
  if(pm_get_pmic_model(charger_dt_config->charger_pmic_index) == PMIC_IS_PMI632)
  {
    haptics_config_ptr->haptics_pmic = PMIC_C;
  }
...
static pm_err_flag_type
pm_misc_gpio_invert(void)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  uint32 bompoka_pmic_index = 0x1; //Bompoka
  uint32 reg_addr = 0x972; //MISC_GPIO_INVERT
  uint8 mask = 0x1; //bit0

  pm_dt_sw_config* sw_config_dt = pm_dt_get_node(PMIC_DT_NODE_PM_SW_CONFIG);
  if(NULL != sw_config_dt)
  {
    if(sw_config_dt->misc_gpio_invert)
    {
	  err_flag |= pm_comm_write_byte_mask(bompoka_pmic_index, reg_addr, mask, 0x0); //clear bit0
    }
  }
  return err_flag;
}
...
pm_device_pre_init(void)
{
  ...
  //WA to enable 5mOhm ext sense for Nebula. No need for scaling
  if(pm_get_pmic_model(PMIC_C) == PMIC_IS_PMI632)
  {
    err_flag |= pm_comm_write_byte(0x2,0x305E,0x82); //ADC_QG7_CMN2_EXT_SNS_RESISTOR_SEL
  }
  
  //ext rsns enable/disable and adc scaling are dt configurable
...
pm_device_post_init ( void )
{
  ...
  //WA to update ksns register settings for tioman
  adc_config_parallel_sns_k();
  //enable BCL peripheral based on dtsi entry
  err_flag |= pm_bclbig_comp_en_ctl();
...
pm_driver_post_init(void)
{
  ...
  //SD card detect abnormality fix- CR3463982
  err_flag |= pm_misc_gpio_invert();
                       ...
                       uint8 vrm_rsrc_arr_size )
{
 ...
 /*Hard Code BCL Peripherals to RPID-200 onwards*/
  (*vrm_cfg_index)=200;
  if(pm_get_pmic_model(PMIC_C) == PMIC_IS_PMI632) //Nebula
  {
    pm_vrm_rsrc_arr[*vrm_cfg_index].slaveId = 0x2;
    pm_vrm_rsrc_arr[*vrm_cfg_index].address = 0x3E00;
  (*vrm_cfg_index)++;
  }
  if(pm_get_pmic_model(PMIC_I) == PMIC_IS_PM7250B) //Tioman
  {
...
pm_err_flag_type pm_tgt_aop_pre_init(void)
{
  ...
  err_flag |= pm_tgt_wlan_clock();
  
  return err_flag;
}

pm_err_flag_type pm_tgt_wlan_clock(void)
{
...
*/
  if((platform == DALPLATFORMINFO_TYPE_IDP) && ((subtype == 0x3) || (subtype == 0x4) || (subtype == 0x5)))
  {

    err_flag |= pm_comm_write_byte(PMIC_A, XO_RF5_FREQ_DIV_REG, XO_FREQ_DIV1);
    err_flag |= pm_ldo_sw_enable(PMIC_B, PM_LDO_11, PM_OFF, TRUE);
    if(pm_get_pmic_model(PMIC_I) == PMIC_IS_PM7250B)
    {
      charger_dt_config = (pm_dt_charger_config*)pm_dt_get_node(PMIC_DT_NODE_PM_CHARGER);
      CORE_VERIFY_PTR(charger_dt_config);

      if(charger_dt_config->sysok_cfg_option == 0x1)
        err_flag |= pm_comm_write_byte_mask(PMIC_I, 0x1694, 0x03, 0x1);
    }
  }
...
pm_err_flag_type pm_bclbig_comp_en_ctl(void)
{
  pm_err_flag_type err_flag = PM_ERR_FLAG_SUCCESS;
  pm_dt_charger_config *charger_dt_config = NULL;
  uint8 bcl_enable = 0;
  ...
  if(pm_get_pmic_model(PMIC_I) == PMIC_IS_PM7250B)
  {
    bcl_enable = charger_dt_config->bcl_enable_option ? 0x80:0x00;
    err_flag |= pm_comm_write_byte(PMIC_I, BCLBIG_COMP_EN_CTL1, bcl_enable);
  }
