...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/netrani/system/src/pm_pon_target.c#3 $
$Author: pwbldsvc $
$DateTime: 2022/01/31 21:53:11 $
...
pm_err_flag_type pm_pon_get_fault_pmic_string(pm_pon_event_type event, char* fault_pmic, int size)
{
  ...
  case PM_PON_EVENT_BOMPOKA_FAULT:
    ...
    break;
  case PM_PON_EVENT_MAKUAC_FAULT:
    strlcpy(fault_pmic, "MakuaC", size);
    break;
...
pm_err_flag_type pm_get_target_specific_pon_trig_string(uint16 pon_dataset, char** pon_trig_str)
{
  ...
  case PON_PBS_BOMPOKA_OCP_RSN:
    *pon_trig_str = "Bompoka OCP";
    break;
  case PON_PBS_MAKUAC_OCP_RSN:
    *pon_trig_str = "MakuaC OCP";
    break;
