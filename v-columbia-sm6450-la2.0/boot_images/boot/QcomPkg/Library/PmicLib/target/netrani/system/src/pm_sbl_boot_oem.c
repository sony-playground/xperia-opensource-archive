...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/netrani/system/src/pm_sbl_boot_oem.c#16 $
$Author: pwbldsvc $
$DateTime: 2023/05/05 00:18:43 $
...
#define PM_EXT_RSNS_2x 0x1
#define PM_EXT_RSNS_2P5x 0x2

#define EXT_RES_1MOHM 0x0
...
pm_update_charger_pmic_index(void)
{
  ...
  /* Updated charger PMIC Index */      
  if((pm_get_pmic_model(PMIC_I) == PMIC_IS_UNKNOWN) && (pm_get_pmic_model(PMIC_D) == PMIC_IS_PM8350B))
  {
      charger_dt_config->charger_pmic_index = PMIC_D; //update config field with Puhi Index
  ...
  /* Updated Haptic PMIC index */
  if(pm_get_pmic_model(charger_dt_config->charger_pmic_index) == PMIC_IS_PM8350B)
  {
    haptics_config_ptr->haptics_pmic = PMIC_D;
  }
  return err_flag;
}
...
static pm_err_flag_type pm_config_ibat_rsns(void)
{
  ...
  if(charger_dt_config->ibat_via_ext_sense)
  {
      ...
      if((pm_get_pmic_model(PMIC_D) == PMIC_IS_PM8350B) || (PMIC_IS_PM7325B == pm_get_pmic_model(charger_dt_config->charger_pmic_index)))
      {
          /* set IBAT_SENSE_OPTION to EXT_ONLY(0x1) */    
          err_flag |= pm_comm_write_byte_mask(PUHI_INDEX, ADC_CMN2_IADC_DIG_PARAM2_ADDR, 0x3, EXT_RSNS_SELECTION);
      }        

      /* Set ADC scaling factor*/
...
pm_driver_post_init(void)
{
  ...
  err_flag |= pm_gpio_cfg_mode(PMIC_B, PM_GPIO_1, PM_GPIO_DIG_IN);
  
  err_flag |= pm_gpio_set_pull_sel(PMIC_B, PM_GPIO_1, PM_GPIO_PULL_UP_1P5UA);
                       ...
                       uint8 vrm_rsrc_arr_size )
{
 ...
 /*Hard Code BCL Peripherals to RPID-200 onwards*/
  (*vrm_cfg_index)=200;
  pm_vrm_rsrc_arr[200].slaveId = 0x4; //MakuaC
  pm_vrm_rsrc_arr[200].address = 0x3E00;
  (*vrm_cfg_index)++;
...
IDP			0		Default (Moselle)
IDP			0		Default (Moselle) (SLT)
IDP			1		Cherokee
IDP			2		RCM+AMOLED (Moselle)
IDP			3		RCM+AMOLED (Cherokee)
IDP			4		Moselle + AMOLED (SLT)
QRD			0		Default (Cherokee)
QRD			1		Moselle
ATP			0		Default (Cherokee)
*/
  if(((platform == DALPLATFORMINFO_TYPE_IDP) && ((subtype == 0x00) || (subtype == 0x02) || (subtype == 0x04)))
    || ((platform == DALPLATFORMINFO_TYPE_QRD) && (subtype == 0x01)))
  {
