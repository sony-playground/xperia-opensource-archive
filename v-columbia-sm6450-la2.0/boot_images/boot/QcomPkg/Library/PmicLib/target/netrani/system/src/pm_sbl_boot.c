...
*  \n
*  \n &copy; Copyright 2013-2022, 2023 Qualcomm Technologies Inc, All Rights Reserved
*/
/* =======================================================================
Edit History
...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/netrani/system/src/pm_sbl_boot.c#13 $
$Author: pwbldsvc $
...
extern pm_err_flag_type pm_update_charger_pmic_index(void);
...
pm_device_auto_boot_check(void) 
{
  ...
  if (pon_reason.rtc && !(rtc_data[0] | rtc_data[1] | rtc_data[2] | rtc_data[3]))
  {
  ...
  if(pm_get_pmic_model(PMIC_D) != PMIC_IS_UNKNOWN) //PMIC detected at SID=3
  {
	if(pm_get_pmic_model(PMIC_D) == PMIC_IS_PM7250B) 
  ...
  { //Enters here if mis-configurations detected
	...
	if(pon_trig_pmic_expected == PMIC_D)
	{
	...
	{ //Clear GPIO5B configs done for false NO-PMI detection
	  err_flag |= pm_comm_write_byte(0x1, 0x8CDA, 0x0f); //Perf reset of GPIO, follow all Resets
	  err_flag |= pm_comm_write_byte(0x1, 0x8C14, 0x1); //clear pending interrupts
	  err_flag |= pm_comm_write_byte(0x1, 0x8C16, 0x1); //Disable interrupts
	}
	...
	{ // PON-Trigger is something valid & needs the system to boot-up (eventually).
	  pm_log_message("Detected Valid PON-Trigger(s). Initiating Hard-Reset");
  ...
  *applicable = FALSE;
  ...
  if( ((platform_type == DALPLATFORMINFO_TYPE_MTP)
    ...
    || (platform_type == DALPLATFORMINFO_TYPE_HDK)) &&
       ((pmic_model == PMIC_IS_PM7250B) || (PMIC_IS_PM8350B == pm_get_pmic_model(PMIC_D))
         || (pmic_model == PMIC_IS_PM7325B)))
  {
