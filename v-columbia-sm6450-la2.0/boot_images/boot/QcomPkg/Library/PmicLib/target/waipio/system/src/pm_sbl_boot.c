...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/waipio/system/src/pm_sbl_boot.c#32 $
$Author: pwbldsvc $
...
pm_get_boot_media(boolean* applicable)
{
   ...
   pm_log_message("boot option is %d",(uint32)(*boot_option_address));
   if((uint32)(*boot_option_address) == USB_FLASHLESS_BOOT_OPTION){ 
     *applicable = FALSE;
     pm_log_message("Leica WA is not applicable as BOOT MEDIA is USB");
    }
    else{
     *applicable = TRUE;
     pm_log_message("Leica WA is applicable");
    }
   return err_flag;
}
...
pm_leica2_detection_workaround()
{
    ...
    if(ufs_return){
       ...
       pm_log_message("UFS: Sleep Mode entry success");
    }

    /*Check the initial state of LDOs
    currently checking only L6C and L9C LDOs
    To-do : generic API to check and log the PON status of all LDOs
    Honor specific Fix */
    err_flag = pm_comm_read_byte_ex(0, PMIC_C, 0xC646, &pon_l6c_status);
    pm_log_message("Initial L6C status is %d",pon_l6c_status);
    err_flag = pm_comm_read_byte_ex(0, PMIC_C, 0xC946, &pon_l9c_status);
    pm_log_message("Initial L6C status is %d",pon_l9c_status);
    ...
    if (PM_ERR_FLAG_SUCCESS != pm_comm_write_byte(PMIC_C, 0xC946, 0))
        goto exit;
    pm_log_message("Disabled LDOs ");

    //Disable BOB 
    pm_comm_write_byte(PMIC_C,0xE446,0);
    pm_log_message("Disable BOB");

    busywait(20000);

    pm_comm_write_byte(PMIC_C,0xE446,0x80);
    pm_log_message("Enable BOB");
...
exit:
    ...
    if(ufs_return){
       ...
       pm_log_message("UFS: Active Mode entry success");
    }
    ufs_close(ufshd);
    return err_flag;
...
pm_leica2_sid_programming (void)
{
  ...
  uint8 leica2_1_aon_trim = 0;
  uint8 leica2_2_aon_trim = 0;
uint8 leica2_1_revid_tp_rev = 0,leica2_2_revid_tp_rev = 0;
  ...
  while(retries <= MAX_RETRY_COUNT)
  {
     if (camera_dt->reset_gpio[0] != PM_LEICA_GPIO_DISABLE) {
       ...
       if(err_flag){
          /* For USB BOOT FLASHLESS BOOT MEDIA do not run the WA*/
         if(FALSE == applicable)
            break;
         pm_log_message("Calling Leica Detection workaround");
         err_flag = pm_leica2_detection_workaround();
         if(err_flag){
           pm_log_message("Failure in Leica Detection Workaround");
           break;
         }
         retries++;
         continue;
           ...
           pm_log_message("Leica2_1 read successfull - Subtype ID is 0x%x",data);
     ...
     if (camera_dt->reset_gpio[1] != PM_LEICA_GPIO_DISABLE) {
       ...
       if(err_flag){
		  ...
		  //WA to update Leia2 AON trim value
