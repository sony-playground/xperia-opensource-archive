...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/PmicLib/target/waipio/system/src/pm_sbl_boot_oem.c#28 $
$Author: pwbldsvc $
...
*/
        ...
        config_param_ptr->swire_cfg.swire_map < PM_SWIRE_MAP_INVALID)
  {
    ...
    if(display_dt)
    {
      ...
      if(map_select != PM_SWIRE_MAP_INVALID)
      {
        swire_map_data = swire_lut[map_select].aswire << 4 | swire_lut[map_select].eswire;
      }
      else
      {
        swire_map_data = display_dt->swire_map_data;
      }
...
pm_driver_post_init(void)
{
  ...
  err_flag |= pm_ext_i2c_charger_rerun_apsd();
