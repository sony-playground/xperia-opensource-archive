...
Copyright (c) 2014,2016,2020,2021 by QUALCOMM Technologies, Incorporated. 
All Rights Reserved.
===========================================================================*/
...
void glink_os_initialize_ipcc( void )
{
  if(gBS->LocateProtocol (&gEfiIpccProtocolGuid, NULL,
                                (VOID**) &pIpccProtocol) == EFI_SUCCESS )
  {
    pIpccProtocol->IpccAttach(&hGlink, EFI_IPCC_P_MPROC);
  }
}
...
boolean glink_os_cs_init( os_cs_type *cs )
{
  ...
  InitRecursiveLock (0, (LockHandle**)cs);  // Make sure string is exactly 8 bytes and return the object ptr
  //*cs = GlinkCrSPtr;
  return TRUE;
}
...
os_cs_type glink_os_cs_create( void )
{
  ...
  LockHandle* GlinkCSPtr = (LockHandle*)0xdeadbeef; /*Initialized it with a dummy value so that its neither null nor a dangling pointer (done to solve MISRA violation)*/
  InitRecursiveLock (0, &GlinkCSPtr);
  return GlinkCSPtr;
...
boolean glink_os_mutex_init(os_mutex_type *cs)
{
  ...
  InitRecursiveLock (0, (LockHandle**)cs);
  //*cs = GlinkMtxPtr;
  return TRUE;
}
...
{
  *event = EventInit(0,0,SCHED_EVENT_FLAG_AUTOUNSIGNAL);
}
...
{
  return EventWaitTimeout (event, (TimeDuration)time_ms);
}
...
boolean glink_os_register_isr( os_ipc_intr_type *intr, os_isr_cb_fn isr, void* cb_data )
{
  if(!pIpccProtocol)
  {
    glink_os_initialize_ipcc();
  }
  if( pIpccProtocol->IpccRegisterInterrupt(hGlink, 
                                           intr->client, 
                                           intr->signal, 
                                           intr->signal, 
                                           (EFIIPCC_CB)isr, 
                                           (EFIIPCC_CBData)cb_data) != EFI_IPCC_EOK)
  {
    return FALSE;
  }
  return TRUE;
...
boolean glink_os_deregister_isr( os_ipc_intr_type *intr )
{
  ...
  if( pIpccProtocol->IpccDeregisterInterrupt(hGlink, 
                                             intr->client, 
                                             intr->signal, 
                                             intr->signal) != EFI_IPCC_EOK)
  {
    return FALSE;
  }
  return TRUE;
...
void glink_os_send_interrupt( os_ipc_intr_type *intr )
{
  ...
  pIpccProtocol->IpccTrigger(hGlink, 
                             intr->client, 
                             intr->signal, 
                             intr->signal);
}
