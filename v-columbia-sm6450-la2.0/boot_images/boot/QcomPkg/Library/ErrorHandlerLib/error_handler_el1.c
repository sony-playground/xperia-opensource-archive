...
Copyright 2009-2021 by Qualcomm Technologies, Inc.  All Rights Reserved.
=============================================================================*/
...
)
{
  ...
  static boolean ramdump_allowed = 0;
  ...
  do
  {
	...
	return_status = boot_config_context_get_ptr(CONFIG_CONTEXT_CRT_HANDLE, CONFIG_CONTEXT_PMIC_INTERFACE, (void *)&pmic_if);
    if(return_status != BL_ERR_NONE || pmic_if == NULL)
    {
  	  break;
    }
    
    // check if ramdump is allowed
    return_status = (boolean) boot_dload_is_security_region_dump_allowed(CONFIG_CONTEXT_CRT_HANDLE, &ramdump_allowed);
