...
/*
  ...
  Copyright (c) 2011, 2022, 2023 Qualcomm Technologies, Inc. All rights reserved.
...
)
{
  ...
  nSize = sizeof(DalPlatformInfoSMemType) + nClusterArraySize + nDisabledFeaturesArraySize + nPMICArraySize;
...
//  AsciiStrnCpy(pSMem->aBuildId, mob_sw_rev, DALPLATFORMINFO_SMEM_BUILD_ID_LENGTH);
  ...
  if (phChipInfo != NULL)
  {
    ...
    */
    ...
    pSMem->eSKU = SKUInfo.eSKU;
    pSMem->nProductCode = SKUInfo.nProductCode;
  ...
  pSMem->nPMICArrayOffset = sizeof(DalPlatformInfoSMemType)
    + nClusterArraySize + nDisabledFeaturesArraySize;
  aPMICInfo =
...
)
{
  ...
  nDTResult = fdt_get_blob_handle(&blob, DEFAULT_BLOB_ID);
  if (nDTResult != 0 || !blob)
  {
    return DAL_ERROR;
  }
