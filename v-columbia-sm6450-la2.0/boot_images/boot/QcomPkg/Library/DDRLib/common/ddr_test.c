...
--------   ---     -------------------------------------------------------------
11/18/20   yps     Fixed compiler upgrading issue
...
================================================================================
                 Copyright 2010,2012-2013,2020 Qualcomm Technologies Incorporated
                              All Rights Reserved
...
)
{
  ...
  for (pattern = 0x1; pattern != 0x0; pattern <<= 1)
  {
    ...
    {
      DDR_TEST_LOG_ERR_PATTERN((uintptr_t)&base_addr[0], pattern);
      return FALSE;
    }
  }
...
)
{
  ...
  for (addr_line = 0x1, i = 1; addr_line <= limit; addr_line <<= 1, i++)
  {
    ...
    {
      DDR_TEST_LOG_ERR_PATTERN((uintptr_t)&base_addr[addr_line], ~addr_line);
      return FALSE;
    }
    else
    {
...
)
{
  ...
  for (offset = 0; offset <= limit; offset++)
  {
    ...
    {
      DDR_TEST_LOG_ERR_PATTERN((uintptr_t)&base_addr[offset], offset);
      return FALSE;
    }
  }
...
)
{
  ...
  for (start_pattern = 0x1; start_pattern != 0x0; start_pattern <<= 1)
  {
    ...
    for (offset = 0; offset <= limit; offset++)
    {
      ...
      {
        DDR_TEST_LOG_ERR_PATTERN((uintptr_t)&base_addr[offset], pattern);
        return FALSE;
      }
      pattern <<= 1;
      if (pattern == 0x0)
...
)
{
  ...
  for (offset = 0; offset <= limit; offset++)
  {
    if(0 != ddr_physical_info.interleaved_memory) {
      ...
      addr_mod = (uint32)(uintptr_t)(&base_addr[offset]) % (DDR_INTERLEAVE_CHANNEL_SIZE * ddr_get_max_channels());
...
{
	//volatile uint32 index1, index2;   //Missing
	//dr_test_error_type test_error_type = DDR_TEST_ERR_NONE; //Missing
#if 0      //Missing		
  ...
  */
  for(cnt_cs=0; cnt_cs<2; cnt_cs++) {
    ...
    if(ddr_physical_info.interleaved_memory & chip_select) {
      ...
      for(cnt_ch=0; cnt_ch<ddr_get_max_channels(); cnt_ch++) {
