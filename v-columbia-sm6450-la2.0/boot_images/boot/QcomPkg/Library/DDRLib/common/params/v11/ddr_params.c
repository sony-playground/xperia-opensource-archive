...
{   
	...
	fptr_HAL_EXT_CORE_SW_Get_Chromebook_Sku_Info = ddr_external_Get_Chromebook_Sku_Info;
	fptr_HAL_EXT_CORE_SW_Get_Warm_Mission_Mode_Info = ddr_external_Get_Warm_Mission_Mode_Info;
 	fptr_HAL_EXT_CORE_SW_Get_DDR_Accessibility_Limit_Info = ddr_external_Get_DDR_Accessibility_Limit_Info;
	fptr_HAL_EXT_CORE_SW_Get_Frequency_Limit_info = ddr_external_Get_Frequency_Limit_info; 
...
void ddr_get_shub_shrm_freq(void)
{
	...
	{
		ddr_shared_data->ddr_aop_params.ptr->mc_states[12].freq_enable = FALSE;
	}
...
boolean ddr_set_params(boolean dload_mode, boolean clock_configured)
{
  ...
  if(dload_mode == 0x0) 
  {
	...
	if(clock_configured == FALSE)
		HAL_DDR_FW_Enter((void*)ddr_fw_buffer_ptr, SIZE_64KB);//
  }
            ...
            HWIO_INF(AOSS_CC_RESET_FSM_STATUS, FIRST_PASS_EXPIRE)||(!HWIO_INF(AOSS_CC_RESET_FSM_CTRL, FIRST_PASS_COMPLETE)))
  {
	...
	ddr_fw_buffer_ptr = (uint8 *)SCL_SBL1_DDR_DSF_CFG_BUF_BASE;//0x0B000000; //aop code ram 
	if(clock_configured == FALSE)
		HAL_DDR_FW_Enter((void*)ddr_fw_buffer_ptr, SIZE_64KB);//SCL_SBL1_DDR_DSF_CFG_BUF_BASE
  }	
