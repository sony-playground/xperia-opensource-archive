...
boolean ddr_get_shared_info_details(ddr_details_entry_ptr_type ddr_details_entry_ptr)
{
    ...
    if(ddr_get_boot_mode() != BOOT_MODE_RAMDUMP)
	{
	  ...
	  //ddr_misc_data.dsf_version = (ddr_shared_data->dsf_code_version.mainline << 16 || ddr_shared_data->dsf_code_version.major << 8 || ddr_shared_data->dsf_code_version.minor);
