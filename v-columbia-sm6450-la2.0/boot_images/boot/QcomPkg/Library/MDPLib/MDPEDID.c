...
MDP_Status MDPDetectPanel(MDP_Display_IDType eDisplayId, MDP_Panel_AttrType *pDisplayInfo)
{
  ...
  else
  {
    ...
    {
        MDP_Log_Message(MDP_LOGLEVEL_ERROR, "MDPLib: MDPPlatformConfigure(MDPPLATFORM_CONFIG_GETPANELCONFIG) failed!\n");
    }
    ...
    MDP_Log_Message(MDP_LOGLEVEL_ERROR, "MDPDetectPanel: pPanelInfo->szName=%s,sPlatformParams.sPlatformPanel.szName=%s\n", pPanelInfo->szName, sPlatformParams.sPlatformPanel.szName);
        //Set panel name
        AsciiStrCpy(pPanelInfo->szName, sPlatformParams.sPlatformPanel.szName);
        ...
        MDP_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayDxe: Resolution %dx%d (PhysConnect %d)\n", pPanelInfo->uDisplayWidth,
