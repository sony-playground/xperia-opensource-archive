...
static const sChip_MMUSidMasks asChip_MMUSidMasks[MDSS_BASEADDRESSMAPINGS_MAX] = {
    {EFICHIPINFO_FAMILY_WAIPIO,   0x402},                // for Waipio 
    {EFICHIPINFO_FAMILY_KAILUA,   0x002},                // for Kailua 
    {EFICHIPINFO_FAMILY_FILLMORE, 0x402},                // for Fillmore 
    {EFICHIPINFO_FAMILY_PALIMA,   0x402},                // for Palima
    {EFICHIPINFO_FAMILY_NETRANI,  0x402},                // for Netrani
    {EFICHIPINFO_FAMILY_CLARENCE, 0x402},                // for Clarence    
};
...
MDP_Status  MDPProfiler(uint8 *pFuncName, uint32 uParam1, bool32 bEntry)
{
  ...
  if (FALSE == MDP_IsRamDumpMode())
  {
        ...
        (gProfileLog->uEntryCount < MDP_PROFILER_LOG_ENTRIES))
    {
      ...
      AsciiStrnCpy((CHAR8 *)pEntry->sFunctionName, (const CHAR8*)pFuncName, sizeof(pEntry->sFunctionName));
...
static MDP_Status AllocateDSCEncoder(MDP_Panel_AttrType* pPanelInfo, uint32 uDSCNumInUse)
{
  ...
  else
  {
    ...
    /* If hardware supports only single pair of dsc and
     * each dsc is being used by single display in a dual display case,
     * update dscid increment by 1
     */
    if ((NULL != pMDPInterfaceCaps) &&
        (pMDPInterfaceCaps->uNumDSCCores <= 2))
    {
      uNumDSCsIncr = 1;
    }
    else
    ...
    for (uDSCId = HAL_MDP_DSC_0; uDSCId < HAL_MDP_DSC_MAX; uDSCId += uNumDSCsIncr)
    {
  ...
  else if (psMDPHwPrivateInfo->sMDPVersionInfo.uMajorVersion == 0)
  {
    switch (psMDPHwPrivateInfo->sEFIChipSetFamily)
    {
     case EFICHIPINFO_FAMILY_NETRANI:
       psMDPHwPrivateInfo->sMDPVersionInfo.uMajorVersion = 0x08;
       psMDPHwPrivateInfo->sMDPVersionInfo.uMinorVersion = 0x03;
       break;
     ...
     case EFICHIPINFO_FAMILY_PALIMA:
       psMDPHwPrivateInfo->sMDPVersionInfo.uMajorVersion = 0x08;
       psMDPHwPrivateInfo->sMDPVersionInfo.uMinorVersion = 0x05;
       break;
     ...
     case EFICHIPINFO_FAMILY_WAIPIO:      
      default:
      // Default, set Waipio family i.e MDSS 8.1
      psMDPHwPrivateInfo->sMDPVersionInfo.uMajorVersion = 0x08;
