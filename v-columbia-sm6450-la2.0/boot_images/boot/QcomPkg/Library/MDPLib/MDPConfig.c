...
MDP_DisplayCtrlPathMapType gMDP_DisplayControlPathMap[MDP_DISPLAY_MAX][MDP_DUALPIPE_NUM_MIXERS] =
{
  // MDP_DISPLAY_PRIMARY
  {
...
const HAL_MDP_SprId gMDP_SPRIdMap[HAL_MDP_DESTINATION_PIPE_MAX] =
{
  ...
  HAL_MDP_SPR_NONE,                                           //HAL_MDP_DESTINATION_PIPE_2
  HAL_MDP_SPR_NONE,                                           //HAL_MDP_DESTINATION_PIPE_3
...
#define MDP_DEFAULT_TE_HEIGHT_MAX                        (0xfff0)
  ...
  if ((NULL != pProgFetchStartConfig) && (NULL != pPanelConfig))
  {
    ...
    if (bEnablePFS)
    {
          ...
          (1 < pPanelConfig->sDSCConfig.uDSCCompressionRatio))
      {
        ...
        uHorzActive = MDP_CEIL((uHorzActive ), pPanelConfig->sDSCConfig.uDSCCompressionRatio / 1000);
      }
      ...
      case MDP_DISPLAY_CONNECT_SECONDARY_DSI_CMD:
        {
          ...
          if ( TRUE == pPanelConfig->sDSCConfig.bDSCEnable)
          {
            ...
            {
              sInterfaceTiming.bTwoPixelsPerPclk     = TRUE;    /*  enable two pixels per pclk */
              ...
              sInterfaceTiming.uVisibleWidthInPixels = MDP_CEIL((sInterfaceTiming.uVisibleWidthInPixels), pPanelConfig->sDSCConfig.uDSCCompressionRatio / 1000);
            }
            else
            {
...
static void displayCommandModeVsyncConfig(MDP_Panel_AttrType *pPanelAttr, HAL_MDP_Interface_VsyncConfigType *pVsyncConfig, uint32 uPathIndex)
{
    ...
    fVsyncHeight = (float)(pPanelAttr->uDisplayHeight + pPanelAttr->sActiveTiming.uVsyncBackPorchLines + pPanelAttr->sActiveTiming.uVsyncFrontPorchLines - 1);
...
static MDP_Status SetupControlPath(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  MDP_LOG_FUNC_ENTRY("SetupControlPath", eStatus);
  // Setup Control Path
  for (uInterfaceId = 0; uInterfaceId < pPanelConfig->uNumInterfaces; uInterfaceId++)
  {
      if (HAL_MDP_INTERFACE_NONE != pPanelConfig->sDisplayControlPath[uInterfaceId].eInterfaceId)
      {
          ...
          if (HAL_MDSS_STATUS_SUCCESS != HAL_MDP_Interface_Enable(pPanelConfig->sDisplayControlPath[uInterfaceId].eInterfaceId,
              FALSE,
              0x0))
          {
  ...
  for (uInterfaceId = 0; uInterfaceId < pPanelConfig->uNumMixers; uInterfaceId++)
  {
    ...
    for (uMixerId = 0; uMixerId < pPanelConfig->uNumMixers; uMixerId++)
    {
  ...
  MDP_LOG_FUNC_EXIT("SetupControlPath", eStatus);
  return eStatus;
}  
...
*
****************************************************************************/
MDP_Status ScaleHelperConfigure(HAL_MDP_PixelExtLayerConfigType *pPixelExtConfig, HAL_MDP_CropRectangleType *pCropRectInfo, uint32 uNumMixers, uint32 uFlags)
{
  MDP_Status eStatus = MDP_STATUS_OK;
...
{
   MDP_Status    eStatus = MDP_STATUS_OK;
   MDP_LOG_FUNC_ENTRY("MDPSetupPipe", eStatus);
   ...
   MDP_LOG_FUNC_EXIT("MDPSetupPipe", eStatus);
   return eStatus;
}
...
MDP_Status  MDPStopPipe(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  else
  {
    ...
    if (MDP_STATUS_OK != CheckInterfacestatus (pPanelConfig, FALSE))
    {
      MDP_Log_Message(MDP_LOGLEVEL_WARN, "MDPLib: CheckInterfacestatus  failed!\n");
    }
