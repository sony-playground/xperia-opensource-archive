...
MDP_Status MDP_SaveFirmwareEnvironmentVariable(MDP_Display_IDType eDisplayId)
{
  ...
  if (MDP_STATUS_OK == (eStatus = MDPPlatformConfigure(eDisplayId, MDPPLATFORM_CONFIG_GETPLATFORMINFO, &sPlatformParams)))
  {
    ...
    sFirmwareEnvInfo.uFrameBufferAddress      = (UINTN)pPrimaryDisplayInfo->sFrameBuffer.pPlane0Offset;
    ...
    sSplashInfo.uFrameAddr = (UINTN)pPrimaryDisplayInfo->sFrameBuffer.pPlane0Offset;
...
EFI_STATUS MDP_Display_LoadFile(CHAR8 *pFileName, UINT8 **pFileBuffer, UINT32 *pBufferSize, UINT32 *pRetSize, UINT32 uFlags)
{
  ...
  DEBUG((EFI_D_ERROR, "MDP_Display_LoadFile pFileName=%s, pFileBuffer=%s, pBufferSize=%s, pRetSize=%d\n", pFileName, pFileBuffer, pBufferSize, pRetSize));
  if ((NULL == pFileName)   ||
      (NULL == pFileBuffer) ||
      (NULL == pBufferSize) ||
      (NULL == pRetSize))
  ...
  if (DISP_LOAD_FILE_FLAG_IMAGEFV & uFlags)
  {
    ...
    if((EFI_SUCCESS != Status) ||
       (NULL        == pFileBuffer))
    {
       DEBUG((EFI_D_ERROR, "MDP_Display_LoadFile: load file from imagefv failed with status(%d)!\n", Status));
    }
    else
    {
  ...
  {
      DEBUG((EFI_D_ERROR, "MDP_Display_LoadFile ReadFromFV\n"));
      UINTN uBufferSize = 0;

      AsciiStrToUnicodeStr(pFileName, (CHAR16*)&LoadFile);
...
MDP_Status MDP_Display_LoadFileEx(CHAR16* pPatitionName, CHAR8* pFileName, UINT8** pFileBuffer, UINT32* pBufferSize, uint64 uReadOffset, UINT32* pRetSize, UINT32 uFlags)
{
  ...
  else if (NULL == AsciiStrToUnicodeStr(pFileName, (CHAR16*)pFileNameStr))
  {
