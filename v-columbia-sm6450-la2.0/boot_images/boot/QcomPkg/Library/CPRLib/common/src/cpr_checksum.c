 ...
 *
 * $DateTime: 2022/11/15 01:53:43 $
 * $Header: //components/rel/core.boot/6.0/QcomPkg/Library/CPRLib/common/src/cpr_checksum.c#3 $
 * $Change: 41150433 $
 */
#include "cpr_data.h"
#include "cpr_utils.h"
    ...
    for(int i = 0; i < rail->vp->modesCount; i++)
    {
        ...
        *checksum_array++ = checksum16((const uint8*)&(mode->freqDelta), sizeof(uint32));
        ...
        if(mode->quotients)
        {
			...
			for (int j = 0; j < mode->quotients->count; j++)
		    {
		    	struct cpr_quotient_cfg *quotient_cfg = &mode->quotients->data[j];
		    	*checksum_array++ = checksum16((const uint8*)&(quotient_cfg->cprRevMin), sizeof(uint8));
		    	*checksum_array++ = checksum16((const uint8*)&(quotient_cfg->cprRevMax), sizeof(uint8));
		    	*checksum_array++ = checksum16((const uint8*)(quotient_cfg->quots), sizeof(cpr_quotient) * quotient_cfg->count);
