 ...
 * $Header: //components/rel/core.boot/6.0/QcomPkg/Library/CPRLib/common/src/cpr_rail.c#7 $
 * $Change: 39171631 $
 */
#include <stdbool.h>
#include <string.h>
...
{
    cpr_results rslts;
	memset( &rslts, 0, sizeof( cpr_results ) );
...
static void cpr_rail_init_rail_data(cpr_rail* rail)
{
    ...
    for(int modeIdx = 0; modeIdx < railState->modeSettingsCount; modeIdx++)
    {
        ...
        {
			#ifdef CPR_REV_BASED_QUOTS_KV
			cpr_quotient_cfg*   baseQuots_rev;
            baseQuots_rev = cpr_utils_get_rev_quotients(railState->railCPRRev, vpMode->quotients);
			if (baseQuots_rev != NULL)
			{
				modeSettings->targetsCount = baseQuots_rev->count;
			}
			#else
    ...
    for (modeIdx = 0; modeIdx < railState->modeSettingsCount; modeIdx++)
    {
                ...
                subModeIdx++)
        {
            ...
            subMode->floor = subModeLo->floor +
                ((int)((subModeHi->floor - subModeLo->floor) * interpAdjFactor)\
                 / 100); //Typecasting to int in case of overflow
    ...
    for(int modeIdx = 0; modeIdx < vp->modesCount; modeIdx++)
    {
        ...
        for(int submodeIdx = 0; submodeIdx < modeSettings->subModesCount; submodeIdx++)
        {
            ...
            if(margin)
            {
              #ifdef CPR_VD
				int32 vdelta = 0;
                if (cpr_image_apply_margin_offset())
					{
                      vdelta = cpr_get_vd_margin(rail->id,modeIdx);
                    }
				submode->ceiling += margin->openLoop + vdelta;
                submode->floor   += margin->openLoop + vdelta;
 ...
 */
#ifdef CPR_REV_BASED_QUOTS_KV
cpr_quotient_cfg   baseQuots = {.count = 0, .cprRevMin = 0, .cprRevMax = 255,  .quots = NULL };
...
void cpr_rail_calculate_target_quotients(cpr_rail* rail, cpr_rail_state* state)
{
    ...
    for(int i = 0; i < vp->modesCount; i++)
    {
        ...
        #ifdef CPR_VD
        if (cpr_image_apply_margin_offset() && (NULL != margin)) {
            int32 vdelta = cpr_get_vd_margin(rail->id,i);
            margin_uv += vdelta;
        }
        #endif
        ...
        {
			#ifdef CPR_REV_BASED_QUOTS_KV
			baseQuots_rev = cpr_utils_get_rev_quotients(state->railCPRRev, vpMode->quotients);
			if (baseQuots_rev != NULL)
			{
				baseQuots.count = baseQuots_rev->count;
                                baseQuots.cprRevMin = baseQuots_rev->cprRevMin;
                                baseQuots.cprRevMax = baseQuots_rev->cprRevMax;
				baseQuots.quots = baseQuots_rev->quots;
				//baseQuots = (cpr_quotient_cfg)(*baseQuots_rev);
			}
			else 
			{
				continue;
