 ...
 * $Header: //components/rel/core.boot/6.0/QcomPkg/Library/CPRLib/common/src/cpr_smem_vp.c#5 $
 * $Change: 39171631 $
 */
#include "cpr_smem.h"
#include "cpr_image.h"
...
static boolean cpr_smem_write_margin_data(cpr_rail_state* state,
        cpr_margin_cfg *margins,int modeIdx)
{
    int i = 0;
	     ...
	     if (cpr_image_apply_margin_offset()) 
 	        {
			  ...
			  s_data->openLoop = data->openLoop + vdelta;
		      s_data->closedLoop = data->closedLoop + vdelta;			
    #else
        ...
        struct cpr_voltage_data* modes, int modeIdx)
{
    ...
    s_margins->data = cpr_smem_write_margin_data(state, margins,modeIdx); 
        ...
        struct cpr_voltage_data *modes, int modeIdx)
{
    ...
    cpr_quotient_cfg* quotients = cpr_utils_get_rev_quotients(state->railCPRRev,modes->quotients );
...
static void cpr_smem_read_quotient_cfg(struct cpr_voltage_data *modes)
{
    ...
    modes->quotients = cpr_image_malloc(sizeof(cpr_quotient_cfg_rev));
    CPR_ASSERT(modes->quotients);

    cpr_quotient_cfg_rev* quotients_rev = modes->quotients;
	
	quotients_rev->data = cpr_image_malloc(sizeof(cpr_quotient_cfg));
    CPR_ASSERT(quotients_rev->data);

    cpr_quotient_cfg* quotients = quotients_rev->data;
    quotients_rev->count =1;
	
	quotients->cprRevMin = 0;
	quotients->cprRevMax = 7;
