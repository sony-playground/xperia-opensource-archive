 ...
 *
 * $DateTime: 2022/04/20 12:17:29 $
 * $Header: //components/rel/core.boot/6.0/QcomPkg/Library/CPRLib/common/src/cpr_utils.c#4 $
 * $Change: 36707691 $
 */
...
cpr_quotient_cfg* cpr_utils_get_rev_quotients(const uint8 railCPRRev, cpr_quotient_cfg_rev* quotlist)
{
    if(quotlist != NULL)
    {
        for(int i = 0; i < quotlist->count; i++)
        {
            if (railCPRRev >= quotlist->data[i].cprRevMin && railCPRRev <= quotlist->data[i].cprRevMax) {
                return &quotlist->data[i];
            }
        }
    }
