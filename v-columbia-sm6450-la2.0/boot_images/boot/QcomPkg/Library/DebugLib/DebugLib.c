  ...
  )
{
  ...
  REPORT_STATUS_CODE ((EFI_ERROR_CODE | EFI_ERROR_UNRECOVERED), 
                      (EFI_SOFTWARE | EFI_SW_EC_ILLEGAL_SOFTWARE_STATE));
...
#ifdef FEATURE_XBOOT
  TargetPanic(); /* Shutdown system in TargetPanic() */
  ...
  * ------------------------------------------------------------- */
  ...
  if (RETAIL)
  {
    // Target Panic may hard reset or shutdown based on PCD config 
    TargetPanic();
  }

  // Deadloop as last resort
  CpuDeadLoop ();
