 ...
 *
 * Copyright (c) 2012-2015,2017-2018, 2020 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
/*=============================================================================
  ...
  ----------   ---     ---------------------------------------------------------
  2020-08-17   jt      Use BlkIo handle from the GPT listener request
  2020-02-20   am      Use aligned buffer during Write operation
  2018-05-09   ai      Register for BlkIoRefresh and prevent subsequent writes
  2018-05-03   pranavj Ported SHMBridge changes to Poipu  
  2018-04-24   jt      GPTListenerInit updates for Poipu 
...
EFI_STATUS RegisterBlockIoRefresh (VOID);
...
EFI_STATUS EFIAPI GPTListenerAllocMem(void)
{
  ...
  UINTN uSize = sizeof(GPT_LISTENER_PARAMS);
  ...
  GPTListenerParams = ShmBridgeProtocol->ShmBridgeAllocate(ShmBridgeProtocol, &uSize, ShmBridgeBootSvcData);
...
EFI_STATUS EFIAPI GPTListenerInit(EFI_BLOCK_IO_PROTOCOL *BlkIo)
{
  ...
  do
  {
    ...
    if (BlkIo->Media->MediaId == SIGNATURE_32('u','f','s',' '))
    {
        IsUFSDevice = TRUE; 
    }
    else if (BlkIo->Media->MediaId == SIGNATURE_32('s', 'n', 'o', 'r'))
    {
      GPTListenerParams->BlkIo = BlkIo; 
    }
    ...
    if (EFI_ERROR(Status))
      DEBUG((EFI_D_WARN, "Blkio refresh registration failed\n"));
...
void GPTListenerPartiInit (void *BufferPtr, UINT32 BufferLen)
{
  ...
  do
  {
    ...
    if (SD_MGR_MSG_GPT_PARTI_INIT_ID == DevInitReq->CmdID)
    {
      ...
      {
        SearchArray = (void *)&(DevInitReq->DevID);
        SearchCount = 1;
      }
...
/* BlkIo handle no longer valid after a GPT update (for eg: fastboot) */
VOID
RefreshBlockIo (
  IN EFI_EVENT   Event,
  IN VOID        *Context
...
{
  GPTListenerParams->HasBlkIoRefreshOccurred = TRUE;
}
...
void GPTListenerReadSectors (void *BufferPtr, UINT32 BufferLen)
{
  ...
  do
  {
    ...
    BlkIo = (EFI_BLOCK_IO_PROTOCOL *)((uintptr_t)GptRWReq->CtxtID);
...
void GPTListenerWriteSectors (void *BufferPtr, UINT32 BufferLen)
{
  ...
  mTzSDGptRWReq = AllocatePages(EFI_SIZE_TO_PAGES(ReqBuffLen)); /* copy of request */
  if (NULL == mTzSDGptRWReq)
  {
    GptRWRsp = (TzSDGptRWRspType *) BufferPtr; 
    GptRWRsp->Status = SD_MGR_MSG_OUT_OF_RESOURCES; 
  ...
  do
  {
    ...
    /* If BlkIoRefresh event has been signaled, then the BlkIo handle
       is no longer valid. It is okay to drop writes, because a new
       partition has just been flashed */
    if (GPTListenerParams->HasBlkIoRefreshOccurred)
    {
      GptRWRsp->Status = TZSDMgrGetErrorCode(EFI_SUCCESS);
    ...
    if (SD_MGR_MSG_GPT_PARTI_WRITE_ID == GptRWReq->CmdID)
    {
      ...
      Buffer = (VOID *) ((UINTN) GptRWReq + GptRWReq->ReqBuffOffset);
  ...
  if (mTzSDGptRWReq)
  {
    FreePages(mTzSDGptRWReq, EFI_SIZE_TO_PAGES(ReqBuffLen)); /* copy of request */
  }
}
