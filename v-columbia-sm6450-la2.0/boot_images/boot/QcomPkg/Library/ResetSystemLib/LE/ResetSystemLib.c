/** @file ResetSystemLib.c
  ...
  Copyright (c) 2011-2016,2018, 2020 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 08/19/20   cm      Workaround for HWIO mapping issue
...
PlatformSpecificReset(UINTN DataSize, CHAR16 *ResetData)
{
  ...
  if (StrCmp(STR_RESET_PLAT_SPECIFIC_EDL, ResetData) == 0)
  {
    ...
    if (Status != EFI_SUCCESS)
       DEBUG ((EFI_D_ERROR, "ERROR: PmicReset failed: 0x%08x\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  if (EFI_SUCCESS != Status)
  {
    ...
    // not returning error when HWIO mapping fails.
    //return EFI_DEVICE_ERROR;
  }
  /*if (gPSHoldAddress == 0)
  {
