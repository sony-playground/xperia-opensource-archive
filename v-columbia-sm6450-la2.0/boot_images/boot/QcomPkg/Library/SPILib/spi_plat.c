/*=============================================================================   
    ...
    Copyright (c) 2019, 2021, 2023 Qualcomm Technologies, Incorporated.
                        All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
08/09/23    KSN     Updated dfs_index in set_frequency api
05/22/23    STR     Support for multi chip select 
11/01/21    rk      Migration to new clock interface
...
{ 
    ...
    for (uint8 i = 0; i < SPI_NUM_CHIP_SELECTS; i++)
    {
        ...
        {
            dalRes = DalTlmm_ConfigGpio(hTlmm, config->cs_encoding[i], DAL_TLMM_GPIO_ENABLE);
            if ( DAL_SUCCESS != dalRes )
            {
                result = FALSE;
...
{ 
    ...
    for (uint8 i = 0; i < SPI_NUM_CHIP_SELECTS; i++)
    {
        ...
        {
            dalRes = DalTlmm_ConfigGpio(hTlmm, config->sleep_cs_encoding[i], DAL_TLMM_GPIO_DISABLE);
            if ( DAL_SUCCESS != dalRes )
            {
                result = FALSE;
                SPI_LOG(LEVEL_ERROR,"spi_plat_gpio_enable : failed to enable config->sleep_cs_encoding , error %d",dalRes);
                goto ON_EXIT;
            }
        }
    }
...
)
{
	...
	{
		SPI_LOG(LEVEL_ERROR,"spi_plat_clock_set_frequency config or div is NULL");
		return FALSE;
	}
	ClockResult res = CLOCK_ERROR;
	boolean result = TRUE;
	...
	*dfs_index = 0;
    ...
    if(clk_cfg == NULL)
    {
       SPI_LOG(LEVEL_ERROR,"spi_plat_clock_set_frequency clock_config is NULL");
       return FALSE;
    }
    ...
    //updating dfs index and div as per required frequency
    *dfs_index = min_index;
    ...
    *div += (min_error == 0) ? 0 : 1;
	...
	res = Clock_SetFrequency(clock_handle, se_id, clk_cfg[min_index].se_clock_frequency_khz, CLOCK_FREQUENCY_MHZ_CLOSEST,  &pnResultFreq);
	if (CLOCK_SUCCESS != res) { 
	    result = FALSE;
