...
----------   ---     --------------------------------------------------- 
2022-02-18	 cm	     Fix Klocwork issue by remove Assert with error code
2022-01-05   jt      Add config check for HS mode 
2021-09-27   ss      Used Config API for removing PCD lib dependency 
2021-05-24   jt      Add device tree support
2021-02-18   jt      Allow restricting SD card speed with PCD variable
2020-10-21   jt      Fix KW errors
2020-06-04   jt      Get SD card spec information 
...
SDCC_CARD_TYPE sdcc_handle_find_card( struct sdcc_device *handle )
{
   ...
   if (NULL == sdcc_pdata)
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_find_card : can't get slot handle\n"));
      return SDCC_CARD_UNKNOWN;
   }
   ...
   else if (sdcc_bsp_get_slot_type(driveno) & SDCC_BSP_SD_SLOT)
   {
      ...
      /* Check whether we are restricting to SDHC mode. Otherwise, make sure PMIC operations 
		 are allowed for UHS-1 cards by enabling the VDD that can be switched. If restricting 
		 the speed or if PMIC operations are not allowed, set the card_type to SDHC instead. */
      if (SDCC_CARD_UHS1 == card_type)
      {
         if (SDCC_HS_MODE == sdcc_pdata->sdcc_config.SpeedMode) 
         {
             card_type = SDCC_CARD_SDHC; 
         }
         else
         {
             if (SDCC_NO_ERROR != sdcc_bsp_get_vreg_switch_status(driveno))
             {
                 card_type = SDCC_CARD_SDHC;
             }
         }
...
sdcc_init_memory_device( sdcc_dev_type  *sdcc_pdev )
{
   ...
   UINT32          count = 100; 
   UINT32          dat_line = 0xFFFFFFFF;  

   if (SDCC_CARD_UHS1 == sdcc_pdev->card_type)
   {
      /* CMD11: Voltage switch */
      sdcc_cmd.cmd       = SD_CMD11_VOLTAGE_SWITCH;
      sdcc_cmd.cmd_arg   = 0;
      sdcc_cmd.resp_type = SDCC_RESP_SHORT;
      sdcc_cmd.prog_scan = 0;
      sdcc_cmd.flags     = SDCC_TRANSFER_FLAG_IS_CMD_ONLY;
      ...
      /* Change VDD from 3V to 1.8V and set IO_PAD_PWR_SWITCH */
      if (SDCC_NO_ERROR != sdcc_bsp_lower_volt_level(sdcc_pdev->driveno))
      {
         return SDCC_ERR_OP_NOT_SUPPORTED; 
      }
      HAL_sdhci_IoPadPwrSwitch(sdcc_pdev->driveno); 

      /* Poll on DAT[3:0] to become all 0s */
      while (count)
      {
         dat_line = HAL_sdhci_GetPresentState (sdcc_pdev->driveno) & 
            HAL_SDHCI_PRESENT_STATE_DAT_3_0_LINE_SIGNAL_LEVEL_BMSK;
         if (dat_line == 0)
         {
            break;
         }
      ...
      /* Wait for the regulator to become stable */
      HAL_sdhci_PowerSave(sdcc_pdev->driveno, TRUE); 
      sdcc_mdelay(5); 
      HAL_sdhci_PowerSave(sdcc_pdev->driveno, FALSE); 

      /* Poll on DAT[3:0] to become all ones */
      count = 100; 
      while (count)
      {
         dat_line = HAL_sdhci_GetPresentState (sdcc_pdev->driveno) & 
            HAL_SDHCI_PRESENT_STATE_DAT_3_0_LINE_SIGNAL_LEVEL_BMSK;
         if (dat_line == HAL_SDHCI_PRESENT_STATE_DAT_3_0_LINE_SIGNAL_LEVEL_BMSK)
         {
            break; 
         }
         sdcc_udelay(10); 
...
sdcc_config_memory_device( sdcc_dev_type  *sdcc_pdev )
{
   ...
   sdcc_bsp_speed_mode intended_speed; 
            ...
            (SDCC_CARD_UHS1 == sdcc_pdev->card_type))
   {
      ...
      /* SD_SPEC = 0, SD_SPEC3 = 0, SD_SPEC4 = 0 corresponds to Version 1.0X */
      if (0 == scr.sd_spec)
      {
         if ((0 == scr.sd_spec3) && (0 == scr.sd_spec4))
         {
            sdcc_pdev->mem.mem_info.spec_version = SDCC_SD_VER_1_0_X;
         }
      }
      /* SD_SPEC = 1, SD_SPEC3 = 0, SD_SPEC4 = 0 corresponds to Version 1.10 */
      else if (1 == scr.sd_spec)
      {
         if ((0 == scr.sd_spec3) && (0 == scr.sd_spec4))
         {
            sdcc_pdev->mem.mem_info.spec_version = SDCC_SD_VER_1_1_0;
         }
      }
      else if (2 == scr.sd_spec)
      {
         /* SD_SPEC = 2, SD_SPEC3 = 0, SD_SPEC4 = 0 corresponds to Version 2.00 */
         if ((0 == scr.sd_spec3) && (0 == scr.sd_spec4))
         {
            sdcc_pdev->mem.mem_info.spec_version = SDCC_SD_VER_2_0_0;
         }
         /* SD_SPEC = 2, SD_SPEC3 = 1, SD_SPEC4 = 0 corresponds to Version 3.0X */
         else if ((0 == scr.sd_spec4) && (1 == scr.sd_spec3))
         {
            sdcc_pdev->mem.mem_info.spec_version = SDCC_SD_VER_3_0_X;
         }
         /* SD_SPEC = 2, SD_SPEC3 = 1, SD_SPEC4 = 1 corresponds to Version 4.XX */
         else if ((1 == scr.sd_spec4) && (1 == scr.sd_spec3))
         {
            sdcc_pdev->mem.mem_info.spec_version = SDCC_SD_VER_4_X_X;
         }
      } 
      ...
      if ( scr.sd_spec >= 1 )
      {
         ...
         intended_speed = sdcc_bsp_get_platform_speed_config(sdcc_pdev->driveno);
         (void) sdcc_sd_switch_to_hs( sdcc_pdev, &intended_speed);
      }
   }
