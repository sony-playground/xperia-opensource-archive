...
---------   ----   ----------------------------------------------------- 
2022-04-26  ssb    HSR updates to configure LOW_FREQ_MODE
2022-02-18  cm     Add Level shifter support for quest
...
)
{
   ...
   case SDCC_IDENTIFICATION_MODE:
      ...
      break;
   case SDCC_DATA_TRANSFER_SDR_LS_MODE:  
...
{
   sdcc_bsp_hsr_settings_type hsr_settings = {0}; 
   ...
   UINT8 mclk_freq_calc = 0;
   UINT8 rclk_delay = 0;
   UINT8 sdc4_config_msb = 0;
   UINT32 dll_user_ctl = 0; 
   UINT32 dll_config_init = 0; 
   if (sdcc_pdev->card_mode == SDCC_DATA_TRANSFER_SDR_LS_MODE)
   {
      speed_mode = SDCC_SDR50_MODE; 
   }
   if (sdcc_bsp_get_hsr_settings (driveno, speed_mode, &hsr_settings) != SDCC_BSP_NO_ERROR) 
   {
       sdcc_bsp_get_default_hsr_settings(driveno, speed_mode, &hsr_settings);
   }
...
SDCC_STATUS sdcc_hs200_execute_tuning (sdcc_dev_type *sdcc_pdev)
{
   ...
   if ((SDCC_DATA_TRANSFER_HS200_MODE != sdcc_pdev->card_mode) &&
       ...
       (SDCC_DATA_TRANSFER_SDR_LS_MODE != sdcc_pdev->card_mode))
   {
