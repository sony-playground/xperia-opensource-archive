...
--------     ---     --------------------------------------------------- 
2022-04-26   ssb     Update HAL_sdhci_DLL2_ClockConfig argument  
2022-01-05   jt      Add capability overwrites 
2021-11-03   ss      Fixed KW issue for sdcc_pdata by adding NULL check
2021-09-27   ss      Used Config API for removing PCD lib dependency
2021-05-24   jt      Add device tree support
2020-11-11   jt      Don't overwrite HC_SPEC_VERSION_NUM
2020-06-04   jt      Power management updates 
2020-05-22   vm      Disable FIFO ALT mode in non adma path
...
#include "HALhwio.h"
#include <Library/SdccCommonLib/sdcc_util.h>
...
#define HWIO_SDC_HC_REG_DLL_USER_CTL_REG_IDX                  (0x0388)
...
inline void
HAL_sdhci_DllEnable (const UINT32 nControllerId, BOOLEAN bEnable)
{
   /* Set DLL_EN bit to 1. */
   UINT32 reg_value = 0;
   reg_value = HWIO_SDC_REG_IN(HC_REG_DLL_CONFIG, (UINTN)sdcc_reg_hc[nControllerId]);
...
inline void
HAL_sdhci_DllCkOutEnable (const UINT32 nControllerId, BOOLEAN bEnable)
{
   UINT32 reg_value = 0;
   reg_value = HWIO_SDC_REG_IN(HC_REG_DLL_CONFIG, (UINTN)sdcc_reg_hc[nControllerId]);
...
int HAL_sdhci_ConfigCmDllPhase(const UINT32 nControllerId, INT8 phase)
{
   ...
   HAL_sdhci_DllCkOutEnable(nControllerId, TRUE);
...
{
   UINT16 uhs_mode = 0, val = 0;

   if (mode == SDCC_DDR50_MODE)
   {
      uhs_mode = 0x4;
   }
   else if ((mode == SDCC_SDR104_MODE) || (mode == SDCC_HS200_MODE))
   {
      uhs_mode = 0x3;
   }
   else if (mode == SDCC_SDR50_MODE)
   {
      uhs_mode = 0x2;
   }
   else if ((mode == SDCC_SDR25_MODE) || (mode == SDCC_HS_MODE))
   {
      uhs_mode = 0x1;
   }

   val = HWIO_SDC_WORD_IN(HC_REG_3E, (UINTN)sdcc_reg_hc[nControllerId]);

   val = ((val & ~(UINT16)HWIO_HC_REG_HST_CTL2_UHS_MODE_SEL_BMSK) | uhs_mode); 
...
void HAL_sdhci_SetCapabilities (const UINT32 nControllerId)
{
  ...
  UINT32 eMMCSlotNum = 0;
  UINT32 ExternalSDSlotNum = 0;
  UINT32 reg_value = 0;
  sdcc_slot_type   *sdcc_pdata = NULL;
  
  sdcc_pdata = sdcc_get_slot_handle (nControllerId);
  ...
  eMMCSlotNum         = sdcc_pdata->sdcc_config.EmmcSlotNumber;
  ExternalSDSlotNum   = sdcc_pdata->sdcc_config.ExtSlotNumber;

  caps0 = HWIO_SDC_REG_IN (HC_VENDOR_CAPS0, (UINTN)sdcc_reg_hc[nControllerId]);
  caps1 = HWIO_SDC_REG_IN (HC_VENDOR_CAPS1, (UINTN)sdcc_reg_hc[nControllerId]);  
  ...
  sdcc_bsp_target_cap_overrides(nControllerId, &caps0, &reg_value); 
...
/**
 * Register SDCC_HC_REG_DLL_CONFIG 
 * Initialize DLL Config
 *
 * @param nControllerId
 *   SD controller number, as described in \ref nControllerId .
 * @param init_value
 *   value of DLL_CONFIG[15:0] to write
 */
inline void
HAL_sdhci_DllInit (const UINT32 nControllerId, UINT32 init_value)
{
  UINT32 val = HWIO_SDC_REG_IN(HC_REG_DLL_CONFIG, (UINTN)sdcc_reg_hc[nControllerId]);

  val = (val & ~HWIO_HC_REG_DLL_CONFIG_INIT_BMSK) | (init_value & HWIO_HC_REG_DLL_CONFIG_INIT_BMSK); 
  HWIO_SDC_REG_OUT (HC_REG_DLL_CONFIG, (UINTN)sdcc_reg_hc[nControllerId], val); 
}
...
/**
 * Register SDCC_HC_VENDOR_SPECIFIC_FUNC 
 * Set IO PAD PWR SWITCH and IO_PAD_PWR_SWITCH_EN
 *
 * @param nControllerId
 *   SD controller number, as described in \ref nControllerId
 */
inline void
HAL_sdhci_IoPadPwrSwitch (const UINT32 nControllerId)
{
  UINT32 val = HWIO_SDC_REG_IN (HC_VENDOR_SPECIFIC_FUNC, (UINTN)sdcc_reg_hc[nControllerId]);

  val = val | HWIO_HC_VENDOR_SPECIFIC_FUNC_HC_IO_PAD_PWR_SWITCH_EN_BMSK; 
...
{
  UINT32 val = HWIO_SDC_REG_IN (HC_REG_DDR_CONFIG, (UINTN)sdcc_reg_hc[nControllerId]);
...
/**
 * Register SDCC_HC_REG_DLL_CONFIG_2 
 * Change MCLK_FREQ_CALC and LOW_FREQ_MODE for DLL configuration
 *
 * @param nControllerId
 *   SD controller number, as described in \ref nControllerId .
 *  
 * @param mclk_freq_calc
 *   MCLK_FREQ_CALC value
 *
 * @param low_freq_mode
 *   LOW_FREQ_MODE value
 * 
 */
inline void
HAL_sdhci_DLL2_ClockConfig (const UINT32 nControllerId, UINT8 mclk_freq_calc, UINT8 low_freq_mode)
{
   UINT32 val = HWIO_SDC_REG_IN(HC_REG_DLL_CONFIG_2, (UINTN)sdcc_reg_hc[nControllerId]);
   
   val = (val & ~HWIO_HC_REG_DLL_CONFIG_2_MCLK_FREQ_CALC_BMSK) | (mclk_freq_calc << HWIO_HC_REG_DLL_CONFIG_2_MCLK_FREQ_CALC_SHFT); 

   val = (val & ~HWIO_HC_REG_DLL_CONFIG_2_LOW_FREQ_MODE_BMSK) | ((low_freq_mode & 0x1) << HWIO_HC_REG_DLL_CONFIG_2_LOW_FREQ_MODE_SHFT); 

   HWIO_SDC_REG_OUT (HC_REG_DLL_CONFIG_2, (UINTN)sdcc_reg_hc[nControllerId], val);
}

/**
 * Register SDCC_HC_REG_DLL_CONFIG_3 
 * DLL Config 3 configuration (only applicable for 7FF)
 *
 * @param nControllerId
 *   SD controller number, as described in \ref nControllerId .
 *
 */
inline void
HAL_sdhci_DllConfig3(const UINT32 nControllerId, UINT8 sdc4_config_msb)
{
   UINT32 controller_ver = HWIO_HC_REG_FC_FE_HC_VENDOR_VERSION_NUM_BMSK & HWIO_SDC_REG_IN(HC_REG_FC_FE, (UINTN)sdcc_reg_hc[nControllerId]); 

   if (controller_ver >= 0x72000000)
   {
     HWIO_SDC_REG_OUT (HC_REG_DLL_CONFIG_3, (UINTN)sdcc_reg_hc[nControllerId], sdc4_config_msb);
   }
}

/**
 * Register SDCC_HC_REG_DLL_USER_CTL 
 * DLL user_ctl register configuration (only applicable for 7FF)
 *
 * @param nControllerId
 *   SD controller number, as described in \ref nControllerId .
 * @param reg_value
 *   value of DLL_USER_CTL to write
 *
 */
inline void
HAL_sdhci_DllUserCtl (const UINT32 nControllerId, UINT32 reg_value)
{
   UINT32 controller_ver = HWIO_HC_REG_FC_FE_HC_VENDOR_VERSION_NUM_BMSK & HWIO_SDC_REG_IN(HC_REG_FC_FE, (UINTN)sdcc_reg_hc[nControllerId]); 

   if (controller_ver >= 0x72000000)
   {
      HWIO_SDC_REG_OUT (HC_REG_DLL_USER_CTL, (UINTN)sdcc_reg_hc[nControllerId], reg_value);
   }
}
