 ...
 *
 * Copyright (c) 2014-2018, 2020-2022 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
...
2022-04-21   gml     Add full open support in the ramdump path
2022-04-09   ssb     Add support for eMMC bootup using Device Tree Config
2021-09-27   ss      Used Config API for removing PCD lib dependency
2021-09-02   ss      Validating slot number before doing operations
2021-08-18   ss      Added check for Unsupported Slot Number
...
2018-08-15   jt      Disable Select In for SD card slot as well  
2018-07-13   jt      Add UHS-1 support  
2017-11-10   jt      Make sure we don't reset at the init frequency  
2017-10-30   jt      Remove busywait init 
...
/******************************************************************************
* Name: sdcc_get_card_cid
*
* Description:
*    This function partially requests the CID from the device and decodes and
*    populates the sdcc_dev_type structure 
*
* Arguments:
*    sdcc_pdev        [IN] : sdcc_dev_structure to populate the CID values
*                            
*
*
* Returns:
...
SDCC_STATUS sdcc_get_card_cid(sdcc_dev_type  *sdcc_pdev)
{
...
/******************************************************************************
* Name: sdcc_partial_handle_open
*
* Description:
*    This function partially initializes the SDCC controller so as to read device tree
*
* Arguments:
...
struct sdcc_device *sdcc_handle_partial_open(INT16 driveno, UINT32 phy_partition_num)
{
   ...
   SDCC_CARD_STATUS card_status = 0;

   /* Check to ensure that function is called only for eMMC device */

   if ( !SDCC_DRIVENO_IS_VALID(driveno) || driveno != 0)
   {
      DEBUG ((EFI_D_ERROR, "Invalid Drive Number: %d", driveno));
      return NULL;
   }

#ifdef SDCC_RAMDUMP_IMAGE
   ret_handle = sdcc_handle_open(driveno, phy_partition_num);

   if (ret_handle == NULL)
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_open failure", driveno));
      return NULL; 
   }

   return ret_handle;
#endif /* SDCC_RAMDUMP_IMAGE */

   bsp_slot_type = sdcc_bsp_get_slot_type (driveno);
   
   if (bsp_slot_type == SDCC_BSP_NO_SLOT)
   {
      DEBUG ((EFI_D_ERROR, "Drive Number %d unsupported", driveno));
      return NULL; 
   }
   ...
   if (NULL == sdcc_pdata)
   {  
      DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: can't get slot handle\n"));
      return NULL;
   }
   ...
   sdcc_pdev->rca                      =  sdcc_bsp_get_rca(driveno);
   ...
   sdcc_pdev->sdhci_adma_enable        =  sdcc_bsp_is_adma_supported();

   HAL_sdhci_InitializeRegisters();

   card_status = sdcc_send_status(sdcc_pdev);

   if (card_status != SDCC_CARD_TRAN)
   {
      DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: Card not in Transaction Mode\n"));
      return NULL;
   }
   ...
   if ( NULL == ret_handle )
   {
      ...
      DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: Registering client handle failed\n"));
      return NULL;
   }

   /* Putting card in Stand By Mode */
   card_status = sdcc_select_card(sdcc_pdev, FALSE);
   if (SDCC_NO_ERROR != rc)
   {
     DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: Failed to put card in StandBy Mode\n"));
     return NULL;
   }
   ...
   if (SDCC_NO_ERROR != rc)
   {
     DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: Get Memory Info failed\n"));
     return NULL;
   }

   rc = sdcc_get_card_cid(sdcc_pdev);
   if (SDCC_NO_ERROR != rc)
   {
     DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: Get CID failed\n"));
     return NULL;
   }

   /* Putting card back in Transaction Mode */
   ...
   if (SDCC_NO_ERROR != rc)
   {
     DEBUG ((EFI_D_ERROR, "sdcc_handle_partial_open: Config mmc mode segment failed\n"));
     return NULL;
   }
...
sdcc_handle_open( INT16 driveno, UINT32 phy_partition_num )
{
   ...
   if ((bsp_slot_type ==  SDCC_BSP_SD_SLOT) && SDCC_NO_ERROR !=  sdcc_bsp_volt_set (driveno, SDCC_BSP_REGULATORS_ON))
   {
      DEBUG ((EFI_D_ERROR, "Failed to enable voltage regulator for drive number %d", driveno));
      return NULL;   
   }
   ...
   sdcc_pdev = &(sdcc_pdata->dev);
    ...
    if (( phy_partition_num == 0 ) &&
      ( sdcc_bsp_get_slot_type(driveno) & SDCC_BSP_SD_SLOT) && 
   ...
   do
   {
      ...
      if (( SDCC_CARD_SD  == sdcc_pdev->card_type ) ||
          ( SDCC_CARD_SDHC == sdcc_pdev->card_type ) || 
          ( SDCC_CARD_UHS1 == sdcc_pdev->card_type ) ||
   ...
   STOR_PROFILE_END(SDCC_PROFILE_INIT_BIT_MASK, "SDCC_INIT",
                    phy_partition_num, 0);
...
)
{
   ...
   do
   {
      ...
      else
      {
         ...
         sdcc_pmem_info->spec_version = sdcc_pdev->mem.mem_info.spec_version;
      }
...
)
{
   ...
   STOR_PROFILE_END(SDCC_PROFILE_READ_BIT_MASK, "SDCC_READ",
                    (UINT64)data_address,(UINT32) num_blocks);
...
)
{
   ...
   STOR_PROFILE_END(SDCC_PROFILE_WRITE_BIT_MASK, "SDCC_WRITE",
                    data_address, (UINT32) num_blocks);
...
SDCC_STATUS sdcc_sdhci_mode_enable (void)
{
   ...
   sdcc_bsp_config sdcc_config;
   
   sdcc_bsp_get_config(0, &sdcc_config); 
 
   extSDSlot = sdcc_config.ExtSlotNumber;
   eMMCSlot  = sdcc_config.EmmcSlotNumber;
   maxSlot   = sdcc_config.MAXSlotNumber;
	
   for (slot_num = 0; slot_num < maxSlot; slot_num++)
   {
	  if (SDCC_BSP_NO_SLOT == sdcc_bsp_get_slot_type (slot_num))
      {
         continue;
      }
      ...
      {
         /* If there is error here, slot_num would be invalid, so move to the next slot */
         if (SDCC_BSP_NO_ERROR != sdcc_bsp_gpio_control(slot_num, SDCC_BSP_GPIO_ON))
         {
            continue; 
         }
...
SDCC_STATUS sdcc_handle_get_operational_mode_capabilities (struct sdcc_device *handle, 
                                                           void *capabilities)
{
   sdcc_bsp_err_type err = SDCC_ERR_UNKNOWN;
   UINT32 driveno = 0; 
   ...
   {
      DEBUG ((EFI_D_INFO, "NULL capabilities \n"));
      return SDCC_ERR_INVALID_PARAM;
   }
   ...
   if (!SDCC_DRIVENO_IS_VALID(driveno))
   {
      DEBUG ((EFI_D_ERROR, "Invalid drive number %d\n", driveno));
      return SDCC_ERR_INVALID_PARAM;
   }

   err = sdcc_bsp_operational_mode_info (driveno, handle->hdev->curr_speed_mode, capabilities);
   if (SDCC_BSP_NO_ERROR == err)
   {
      return SDCC_NO_ERROR;
   }
   if (SDCC_BSP_ERR_INVALID_PARAM == err)
   {
      return SDCC_ERR_INVALID_PARAM;
   }
   else if (SDCC_BSP_ERR_FEATURE_UNSUPPORTED == err)
   {
      return SDCC_ERR_FEATURE_UNSUPPORTED;
   }
   else
...
/******************************************************************************
* Name: sdcc_handle_operational_mode_info
*
* Description:
                                                   ...
                                                   sdcc_op_mode_info *curr_info)
{
   ...
   if (NULL == curr_info)
   {
      DEBUG ((EFI_D_INFO, "NULL curr_info \n"));
      return SDCC_ERR_INVALID_PARAM;
   }

   curr_info->speed_mode = handle->hdev->curr_speed_mode;
   curr_info->volt_corner = handle->hdev->curr_volt_corner; 
...
SDCC_STATUS sdcc_handle_set_operational_mode (struct sdcc_device *handle, 
                                              ...
                                              UINT8 speed_mode)
{
   sdcc_bsp_err_type err = SDCC_BSP_ERR_UNKNOWN;
   UINT32 driveno = 0; 
   ...
   if ((handle->hdev->curr_volt_corner == volt_corner) && (handle->hdev->curr_speed_mode == speed_mode)) 
   {
      return SDCC_NO_ERROR;
   }
   ...
   /* Check to see if the volt_corner and speed mode combination is valid */
   err = sdcc_bsp_check_valid_operational_mode (driveno, volt_corner, speed_mode);
   if (SDCC_BSP_ERR_FEATURE_UNSUPPORTED == err)
   {
      return SDCC_ERR_FEATURE_UNSUPPORTED;
   }
   else if (SDCC_BSP_NO_ERROR != err) 
   {
      return SDCC_ERR_INVALID_PARAM;
   }

   /* Compare volt_corner with curr_volt_corner to decide if we are clock scaling up or down, or
      just changing the speed_mode */
   if (handle->hdev->curr_volt_corner < volt_corner)
   {
      return sdcc_clock_scale_up (handle->hdev, volt_corner, speed_mode); 
   }
   else if (handle->hdev->curr_volt_corner > volt_corner)
   {  
      return sdcc_clock_scale_down (handle->hdev, volt_corner, speed_mode);
   }
   else
   {
      return sdcc_change_speed (handle->hdev, speed_mode);
   }
}
...
SDCC_STATUS sdcc_handle_set_speed (struct sdcc_device *handle, UINT8 speed_mode)
{
   ...
   return sdcc_handle_set_operational_mode (handle, handle->hdev->curr_volt_corner, 
                                            speed_mode);
}
                                               ...
                                               UINT8 volt_corner)
{
   ...
   return sdcc_handle_set_operational_mode (handle, volt_corner, 
                                            handle->hdev->curr_speed_mode);
}
...
SDCC_STATUS sdcc_handle_enable_retuning (struct sdcc_device *handle)
{
   ...
   handle->hdev->tuning_done = FALSE; 

   return SDCC_NO_ERROR; 
}

/* Test code for R/w profiling - Please note this may corrupt the images in DDR
 * if the test area overlap with any image area
 */
#if 0
void storage_emmc_test(void)
{
  struct sdcc_device * uh;
  uint8 *buf;
  int32 try_count = 0;
  int32 chunk_in_sectors[] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192};
  int32 i = 0, j = 0;
  UINT32 sector_size = 512, sector_addr = 0;
  UINT32 offset = 204800; //Test starts @ 100MB

  buf = (uint8 *) 0x80080000;
  uh = sdcc_handle_open(0, 0);
  if (uh == NULL) return;
  ...
  for (j=0; j<=13;j++) // For different chunk sizes
  {
    ...
    for (i=0;i<try_count;i++)
    {
      ...
      sdcc_handle_read(uh, sector_addr, buf, chunk_in_sectors[j]);
    }

    STOR_PROFILE_END(STOR_PROFILE_BENCHMARK_BIT_MASK, "EMMC_READ",
  ...
  for (j=0; j<=13;j++) // For different chunk sizes
  {
    ...
    {
      sector_addr = offset + (i * chunk_in_sectors[j]);
      sdcc_handle_write(uh, sector_addr, buf, chunk_in_sectors[j]);
    }

    STOR_PROFILE_END(STOR_PROFILE_BENCHMARK_BIT_MASK, "EMMC_WRITE",
      chunk_in_sectors[j], try_count);
