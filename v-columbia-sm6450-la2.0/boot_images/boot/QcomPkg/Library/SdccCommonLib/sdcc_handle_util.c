 ...
 *
 * Copyright (c) 2014,2023 Qualcomm Technologies Incorporated.
 * All Rights Reserved.
...
----------   ---     ----------------------------------------------------------
2023-02-09   ssb     Added ramdump support for PIPO.
...
/* counter to indicate the number of client handle list count */
static INT32 sdcc_client_handle_list_count = 0;
...
static void sdcc_purge_client_handle_entries( UINT32 driveno )
{
   ...
   while ( pch <= SDCC_CLIENT_HANDLE_LIST_END_ADDR )
   {
      ...
      /* check if client handle list count has no available handles */
      ...
      if ( 0 == sdcc_client_handle_list_count )
      {
         break;
      }
                             ...
                             UINT32 phy_partition_num )
{
   ...
   /* only proceed to search and update the client handle list count */
   /* if client handle list has available entry */
   if ( sdcc_client_handle_list_count >= SDCC_CLIENT_HANDLE_MAX_NUM )
   {
      /* no available entry */
...
BOOLEAN sdcc_is_device_still_in_use( sdcc_dev_type *pdev )
{
   ...
   /* check if client handle list count have no registered handles */
