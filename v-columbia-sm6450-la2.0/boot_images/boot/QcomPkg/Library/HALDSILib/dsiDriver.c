...
MDP_Status DSIDriver_Read(MDP_Display_IDType eDisplayId, uint8 uCmdType, void *pPayload, uint32 uPayloadSize, void *pReadData, uint32 *pReadSize, DSIDisplay_CmdFlags eFlags)
{
  ...
  if (*pReadSize > DSI_READ_MAX_PACKET_SIZE)
  {
    /* DSI can only support max 10 bytes return data for one read */
    sReadPacket.uReadPacketBufferSize = DSI_READ_MAX_PACKET_SIZE;
  }
  else
  {
...
MDP_Status DSIDriver_Init(MDP_Panel_AttrType *pPanelConfig)
{
  ...
  if (MDP_STATUS_OK == eStatus)
  {
    ...
    for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)
    {
      ...
      {
        DSI_Log_Message(MDP_LOGLEVEL_ERROR, "DixplayDxe: DSI_Device_Open failed\n", eDSIDeviceId);
        eStatus = MDP_STATUS_FAILED;
        break;
      }
    }
...
MDP_Status DSIDriver_SetMode(MDP_Panel_AttrType *pPanelConfig)
{
        ...
        (pPanelConfig->ePhysConnect == MDP_DISPLAY_CONNECT_SECONDARY_DSI_VIDEO))
    {
      ...
      if (pPanelConfig->sDSCConfig.bDSCEnable)
      {
        if (1 < pPanelConfig->sDSCConfig.uDSCCompressionRatio)
        {
          ...
          sDisplayConfig.sDisplayInfo.iVisWidthPx                        = HAL_DSI_Phy_Roundup((float)(sDisplayConfig.sDisplayInfo.iVisWidthPx) / (pPanelConfig->sDSCConfig.uDSCCompressionRatio / 1000));
  ...
  if (MDP_STATUS_OK == eStatus)
  {
    ...
    DSI_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayDxe: eClockLaneCfg =%d\n", eClockLaneCfg);
    ...
    if (DSI_Display_ClockLane_ForceHS == eClockLaneCfg)
    {
      ...
      for (uDevIdx = 0; uDevIdx < pDriverCtx->uNumDSIControllers; uDevIdx++)    
      {
        ...
        {
          DSI_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayDxe: Clock lane failed to enter HS pre-init\n");
        }
      }
    }
  ...
  if (MDP_STATUS_OK == eStatus)
  {
    ...
    if (MDP_STATUS_OK == eStatus)  
    {
      ...
      else
      {
        ...
        {
          DSI_Log_Message(MDP_LOGLEVEL_ERROR, "DixplayDxe: MDPPlatformConfigure(MDPPLATFORM_CONFIG_RESETPANEL) failed\n" );
        }
...
MDP_Status DsiPanelInit(MDP_Panel_AttrType    *pPanelInfo)
{
  ...
  MDP_LOG_FUNC_ENTRY("DsiPanelInit", 0x0);
  ...
  MDP_LOG_FUNC_EXIT("DsiPanelInit", 0x0);
...
static MDP_Status SendPanelXMLSequence(DSI_DriverCtx *pDriverCtx, void *pDSICommandBuffer, MDP_Panel_AttrType *pPanelConfig)
{
  ...
  MDP_LOG_FUNC_ENTRY("SendPanelXMLSequence", 0x0);
  if ((NULL == pDriverCtx  ) ||
      (NULL == pPanelConfig))
  {
      eStatus = MDP_STATUS_BAD_PARAM;
  ...
  else
  {
    ...
    while (0 != sXMLBinaryData.uPacketLength)
    {
      ...
      MDP_Log_Message(MDP_LOGLEVEL_ERROR, "DisplayDxe: SendPanelXMLSequence pDsiCmd=0x%x\n", *pDsiCmd);
  ...
  MDP_LOG_FUNC_EXIT("SendPanelXMLSequence", 0x0);
