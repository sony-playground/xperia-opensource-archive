...
$Header: //components/rel/core.boot/6.0/QcomPkg/Library/SmemLib/src/smem_xbl_loader.c#3 $
...
void smem_boot_targ_init(void)
{
  ...
  DALSYS_PROPERTY_HANDLE_DECLARE(icb_prop_handle);
  ...
  /*code added to support ICB between ADSP and SLPI*/
  result = DALSYS_GetDALPropertyHandleStr("/core/mproc/icb", icb_prop_handle);
  if (result == DAL_SUCCESS)
  {
    result = DALSYS_GetPropertyValue(icb_prop_handle, "icb_base", 0, &prop);  
    if(result != DAL_SUCCESS) 
    {
      SMEM_OS_LOG_FATAL(1, "ICB Base address not found", result);      
    }
    icb_base = prop.Val.pdwVal;

    result = DALSYS_GetPropertyValue(icb_prop_handle, "icb_size", 0, &prop);  
    if(result != DAL_SUCCESS) 
    {
      SMEM_OS_LOG_FATAL(1, "ICB size not found", result);      
    }
    icb_size = (uint32)prop.Val.dwVal;

    //zero out ICB region
    //TODO: Use the memset API which does 4-byte aligned operations
    // to zero out the memory
    //memset(icb_base, 0, icb_size);  
    for(count = 0; count < icb_size/sizeof(uint32); count++) 
    {
      icb_base[count] = 0;
    }
  }
