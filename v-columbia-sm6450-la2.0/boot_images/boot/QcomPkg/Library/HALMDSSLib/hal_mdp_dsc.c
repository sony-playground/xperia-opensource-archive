                                   ...
                                   HAL_MDP_DSCInfoType    *pDSCInfo)
{
  ...
  else
  {
    ...
    if (1 == uActiveSSNum)
    {
      // If total active slice is 1 && reduce dsc buffer limit based on scaling factor
      uObMaxaddr = pgsMdpHwInfo->uReducedObMaxBuffer ? (pDSC->pCoreConfig->ob_max_addr_1ss * pgsMdpHwInfo->uReducedObMaxBuffer / 100) : pDSC->pCoreConfig->ob_max_addr_1ss;
    }
    else
    {
      // If total active slice are greater than 1 && reduce dsc buffer limit from 20k to 10k based on scaling factor
      uObMaxaddr = pgsMdpHwInfo->uReducedObMaxBuffer ? (pDSC->pCoreConfig->ob_max_addr_2ss * pgsMdpHwInfo->uReducedObMaxBuffer / 100) : pDSC->pCoreConfig->ob_max_addr_2ss;
    }

    uRegValue = HWIO_OUT_FLD(uRegValue, MDP_DSC_0_DSC_0_ENC_ENC_DF_CTRL, OB_MAX_ADDR, uObMaxaddr);
