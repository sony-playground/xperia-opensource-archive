...
HAL_MDSS_ErrorType HAL_MDP_TrafficCtrl_Setup (HAL_MDP_PerfCtrlParamsType *psPerfCtrlSetting,
                                              ...
                                              uint32                      uFlags )
  {
     ...
     for(uIndex = 0; uIndex < uNumOfSettings; uIndex++)
     {
        ...
        if (HAL_MDSS_STATUS_SUCCESS == eStatus)
        {
          /* configures the traffic shaper for the client */
          ...
          // SSPP QoS setting (configure danger and safe signaling)
          if (NULL != psSetting->psQoSSetting) 
          {
            // Setup Rotator SSPP QoS
            if ((NULL != pgsMdpHwInfo->sIpFxnTbl.RotSSPPQosConfig) &&
                (HAL_MDP_TRFCTRL_CLIENT_ROT_SSPP == psSetting->eClientId))
            {
              pgsMdpHwInfo->sIpFxnTbl.RotSSPPQosConfig(psSetting->eReadClientLatency, 
                                                      psSetting->psQoSSetting,
                                                      psSetting->uSrcSurfFlags);
            }
            // Setup ViG, DMA SSPP QoS
            else if ((NULL != pgsMdpHwInfo->psSSPPQoSInfo)                  &&
                     (NULL != pgsMdpHwInfo->sIpFxnTbl.SSPPQosConfig)        &&
                     (psSetting->eClientId >= HAL_MDP_TRFCTRL_CLIENT_VIG0)  &&
                     (psSetting->eClientId <= HAL_MDP_TRFCTRL_CLIENT_DMA3))
            {
              pgsMdpHwInfo->sIpFxnTbl.SSPPQosConfig(psSetting->eSourcePipe, 
                                                    psSetting->eReadClientLatency, 
                                                    psSetting->psQoSSetting,
                                                    psSetting->uSrcSurfFlags);
            }
          }
