/** @file QcomLib.c
  ...
  Copyright (c) 2010-2019,2020,2021 Qualcomm Technologies, Inc. All Rights Reserved.
  ...
  )
{
  ...
  for (i = 0; i < BlkIoHandleCount; i++)
  {
    ...
    if ((SelectionAttrib & FILTERS_NEEDING_DEVICEPATH) != 0)
    {
      ...
      if (SelectionAttrib & BLK_IO_SEL_SELECT_ROOT_DEVICE_ONLY)
      {
        if ((!boot_from_nvme()) && (DevicePathDepth > 1))
          continue;
      }
...
)
{
  ...
  */
  ...
  if (RootDeviceType != NULL)
  {
    Attrib |= BLK_IO_SEL_SELECT_ROOT_DEVICE_ONLY;
                    ...
                    OUT     CHAR8** ChipVerStr       OPTIONAL)
{
  ...
  AsciiStrnCpy (TempPtr, PlatformTypeStrings[PlatformInfo.platform], BufferSize);
