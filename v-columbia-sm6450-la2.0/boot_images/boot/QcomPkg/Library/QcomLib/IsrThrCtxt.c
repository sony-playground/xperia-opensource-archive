...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 04/30/20   cp      Correcting sequence for event signal / masking.
...
EFI_STATUS RegisterThrCtxtIntrHandler (UINT32 Vector, ThrCtxtIntrHandler Cbfn, VOID* Arg)
{
  ...
  Cpu = InfoBlkPtr->BootCore;

  ThreadSetPinnedCpu (Thr, Cpu);  // make sure this thread runs only on boot core
  ThreadDetach (Thr);
