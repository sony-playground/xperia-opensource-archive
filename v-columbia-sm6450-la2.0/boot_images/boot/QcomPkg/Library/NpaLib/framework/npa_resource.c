                        ...
                        npa_resource_state_delta old_headroom )
{
         ...
         (resource->active_headroom != old_headroom) ) )
  {
    ...
    in_stm_state = npa.in_stm_resource->active_state;
    ...
    CORE_DAL_VERIFY(
      Core_WorkQueueInsert( npa.npa_event_queue_handle, action,
                            CORE_WORKQUEUE_TAIL, FALSE, in_stm_state ? TRUE : FALSE ) );
                                                   ...
                                                   npa_client   *client )
{
   ...
   */
  if ( npa_resource_has_internal_attribute( resource, NPA_RESOURCE_DRIVER_UNCONDITIONAL )
...
static NPA_INLINE void _npa_request_batched( npa_batch_request *batch, npa_client *client )
{
  npa_resource *resource = client->resource;
  ...
  {
    ULOG_RT_PRINTF_3( client->log_handle, "\trequest batched (batch: 0x%08x) "
                      "(client: 0x%08x) (sequence: 0x%08x)",
                      batch, client, client->sequence );
  }
  else
  {
    ULOG_RT_PRINTF_4( client->log_handle, "\trequest batched (batch: 0x%08x) "
                      "(client: 0x%08x) (sequence: 0x%08x) (request state: %d)",
                      batch, client, client->sequence,
                      resource->request_state );
  }
}
...
/**
 * <!-- npa_process_batch_request -->
 *
 * @brief The core of NPA's request pipeline.
 * Aggregates and applies the request from the clients.
 *
 */
void npa_process_batch_request( npa_batch_request *batch )
{
  npa_batch_resource *head, *tail, *batch_resource;
  npa_batch_request  *ptr, *temp;
  npa_client          batch_client = { 0 };
  npa_resource       *resource;
  npa_client_handle   client;
  ...
  CORE_VERIFY_PTR(batch);
 
  /* Aggregate all client requests. */
  for ( ptr = batch, resource = NULL, head = tail = NULL; ptr; /* empty */ )
  {
    client = ptr->client;
    CORE_VERIFY_PTR( client );
    if (client->resource != resource)
    {
      ...
      /* Lock down this resource */
      Core_MutexLock( resource->node_lock );
 
      batch_resource = (npa_batch_resource *)Core_PoolAlloc( &npa.batch_resource_pool );
      batch_resource->old_state = resource->active_state;
      batch_resource->old_required = NPA_RESOURCE_REQUIRED_REQUEST( resource );
      batch_resource->old_suppressible = NPA_RESOURCE_SUPPRESSIBLE_REQUEST( resource );
      batch_resource->old_semiactive = NPA_RESOURCE_SEMIACTIVE_REQUEST( resource );
      batch_resource->old_suppressible2 = NPA_RESOURCE_SUPPRESSIBLE2_REQUEST( resource );
      batch_resource->old_headroom = resource->active_headroom;
      batch_resource->next = NULL;
      batch_resource->resource = resource;
 
      /* Organize batch resource in same order as client list */
      if (NULL == head)
      {
        head = batch_resource;
      }
    ...
    /* Aggregate this client's request */
    npa_invoke_update_function( resource, ptr->client );
    ...
    /* Swap PENDING and ACTIVE work requests
     *
     * We do this despite the fact that the client request is not processed yet.
     * Client structures are unprotected by design and calling code must not
     * use/rely on fields of a batched client until return from npa_issue_batch_request"
     */
    client->index ^= 1;
    ...
    /* Pass all request attributes to the batch client.
     * A future FR would be needed if any clients require more advanced
     * support where request attributes varied within a batch.
     */
    npa_pass_request_attributes( client, &batch_client );

    _npa_request_batched( batch, client );
   
    /* Return batch request to pool. */
    temp = ptr;
    ptr = ptr->next;
    Core_PoolFree( &npa.batch_request_pool, temp );
  }
 
  CORE_VERIFY_PTR(head);
 
  /* Invoke driver function with batch data. */
  batch_client.type = NPA_CLIENT_BATCH;
 
  for ( driver_count = 0, batch_resource = head, resource_data = NULL;
        batch_resource; batch_resource = batch_resource->next )
  {
    resource = batch_client.resource = batch_resource->resource;
    CORE_VERIFY_PTR( resource );
 
    if (NULL == batch_resource->next)
    {
      npa_set_request_attribute( &batch_client,
                                 (npa_request_attribute)NPA_REQUEST_LAST_IN_BATCH );
 
      /* Bypass short-circuiting on last in batch to trigger resource sync. */
      if (driver_count)
      {
    ...
    if ( npa_resource_has_attribute( resource,
                                      (NPA_RESOURCE_DRIVER_UNCONDITIONAL | NPA_RESOURCE_DRIVER_UNCONDITIONAL_NEXT) )  // NPA_RESOURCE_DRIVER_UNCONDITIONAL_ALL
         || new_state != resource->active_state
         || batch_resource->old_required != NPA_RESOURCE_REQUIRED_REQUEST(resource)
         || batch_resource->old_semiactive != NPA_RESOURCE_SEMIACTIVE_REQUEST(resource)
         || ( batch_resource->old_suppressible != NPA_RESOURCE_SUPPRESSIBLE_REQUEST(resource)
    ...
    {
      /* Share common data for all resources in this batch */
      batch_client.resource_data = resource_data;
      new_state = resource->node->driver_fcn( resource, &batch_client, new_state );
 
      resource_data = batch_client.resource_data;
 
      _npa_update_active_state( resource, new_state );
 
      /* Keep track of requests queued by resource */
      driver_count++;
 
    }
  }
 
  /* Perform post change events. */
  for ( batch_resource = head; batch_resource; /* empty */ )
  {
    resource = batch_resource->resource;
    CORE_VERIFY_PTR( resource );
    ...
    _npa_post_async_events( resource, batch_resource->old_state,
                            batch_resource->old_headroom );

    Core_MutexUnlock( resource->node_lock );
 
    /* Return batch resource to pool. */
    head = batch_resource;
    batch_resource = batch_resource->next;
    Core_PoolFree( &npa.batch_resource_pool, head );
  }
}
