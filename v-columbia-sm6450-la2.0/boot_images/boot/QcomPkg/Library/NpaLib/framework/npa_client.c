...
Copyright (c) 2009-2015,2021 Qualcomm Technologies Inc.
All Rights Reserved.
...
/*=============================================================================
  ...
  ----------   ---     --------------------------------------------------------
  05/27/21     amr     Adding support for processing Batch request from cleints.
...
/**
 * <! -- npa_add_client_to_batch -->
 * 
 * @brief Adds a client request to the provided batch list.
 *
 * This function creates dynamic memory allocations to organize
 * a linked list of client requests within a given batch set.
 * Requests are inserted in ascending order of resource address.
 *
 * @param batch : Pointer to first batch request in a linked list.
 * @param client : client handle that will have its work model changed
 */
static void
npa_add_client_to_batch( npa_batch_request **batch, 
                         npa_client_handle   client)
{
  npa_batch_request *ptr, *next, *temp;

  CORE_VERIFY_PTR( batch );
  CORE_VERIFY_PTR( client );

  /* Acquire pool resources for a new batch */
  if ( NULL == *batch )
  {
    temp = (npa_batch_request *)Core_PoolAlloc( &npa.batch_request_pool );
    temp->client = client;
    temp->next = NULL;
    *batch = temp;
  ...
  /* Add clients in order of descending resource address. */
  ptr = *batch;
  CORE_VERIFY_PTR( ptr->client );
  if ( ptr->client->resource >= client->resource )
  {
    temp = (npa_batch_request *)Core_PoolAlloc( &npa.batch_request_pool );
    temp->client = client;
    temp->next = ptr;
  ...
  {
    next = ptr->next;
    while(next && next->client->resource < client->resource)
    {
      ptr = next;
      next = ptr->next;
    }
...
/** 
 * <!-- npa_batch_scalar_request -->
 *
 * @brief Add the client scalar request to the specified batch.
 *
 * The request must be of scalar work model
 * 
 * @param handle : Handle to batch being used
 * @param client : Handle to client being used
 * @param state : Requested resource state
 */
void
npa_batch_scalar_request( npa_batch_request_handle *handle, 
                          npa_client_handle         client,
                          npa_resource_state        state )
{
  char *client_type = "custom";
  ...
  /* Log request from this client */
  if ( client->resource->active_plugin->map_request_fcn )
  {
    NPA_PENDING_REQUEST(client).state = 
      client->resource->active_plugin->map_request_fcn( client, state );
    ULOG_RT_PRINTF_7( client->log_handle, "npa_batch_%s_request "
  ...
  {
    NPA_PENDING_REQUEST(client).state = state;
    ULOG_RT_PRINTF_6( client->log_handle, "npa_batch_%s_request "
                      "(client: 0x%08x) (client_name: \"%s\") "
                      "(request: %u) "
                      "(resource_name: \"%s\") (request_attr: 0x%08x)",
                      client_type, client, client->name, 
  ...
  if( NPA_PENDING_REQUEST(client).state != NPA_ACTIVE_REQUEST(client).state )
  {
    npa_add_client_to_batch(handle, client);
  }
  else
  {
...
/** 
 * <!-- npa_batch_vector_request -->
 *
 * @brief Add the client vector request to the specified batch.
 *
 * The request must be of vector work model
 * 
 * @param handle : Handle to batch being used
 * @param client : Handle to client being used
 * @param num_elems : Number of elements in the vector
 ...
 */
void
npa_batch_vector_request( npa_batch_request_handle *handle, 
                          npa_client_handle         client,
                          unsigned int              num_elems,
                          npa_resource_state       *vector )
{
  ...
  if ( npa_vector_logging_limit != 0 )
  {
    ...
    ULogFront_RealTimeMultipartMsgBegin( client->log_handle );
    ULOG_RT_PRINTF_6( client->log_handle, "npa_batch_%s_vector_request "
                      "(client: 0x%08x) (client_name: \"%s\") "
                      "(resource_name: \"%s\") (request_attr: 0x%08x) "
                      "(num_elems: %d) vector: ",
                      client_type, client, client->name, 
                      client->resource_name, client->request_attr,
                      num_elems );
    ULogFront_RealTimeVector( client->log_handle, "0x%08x ",
                              ( unsigned short )sizeof(npa_resource_state),
                              ( unsigned short )log_vector_len, 
                              vector );
    ULogFront_RealTimeMultipartMsgEnd( client->log_handle );
  ...
  {
    ULOG_RT_PRINTF_7( client->log_handle, "npa_batch_%s_vector_request "
                      "(client: 0x%08x) (client_name: \"%s\") "
                      "(resource_name: \"%s\") (request_attr: 0x%08x) "
...
/** 
 * <!-- npa_issue_batch_request -->
 *
 * @brief Issue the batched requests.
 *
 * Resource state should be updated to account for all request in the batch. 
 *  
 * @param handle : Handle to batch being used
 */
void
npa_issue_batch_request(npa_batch_request_handle handle)
{
  void              *log;
  npa_batch_request *batch = handle;

  CORE_VERIFY_PTR( batch );
  CORE_VERIFY_PTR( batch->client );

  log = batch->client->log_handle;
  ULOG_RT_PRINTF_1( log, "npa_issue_batch_request "
                    "(batch: 0x%08x)", batch );

  /* Forward single request batches to the non-batch path */
  if ( NULL == batch->next )
  {
    /* @brief This is needed because this fall through case we end up using
     * issue_sync which needs the INCOMING field to be populated.
     */
    
    /*
     * INCOMMING Request types are not supported in UEFI becouse npa does not support async client requests.
     */
    #if 0
        NPA_INCOMING_REQUEST(batch->client) = NPA_PENDING_REQUEST(batch->client);
    #endif

    batch->client->issue_request( batch->client,
                                  ( NPA_PENDING_REQUEST(batch->client).state != 
                                    NPA_ACTIVE_REQUEST(batch->client).state ) );

    /* Pool resources for batched requests are deallocated in
     * npa_process_batch_request.  Any batch with one request bypasses that 
     * path and is processed as a regular request so we must return the
     * resources allocated for the single batched request here.
     */
    Core_PoolFree( &npa.batch_request_pool, batch );
  }
  else
  {
    npa_process_batch_request(batch);
  }

  ULOG_RT_PRINTF_1( log, "batch processed "
                    "(batch: 0x%08x)", batch );
}
