...
cleanup:
  ...
  if (ret_handle == NULL)
    DP_LOGE("Failed to parse xml, error at offset %d, %d", xml_ptr - xml_str, len);
...
int devprg_xml_resp_rawmode(struct devprg_xml_write *xmlw, int rawmode)
{
  char *rawmode_str;

  rawmode_str = (rawmode == 1) ? "true" : "false";

  return devprg_xml_resp_attr_s(xmlw, "rawmode", rawmode_str);
...
int devprg_xml_add_log_tags(void *buffer, size_t buf_size, size_t log_size)
{
  ...
  memsmove(log_loc, buf_size - xml_tag_size, buf, log_size);
