...
//#include "qmi_idl_internal_lib.h"
#include "qmi_csi.h"
...
LIST(qmi_csi_service_type, qcsi_service_list);
static qmi_csi_lock_type client_list_lock;
LIST(qmi_csi_client_type, qcsi_client_list);
...
static qmi_csi_service_handle add_service(qmi_csi_service_type *svc)
{
  ...
  LIST_ADD(qcsi_service_list, svc, link);
...
{
  LOCK(service_list_lock);
  LIST_REMOVE(qcsi_service_list, svc, link);
  UNLOCK(service_list_lock);
}
...
static qmi_csi_service_type *find_service(qmi_csi_service_handle _handle)
{
  ...
  qmi_csi_service_type *svc = LIST_HEAD(qcsi_service_list);
...
static void add_client(qmi_csi_service_type *svc, qmi_csi_client_type *clnt)
{
  ...
  LIST_ADD(qcsi_client_list, clnt, global);
}
...
static qmi_csi_client_type *find_client(qmi_client_handle handle)
{
  ...
  LIST_FIND(qcsi_client_list, clnt, global, clnt->handle == to_find);
  return clnt;
}
...
static void free_client(qmi_csi_service_type *svc, qmi_csi_client_type *clnt)
{
  ...
  LIST_REMOVE(qcsi_client_list, clnt, global);
...
{
  qmi_csi_os_init();          //Define empty function in qmi_csi_target.h 
