...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 11/24/23   sd      Renaming quantum_dt to quantum,to reach EBL shell directly with enter_quantum flag 
 12/01/21   cm      Temp workaround to have all uefi menu options displayed after deleting RT variables
  ...
  )
{
  ...
  {
    DEBUG ((EFI_D_ERROR, "Skipped XLogSave because of already saved\n"));
  }
#endif /* FEATURE_XBOOT */
...
UINT32 CheckifEnterQuantumFv()
{
  ...
  Ret = FdtGetNodeHandle(&Node, "/soc/quantum");
  if (Ret) {
...
EFI_STATUS ProcessFvLoadingForFlashless(ram_partition_type_t partition_type)
{
  ...
  if (partition_type == RAM_PARTITION_APPSBL_MEMORY)
  {
