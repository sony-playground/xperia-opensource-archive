...
#define ADSP_IMAGE_ID                            0x32// 2 ASCII char hex value
/*===========================================================================*/
...
#define SUBSYS_NAME_MAX_LENTH		16
...
EFI_STATUS ChargerLibTarget_LoadChargerFW()
{
  ...
  CHAR8       AsciiImageName[SUBSYS_NAME_MAX_LENTH]= {0};
  CHAR16		ImageName[SUBSYS_NAME_MAX_LENTH] = {0};
  ...
  if (EFI_SUCCESS != Status)
  {
    CHARGER_DEBUG(( EFI_D_ERROR, "ChargerLib:: %a failed to read chip id %r \r\n", __FUNCTION__, Status));
  }
  if (ChipId == EFICHIPINFO_ID_SM_TOFINO)
  {
    UnicodeSPrint(ImageName, SUBSYS_NAME_MAX_LENTH, L"%s", PreStr);
    AsciiStrToUnicodeStr("adsp", &ImageName[StrnLenS(PreStr, SUBSYS_NAME_MAX_LENTH)]);
    ...
    if (EFI_SUCCESS != Status)
    {
      CHARGER_DEBUG(( EFI_D_ERROR, "ChargerLib:: %a adsp Failed to get pil cfg of %s \r\n", __FUNCTION__,  PilSubsysCfg.ImagePath));
	  return Status;
    }
    UnicodeStrToAsciiStr(PilSubsysCfg.ImagePath, AsciiImageName);
    PilSubsysCfg.ImagePath[AsciiStrLen(AsciiImageName)] =ADSP_IMAGE_ID;
    PilSubsysCfg.ImagePath[AsciiStrLen(AsciiImageName) + 1] = '\0';
    CHARGER_DEBUG((EFI_D_INFO, "Overriding PIL cfg ImagePath by Qfwk %s\n", PilSubsysCfg.ImagePath));
