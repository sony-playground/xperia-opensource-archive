...
#define MaxInitLogBufferSize    8192  // 8KB max init buffer read size

STATIC EFI_PM_GLINK_LINK_STATUS  PmicGLinkStatus = {0};
...
EFI_STATUS ChargerLib_EnableRTCAlaram(BOOLEAN enable);
...
EFI_STATUS ChargerLibCommon_Init( chargerlib_cfgdata_type *pChargerLibConfig )
{
  ...
  ChargerLibTarget_GetIPCStatus(&PmicGLinkStatus);
...
EFI_STATUS ChargerLib_GetBatteryStatus(chargerlib_batt_status_info *pBatteryStatus)
{
  ...
  {
    pBatteryStatus->BatteryVoltage = 3500;
    pBatteryStatus->StateOfCharge = 10;
    pBatteryStatus->ChargeCurrent = 500;
...
/*Diasble RTC Alarm for avoiding continuous reboot in low vbatt */
  
  DEBUG(( EFI_D_ERROR, "%a FDBG : ChargerLib_ForceSysShutdown called : %r \r\n", __FUNCTION__, Status));
  Status = ChargerLib_EnableRTCAlaram(FALSE);
  if((EFI_SUCCESS != Status))
  {
	DEBUG((EFI_D_ERROR, "ChargerLib:: %a  Error in switching off RTC Alarm = %r \r\n", __FUNCTION__, Status));
  }
  ...
  /* Send shutdown notification if necessary */
  Status = ChargerLib_GetChargingPath(&AttachedCharger);
  if((CHARGERLIB_ATTACHED_CHGR__USB == AttachedCharger || CHARGERLIB_ATTACHED_CHGR__DCIN == AttachedCharger) && 
      gEnableChargerFW && gChargerSharedInfo.uefi_charger_fw_mode == CHARGER_FW_MODE)
  {
    Status = ChargerLibTarget_SendShdnNotification();
    if((EFI_SUCCESS != Status))
    {
      CHARGER_DEBUG((EFI_D_ERROR, "ChargerLib:: %a  Error sending shutdown notification = %r \r\n", __FUNCTION__, Status));
    }
  }
...
EFI_STATUS ChargerLib_GetBatteryVoltageStatus(CHARGERLIB_VBATT_STATUS *pBatteryVoltageStatus)
{
  ...
  Status = ChargerLibTarget_GetBatteryVoltageStatus(pBatteryVoltageStatus);
  if( EFI_SUCCESS != Status )
  {
    CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a Error getting battery voltage status = %r \r\n", __FUNCTION__, Status));
    return Status;
  }
...
EFI_STATUS ChargerLib_GetErrors(BOOLEAN vbattChecking, CHARGERLIB_CHARGING_ERROR_TYPES  *pChargingError)
{
  ...
  *pChargingError = CHARGERLIB_CHARGING_ERROR_NONE;
  ...
  if (firstTime && gEnableChargerFW && (gChargerSharedInfo.uefi_charger_fw_mode == CHARGER_FW_MODE))
  {
    /* Check Critical errors first time, do not want to keep checking.*/
    Status = ChargerLibTarget_GetIPCError(pChargingError);
    if (*pChargingError == CHARGERLIB_CHARGING_FW_LINK_ERROR)
    {
      firstTime = FALSE;
      return Status;
    }
  ...
  CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a pChargingError = %d \r\n", __FUNCTION__, *pChargingError));
  return Status;
}
...
EFI_STATUS ChargerLib_GetDefectiveParts(BOOLEAN *pIsDefective)
{
  ...
  Status = ChargerLibTarget_GetDefectiveParts(pIsDefective);

  CHARGER_DEBUG((EFI_D_WARN, "ChargerLib:: %a Part EFICHIPINFO_PART_AUDIO IsDefectiv = %d\r\n", __FUNCTION__, *pIsDefective));
  return Status;
}
...
EFI_STATUS ChargerLib_HandleRecovery()
{
  ...
  /* Display UI */
  Status = ChargerLibTarget_HandleRecovery();
  return Status;
}
...
{
  EFI_STATUS         Status      = EFI_SUCCESS;
  Status = ChargerLibTarget_LoadChargerFW();
...
EFI_STATUS ChargerLib_LinkChargerFW()
{
  ...
  Status = ChargerLibTarget_LinkChargerFW();
...
EFI_STATUS ChargerLib_GetPlatformPwrOnReason(CHARGERLIB_PLATFORM_PWRON_REASON_TYPE *pwron_reason_type )
{
  ...
  if (PmicPwronProtocol == NULL)
  {
    ...
    if (Status != EFI_SUCCESS)
    {
      CHARGER_DEBUG((EFI_D_ERROR, "ChargerLib:: %a Error locating PmicPwronProtocol %r  \r\n", __FUNCTION__, Status));
      return Status;
    }
  }

  Status = PmicPwronProtocol->GetPonReason(PM_DEVICE_0 ,&PM_PonReasonType);
  if (Status != EFI_SUCCESS)
  {
  ...
  pwron_reason_type->HARD_RESET  = PM_PonReasonType.HARD_RESET;
  pwron_reason_type->SMPL        = PM_PonReasonType.SMPL;
  pwron_reason_type->RTC         = PM_PonReasonType.RTC;
  pwron_reason_type->DC_CHG      = PM_PonReasonType.DC_CHG;
  pwron_reason_type->USB_CHG     = PM_PonReasonType.USB_CHG;
  pwron_reason_type->PON1        = PM_PonReasonType.PON1;
  pwron_reason_type->CBLPWR      = PM_PonReasonType.CBLPWR;
  pwron_reason_type->KPDPWR      = PM_PonReasonType.KPDPWR;
...
EFI_STATUS ChargerLib_CpdlogSetProperties(UINT64 LogCategories, UINT32 LogLevel){
    ...
    Status = ChargerLibTarget_CpdlogSetProperties(LogCategories, LogLevel);
...
EFI_STATUS ChargerLib_CpdlogGetProperty(UINT64* LogCategories){
    ...
    Status = ChargerLibTarget_CpdlogGetProperty(LogCategories);
...
EFI_STATUS ChargerLib_CpdlogGetBuffer(UINT32 LogMaxSize){
    ...
    Status = ChargerLibTarget_CpdlogGetBuffer(LogMaxSize);
...
EFI_STATUS ChargerLib_CpdlogGetQbgDump(){
    ...
    Status = ChargerLibTarget_CpdlogGetQbgDump();
...
EFI_STATUS ChargerLib_GetChargingStateForLed(BOOLEAN *pToggleLed)
{
  ...
  Status = ChargerLibTarget_GetChargingStateForLed(pToggleLed);
...
EFI_STATUS ChargerLib_GetBatteryCellType(EFI_PM_FG_BATT_TYPE *battery_cell_type)
{
  ...
  Status = ChargerLibTarget_GetBatteryCellType(battery_cell_type);
...
EFI_STATUS ChargerLib_CpdlogGetInitBuffer(){
    ...
    {
      Status = ChargerLibTarget_CpdlogGetInitBuffer(MaxInitLogBufferSize);
    }
    return Status;
}
 ...
 EFI_STATUS ChargerLib_EnableRTCAlaram(BOOLEAN enable){
	...
	UINT32 RtcPmicIndex = 0x0;
	DEBUG(( EFI_D_ERROR, "%a FDBG : ChargerLib_EnableRTCAlaram : %r \r\n", __FUNCTION__, Status));
	Status = ChargerLib_GetPlatformPwrOnReason(&PonReasonType);
	if(EFI_SUCCESS != Status)
	{
	   CHARGER_DEBUG(( EFI_D_ERROR, "%a failed to get PwronReason: %r \r\n", __FUNCTION__, Status));
	   return Status;
	}
	if(PonReasonType.RTC) 
	{
	  ...
	  {
		  CHARGER_DEBUG(( EFI_D_ERROR, "%a Cannot locate PmicRtcProtocol: %r \r\n", __FUNCTION__, Status));
		  return Status;
	  }
		  
	  Status = PmicRtcProtocol->AlarmEnable(RtcPmicIndex, enable);
	  if(EFI_SUCCESS != Status)
	  {
		  CHARGER_DEBUG(( EFI_D_ERROR, "%a RTC Alarm disable failed: %r \r\n", __FUNCTION__, Status));
	  }
	}
	return Status;
