/** @file UefiPlatCfg.c
  ...
  Copyright (c) 2012-2019, 2021 Qualcomm Technologies, Inc.  All 
...
STATIC UINT64 UnusableDDRMemoryAtBeginning = 0;
STATIC UINT64 UnusableDDRMemorySizeAtBeginning = 0;
STATIC UINT64 UnusableDDRMemoryStartAddr = 0;
...
ParseBuildHobOption (CHAR8* BuildHobOptionStr)
{
  ...
  else if (AsciiStrCmp (BuildHobOptionStr, "HobOnlyNoCacheSetting") == 0)
    return HobOnlyNoCacheSetting;

  else if (AsciiStrCmp (BuildHobOptionStr, "NoMap") == 0)
    return NoMap;
	
  if (AsciiStrCmp (BuildHobOptionStr, "AddDynamicMem") == 0)
    return AddDynamicMem;
...
ProcessConfigTokens (UINT8* Section, UINT8* Key, UINT8* Value)
{
  ...
  else if (AsciiStrCmp (mKey, "UnusableDDRMemoryAtBeginning") == 0)
  {
     UnusableDDRMemoryAtBeginning = AsciiStrDecimalToUintn (mValue);
  }
  else if (AsciiStrCmp (mKey, "UnusableDDRMemoryStartAddr") == 0)
  {
     UnusableDDRMemoryStartAddr = AsciiStrHexToUint64 (mValue);
  }
  else if (AsciiStrCmp (mKey, "UnusableDDRMemorySizeAtBeginning") == 0)
  {
     UnusableDDRMemorySizeAtBeginning = AsciiStrHexToUint64 (mValue);
  }
...
EFI_STATUS
AddNonFdRegionRemainder(UefiInfoBlkType *UefiInfoBlkPtr)
{
  ...
  AsciiStrCpy (mMemRegions[mNumMemRegions].Name, NonFdRegion.Name);
...
/* Gets information on unusable memory at start of DDR by UEFI */
EFI_STATUS EFIAPI
GetUnusableMemoryAtStartInfo ( UINT64* UnusableStartAddr , UINT64* UnusableMemorySize )
{
  if ((UnusableStartAddr == NULL) || (UnusableMemorySize == NULL))
  {
    return EFI_INVALID_PARAMETER;
  }

  if (UnusableDDRMemoryAtBeginning)
  {
     *UnusableStartAddr = UnusableDDRMemoryStartAddr;
     *UnusableMemorySize = UnusableDDRMemorySizeAtBeginning;
  }
  else
  {
     *UnusableStartAddr = 0;
     *UnusableMemorySize = 0;
  }
  return EFI_SUCCESS;
}
...
AddFdRegionRemainder(UefiInfoBlkType *UefiInfoBlkPtr)
{
  ...
  EFI_STATUS    Status;
  UINT64 UefiFdBase = FixedPcdGet64(PcdEmbeddedFdBaseAddress);
  UINT64 UnusableMemoryStartAddress = 0;
  UINT64 UnusableMemorySize = 0;

  Status = GetPartitionEntryByAddr (UefiFdBase, &FdRegion);
  if (EFI_ERROR(Status))
    return EFI_LOAD_ERROR;
  
  /* Once we read the partition info, we read the config parameters to check if we 
  need to create a hole at the start of DDR memory and not mark as Conventional memory */ 
  Status = GetUnusableMemoryAtStartInfo(&UnusableMemoryStartAddress, &UnusableMemorySize);
  if (Status != EFI_SUCCESS)
  {
     UnusableMemoryStartAddress = 0;
     UnusableMemorySize = 0;
  }
  
  /* if hole required and address matches the FdRegion.Membase, update the address
  and size */
  if (FdRegion.MemBase == UnusableMemoryStartAddress )
  {
     FdRegion.MemBase = FdRegion.MemBase + UnusableMemorySize;
     
  }
  UefiInfoBlkPtr->FdMemBase = FdRegion.MemBase;
  RegionEndAddr = FdRegion.MemBase +  FdRegion.MemSize - UnusableMemorySize;
...
UpdateDynamicMemoryRegions (VOID)
{
...
UpdateSystemMemoryRegions (VOID)
{
  ...
  /* if hole required and address matches the EntryList.Membase, update the address
  and size */
  for (UINT32 i = 0; i<EntryCount; i++ )
  {
     if (EntryList[i].MemBase == UnusableMemoryStartAddress)
     {
         EntryList[i].MemBase = EntryList[i].MemBase + UnusableMemorySize;
         EntryList[i].MemSize = EntryList[i].MemSize - UnusableMemorySize;
     }
  }
...
ValidateParsedMemoryRegions()
{
  ...
  for (UINT32 j = 0; j < NumMemoryMapRegion; j++)
  {
    ...
    if (mMemRegions[j].BuildHobOption == AddDynamicMem) 
       continue;
