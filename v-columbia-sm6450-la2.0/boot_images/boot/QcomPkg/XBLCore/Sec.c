/** @file Sec.c
  ...
  Copyright (c) 2010-2022, 2023 Qualcomm Technologies, Inc. All rights reserved.
...
/*=============================================================================
 ...
 --------   ---     -----------------------------------------------------------
 07/23/22   cm      fix disabledcoremask logic
...
SetFwVersionStringInInfoBlk(VOID)
{
  ...
  AsciiStrCpy(Destination, FwVerStr);
...
//  ArmEnableBranchPrediction ();
  ...
  /* get the DisabledCoreMask after ChipInfo init */
  DalStatus = ChipInfo_Init();
  if (DalStatus != DAL_SUCCESS)
  {
     DEBUG ((EFI_D_ERROR, "ChipInfo_Init: Failed to initialize Status : %d \n", Status)); 
     DisabledCoreMask = ~ 0x1;
     MaxCoreCnt = 0x1;
  }
  else
  {      
    DalStatus = ChipInfo_GetDisabledCPUs(0, &DisabledCoreMask);
    if (DalStatus != DAL_SUCCESS)
    {
      DEBUG ((EFI_D_ERROR, "ChipInfo_GetDisabledCPUs: Failed to get DisabledCoreMask, Status : %d \n", Status));
      DisabledCoreMask = ~ 0x1;
      MaxCoreCnt = 0x1;
    }
  }
  
  BootContinueParameterBlkPtr.AvailMaxCoreMask = DisabledCoreMask ^ MAX_CORE_MASK;
...
void DecompressFV(IN UINT32 DecompressionRqd)
{
  ...
  */
  Status = StartFvDecompression ((void *)UefiFdBase, UefiFdSize, 
                                  &gCompressedDxeCoreContainerFvGuid, SYNCHRONOUS);
  if (Status != EFI_SUCCESS)
  {
  ...
  BeforeCount = GetTimerCountms (); 
  Status = StartFvDecompression ((void *)UefiFdBase, UefiFdSize, 
                                 &gCompressedAuxFvGuid, LaunchMode);
