...
class phy_table(object):
    def __init__ (self, input, output, prefix, postfix, table, index, comment, dft, eud):
        self.addr_table = []
        self.data_table = []
        self.except_table = []
        self.phy_table = []
            ...
            if (ent_len != 0):
                if ((ent_len == 1) and row[0]!="EMPTY" and row[0]!="END"):
                    ...
                    try:
                        ...
                        #print ent
                        if(self.eud and (total_row ==0)):
                            ent_dummy = ent + "  DummyRow for EUD workaround"
                            self.addr_table.append(ent_dummy)
                        ...
                        if(self.eud and (total_row ==0)):
                            ent_dummy = ent + "  DummyRow for EUD workaround"
                            self.data_table.append(ent_dummy)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--eud_flag", default = 0, help="eud format flag")
    ...
    if (args.eud_flag):
        print("eud_flag: " + str(args.eud_flag))
        eud_flag = int(args.eud_flag)

    builder = phy_table(input=input_path, output=output_tag, prefix=prefix, postfix=postfix, table=table_name, index=index_flag, comment=comment_flag, dft=dft_flag, eud=eud_flag)
