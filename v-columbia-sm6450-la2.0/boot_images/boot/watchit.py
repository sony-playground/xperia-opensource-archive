...
# --------   ---     -----------------------------------------------------------
# 07/20/21   ka      Update syntax for QcomCopyright.py to exclude desired dirs
...
def qcom_copyright_check(qcom_copyright_dir,
    ...
    for current_file in files:
      if current_file == 'QcomCopyright.py':
        script_path = os.path.join(root,
                                   'QcomCopyright.py')
      if current_file == 'exclude_paths.txt':
        exclude_files_path = os.path.join(root,
                                   'exclude_paths.txt')                                   
  ...
  if not len(exclude_files_path):
    raise NameError("ERROR: watchit::qcom_copyright_check: " + \
                    "Unable to locate exclude_paths.txt file")
