...
def create_target_specific_build_artifacts():
  ...
  if os.path.exists('../core/bsp/aop/build/'+'/AOP_'+build_variant+'.mbn') or \
    ...
    if os.environ['MSM_ID'] not in ['waipio','clarence','netrani']:
...
def start_build():
  ...
  if (chipset == "waipio"):
    chipset_pmic = os.environ.get('CHIPSET_PMIC')
    GenerateXML("build-cfg-%s.xml" %chipset_pmic)
