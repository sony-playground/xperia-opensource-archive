...
#ifdef PDC_GLOBAL_SERVICE
  rex_set_interrupt_vector(SOC_SLEEP_PDC_AOP_LOCK_REQ, (rex_isr_func_type)aop_services_isr);
  rex_clear_interrupt(SOC_SLEEP_PDC_AOP_LOCK_REQ);
  rex_enable_interrupt(SOC_SLEEP_PDC_AOP_LOCK_REQ);
  rex_set_interrupt_vector(SOC_SLEEP_PDC_AOP_UNLOCK_REQ, (rex_isr_func_type)aop_services_isr);
  rex_clear_interrupt(SOC_SLEEP_PDC_AOP_UNLOCK_REQ);
  //enable unlock on getting a lock
#endif
