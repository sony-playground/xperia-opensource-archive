...
/**
 * <!-- cmd_db_modify_aux_data -->
 *
 * @brief Modify command db for aux data.
 *
 *  This is used to modify a command DB entry aux data.
 *
 *  @param res_id : Resource index for AUX Data modification.
 *  @param len : Caller provides size of data buffer passed in.
 *             Returns size of data copied to data buffer.  Will
 *             fail if the len passed in does not match exactly
 *             what already exists.
 ...
 *          cmd_db_err_type.
 */
cmd_db_err_type cmd_db_modify_aux_data (const char *res_id,
