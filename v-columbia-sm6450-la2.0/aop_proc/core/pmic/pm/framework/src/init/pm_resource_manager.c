...
$Header: //components/rel/aop.ho/4.0/aop_proc/core/pmic/pm/framework/src/init/pm_resource_manager.c#1 $ 
...
void pm_resource_manager_init(void)
{
    ...
    for(; pmic_index < PM_MAX_NUM_PMICS ; pmic_index++)
    {
        ...
        {
            num_slaves_per_pmic = pm_get_num_of_sid_supported(pmic_index);
