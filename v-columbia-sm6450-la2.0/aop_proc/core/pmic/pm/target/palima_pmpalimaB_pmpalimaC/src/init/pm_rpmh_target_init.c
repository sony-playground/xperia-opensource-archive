...
$Header: //components/rel/aop.ho/4.0/aop_proc/core/pmic/pm/target/palima_pmpalimaB_pmpalimaC/src/init/pm_rpmh_target_init.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
08/26/22   rajivr   Set S6C(CX) always ON for Tofino to avoid stability issues
...
pm_err_flag_type pm_rpmh_platform_pre_init(void)
{
   ...
   ChipInfoIdType chip_id = CHIPINFO_ID_UNKNOWN;
   ...
   if(CHIPINFO_ID_SM_TOFINO == chip_id)
   {
       	...
       	pm_rail_info = (pm_vrm_vreg_rail_info_type*)set_ddr_access((uint64_t)rail_info[PMIC_C]);
       	CORE_VERIFY_PTR(pm_rail_info);
     
        /* Set S6C always ON for Tofino */
        pm_rail_info[SMPS_6].AlwaysOn = PM_ON;
        set_page_select(0);
   }
