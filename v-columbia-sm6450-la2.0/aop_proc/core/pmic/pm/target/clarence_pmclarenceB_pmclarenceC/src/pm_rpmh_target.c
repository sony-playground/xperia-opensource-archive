...
$Header: //components/dev/aop.ho/4.0/tmajumde.aop.ho.4.0.05202022_clarence_build_support/aop_proc/core/pmic/pm/target/netrani_pmnetraniB_pmnetraniC/src/pm_rpmh_target.c#1 $
...
pm_err_flag_type pm_rpmh_exit_sleep (pm_sleep_mode_type sleep_mode)
{
    ...
    err_flag = pm_comm_write_byte (0xE, 0x540, 0x1, 0x0);
    if (result != VRM_SUCCESS)
    {
       err_flag |= PM_ERR_FLAG_FAILURE;
    }
