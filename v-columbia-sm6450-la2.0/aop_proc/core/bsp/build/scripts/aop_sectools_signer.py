...
def generate(env):
  ...
  if env['MSM_ID'] in ['waipio']: #SPF support
    target = env['CHIPSET_PMIC']
  else:
    target = env['MSM_ID']
...
authenticator_map = {'waipio': '${BUILD_ROOT}/core/bsp/build/authenticators/waipio/waipio_tme_security_profile.xml',
                    ...
                    'clarence': '${BUILD_ROOT}/core/bsp/build/authenticators/clarence/clarence_tme_security_profile.xml',
					'palima': '${BUILD_ROOT}/core/bsp/build/authenticators/palima/palima_tme_security_profile.xml',
					'halliday': '${BUILD_ROOT}/core/bsp/build/authenticators/halliday/halliday_tme_security_profile.xml'
                    }

cass_cap_map = {'waipio': 'srot_prod_v6_waipio_srot_release',
                'fillmore': 'srot_prod_v6_fillmore_srot_release',
                'aurora': 'srot_prod_v6_aurora_srot_release',
                'kailua': 'srot_test_v6_srot_release',
                'lassen': 'srot_test_v6_srot_release',
				'netrani': 'srot_prod_v6_netrani_srot_release',
                'clarence': 'srot_prod_v6_clarence_srot_release',
				'palima': 'srot_prod_v6_palima_srot_release',
				'halliday': 'srot_prod_v6_halliday_srot_release'
                }

def sectools_find_authenticator(env):
  #Finds the authenticator per target and returns. Else fatals
  if env['MSM_ID'] in ['waipio']:
    auth_xml = env.RealPath(authenticator_map[env['CHIPSET_PMIC']])
  else:
...
def aop_img_sign_wrapper(env, target, source, **kwargs):
  ...
  if env['MSM_ID'] in ['waipio']:
    env.AOPSignXMLGen(env['CHIPSET_PMIC']) #signing manifest
  else:
...
def aop_devcfg_sign_wrapper(env, target, source, **kwargs):
  if kwargs.has_key('OUTPATH'):
    ...
    raise RuntimeError, "OUTPATH is not defined"
  if env['MSM_ID'] in ['waipio']:
    env.AOPSignXMLGen(env['CHIPSET_PMIC'])
  else:
...
def gen_signing_manifest_wrapper(env, msm_id):
  ...
  if os.environ.has_key("SPF"):
    if (os.environ['SPF'] == 'true'):
      supported_targets = ['waipio','palima','fillmore','netrani','clarence']
