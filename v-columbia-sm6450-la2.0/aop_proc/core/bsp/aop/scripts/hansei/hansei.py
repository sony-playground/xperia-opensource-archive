...
for arg in args.files :
  if os.path.isdir(arg):
    if os.path.exists(os.path.join(arg,"load.cmm")):
      with file(os.path.join(arg,"load.cmm")) as f :
      ...
      if dft['path'] != '':
        with open(dft['path'], 'rb') as data:
          ...
          if dft['display_name'] == 'md_SHRDIMEM':
            minidump = 1
            pretty.p("\tRecovering minidump RPMH Binary1...")
        break
  if loaded:
    try:
        #import pdb; pdb.set_trace()
        ...
        for dft in dump_file_types:
          if dft.get('no_load',False) == True:
            if "DDRCS" in dft.get('display_name','') or "md" in dft.get('display_name',''):
              #import pdb; pdb.set_trace()
              if dft['path'] != '':
                path = dft['path']
                if minidump:
        ...
        #block_reg_addr = 0x00000000A4267958
        #block_data_addr = 0x00000000A42698D8
    ...
    try:
        ...
        if hansei_target in ['24','55']:
            ...
            cmd_db_size = 0x2000
        elif hansei_target in ['8250','Saipan', 'lahaina','cedros','waipio','fillmore','aurora','netrani','clarence','palima']:
            cmd_db_addr = 0x80860000
            cmd_db_load_addr = 0x60860000
            cmd_db_size = 0x2000
        elif hansei_target in ['halliday']:
...
if (args.parser == ''):
    args.parser = os.path.join(os.path.realpath(__file__).split('aop_proc')[0], 'aop_proc','core','bsp','aop','scripts')
...
debug_data['bcm_json_dir'] =  os.path.join(os.path.realpath(__file__).split('aop_proc')[0]+'aop_proc','core','bsp','aop','scripts','hansei','clock_plan')
...
os.system("python " + os.path.dirname(os.path.abspath(__file__)) + os.sep + "tcs_content_dump" + os.sep + "printTcs.py " + os.path.dirname(list(args.files)[0]) + " " + args.output)
    ...
    if (dfn in dft['name']) or ((dfe.upper() in dft['ext']) and (dfne.upper() in dft['basename'])):
        ...
        if hansei_target in ['24','55','Olympic']:
          ...
          #import pdb;pdb.set_trace()
            ...
            os.system("python " + debug_data['ddr_mgr_log_parser'] + os.sep + args.target + os.sep + "decode_msg11.py " + dumpfile + "  > " + args.output + os.sep + "aop_ddr_logs_msgram11.txt" )
