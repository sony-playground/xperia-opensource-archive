...
def locate_by_elf(memory, debug_info):
    if b'aop_core_dump' not in debug_info.variables:
        logger.debug('Symbol "aop_core_dump" not found in ELF!')
        return None

    address = debug_info.variables[b'aop_core_dump'].address
...
def locate_by_scan(memory):
    for address in range(DATA_RAM_BASE, DATA_RAM_BASE + DATA_RAM_SIZE, 4):
        raw_cookie = memory.read(address, 4)
        cookie, = struct.unpack('<L', raw_cookie)
        if cookie == 0xbaff1ed:
