...
def dump(debug_data): #dump_path, memory, debug_info, fill_char):
	...
	try:
		...
		elif target in ['8250','Saipan','lahaina','cedros','waipio','fillmore','aurora','halliday','palima','netrani','clarence']:
			ver_data = memory.read(0x80860000, 4)
		elif target in ['halliday']:
	...
	with open(cmddb_file_name, 'w') as cmd_db_file:
		...
		elif target in ['8250','Saipan', 'lahaina','cedros','waipio','fillmore','netrani','clarence','halliday','aurora','palima']:
			smem_db_data_addr = 0x80860090
		elif target in ['halliday']:
			smem_db_data_addr = 0x81C60000
		else:
