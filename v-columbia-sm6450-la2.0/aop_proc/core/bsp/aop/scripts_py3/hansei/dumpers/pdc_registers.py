...
def dump(debug_data): #dump_path, memory, debug_info, fill_char):
	...
	if target_ver in targ_spec_config:
		target = target_ver
	debug_data['printer'].p("Dumping subsystem pdc registers...")
	...
	for i in range(len(subs_list)):
		...
		if ((subs_list[i]!="gpu") & (subs_list[i]!="display") & (subs_list[i]!="display2")):
	...
	for xx in range (len(subs_list)):
		...
		with open(pdc_mod_file_name, 'w') as pdc_module_file:
			...
			if ((subs_list[xx]!="gpu") & (subs_list[xx]!="display") & (subs_list[xx]!="display2")):
