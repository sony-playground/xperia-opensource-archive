...
def dump(debug_data): #dump_path, memory, debug_info, fill_char):
    ...
    with open(aop_log_file_name, 'w') as aop_log_file:
        ...
        log_address = debug_info.variables[b'aop_log_data'].address
        #print(hex(log_address))
        log_type = debug_info.variables[b'aop_log_data'].die
...
def conhexToAscii(message):
    ...
    try:
      messageStr = binascii.unhexlify(message2.strip())
    except: 
      messageStr = "--"

    if type(messageStr) is bytes:
      return messageStr[::-1].decode('utf-8')
    return messageStr[::-1]
