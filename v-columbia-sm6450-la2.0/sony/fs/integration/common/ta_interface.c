...
int format_ta(uint8_t partition)
{
	...
	if (err != TA_Success && err != TA_Invalid) {
		FCS_LOG_ERR_TA("Unable to open TA partition %d, err=%d\n",
			partition, err);
		res = -FCSERR_EOPEN;
		goto exit_format_ta;
	}
	...
	if (err != TA_Success) {
		FCS_LOG_ERR_TA("Unable to format TA partition %d, err=%d\n",
			partition, err);
		res = -FCSERR_EFORMAT;
...
int get_ta_unit_size(uint8_t partition, uint32_t unit, int *unit_size)
{
	...
	if (!unit_size) {
		FCS_LOG_ERR_TA("Invalid argument\n");
	...
	if (err == TA_Invalid)
		ta_opened = true;
	if (err != TA_Success) {
		FCS_LOG_ERR_TA("Unable to open TA partition %d, error=%d\n",
			partition, err);
		res = -FCSERR_EINVAL;
	...
	*unit_size = TA_GetUnitSize(unit);
	if (*unit_size == 0) {
		...
		FCS_LOG_INF_TA("Unit does not exist (%d)\n", unit);
		res = FCSERR_OK;
		goto exit_get_ta_unit_size;
	}

	FCS_LOG_INF_TA("Size of unit %d is %d bytes\n", unit, *unit_size);
...
int read_ta(uint8_t partition, uint32_t unit, uint8_t *buf, int buf_size)
{
	...
	if (err == TA_UnitNotFound) {
		FCS_LOG_WRN_TA("Unit %d does not exist\n", unit);
		res = -FCSERR_ENOTF;
		goto exit_read_ta;
	}

	if (err != TA_Success) {
		FCS_LOG_WRN_TA("Unable to read TA unit %d, error=%d\n",
			unit, err);
		res = -FCSERR_EIO;
		goto exit_read_ta;
	}
...
int get_all_ta_size(uint8_t partition, int *ta_size, bool include_hi_ta)
{
	...
	*ta_size = 0;
	...
	FCS_LOG_INF_TA("TA total size=%ld\n", *ta_size);
		...
		bool include_hi_ta)
{
	...
	while (TA_Inc_GetSize(&unit_size) == TA_Success) {
		pData = malloc(unit_size);
		if (pData == NULL) {
			FCS_LOG_ERR_TA("malloc failed!\n");
			res = -FCSERR_ENOMEM;
			goto exit_read_all_ta;
		}

		TA_Inc_Read(&unit, pData, unit_size);
		FCS_LOG_INF_TA("TA unit=%ld, size=%ld\n", unit, unit_size);
		/*
...
int write_ta(uint8_t partition, uint32_t unit, const uint8_t *buf, int buf_size)
{
	...
	if (err != TA_Success) {
		FCS_LOG_ERR_TA("Unable to write to TA unit %d\n", unit);
		res = -FCSERR_EINVAL;
	}
...
int delete_ta(uint8_t partition, uint32_t unit)
{
	...
	if (err != TA_Success) {
		FCS_LOG_ERR_TA("Unable to delete TA unit %d\n", unit);
		res = -FCSERR_EINVAL;
	}
