...
// BoringSSL does not support extra_data in the store context
// and as a result, it does not have set/get functions for that.
// here is the local implementation that brings the minimal changes
// for the existing code and still can handle the logic
static struct rule_verifier* p_rule_verifier = NULL;

static int CRYPTO_set_ex_data(void* empty, int empty2, struct rule_verifier* rule_handle)
{
	(void) empty;
	(void) empty2;
	p_rule_verifier = rule_handle;
	return 1;
}

static struct rule_verifier* CRYPTO_get_ex_data(void* empty, int empty2)
{
	(void) empty;
	(void) empty2;
	return p_rule_verifier;
}
