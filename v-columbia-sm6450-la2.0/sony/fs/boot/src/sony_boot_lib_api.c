	...
	struct xboot_ctx *boot_context)
{
	...
	/* Parse the bootconfig to add Sony related additions to cmdline.
	 * We don't care about the result since we are on our way to
	 * service mode anyway.
	 */
	(void)sony_boot_parse_bootconfig(boot_context);
	FCS_LOG("Populate command line before entering service mode\n");
