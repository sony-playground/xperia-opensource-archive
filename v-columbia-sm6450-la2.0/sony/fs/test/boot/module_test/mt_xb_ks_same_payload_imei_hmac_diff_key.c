...
/*
 * Description:
 * If xflkeystore and xflkeystorebak have same payload, both are bound to
 * an HMAC of the IMEI, both have the same keystore counter, but the IMEI HMAC
 * is generated with different HMAC keys, the new valid HMAC key from MiscTA
 * unit 2164 should be copied to MiscTA unit 66673, and xflkeystore copied to
 * xflkeystorebak.
 *
 * Setup:
 * 1. xflkeystorebak is valid and bound to IMEI HMAC.
 * 2. xflkeystore is valid, bound to IMEI HMAC, has same counter as
 *    xflkeystorebak, but the HMAC is generated with a different key.
 * 3. There is a valid HMAC key in MiscTA unit 66673 for xflkeystorebak.
 * 4. There is a valid HMAC key in MiscTA unit 2164 for xflkeystore.
 *
 * Execution:
 * 1. xflkeystore will be validated successfully.
 * 2. The new HMAC key will be copied to MiscTA unit 66673.
 * 3. MiscTA unit 2164 will be erased.
 * 4. xflkeystore will be copied to xflkeystorebak.
		...
		const char *match)
{
	...
	case MT_XB_EXECUTE:
		...
		/* Set a keystore that is bound to HMAC of IMEI in backup */
		...
		/*
		 * Set a keystore that is bound to HMAC of IMEI with different
		 * HMAC key in primary.
		 */
		res = integration_set_property(INTG_PROP_KS,
			sony_keystore_bound_to_hmac_imei_new_key,
			sony_keystore_bound_to_hmac_imei_new_key_len);
		if (res != FCSERR_OK) {
			log("Set keystore failed with %d\n", res);
			return res;
		}

		/* Put a new valid HMAC key to low MiscTA */
		res = integration_set_property(TA_KEYSTORE_HMAC_KEY,
					hmac_key_new, hmac_key_new_len);
		if (res != FCSERR_OK) {
			log("Put HMAC key to low MiscTA failed with %d\n", res);
			return res;
		}
		...
		/* Check that the HMAC key has been copied to high MiscTA */
