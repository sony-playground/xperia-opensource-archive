		...
		const char *match)
{
	...
	unsigned char called_not_expected = 0;
	...
	case MT_XB_EXECUTE:
		...
		/* Replace primary KS partition with XCS KS */
		res = integration_set_property(
			INTG_PROP_KS,
			const_dut_ks_xcs_valid,
			const_dut_ks_xcs_valid_len);
		if (res != FCSERR_OK) {
		...
		/* User sensitive data should have been erased */
		res = integration_is_equal_property(INTG_PROP_CNT_ERASEUSER,
			(const void *)&called_expected,
			sizeof(called_expected));
		if (res != FCSERR_OK) {
			log("CNT_ERASEUSER check failed with %d\n", res);
			goto test_failure;
		}

		/* TA part of DK should have been erased */
		res = integration_is_equal_property(INTG_PROP_CNT_ERASETADK,
			(const void *)&called_expected,
			sizeof(called_expected));
		if (res != FCSERR_OK) {
			log("CNT_ERASETADK check failed with %d\n", res);
			goto test_failure;
		}

		/* BootLoader should have been locked */
		res = integration_is_equal_property(INTG_PROP_CNT_LOCK,
			(const void *)&called_expected,
			sizeof(called_expected));
		if (res != FCSERR_OK) {
			log("CNT_LOCK check failed with %d\n", res);
			goto test_failure;
		}
		...
		/* ORG KS Partition should not have been updated */
		res = integration_is_equal_property(INTG_PROP_CNT_WRKSORG,
			(const void *)&called_not_expected,
			sizeof(called_not_expected));
		if (res != FCSERR_OK) {
			log("CNT_WRKSORG check failed with %d\n", res);
			goto test_failure;
		}

		/* BAK KS Partition should have been updated */
		res = integration_is_equal_property(INTG_PROP_CNT_WRKSBAK,
			(const void *)&called_expected,
			sizeof(called_expected));
		if (res != FCSERR_OK) {
			log("CNT_WRKSBAK check failed with %d\n", res);
			goto test_failure;
		}
