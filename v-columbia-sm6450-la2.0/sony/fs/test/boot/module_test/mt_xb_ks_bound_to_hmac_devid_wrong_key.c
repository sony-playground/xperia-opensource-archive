		...
		const char *match)
{
	...
	case MT_XB_EXECUTE:
		...
		/* Set a keystore that is bound to HMAC of DevID */
		res = integration_set_property(INTG_PROP_KS,
			xcs_keystore_bound_to_hmac_devid,
			xcs_keystore_bound_to_hmac_devid_len);
		if (res != FCSERR_OK) {
			log("Set keystore failed with %d\n", res);
			return res;
		}

		/* Put an erroneous HMAC key to low TA */
		...
		/* Check that there is no HMAC key in high TA */
