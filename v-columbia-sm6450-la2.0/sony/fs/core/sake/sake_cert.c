					      ...
					      certificate_datalist)
{
	...
	while (current_cert != NULL) {
		...
		if (current_cert->publicKey.item_m.data != NULL) {
			free(current_cert->publicKey.item_m.data);
			current_cert->publicKey.item_m.data = 0;
		}
		if (current_cert->publicKey.item_e.data != NULL) {
			free(current_cert->publicKey.item_e.data);
			current_cert->publicKey.item_e.data = 0;
		}
		free(current_cert);
